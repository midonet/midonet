...
class UplinkDualStack(SingleTenantWithNeutronIPv6FIP):
    ...
    def build_uplink(self, cidr, host, interface):
        ...
        ipv4_gw_ip = '10.2.0.1'
        ipv6_gw_ip = net_pref + "1"
        self.create_subnet("uplinksubnet4_%d" % id, uplinknet, '10.2.0.0/24',
...
class FullDualStack(SingleTenantWithNeutronIPv6FIP):

    def build_public_subnets(self):
        pubsubnets = super(FullDualStack, self).build_public_subnets()
        pubsubnet4 = self.create_subnet("publicsubnet4",
                                        self.pubnet,
                                        "200.0.0.0/24",
                                        version=4)
...
def test_dual_stack_uplink():
    ...
    """
    ping_from_inet('quagga1', 'cccc:bbbb::3', 10, namespace='ip6')
    ping_from_inet('quagga1', '10.2.0.1', 10)

...
class SingleTenantWithNeutronIPv6FIP(UplinkWithVPP):
    ...
    def build_external_subnets(self):
    ...
    def build_tenant(self):
        ...
        pubsubnets = self.build_external_subnets()
...
class TenantDualStack(SingleTenantWithNeutronIPv6FIP):
    ...
    def build_external_subnets(self):
        pubsubnets = super(TenantDualStack, self).build_external_subnets()
...
def ping_from_inet(container, ip='2001::1', count=4, namespace=None, ip_version=6):
    count = max(1, count)
    ping_cmd = "ping"
    if ip_version == 6:
        ping_cmd += "6"
...
def test_tenant_dual_stack():
    ...
    """
    ping_from_inet('quagga1', 'cccc:bbbb::3', 10, namespace='ip6')
    cont_services = service.get_container_by_hostname('midolman2')
    (ret, vmnamespace) = cont_services.exec_command_and_get_output('ip netns', 60)
    vmnamespace = [el for el in vmnamespace.split('\n') if el != '']
    assert(len(vmnamespace) == 1)
    ping_from_inet('midolman2', '200.0.0.1', 10,
                   namespace=vmnamespace[0], ip_version=4)

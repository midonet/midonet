...
class UplinkWithVPP(NeutronVPPTopologyManagerBase):
    ...
    def build_uplink(self, id, host, interface):
        uplinknet = self.create_network("uplink%d" % id, uplink=True)
        net_pref = "%d001::" % (id + 1)
        subnet = net_pref + "/64"
        ipv6_gw_ip = net_pref + "1"
        self.create_subnet("uplinksubnet6_%d" % id, uplinknet, subnet,
    ...
    def build(self, binding_data=None):
        ...
        self.build_uplink(1, "midolman1", "bgp0")
...
class DualUplinkAssymetric(SingleTenantWithNeutronIPv6FIP):
    ...
    def build(self, binding_data=None):
        self._edgertr = self.create_router("edge")
        self.build_uplink(1, "midolman1", "bgp0")
        self.build_uplink(2, "midolman2", "bgp0")
    ...
    def destroy(self):
        super(SingleTenantWithNeutronIPv6FIP, self).destroy()

...
def ping(src_port, dst_port, retries=3):
    try:
        ...
        ping(src_port, dst_port, retries-1)
...
binding_onehost_inter_tenant = copy.deepcopy(binding_onehost_intra_tenant)
binding_onehost_inter_tenant.update({
...
def test_ping_between_three_sites():
    ...
    # Ping all to all
    ports = ['port_left', 'port_right', 'port_up']
    for src, dst in itertools.permutations(ports, 2):
        ping(src, dst)

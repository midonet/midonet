...
def disable_and_assert_traffic_fails(sender, action_fun, vips):
    # Do action
    action_fun("disable")
...
def enable_and_assert_traffic_succeeds(sender, action_fun, vips):
    # Do action
    action_fun("enable")

    # Make one request to the non sticky loadbalancer IP, should succeed
    assert_request_succeeds_to(sender, vips['non_sticky_vip'])

    # Make one request to the sticky loadbalancer IP, should succeed
    assert_request_succeeds_to(sender, vips['sticky_vip'])
...
def action_vips(fun_name, vips):
    for current_vip in vips.values():
...
def test_disabling_topology_loadbalancing():
    ...
    disable_and_assert_traffic_fails(sender, action_pool_members, vips)
    enable_and_assert_traffic_succeeds(sender, action_pool_members, vips)

    disable_and_assert_traffic_fails(sender, action_pool, vips)
    enable_and_assert_traffic_succeeds(sender, action_pool, vips)

    # Disabled due to MN-1536
    disable_and_assert_traffic_fails(sender, action_vips, vips)
    enable_and_assert_traffic_succeeds(sender, action_vips, vips)

    # Disabled due to MN-1536
    disable_and_assert_traffic_fails(sender, action_loadbalancer, vips)
    enable_and_assert_traffic_succeeds(sender, action_loadbalancer, vips)

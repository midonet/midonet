...
LOG.info("IKDEBUG container1 %s" % (get_container_by_hostname('cassandra1')))
#LOG.info("IKDEBUG container %s" % (get_container_by_hostname('cassandra')))


cass_host = get_container_by_hostname('cassandra1').get_ip_address()
LOG.info("IKDEBUG container1 ip %s" % (cass_host))
cassandra = None
try:
    cluster = Cluster([ cass_host ])
    LOG.info("IKDEBUG connecting")
    cassandra = cluster.connect()
    LOG.info("IKDEBUG set keyspace")
    cassandra.set_keyspace('midonetflowtracing')
    LOG.info("IKDEBUG done")
except cassandra.InvalidRequest as e:
    LOG.info("IKDEBUG got exception %s" % (e))
    raise e
...
def get_flow_traces(tr):
    rows = cassandra.execute(
        "SELECT flowtraceid FROM flows WHERE tracerequestid = %s",
        [UUID(tr)])
    return [str(r.flowtraceid) for r in rows]

def get_hosts(tr, flowtraceid):
    rows = cassandra.execute(
        "SELECT host FROM flow_events WHERE tracerequestid = %s AND flowtraceid = %s",

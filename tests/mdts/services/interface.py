...
class Interface(object):
    ...
    def verify_bandwidth(self, target_iface, max_kbps, burst_kb,
        ...
        assert_that(listen_result, equal_to(0),
                    "Listening nc process failed with error: %d" % results[0])
        assert_that(send_result, equal_to(0),
                    "Sending nc process failed with error: %d" % results[1])

        assert_that(elapsed_time, greater_than(min_time),
                    "Transfer completed in %f seconds, expected > %f" %
                    (elapsed_time, min_time))
        assert_that(elapsed_time, less_than(max_time),
                    "Transfer completed in %f seconds, expected < %f" %

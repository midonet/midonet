...
from paramiko.client import MissingHostKeyPolicy, RejectPolicy
...
class CustomHostKeyPolicy(MissingHostKeyPolicy):
    ...
    def __init__(self, strict_host_key_checking, known_hosts_file):
        self._strict_host_key_checking = strict_host_key_checking
        self._known_hosts_file = known_hosts_file

    def missing_host_key(self, client, hostname, key):
        if self._strict_host_key_checking == 'no':
            return
        else:
            if HostKeys(self._known_hosts_file).check(hostname, key):
                return
...
class SshClient(object):
    ...
    def __init__(self, containers_file, extra_ssh_config_file):
        ...
        self._missing_host_key_policy = CustomHostKeyPolicy(self._ssh_config,
                                                            RejectPolicy())
    ...
    def _get_ssh_connection(self, container_name):
        ...
        ssh_connection.set_missing_host_key_policy(
                CustomHostKeyPolicy(self._get_strict_host_key_checking_option(config),
                                     self._get_user_known_hosts_file(config)))

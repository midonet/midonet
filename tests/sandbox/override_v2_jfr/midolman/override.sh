...
sed -i "\$a JVM_OPTS=\"\$JVM_OPTS -XX:+UnlockCommercialFeatures -XX:+FlightRecorder -XX:StartFlightRecording=filename=duration=0s,/midolman.jfr,compress=True -Djava.rmi.server.hostname=$IPADDRESS\"" /etc/midolman/midolman-env.sh

...
First, you must clone this repository into a fresh directory:

```git clone http://github.com/midonet/mdts```

So, for a quick and dirty set up:

```
cd ~
mkdir -p src/midonet
cd src/midonet
git clone http://github.com/midonet/mdts mdts
cd mdts
```

Now that MDTS is downloaded and ready, make sure all the dependencies
are installed.

Pre-requisites
-------------

* MDTS currently can only be used on Ubuntu Linux.
* MDTS requires git to be installed and running separately.

### Development Environment

As per standard development principles, testing in a clean environment
is highly recommended.  Using a VM (such as Vagrant, VMWare, or even a
Docker Container) can be useful to isolate your testing setup so that
other environmental factors do not interfere with the test.  Prime
candidates for interference with MDTS tests are: any networking software
or extra interfaces, running processes with open network ports, active
network traffic, packet traces, active zookeeper or cassandra processes,
complicated internal routing or bridge setups, and/or active IP net
namespaces.  Making sure the system is as clean as possible will go a
long way to ensure clean and reliable results.  It also ensures that the
environment itself can easily be destroyed and recreated (from
snapshots, for example) if something goes wrong, or if the system
becomes corrupted).

Furthermore, there are several package dependencies for MDTS to run.
In order to install them, the mdts_cbt command can be used with the ‘env’
target (see below).  The main mdts_cbt command will also install the
environmental dependencies by default, unless a flag is set to state
otherwise (“-ne”, see below).  Of course, an image and/or snapshot can
be created after the environment install, and then used for further
testing without taking the time to reinstall.  It will remain accurate
for as long as the environmental dependencies remain identical.

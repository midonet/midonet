...
class RtnetlinkTest extends FeatureSpec
    ...
    feature("rtnetlink operations for links") {
        ...
        scenario("the created interface should equal to the original one.") {
            val link = new Link
            link.ifi.family = 0
            link.ifi.`type` = 0
            link.ifi.index = 0
            ...
            try {
                whenReady(obs.test, timeout(TestTimeoutSpan)) { result =>
                    result should be (OK)
                }
            } finally {
                s"ip link del ${tapName}_".!
            }
        }

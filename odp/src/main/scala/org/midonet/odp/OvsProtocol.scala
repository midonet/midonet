...
sealed class OvsProtocol(pid: Int,
    ...
    private def portRequest(buf: ByteBuffer, datapathId: Int, port: DpPort,
        ...
        val message = messageFor(buf, datapathId, ctx).withFlags(NLFlag.REQUEST)
        NetlinkMessage.writeIntAttr(buf, Attr.UpcallPID, pid)
    ...
    def prepareFlowCreate(datapathId: Int, flow: Flow, ack: Boolean,
                          buf: ByteBuffer): Unit = {
        import org.midonet.odp.OpenVSwitch.Flow.Attr
        ...
        if (!flow.hasEmptyMask) {
            NetlinkMessage.writeAttrSeq(buf, Attr.Mask, flow.getMask.getKeys, FlowKeys.writer)
        }
        message.finalize(pid)
    }

    def prepareFlowSet(datapathId: Int, flow: Flow, buf: ByteBuffer): Unit = {
        import org.midonet.odp.OpenVSwitch.Flow.Attr

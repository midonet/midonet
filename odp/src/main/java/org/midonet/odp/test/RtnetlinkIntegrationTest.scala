...
object MockSelectorBasedRtnetlinkConnection {
    ...
    def apply(addr: Netlink.Address, sendPool: BufferPool, groups: Int = 0) =
        try {
...
trait RtnetlinkTest {
    ...
    def newLinkNotificationTest: Test = {
        ...
        implicit val promise = Promise[String]()
        var linkNum = 0
        // conn.requestBroker.readReply(conn.notificationObserver)
    ...
    def listRouteTest: Test = {
        val desc = """the number of IPv4 entries of the default routing table
                     |should be identical to the result of `ip routes list`.
                   """.stripMargin.replaceAll("\n", " ")
    ...
    def linkTests: TestSuite = Seq(listLinkNumberTest, getLinkTest)
    def linkCreateTests: TestSuite = Seq(createLinkTest)
    // def addrTests: TestSuite = Seq(getAddrTest,
    //    (listAddrTest._1, getAddrTest._2.flatMap(_ => listAddrTest._2)))
    def addrTests: TestSuite = Seq(listAddrTest)
    def routeTests: TestSuite = Seq(
        (getRouteTest._1, listAddrTest._2.flatMap(_ => getRouteTest._2)),
        (listRouteTest._1, getRouteTest._2.flatMap(_ => listRouteTest._2)))
    def neighTests: TestSuite = Seq(listNeighTest)
    def notificationTests: TestSuite = Seq(newLinkNotificationTest)
}
...
class RtnetlinkIntegrationTestBase extends RtnetlinkTest {
    ...
    def run(): Boolean = {
        ...
        try {
            ...
            passed &= printReport(runSuite(linkCreateTests))
            // passed &= printReport(runSuite(addrTests))
            // passed &= printReport(runSuite(routeTests))
            // passed &= printReport(runSuite(neighTests))
            // passed &= printReport(runSuite(notificationTests))
        } finally {
            stop()
            conn.stop()
        }

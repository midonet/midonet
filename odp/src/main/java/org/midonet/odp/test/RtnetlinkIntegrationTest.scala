...
trait RtnetlinkTest {
    ...
    def listLinksAndAddrs: Test = {
        ...
        val linksListRequestObserver = DanglingObserver(linksSubject)
        while (linksListRequestObserver.isDangling) {
            try {
                conn.requestBroker.readReply()
            } catch {
                case t: Throwable =>
        ...
        val addrsListRequestObserver = DanglingObserver(addrsSubject)
        while (addrsListRequestObserver.isDangling) {
            try {
                conn.requestBroker.readReply()
            } catch {
                case t: Throwable =>

...
public class FlowKeyTunnel implements FlowKey<FlowKeyTunnel> {
    ...
    public void serialize(BaseBuilder<?,?> builder) {
        if ((usedFields & TUN_ID_MASK) != 0)
            builder.addAttr(FlowKeyTunnelAttr.OVS_TUNNEL_KEY_ATTR_ID, tun_id,
        ...
        if ((tun_flags & OVS_TNL_F_DONT_FRAGMENT) == OVS_TNL_F_DONT_FRAGMENT)
            builder.addAttr(FlowKeyTunnelAttr.OVS_TUNNEL_KEY_ATTR_DONT_FRAGMENT);
    ...
    public boolean deserialize(NetlinkMessage message) {
        try {
            Long tun_id = message.getAttrValueLong(
                    FlowKeyTunnelAttr.OVS_TUNNEL_KEY_ATTR_ID, ByteOrder.BIG_ENDIAN);
            if (tun_id != null) {
                this.tun_id = tun_id;
            ...
            Integer ipv4_src = message.getAttrValueInt(
                    FlowKeyTunnelAttr.OVS_TUNNEL_KEY_ATTR_IPV4_SRC, ByteOrder.BIG_ENDIAN);
            if (ipv4_src != null) {
            ...
            Integer ipv4_dst = message.getAttrValueInt(
                FlowKeyTunnelAttr.OVS_TUNNEL_KEY_ATTR_IPV4_DST, ByteOrder.BIG_ENDIAN);
            if (ipv4_dst != null) {
    ...
    public String toString() {
        return "FlowKeyTunnel{"
            + "tun_id=" + tun_id + ", "
            + "ipv4_src=" + IPv4Addr.fromInt(ipv4_src).toString() + ", "
            + "ipv4_dst=" + IPv4Addr.fromInt(ipv4_dst).toString() + ", "
            + "tun_flag=" + tun_flags + ", "
            + "ipv4_tos=" + ipv4_tos + ", "
            + "ipv4_ttl=" + ipv4_ttl
            + '}';

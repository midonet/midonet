...
class FlowMaskTest  extends FlatSpec with ShouldMatchers {
    ...
    private def hasKey(fmatch: FlowMatch, id: Short): Boolean = {
        for (k <- fmatch.getKeys) {
            if (k.attrId() == id)
                return true
        }
        false
    ...
    private def expect[K <: FlowKey](f: K => Array[Byte])
                                    (implicit m: Manifest[K]): Array[FlowKey] => Array[Byte] =
        keys => {
            val ctor = m.runtimeClass.getDeclaredConstructor()
            ctor.setAccessible(true)
            val id = ctor.newInstance().asInstanceOf[K].attrId()
            var k = keys(id).asInstanceOf[K]
            if (k eq null) {
                k = FlowKeys.newBlankInstance(id).asInstanceOf[K]
                k.wildcard()
            }
            f(k)
        }
    ...
    private def seeSomeFields(fmatch: FlowMatch) = {
        var expected = Set[Array[FlowKey] => Array[Byte]]()

        fmatch.getInputPortNumber
        expected += expect[FlowKeyInPort](_.portNo)

        if (hasKey(fmatch, OpenVSwitch.FlowKey.Attr.Tunnel)) {
            if (rand.nextBoolean()) {
                fmatch.getTunnelKey
        ...
        if (hasKey(fmatch, OpenVSwitch.FlowKey.Attr.IPv4)) {
            if (rand.nextInt(2) == 0) {
            ...
            if (hasKey(fmatch, OpenVSwitch.FlowKey.Attr.UDP)) {
                if (rand.nextInt(2) == 0) {
                    fmatch.getSrcPort
            ...
            } else if (hasKey(fmatch, OpenVSwitch.FlowKey.Attr.TCP)) {
                if (rand.nextInt(2) == 0) {
                    fmatch.getSrcPort
            ...
            } else if (hasKey(fmatch, OpenVSwitch.FlowKey.Attr.ICMP)) {
                if (rand.nextInt(2) == 0) {
                    fmatch.getSrcPort
        ...
        } else if (hasKey(fmatch, OpenVSwitch.FlowKey.Attr.ARP)) {
            if (rand.nextInt(2) == 0) {
                fmatch.getNetworkSrcIP
    ...
    private def verify(flowMatch: FlowMatch, flowMask: FlowMask,
                       expected: Set[Array[FlowKey] => Array[Byte]]) = {
        ...
        expected map (_(flowKeys)) foreach allOnes
    }

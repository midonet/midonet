...
import org.midonet.util.concurrent.{NanoClock, InstanceStash0}
...
class RecircTest extends FeatureSpec
    ...
    val recircHostMac = MAC.fromString("02:0a:08:06:04:02")
    val recircHostAddr = IPv4Subnet.fromString("80.254.123.111/30", "/")
    val recircMnMac = MAC.fromString("0a:fe:88:90:22:33")
    val recircMnAddr = IPv4Addr("80.254.123.112")

    val config = new MidolmanConfig(ConfigFactory.parseString(
        s"""
           |agent.datapath.recircExternalName : "recirc-host"
           |agent.datapath.recircInternalName : "recirc-mn"
           |agent.datapath.recircExternalMac : "${recircHostMac.toString}"
           |agent.datapath.recircInternalMac : "${recircMnMac.toString}"
    ...
    override def beforeAll(): Unit = {
        ...
        val LinkOps.Veth(hostSide, mnSide) = LinkOps.createVethPair(
        ...
        LinkOps.setAddress(mnSide, config.datapath.recircMnAddr.subnet())
        veth = hostSide
    ...
    feature("Packets are recirculated") {
        scenario ("For encapsulation") {
            ...
            context.encap(
                ...
                encapSrcUdp,
                UDP.VXLAN,
                new InstanceStash0[FlowMatch](() => new FlowMatch()))

...
class HaproxyHelperTest extends FeatureSpec
    ...
    def verifyStatus(haproxy: HaproxyHelper,
        ...
        eventually {
            ...
            val memberIds = lbConfig.pools.flatMap(p => p.members)
                                          .map (_.id.toString)
    ...
    val pool1 = PoolV2Config(UUID.randomUUID(), true, Set(member1, member2),
                             healthMonitor)
    val pool2 = PoolV2Config(UUID.randomUUID(), true, Set(member4, member5),
                             healthMonitor)

    val pool1Updated = PoolV2Config(pool1.id, true,

...
class VppControllerIntegrationTest extends FeatureSpec with Matchers
    ...
    private var log = LoggerFactory.getLogger(classOf[VppControllerIntegrationTest])
    private val test_device = "vpp_test_device"
    ...
    class VppTestDatapathState extends TestDatapathState {
        ...
        private val dpPort = ovs.createDpPort(test_device)
    ...
    before {
        ...
        log info s"Creating test device $test_device"
        cmd(s"sudo ip link add name $test_device type dummy", 1)
        cmd(s"sudo ip l set up dev $test_device")
    ...
    after {
        ...
        cmd(s"sudo ip link del $test_device", 1)
        log info "Done"
    }
    ...
    private def createVppContoller(): (Actor, ActorRef) = {
        val ref = TestActorRef[VppController](
            Props(new VppController(config, dpConnManager, dpState, backend)),
                "TestVppController")
    ...
    feature("VPP controller handles uplink ports") {
        scenario("VPP controller launches VPP process") {
            ...
            Thread.sleep(10000)
            assertCmd("vppctl show version", 1)
            ...
            log info "All checks passed, Something wrong with Datapath"

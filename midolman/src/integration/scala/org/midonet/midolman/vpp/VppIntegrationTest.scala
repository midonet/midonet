    ...
    feature("VXLan downlink") {
        scenario("ping6 through vxlan") {
            ...
            try {
                ...
                assertVppCtl("set int ip table vxlan_tunnel0 1")

                // this tells all traffic on vrf 1 to go to loop0,
                // but prepends an ethernet header first
                // dst de:ad:be:ef:00:03
                // src de:ad:be:ef:00:04
                // ethertype 0800 (IPv4)
                // dst should match the other end of the tunnel when
                // working with linux kernel vxlan, but shouldn't matter
                // with OVS.
                assertVppCtl("ip route add table 1 0.0.0.0/0"
                                 +" via loop0 0xdeadbeef0003deadbeef00040800")

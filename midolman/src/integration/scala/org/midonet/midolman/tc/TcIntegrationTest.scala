...
class TcIntegrationTest extends FeatureSpec with Eventually with ShouldMatchers {
    val log = LoggerFactory.getLogger(classOf[TcIntegrationTest])

    def verifyIngressQdisc(dev: String, noQdisc: Boolean = false): Boolean = {
        eventually {
            val qdiscs = s"tc -s qdisc show dev $dev".!!
            (qdiscs contains "ingress") == !noQdisc
        }
    }

    def verifyIngressFilter(dev: String, rate: Int, burst: Int): Boolean = {
        eventually {
            val filters = s"tc filter ls dev $dev parent ffff:".!!
            filters contains s"rate ${rate}000bit burst ${burst}Kb"
        }
    }
    ...
    feature("write TC config") {
        scenario("add tc config") {
            ...
            try {
                ...
                verifyIngressQdisc(devName) shouldBe true
                verifyIngressFilter(devName, 300, 200) shouldBe true
        ...
        scenario("add new tc config") {
            ...
            try {
                ...
                verifyIngressQdisc(devName) shouldBe true
                verifyIngressFilter(devName, r, b) shouldBe true
        ...
        scenario("delete added TC conf") {
            ...
            try {
                ...
                verifyIngressQdisc(devName, noQdisc = true) shouldBe true
    ...
    feature("multiple devices") {
        scenario("multiple devices are created") {
            ...
            try {
                ...
                verifyIngressQdisc(devName1) shouldBe true
                verifyIngressFilter(devName1, r1, b1) shouldBe true
                ...
                verifyIngressQdisc(devName2) shouldBe true
                verifyIngressFilter(devName2, r2, b2) shouldBe true

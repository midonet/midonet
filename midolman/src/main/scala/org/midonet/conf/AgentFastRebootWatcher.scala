...
class AgentFastRebootBarriers(zk: CuratorFramework,
    ...
    def clear() = {
        stage1.removeBarrier()
        stage2.removeBarrier()
        stage3.removeBarrier()
    }
...
class AgentFastRebootWatcher(minionProcess: MonitoredDaemonProcess,
    ...
    private def startFastRebootWorkflow() = executor submit makeRunnable {
        ...
        RebootLog.info("Fast reboot (main agent): waiting on stage1 barrier.")
        barriers.stage1.waitOnBarrier(2, TimeUnit.SECONDS)
        ...
        RebootLog.info("Fast reboot (main agent): remove stage2 barrier, " +
                       "waiting on stage3 barrier.")
        barriers.stage3.waitOnBarrier(2, TimeUnit.SECONDS)

...
case class VpnServiceConfig(script: String,
    ...
    def initConnsCmd = {
        var cmd = new StringBuilder(s"""$script init_conns -n ${vpnService.name} -p ${vpnService.filepath}""")
 ...
 * script.
 */
trait VpnServiceContainerFunctions {
    private final val log = LoggerFactory.getLogger(s"midonet.servcon.vpn")

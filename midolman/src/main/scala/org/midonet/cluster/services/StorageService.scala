...
import org.midonet.cluster.models.Topology.{Host, Rule, Chain, TunnelZone, Port}
...
class StorageService @Inject() (directory: Directory,
    ...
    protected def buildStorage(): Unit = {
        ...
        List(classOf[Port], classOf[Host], classOf[TunnelZone], classOf[Chain],
             classOf[Rule]).foreach(clazz =>

...
import org.midonet.cluster.state.StateStorage.{ReplicatedRouteSet, UntaggedVlanId}
...
class ZookeeperStateStorage @Inject() (backendCfg: MidonetBackendConfig,
    ...
    /**
     * An implementation of a [[ReplicatedRouteSet]].
     */
    @throws[StateAccessException]
    private class ReplicatedRouteTable(routerId: UUID)
        extends ReplicatedRouteSet(
    ...
    override def logSource = "org.midonet.state-storage"
    ...
    /** Ensures that the path for the specified router is created in the legacy
      * storage. */
    @throws[StateAccessException]
    private def ensureRouter(routerId: UUID): Unit = {
        // Create path.
        ...
        if (!zkManager.exists(routerPath)) {
            log.warn("Router {} does not exist in state storage, creating.",
                     routerId)
            createPath(routerPath)
            if (!zkManager.exists(routerArpTablePath)) {
                createPath(routerArpTablePath)
            }
            if (!zkManager.exists(routerRoutingTablePath)) {

...
trait FlowStateRequestClient {
    ...
    def getFlowStateConfig: FlowStateConfig

    protected def initSocket(clientHost: String = "0.0.0.0") = {
        val socket = new Socket(clientHost, getFlowStateConfig.port)
        socket.setSoTimeout(getFlowStateConfig.connectionTimeout)
        socket
    }
 ...
 */
class FlowStateInternalClient(flowStateConfig: FlowStateConfig)
     extends FlowStateRequestClient {

    override def getFlowStateConfig = flowStateConfig
 ...
 */
class FlowStateRemoteClient(flowStateConfig: FlowStateConfig)
    extends FlowStateRequestClient {

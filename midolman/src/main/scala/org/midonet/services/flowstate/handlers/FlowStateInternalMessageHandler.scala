...
/** Handler used to receive, parse and submit flow state messages from agents
  ...
  * garbage collection. */
@Sharable
class FlowStateInternalMessageHandler(session: Session,
                                      flowStateStorageProvider: FlowStateStorageProvider,
                                      config: FlowStateConfig)
    extends SimpleChannelInboundHandler[DatagramPacket] {

    private val legacyPushState = config.legacyPushState
    ...
    @VisibleForTesting
    protected[flowstate] def getLocalStateStorage = flowStateStorageProvider
    ...
    private def writeInLocalStorage(portId: UUID, encoder: SbeEncoder): Unit =
        getLocalStateStorage.get(portId).out.write(encoder)

...
class FlowStateService @Inject()(nodeContext: Context, curator: CuratorFramework,
    ...
    class BlockInvalidator extends Runnable {
        override def run(): Unit = {
            ...
            Log debug "Flow state block invalidator task took " +
                      s"${System.currentTimeMillis - startTime} ms and " +
                      s"invalidated $invalidatedBlocks blocks."
        }
    }
    ...
    private[flowstate] def startBlockInvalidator() = {
        if (config.flowState.localPushState) {
            executor.scheduleAtFixedRate(blockInvalidator,
                                         config.flowState.expirationRate toMillis,
                                        config.flowState.expirationRate toMillis,

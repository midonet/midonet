...
import org.midonet.services.flowstate.stream.{FlowStateInputStream, _}
...
class FlowStateReadHandler(config: FlowStateConfig)
    ...
    private def respondRaw(portId: UUID): Unit = {
        ...
        try {
            val in = ByteBufferBlockInputStream(config, portId)
    ...
    private def respondRemote(portId: UUID, host: String): Unit = {
        try {
            ...
            val out = ByteBufferBlockOutputStream(config, portId)
            tcpClient.rawPipelinedFlowStateFrom(host, config.tcpPort, portId, out)
    ...
    protected[flowstate] def writeFromLocalState(portId: UUID): Unit = {
        val in = FlowStateInputStream(config, portId)
    ...
    private def writeAndFlushWithHeader(data: Array[Byte]): Unit = {
        ctx.write(toBytes(data.size))
        ctx.writeAndFlush(data)
    }

    private def toBytes(value: Int): Array[Byte] =

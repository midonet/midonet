...
object FlowStateStandaloneMinion extends App {
    ...
    private def runReporter = {
        val reporter = new Runnable {
            def run = {
                try {
                    val write = metrics.getTimers().get("writeRate")
                    val compression = metrics.getTimers().get("compressionRate")
                    for ((name, m) <- Seq(("write", write), ("compression", compression))) {
                        val snapshot = m.getSnapshot
                        Log info s"\t$name " +
                                 s"${clock.tick} " +
                                 f"1minrate ${m.getOneMinuteRate}%.2f\t" +
                                 f"5minrate ${m.getFiveMinuteRate}%.2f\t" +

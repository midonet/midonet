...
object FlowStateMinionBenchmark extends App {
    ...
    val defaults = Map('writeRate -> 100000,
                       'readRate -> 0,
                       'numPorts -> 1,
                       'duration -> 100,
                       'messagesPerPort -> 1)
    ...
    private def runBenchmark = {
        ...
        val messagesPerPort = options('messagesPerPort)
        ...
        val messagesMap = mutable.HashMap.empty[UUID, mutable.MutableList[WriteMessage]]
        for (i <- 0 until numports) {
            val ingressPort = ingressPorts(i)
            val messages = mutable.MutableList.empty[WriteMessage]
            for (j <- 0 until messagesPerPort) {
                messages += WriteMessage(validFlowStateInternalMessage(
        ...
        for (i <- 0 until numWrites) {
            ...
            val message = messagesMap.get(port).get(Random.nextInt(messagesPerPort))
            writeThrottler ! message
        }
        ...
        /*val writerThread = new Thread {
            override def run = {
                ...
                for (i <- 0 until numWrites) {
                    // Pick one port at random, one message at random from the list
                    val port = ingressPorts(Random.nextInt(ingressPorts.length))
                    val message = messagesMap.get(port).get(Random.nextInt(messagesPerPort))
                    writeThrottler ! message
                }

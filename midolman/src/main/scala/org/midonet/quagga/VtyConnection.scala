...
object VtyConnection {
    ...
    case class FlatCommand(override val commands: List[String]) extends Command

    case class NestingCommand(open: String, inner: Command = NoOp) extends Command {
        override def commands = open :: inner.commands ::: List("exit")
    }

    val NoOp = FlatCommand(Nil)

    def ConfigureCommand(cmd: Command) = NestingCommand("configure terminal", cmd)

    val SetExecTimeout: Command = {
        ConfigureCommand(
            NestingCommand("line vty",
                FlatCommand(List("exec-timeout 0")))
        )
    }
...
abstract class VtyConnection(val addr: String, val port: Int) extends Closeable {
    ...
    def exec(command: Command): Seq[(String, Seq[String])] = {
        requireConnection()
...
class BgpVtyConnection(addr: String, port: Int) extends VtyConnection(addr, port)
    ...
    private def SetAs(as: Int)(subcommands: List[String]): Command = {
        ConfigureCommand(
            NestingCommand(s"router bgp $as",
                FlatCommand(subcommands)))
    }

    private def SetAs(as: Int, subcommand: String): Command = SetAs(as)(List(subcommand))

...
import java.util.concurrent.{TimeUnit, ThreadFactory, Executors, ScheduledExecutorService}
...
/**
  ...
  */
class ContainerExecutors(config: ContainerConfig, log: Logger) {
    ...
    log debug s"Container service is using $threadCount threads"
    ...
    /**
      ...
      */
    def nextExecutor(): ScheduledExecutorService = {
    ...
    def shutdown(): Unit = {
        ...
        for (executor <- executors) {
            if (!executor.awaitTermination(config.shutdownGraceTime.toMillis,
                ...
                log warn "Shutting down container executor timed-out after " +
                         s"${config.shutdownGraceTime}"
    ...
    /**
      ...
      * executor type.
      */
    protected def newExecutor(index: Int): ScheduledExecutorService = {

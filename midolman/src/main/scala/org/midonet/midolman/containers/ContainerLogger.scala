...
class ContainerLogger(config: ContainerConfig, log: Logger) {
    ...
    def log(`type`: String, config: ContainerConfiguration): Unit = sync.synchronized {
        // Validate the input.
        validate(`type`, "type")
        validate(config.id, "identifier")
        validate(config.config, "configuration")
    ...
    /**
      * Validates that the input item can be logged.
      */
    @throws[IllegalArgumentException]
    private def validate(item: String, name: String): Unit = {
        if (item.contains('|') || item.contains('\n') || item.contains('\r') ||
            item.contains('\f') ) {
            throw new IllegalArgumentException(
                s"Container $name '$item' contains an invalid character " +
                "(|, CR, LF or FF)")
        }
    }

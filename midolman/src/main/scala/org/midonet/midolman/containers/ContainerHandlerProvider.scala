...
/**
  * Scans the current classpath of the package prefix for service container
  * handlers.
  */
class ContainerHandlerProvider(prefix: String, vt: VirtualTopology,
    ...
    /** Returns all container classes that have been loaded by the provider,
      * included those of which there are multiple versions for the same
      * container name.
      */
    def all = allContainers

    /** Returns the current set of container classes. If for the same container
      * name there are multiple classes with different version, this set

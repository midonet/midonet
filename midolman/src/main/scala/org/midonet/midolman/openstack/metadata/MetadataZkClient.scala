...
case class MetadataAddressInfo(
    val vmAddr: String,
    val portId: UUID,
    val tenantId: String,
    val instanceId: String)

class MetadataZkClient(val store: ReadOnlyStorage) {
    private val log: Logger = LoggerFactory.getLogger(classOf[MetadataZkClient])

    def getComputePortInfo(portId: UUID): Option[MetadataAddressInfo] = {
        val port = try {

...
object MetadataProxy {
    private val log: Logger = LoggerFactory.getLogger(MetadataProxy.getClass)
    private val ip = InetAddress.getByName("169.254.169.254")
    private val port = 9697
    private var server: Server = _

    def start(store: ReadOnlyStorage) = {
        val sa = new InetSocketAddress(ip, port)
        log.info(s"Starting metadata proxy on ${sa}")
        val s = new Server(sa)
        s.setHandler(new MetadataProxyHandler(store))
        try {
            s.start
            server = s
        } catch {

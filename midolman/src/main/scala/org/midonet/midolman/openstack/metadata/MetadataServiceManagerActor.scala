...
class MetadataServiceManagerActor @Inject() (
    ...
    override def preStart() {
        ...
        MetadataServiceWorkflow.mdInfo = mdInfo  // XXX
        Proxy start config
    }
    ...
    override def receive = {
        case LocalPortActive(portId, true) =>
            ...
            /*
             * XXX While unlikely, this can be race with metadata requests
             * from VM.  If we lose the race, the requests end up with
             * UnknownRemoteAddressException.
             */
            // XXX should avoid synchronous calls

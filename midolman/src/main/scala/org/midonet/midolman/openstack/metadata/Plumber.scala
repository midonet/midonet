...
class Plumber(val injector: Injector) {
    ...
    def calculateRemoteAddr(dpPortNo: Int) = {
        // XXX skip 169.254.169.254
        val hi = (dpPortNo >> 8) & 0xff
        val lo = dpPortNo & 0xff
        s"169.254.${hi}.${lo}"
    }
    ...
    def write(keys: ArrayList[FlowKey], actions: ArrayList[FlowAction]) = {
        // XXX we roll our own as calculateFor doesn't take care of TcpFlags
    ...
    def unplumb(vmInfo: InstanceInfo, mdInfo: ProxyInfo) = {
        val vmDpPortNo =
            writer.dpState getDpPortNumberForVport vmInfo.portId
        val addr = calculateRemoteAddr(vmDpPortNo)
        // XXX implement

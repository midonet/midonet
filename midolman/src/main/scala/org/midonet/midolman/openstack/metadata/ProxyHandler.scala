...
class ProxyHandler extends AbstractHandler {
    private val log: Logger = LoggerFactory getLogger classOf[ProxyHandler]

    def handle(target: String, baseReq: Request, req: HttpServletRequest,
               res: HttpServletResponse) = {
        baseReq setHandled true
        try {
            val result = NovaMetadataClient.get_metadata(req getPathInfo,
                                                         req getRemoteAddr)
            res.getWriter print result
        } catch {
            case e: UniformInterfaceException =>
                log error s"Unexpected HTTP response: ${e}"

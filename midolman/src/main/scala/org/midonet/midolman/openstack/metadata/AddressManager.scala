...
object AddressManager {
    def dpPortToRemoteAddress(dpPortNo: Int) = {
        require(dpPortNo >= 4)
        require(dpPortNo <= 65534)
        val x =
            if (dpPortNo <= 43520)
                dpPortNo - 3
            else
    ...
    def remoteAddressToDpPort(addr: String) = {
        require(addr != "169.254.169.254")
        val n = IPv4Addr.stringToInt(addr)
        require(n >= (169 << 24) + (254 << 16) + (0 << 8) + 1)
        require(n <= (169 << 24) + (254 << 16) + (255 << 8) + 252)
        val hi = (n >> 8) & 0xff
        val lo = n & 0xff
        val x = (hi << 8) + lo
        val dpPortNo =

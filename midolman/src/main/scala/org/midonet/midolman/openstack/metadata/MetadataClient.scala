...
object MetadataClient {
    private val log: Logger = LoggerFactory.getLogger(MetadataClient.getClass)
    ...
    def get_metadata(path: String, remoteAddr: String,
        ...
        val vmAddr = "1.0.0.3"  // XXX consult VM IP mappings
        val portId = "f1fd2a83-6590-431b-a554-9f89caf48109" // XXX
        val zk = new MetadataZkClient(store)
        val (tenantId, instanceId) = zk.getPortInfo(portId)
        log.info(s"Port-Id: ${portId}, Tenant-Id: ${tenantId}, Instance-Id: ${instanceId}")
        val instanceIdSig = sign_instance_id(instanceId)

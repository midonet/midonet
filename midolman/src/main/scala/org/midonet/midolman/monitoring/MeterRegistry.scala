...
class MeterRegistry(val maxFlows: Int) {
    ...
    private val metadataPool = new ArrayObjectPool[FlowData](maxFlows) {
        override def allocate = new FlowData()
    }

    @volatile var meters: Map[String, FlowStats] = Map.empty
    private val trackedFlows = new JHashMap[FlowMatch, FlowData]()
    private val DELTA = new FlowStats()

    def trackFlow(flowMatch: FlowMatch, tags: Array[FlowTag]) = {
        val metadata = metadataPool.take.getOrElse(metadataPool.allocate)
        metadata.reset()
            ...
            tags(i) match {
                case meter: MeterTag =>
                    ...
                    if (meters.contains(meter.meterName)) {
                        ...
                        meters += meter.meterName -> new FlowStats()

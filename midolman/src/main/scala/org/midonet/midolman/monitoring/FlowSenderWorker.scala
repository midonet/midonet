...
import org.midonet.cluster.services.discovery.{MidonetDiscovery, MidonetDiscoveryClient, MidonetServiceHostAndPort}
...
class DisruptorFlowSenderWorker(config: FlowHistoryConfig,
    ...
    private val executor = Executors.newSingleThreadExecutor(
        new NamedThreadFactory("disruptor-flow-sender-worker", true))
    ...
    private val flowSender = new FlowSender(config, backend.discovery)
...
class FlowSender(config: FlowHistoryConfig, discovery: MidonetDiscovery)
    ...
    override def doStart(): Unit = {
        clioDiscoveryClient =
            discovery.getClient[MidonetServiceHostAndPort](

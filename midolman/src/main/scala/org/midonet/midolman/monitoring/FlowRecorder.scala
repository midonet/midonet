...
object FlowRecorder {
    final val TargetServiceName = "clio"
...
abstract class AbstractFlowRecorder(config: FlowHistoryConfig,
    ...
    val clioDiscoveryClient = discovery.getClient[MidonetServiceHostAndPort](
        FlowRecorder.TargetServiceName)

    val _endpoint = new AtomicReference[Option[InetSocketAddress]](None)

    def endpoint: Option[InetSocketAddress] = _endpoint.get
    ...
    clioDiscoveryClient.observable.subscribe(
        new Observer[Seq[MidonetServiceHostAndPort]] {
            override def onCompleted(): Unit = {
                log.debug("Service discovery completed for clio")
                _endpoint.lazySet(None)
            }

            override def onError(e: Throwable): Unit = {
                log.error("Error on clio service discovery")
                _endpoint.lazySet(None)
            }
            ...
            override def onNext(t: Seq[MidonetServiceHostAndPort]): Unit = {
                val chosenEndpoint =
                    if (t.nonEmpty) {
                        ...
                        None
                _endpoint.lazySet(chosenEndpoint)
                log.debug("")
            }
        }
    )
    ...
    final override def record(pktContext: PacketContext, simRes: MMSimRes):
            Unit = {
        try {
            endpoint.foreach { currentEndpoint =>
                val buffer = encodeRecord(pktContext: PacketContext, simRes)
                socket.send(buffer, currentEndpoint)
            }
        } catch {

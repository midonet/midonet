...
class WatchedProcess {
    ...
    def start(initializationWitness: Promise[_], timeout: Int): Unit = sync.synchronized {
        if (running)
            return
        if (!readConfig())
            return
        ...
        timer.schedule(initializationTimeout(initializationWitness),
                       timeout * 1000)
    ...
    def close(): Unit = sync.synchronized {

...
import org.midonet.odp.{OpenVSwitch, OvsNetlinkFamilies, OvsProtocol, FlowMatch}
...
sealed class FlowProcessor(flowEjector: FlowEjector,
    ...
    private def debug(buf: ByteBuffer): Unit = {
        log.debug(s"Netlink length = ${buf.getInt(NetlinkMessage.NLMSG_LEN_OFFSET)}")
        log.debug(s"Netlink type = ${buf.getShort(NetlinkMessage.NLMSG_TYPE_OFFSET)}")
        log.debug(s"Netlink flags = ${buf.getShort(NetlinkMessage.NLMSG_FLAGS_OFFSET)}")
        log.debug(s"Netlink seq = ${buf.getShort(NetlinkMessage.NLMSG_SEQ_OFFSET)}")
        log.debug(s"Netlink pid = ${buf.getShort(NetlinkMessage.NLMSG_PID_OFFSET)}")
        log.debug(s"Netlink cmd = ${buf.getShort(NetlinkMessage.GENL_CMD_OFFSET)}")
        buf.position(NetlinkMessage.GENL_HEADER_SIZE)
        log.debug(s"Datapath id ${buf.getInt()}")
        debugAttrs(buf, OpenVSwitch.Flow.Attr.Key, FlowMatch.reader)
        debugAttrs(buf, OpenVSwitch.Flow.Attr.Actions, FlowActions.reader)
    }

    private def debugAttrs[T](bb: ByteBuffer, key: Short,
                              reader: Reader[T]) = {
        ...
        val limit = bb.limit()
        bb.limit(next)
        log.debug(reader.deserializeFrom(bb).toString)
        bb.position(next)
        bb.limit(limit)
    }
}

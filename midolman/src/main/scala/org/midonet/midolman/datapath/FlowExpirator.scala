...
class FlowExpirator(val flows: NativeFlowMatchList, config: MidolmanConfig,
    ...
    def expireAllFlows(): Unit = {
        ...
        try {
            ...
            val elapsed = TimeUnit.NANOSECONDS.toSeconds(init - System.nanoTime())
            val rate = numFlows / elapsed
            channel.close()
            flows.delete()
            log.info(s"Finished deleting flows from $datapath in ${elapsed}s " +
                     s"at $rate flows per second}")
        }
    }

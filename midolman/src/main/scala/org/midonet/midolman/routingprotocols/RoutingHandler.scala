    ...
    def apply(rport: RouterPort, bgpIdx: Int,
        ...
        new RoutingHandler(rport, bgpIdx, flowInvalidator.tell,
            ...
            override protected val bgpd = new DefaultBgpdProcess(
                rport.id, bgpIdx, bgpVtyLocalIp,
                bgpVtyPeerIp, new IPv4Subnet(rport.portAddress,
                                             rport.portSubnet.getPrefixLen),
            ...
            override def preStart(): Unit = {
                log.info(s"Starting routing handler for port ${rport.id}:$bgpIdx")
                super.preStart()
            ...
            override def postStop(): Unit = {
                log.info(s"Stopping routing handler for port ${rport.id}:$bgpIdx")
                super.postStop()
                if (portBgpSub != null)
...
abstract class RoutingHandler(var rport: RouterPort, val bgpIdx: Int,
    ...
    override def logMark = s"bgp:${rport.id}:$bgpIdx"

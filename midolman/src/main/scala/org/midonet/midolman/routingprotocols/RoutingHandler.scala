...
class RoutingHandler(var rport: RouterPort, val bgpIdx: Int,
        ...
        case RemoveBgpSession(bgpID) =>
            phase match {
                ...
                case Started if bgps.contains(bgpID) =>
                    ...
                    RoutingWorkflow.routerPortToBgp -= bgp.getPortId
                    RoutingWorkflow.inputPortToBgp -= bgp.getQuaggaPortNumber
                    invalidateFlows(bgp)
    ...
    private def netdevPortReady(newPort: NetDevPort, uplinkPid: Int) {
        ...
        RoutingWorkflow.routerPortToBgp += bgp.getPortId -> bgp
        RoutingWorkflow.inputPortToBgp += bgp.getQuaggaPortNumber -> bgp
        invalidateFlows(bgp)

...
abstract class RoutingHandler(var rport: RouterPort, val bgpIdx: Int,
    ...
    def bgpdShouldRun = {
        val shouldRun = zookeeperConnected && portActive && updatesActive &&
                        bgpConfig.neighbors.nonEmpty
        log.debug("BGP should run {} ({} {} {} {} {})", Boolean.box(shouldRun),
                  Boolean.box(zookeeperConnected), Boolean.box(portActive),
                  Boolean.box(updatesActive), bgpConfig.neighbors,
                  Int.box(System.identityHashCode(bgpConfig)))
        shouldRun
    }
    ...
    private val eventHandlerV2: PartialFunction[Any, Future[_]] = eventHandlerBase orElse {
        case BgpPort(port, router, neighborIds) =>
            ...
            onPortUpdated.flatMap[Any] { _ =>
                log.info("BGP configuration changed")
                Try(update(router, neighborIds)).getOrElse(checkBgpdHealth())

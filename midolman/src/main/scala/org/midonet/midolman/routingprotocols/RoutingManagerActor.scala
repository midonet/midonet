...
class RoutingManagerActor extends ReactiveActor[AnyRef]
    ...
    private def stopping: Actor.Receive = {
        case RoutingHandlerStopped =>
            ...
            checkZkConnection()
            if (bgpActorCount == 0) {
                log.debug("XLDEBUG ALL BGP routing handlers stopped. " +
    ...
    private def stopAllHandlers(): Unit = {
        ...
        log.debug("XLDEBUG stopAllHandlers sending done! to actor service")

    ...
    private def cleanStalePorts(): Unit = {
        VirtualToPhysicalMapper.allActivePorts
            ...
            case Success((vtpmPorts, dpPorts)) =>
                ...
                // Note: we are only interested in removing ports related to VMs
                // that have been killed while midolman was down. VM ports
                // are of type NetDev and their name has UUID format. We also
                // check that the ports do not currently belong to current
                // instances.
                // TODO: we should verify that port name format does not change.
                dpPorts
                    .withFilter(_.getType == DpPort.Type.NetDev)
                    .withFilter(p => !knownPorts.contains(p.getPortNo))
                    .withFilter(p => Try(UUID.fromString(p.getName)).isSuccess)
                    .foreach { p =>

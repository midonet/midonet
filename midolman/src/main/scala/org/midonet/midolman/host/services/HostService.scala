...
class HostService @Inject()(config: MidolmanConfig,
    ...
    private final val storageTimeout = 5 seconds
    ...
    private val ownerObserver = new Observer[Set[String]] {
        ...
        override def onNext(o: Set[String]): Unit = o.headOption match {
            case Some(id) =>
                ...
                store.get(classOf[Host], hostId) onComplete {
                    case Success(host) if host.getName != hostName =>
                        recreateHostInV2OrShutdown()
                    case Success(_) => // Do nothing
                    case Failure(e) => recreateHostInV2OrShutdown()
                }
            case None =>
                recreateHostInV2OrShutdown()
        }
    ...
    private def getCurrentHost: Host = {
        if (store.exists(classOf[Host], hostId).await(storageTimeout)) {
            store.get(classOf[Host], hostId).await(storageTimeout)
        } else null
    }

...
class HostService @Inject()(mmConfig: MidolmanConfig,
    ...
    extends AbstractService with HostIdProviderService
    ...
    override def doStop(): Unit = {
        ...
        // disappear.
        if (mmConfig.isLegacyStorageEnabled) {
        ...
        if (mmConfig.isClusterStorageEnabled) {
            try {
                storage.deleteOwner(classOf[Host], hostId, hostId.toString)
            } catch {
    ...
    private def createCluster(id: UUID, metadata: HostMetadata): Boolean = {
        if (mmConfig.isClusterStorageEnabled) {
            try {
                ...
                if (Await.result(storage.exists(classOf[Host], id), timeout)) {
                    // Read the current host.
                    val currentHost =
                        Await.result(storage.get(classOf[Host], id), timeout)
                ...
                case e @ (_: NotFoundException |
                          ...
                          _: OwnershipConflictException) => return false
            }
        }
        true
    }

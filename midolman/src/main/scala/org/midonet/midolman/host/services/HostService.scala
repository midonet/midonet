...
import org.midonet.cluster.models.Topology.{HostInterfaces, Host}
import org.midonet.cluster.models.Topology.HostInterfaces.Interface
...
class HostService @Inject()(config: MidolmanConfig,
    ...
    private def createInV2(): Boolean = {
        ...
        try {
            ...
            Await.result(stateStore.addValue(classOf[Host], hostId, AliveKey,
                                             AliveKey), timeout)
            updateInterfacesInV2()
    ...
    private def updateInterfacesInV2(): Unit = {
        ...
        try {
            Await.result(stateStore.addValue(classOf[Host], hostId,
                                             InterfacesKey, getInterfaces),
                         timeout)
        } catch {
            case NonFatal(e) =>
    ...
    private def getInterfaces: String = {
        HostInterfaces.newBuilder()
            .setHostId(UUIDUtil.toProto(hostId))
            .addAllInterfaces(
                currentInterfaces.map(
                    ZoomConvert.toProto(_, classOf[Interface])).asJava)
            .build()
            .toString
    }

...
class DefaultInterfaceScanner(channelFactory: NetlinkChannelFactory,
    ...
    private val ovsNotificationChannel: NetlinkChannel =
        channelFactory.create(blocking = true, NetlinkProtocol.NETLINK_GENERIC,
            ...
            case OpenVSwitch.Type.OVS_PORT =>
                ...
                if (ver == OpenVSwitch.Port.version &&
                    ...
                    val pairOption: Option[(Int, Link)] = links.find {
                        ...
                        case _ =>
                            log.debug(s"Couldn't find the interface $ifname")
                            false
                    }
                    pairOption match {
                        case Some(_) =>
                            ...
                            Observable.just(filteredIfDescSet)
                        case None =>
                            log.debug(s"The endpoint of $ifname has been " +
                                "already updated. So did nothing")
                            Observable.empty[Set[InterfaceDescription]]
                    }
                } else {
                    Observable.empty[Set[InterfaceDescription]]

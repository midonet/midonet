...
class DefaultInterfaceScanner(channelFactory: NetlinkChannelFactory,
    ...
    private val addrs = mutable.Map.empty[Int, Addr]
            ...
            nlType match {
                ...
                case Rtnetlink.Type.NEWADDR =>
                    ...
                    addrs.get(addr.ifa.index) match {
                        case Some(previous: Addr) if addr == previous =>
                            Observable.empty[Set[InterfaceDescription]]
                        case _ =>
    ...
    override def start(): Unit = {
        ...
        val linkSubject = PublishSubject.create[Set[Link]]()
        val addrSubject = PublishSubject.create[Set[Addr]]()
        val obs = Observable.zip[Set[Link], Set[Addr], Set[InterfaceDescription]](
            linkSubject, addrSubject, makeFunc2((links, addrs) => {
                log.debug("Composing the initial state from the retrieved data")
                composeIfDesc(links, addrs)
            }))
        obs.subscribe(ResourceObserver { ifDescs: Set[InterfaceDescription] =>
            log.debug("Composed the initial interface descriptions: ", ifDescs)
        })

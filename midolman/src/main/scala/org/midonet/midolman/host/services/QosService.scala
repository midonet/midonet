...
object QosService {
    ...
    def apply(scanner: InterfaceScanner,
        ...
        val requestHandler = new TcRequestHandler()
        apply(scanner, hostId, requestHandler)
    }
}
...
class QosService(scanner: InterfaceScanner,
    ...
    val boundPortsObvrs = new m.HashMap[UUID, Subscription]()
    val ifaceNameToId = new m.HashMap[String, Int]()
    val portIdToPort = new m.HashMap[UUID, Port]()
    val currentConfs = new m.HashSet[TcConf]()

    def generateRequests(
        newInterfaceSet: Option[Set[InterfaceDescription]] = None): Unit = {

        newInterfaceSet match {
            case Some(data) =>
                // Update the iface set here so that the read ops are protected
                ifaceNameToId.clear()
                data.foreach(ifd => ifaceNameToId.put(ifd.getName, ifd.getIfindex))
            case None =>
        }

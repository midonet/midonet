...
class QosService(scanner: InterfaceScanner,
    ...
    /* Make this a list so that we can preservce order. This helps
     * in making unit tests deterministic, but it not otherwise required
     * for the feature. */
    var currentConfs = List[TcConf]()
    ...
    def generateRequests(): Unit = {
        ...
        currentConfs = (currentConfs diff removeConfs) ::: addConfs
    }

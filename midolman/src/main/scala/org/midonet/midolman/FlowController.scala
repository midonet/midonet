...
object FlowController extends Referenceable {
    ...
    sealed class FlowDeleteCommand(completedRequests: SpscArrayQueue[FlowDeleteCommand])
                                  (implicit actorSystem: ActorSystem)
        extends FlowOvsCommand[FlowDeleteCommand](completedRequests) {
...
class FlowController extends Actor with ActorLogWithoutPath
    ...
    var pooledFlowDeleteCommands: ArrayObjectPool[FlowDeleteCommand] = _
    var completedFlowDeleteCommands: SpscArrayQueue[FlowDeleteCommand] = _
    ...
    override def preStart() {
        ...
        pooledFlowDeleteCommands = new ArrayObjectPool(ejector.maxPendingRequests,
                                                       _ => new FlowDeleteCommand(completedFlowDeleteCommands))
    }
    ...
    private def processRemovedFlows(): Unit = {
        var req: FlowDeleteCommand = null
        while ({req = pooledFlowDeleteCommands.take; req } ne null) {
            if (req.isFailed) {
    ...
    private def flowDeleteFailed(req: FlowDeleteCommand): Unit = {
    ...
    private def scheduleRetry(req: FlowDeleteCommand): Unit = {
    ...
    private def notifyRemoval(req: FlowDeleteCommand): Unit = {
        log.debug(s"DP confirmed removal of flow with match ${req.flowMatch}")
        req.clear()
    }

    private def giveUp(req: FlowDeleteCommand): Unit = {
        log.error(s"Failed to delete flow with match: ${req.flowMatch}", req.failure)
        req.clear()
    }

    sealed class FlowManagerInfoImpl() extends FlowManagerHelper {

        val awaitCompletedCommands = new Parkable {

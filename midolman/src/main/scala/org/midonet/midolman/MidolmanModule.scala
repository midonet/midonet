...
class MidolmanModule(injector: Injector,
    ...
    protected def ruleLogEventChannel(capcity: Int): RuleLogEventChannel = {
        DisruptorRuleLogEventChannel(capcity)
    }
    ...
    protected def virtualTopology(simBackChannel: SimulationBackChannel) = {
        ...
        new VirtualTopology(
            ...
            config,
            simBackChannel,
            ruleLogEventChannel(4096), // TODO: Make capacity configurable
            metricRegistry,
            vtExecutor,
            ioExecutor,
            vtExecutorCheck

...
import org.midonet.midolman.logging.rule.{DisruptorRuleLogEventChannel, RuleLogEventChannel, RuleLogEventHandler}
...
class MidolmanModule(injector: Injector,
    ...
    protected def ruleLogEventChannel(): RuleLogEventChannel = {
        ...
        val ringBuffer = RingBuffer
            .createMultiProducer(DisruptorRuleLogEventChannel.Factory, 256)
        val barrier = ringBuffer.newBarrier()
        val eventHandler = new RuleLogEventHandler()
        val batchProcessor =

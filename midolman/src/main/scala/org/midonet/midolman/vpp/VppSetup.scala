...
/**
  ...
  */
class Fip6Setup(floatingIp: IPv6Addr,
                fixedIp: IPv4Addr,
                vppApi: VppApi,
                fip64conf: Fip64Config,
                log: Logger,
                ...
                vrf: Int = -1, vni: Int = -1)
               (implicit ec: ExecutionContext)
    extends VppSetup(s"Fip6 setup", log)(ec) {

    import VppSetup.{VppAddRoute, VppAddFip64}

    val operation = if (vrf != -1) "Add" else "Delete"
    val fip6 = new VppAddFip64(s"$operation fip6 $floatingIp <-> $fixedIp",
                               floatingIp, fixedIp,
                               poolStart,
                               poolEnd,
                               vrf, vni, vppApi)
    val routeToVxlan = new VppAddRoute(s"$operation route to vxlan for $fixedIp",

...
trait UnderlayTrafficHandler { this: PacketWorkflow =>
    ...
    private var toPortActionsCache: Map[Int, List[FlowAction]] = Map[Int, List[FlowAction]]()
    ...
    private def outputToPort(port: DpPort) = {
        if (! toPortActionsCache.contains(port.getPortNo))
            toPortActionsCache += (port.getPortNo.toInt -> List[FlowAction](port.toOutputAction))

        toPortActionsCache(port.getPortNo)
    }
    ...
    private def wildflowForTunnelPacket(context: PacketContext,
        ...
        WildcardFlow(
            ...
            actions =  outputToPort(forwardTo),

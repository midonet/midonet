...
class PacketWorkflow(protected val dpState: DatapathState,
    ...
    def processSimulationResult(context: PacketContext,
        ...
        def calculateOutputPorts(actions: List[FlowAction]) {
            context.outPorts.clear()
            var actionList: List[FlowAction] = actions
            while (actionList.nonEmpty) {
                actionList.head match {
                    case FlowActionOutputToVrnPort(port) =>
                        context.outPorts.append(port)
                    case FlowActionOutputToVrnBridge(_, ports) =>
                        context.outPorts.appendAll(ports)
                    case _ => // do nothing
                }
        ...
        result match {
            case AddVirtualWildcardFlow(flow) =>
                calculateOutputPorts(flow.actions)
                addVirtualWildcardFlow(context, flow)
            case SendPacket(actions) =>
                calculateOutputPorts(actions)
                context.runFlowRemovedCallbacks()

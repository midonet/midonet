...
abstract class AllPacketWorkersService extends AbstractService {
    def workers: IndexedSeq[PacketWorker]
}

class AllPacketWorkersServiceImpl(config: MidolmanConfig,
        ...
        extends AllPacketWorkersService with Runnable with MidolmanLogging {

    override def logSource = "org.midonet.packet-worker-supervisor"
    ...
    val natLeaser: NatLeaser = new NatLeaser {
        ...
        val clock = AllPacketWorkersServiceImpl.this.clock
    }
    ...
    val workers: IndexedSeq[DisruptorPacketWorker] =
        0 until NUM_WORKERS map createWorker
    ...
    protected def createWorker(index: Int): DisruptorPacketWorker = {
        ...
        val capacity = Util.findNextPositivePowerOfTwo(
            config.datapath.globalIncomingBurstCapacity) // TODO should probably create a different config here
        val ringBuffer = RingBuffer
            .createSingleProducer(PacketWorkflow.PacketRefFactory, capacity,
                                  new TimeoutBlockingWaitStrategy(50, TimeUnit.MICROSECONDS))
        ...
        new DisruptorPacketWorker(ringBuffer, workflow, metrics, index)
    }
}

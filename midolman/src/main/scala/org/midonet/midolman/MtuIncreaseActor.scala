 ...
 * port is not a materialized bridge port, it returns a None-filled Future
 * instance and does nothing. Or it returns the Future contains a Some-wrapped
 * returned status code of the ip command, otherwise the exception thrown by the
 * command.
 */
class MtuIncreaseActor extends PortActivationStateMachine {
    ...
    private def increaseMtu(vPortId: UUID,
                            ifDesc: InterfaceDescription): Future[_] = {
        val itfName: String = ifDesc.getName
        val vPortFuture: Future[Port] =
            Try(VirtualTopologyActor.tryAsk[Port](vPortId)) match {
                case Success(vPort) =>
                    Future.successful(vPort)
    ...
    override def handlePortActivation(msg: LocalPortActive): Future[_] = {
        val increaseMtuWithIfDescOption: Option[Future[Int]] = for {
            ...
            vPortId <- dpState.getVportForDpPortNumber(portNo)
        } yield increaseMtu(vPortId, ifDesc) flatMap {
                case Some(0) =>
                ...
                case t: Throwable =>
                    log.warn(s"Failed to increase the MTU of $itfName", t)
                    Future.failed(t)
                case _ =>
                    log.debug(s"Didn't increase the MTU of $itfName")
                    Future.successful(-1)
            }
        increaseMtuWithIfDescOption.getOrElse {
            log.debug("Didn't increase the MTU of the interface associated " +
                s"with the port ${msg.portID}")
            Future.successful(-1)
        }
    }

...
class VppController @Inject() (config: MidolmanConfig)
    ...
    private def handlePortActive(portId: UUID): Unit = {
        try {
            ...
            port match {
                case routerPort: RouterPort if isIPv6(routerPort) =>
                    log debug s"IPv6 router port attached: $routerPort"
                    attachPort(portId, routerPort)
                case _ =>
                    log debug s"Port not IPv6 router port: $portId"
            }
        } catch {
            case NotYetException(future, msg) =>
                log debug s"Port $portId not yet available: $msg"
                future onComplete {
                    case Success(_) => handlePortActive(portId)
                    case Failure(err) =>
                        log warn s"Failed to read port $portId: $err"
                }
            case NonFatal(err) =>
    ...
    private def dettachPort(portId: UUID): Unit = {

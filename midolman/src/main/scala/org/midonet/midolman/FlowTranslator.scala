    ...
    def outputActionsToPeer(key: Long, peer: UUID,
        ...
        if (routeInfo.isEmpty) {
            ...
            val tunnelTOS = if (config.datapath.setTosOnTunnelHeader) {
                ...
                // masking the ECN bits, as the tunnelling protocol (UDP)
                // doesn't handle congestion. Congestion should be handled
                // by the end to end transport layer of the inner flow.
                context.wcmatch
                    .setTunnelTOS((context.wcmatch.getNetworkTOS & 0xfc).toByte)
                    .getTunnelTOS
            } else {

...
object FlowTranslator {
    ...
    val ALL_ZEROS_MAC = MAC.fromAddress(new Array(6))
...
trait FlowTranslator {
        ...
        while (i < actionsCount) {
            virtualActions.get(i) match {
                ...
                case Encap(vni) =>
                    ...
                    addActionsToPrepareOuterPacket(context, i + 1, addActionAfterEncap)
                    addFlowAndPacketAction = addActionAfterEncap
                case Decap(vni) =>
    ...
    private val addActionAfterEncap: AddFlowAction = (c: PacketContext, a: FlowAction) =>
        c.flowActions.add(a)

    private def uniquifyEncap(key: FlowKeyTunnel) = {
        // TOOD(duarte): can also safely use the tunnel flags?
        key.ipv4_tos = encapUniquifier.toByte
        key.ipv4_ttl = (encapUniquifier >>> 8).toByte
        encapUniquifier += numWorkers
    }

    private def uniquifyEncap(key: FlowKeyIPv4) = {
        key.ipv4_tos = decapUniquifier.toByte
        key.ipv4_ttl = (decapUniquifier >> 4).toByte
    ...
    private def recircInnerPacket(
        ...
        val innerFlowKey = FlowKeys.tunnel(
            ...
            config.datapath.recircHostCidr.toNetworkAddress.toInt,
            0)
    ...
    private def matchOnOuterPacket(fmatch: FlowMatch, tunnelFk: FlowKeyTunnel): Unit = {
        ...
        fmatch.addKey(FlowKeys.ethernet(ALL_ZEROS_MAC, ALL_ZEROS_MAC))

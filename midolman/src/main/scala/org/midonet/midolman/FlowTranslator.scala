...
trait FlowTranslator {
    ...
    def outputActionsToPeer(key: Long, peer: UUID,
                            context: PacketContext): Unit = {
        context.log.debug(s"Emitting towards remote host $peer with tunnek key $key")
        ...
        if (routeInfo.isEmpty) {
            ...
            // FlowActionOutput.
            context.addFlowAction(setKey(FlowKeys.tunnel(key, src, dst)))
    ...
    private def outputActionsToVtep(vni: Int, vtepIp: IPv4Addr, tzId: UUID,
        ...
        context.addFlowAction(setKey(FlowKeys.tunnel(vni.toLong, localIp, vtepIntIp)))
        ...
        def addVtepActions(br: Bridge): Unit =
            if (br.vxlanPortId.isDefined && br.vxlanPortId.get != context.inputPort) {
                ...
                tryAsk[Port](vxlanPortId) match {
                    case p: VxLanPort =>
                        outputActionsToVtep(p.vni, p.vtepTunAddr, p.tunnelZoneId, context)
                    case _ =>
    ...
    private def expandPortAction(port: UUID, context: PacketContext): Unit =
        dpState.getDpPortNumberForVport(port) map { portNum =>
            ...
            tryAsk[Port](port) match {
                case p: VxLanPort =>
                    outputActionsToVtep(p.vni, p.vtepTunAddr, p.tunnelZoneId,
                                        context)

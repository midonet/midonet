...
import org.midonet.util.logging.{DailyRollingStaticHeaderBufferedOutputStream, RollingOutputStream, SizeBasedRollingStaticHeaderBufferedOutputStream, TimeBasedRollingStaticHeaderBufferedOutputStream}
            ...
            os = parseRotationFrequency(config.rotationFrequency) match {
                case Left(dur) =>
                    new TimeBasedRollingStaticHeaderBufferedOutputStream(
                        logPath, config.maxFiles, header, dur.toSeconds)
                case Right(size) =>
                    new SizeBasedRollingStaticHeaderBufferedOutputStream(
                        logPath, config.maxFiles, header, size)
            }
        } catch {
            case NonFatal(ex) =>

...
private[topology] class VxLanPortService(vt: VirtualTopology)
    ...
    private def vtepUpdated(vtep: Vtep): Vtep = {
        ...
        val prevNetworksIt = prevBoundNetworks.iterator
        while (prevNetworksIt.hasNext) {
            val netId = prevNetworksIt.next

            if (!newBoundNetworks.contains(netId)) {
                val index =
        ...
        val newtNetworksIt = newBoundNetworks.iterator
        while (newtNetworksIt.hasNext) {
            val netId = newtNetworksIt.next

            if (!prevBoundNetworks.contains(netId)) {

...
object Host {
    def toNewHost(oldHost: OldHost): Host = {
        val newHost = new Host
        newHost.id = oldHost.id
        newHost.portToInterface = oldHost.ports
        newHost.tunnelZoneIds = oldHost.zones.keySet
        newHost.tunnelZones = oldHost.zones.map(idHostConfig =>
            (idHostConfig._1, idHostConfig._2.getIp)
        )
        newHost.alive = oldHost.alive
        newHost
    }
}

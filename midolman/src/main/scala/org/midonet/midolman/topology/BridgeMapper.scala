...
object BridgeMapper {
    ...
    private class PortState[D >: Null <: Port](val portId: UUID)
        ...
        def :==(portState: PortState[_ <: Port]): PortState[_ <: Port] = {
...
final class BridgeMapper(bridgeId: UUID, implicit val vt: VirtualTopology)
    ...
    private val macUpdatesSubscription = Observable
        ...
        .doOnSubscribe(makeAction0 { log.debug("Subscribing")})
     ...
     *
     * +---------+    +------------+
     * | Storage |--->| bridgeNext |---> Observable[TopologyBridge]
     * +---------+    +------------+
     *                       |
     *          Add: portsSubject onNext portObservable
     *          Remove: portObservable complete()
     */
    private def onBridge(br: TopologyBridge): Unit = {
        assertThread()
    ...
    private def updateLocalPort(port: Port): Unit = {
        ...
        if (null != port.peerId && !peerPorts.contains(port.peerId)) {
            ...
            val previousPeer = portState :== peerPortState
            if ((previousPeer ne null) && !peerPorts.contains(previousPeer.portId)){
    ...
    private def mapDevice(br: TopologyBridge): SimulationBridge = {
        log.debug("Refreshing state for bridge {}", bridgeId)
        assertThread()
        ...
        if (vt.config.getMidolmanBridgeArpEnabled && (ipv4MacMap eq null)) {
            try {
                ...
                case e: StateAccessException =>
                    log.warn("Error retrieving ARP table for bridge {}",
                             bridgeId)

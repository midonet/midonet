...
final class BridgeMapper(bridgeId: UUID, implicit val vt: VirtualTopology)
    ...
    /**
     * Processes updates from the topology bridge observable. This examines any
     * the addition/removal of the bridge ports, and adds/removes the
     * corresponding port observables.
     *
     *                  +-----------------------+
     ...
     * for local ports triggers an update of the corresponding peer port state.

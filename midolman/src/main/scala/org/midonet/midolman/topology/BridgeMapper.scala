...
object BridgeMapper {
     ...
     */
    private[topology] class MacTableMergedMap(vt: VirtualTopology,
                                              bridgeId: UUID,
                                              vlanId: Short)
        ...
        val map = vt.backend.stateTableStore.bridgeMacTable(bridgeId, vlanId)
    ...
    private class ArpTableMergedMap(vt: VirtualTopology, bridgeId: UUID)
        ...
        private val map = vt.backend.stateTableStore.bridgeArpTable(bridgeId)
...
final class BridgeMapper(bridgeId: UUID, implicit val vt: VirtualTopology)
    ...
    private def deviceUpdated(br: TopologyBridge): SimulationBridge = {
        ...
        if (vt.config.bridgeArpEnabled && (ipv4MacMap eq null)) {
            try {
                ipv4MacMap = if (vt.backend.isEnabled) {
    ...
    private def createMacLearningTable(vlanId: Short): Unit = {
        ...
        try {
            val table = if (vt.backend.isEnabled) {

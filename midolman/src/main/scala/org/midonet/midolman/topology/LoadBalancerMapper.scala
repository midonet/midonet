...
final class LoadBalancerMapper(loadBalancerId: UUID, vt: VirtualTopology)
    ...
    private lazy val poolsObservable = Observable
        .merge(poolsSubject)
        .map[TopologyLb](makeFunc1(poolUpdated))
    ...
    /** Processes updates to the pool. */
    private def poolUpdated(pool: SimulationPool): TopologyLb = {
        assertThread()
    ...
    private def buildLoadBalancer: SimulationLb = {
        ...
        var vipsCount = 0
        for (pool <- pools.values) vipsCount += pool.device.vips.length
        val vips = new ArrayBuffer[Vip](vipsCount)
        for (pool <- pools.orderAsKeys(poolIds)) {

...
final class LoadBalancerMapper(lbId: UUID, vt: VirtualTopology)
    ...
    private def deviceUpdated(update: Any): SimLB = {
        ...
        log.debug("Processing and creating load-balancer {} device", lbId)
        new SimLB(lbId, currentLB.getAdminStateUp, currentLB.getRouterId.asJava,
                  vips.values.map(_.vip).toArray)
    }

    private def isLoadBalancerReady(update: Any): Boolean = {
        assertThread()

        update match {
            case loadBalancer: TopologyLB =>
                val vipIds = Set(
                    loadBalancer.getVipIdsList.asScala.map(_.asJava).toArray: _*)
            ...
            case _ =>
                log.debug("Unexpected update of class: {}, ignoring",
                          update.getClass)
        }
        val ready = (currentLB ne null) && (vips.forall(_._2.isReady))
        log.debug("Load-balancer ready: {}", Boolean.box(ready))
        ready
    }
    ...
    // observables. Publish subjects such as the vip observable must be
    // added to the merge before observables that may trigger their update,
    // such as the load-balancer observable, which ensures they are subscribed
    // to before emitting any updates.
    protected override lazy val observable: Observable[SimLB] =
        Observable.merge[Any](Observable.merge(vipSubject),
            ...
            .filter(makeFunc1(isLoadBalancerReady))

...
final class PortMapper(id: UUID, vt: VirtualTopology,
    ...
    private def handleQosPolicyIdChanges(port: TopologyPort): Unit = {
        if (port.hasQosPolicyId) {
            ...
            // whose policy ID was cleared. Either way we need to check the
            // bridge if it has one.
            if (port.hasNetworkId)
                bridgeTracker.requestRefs(Set(port.getNetworkId.asJava))

            // If this was an existing port, we need to stop tracking its old
            // policy ID.

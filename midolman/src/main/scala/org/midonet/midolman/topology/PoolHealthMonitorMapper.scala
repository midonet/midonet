...
import org.midonet.midolman.simulation.{PoolMember, Pool}
...
class PoolHealthMonitorMapper(vt: VirtualTopology)
    ...
    private final class PoolHealthMonitorState(val poolId: UUID) {
        ...
        private val funnel = PublishSubject.create[Observable[Any]]()
            ...
            info match {
                case p: Pool =>
                    if (hMon == null && p.healthMonitorId != null)
                        ...
                        hMon.complete()
                        hMon = if (p.healthMonitorId == null) null
                        else new HealthMonitorState(p.healthMonitorId)
                    }
                case m: HealthMonitor =>
            }
            PoolHealthMonitor(if (hMon == null) null else hMon.healthMonitor,
                              null, null, pool.poolMembers)
        }

        /** An observable for pool health monitor updates */
        val observable: Observable[PoolHealthMonitor] =
            Observable.merge(Observable.merge(funnel),
                             pool.observable.doOnCompleted(
                                 makeAction0({complete()})))
    ...
    private def addPool(poolId: UUID): Unit = {
        ...
        state.observable.observeOn(vt.scheduler)
            .doOnNext(
                makeAction1(phm => {mappings = mappings updated (poolId, phm)
                    subject.onNext(mappings)}))
            .doOnCompleted(
                makeAction0({mappings = mappings - poolId
                                subject.onNext(mappings)}))
    }
    ...
    Observable.merge(
        ...
        }))).observeOn(vt.scheduler).doOnNext(makeAction1(addPool))

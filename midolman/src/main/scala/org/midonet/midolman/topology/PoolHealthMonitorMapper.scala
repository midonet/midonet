...
class PoolHealthMonitorMapper(vt: VirtualTopology)
            ...
            info => {
                info match {
                    case p: Pool if pool == null =>
                        log.debug(s"pool contents received: $poolId: {}", p)
                        ...
                        pool = p
                    case p: Pool =>
                        log.debug(s"pool contents updated: $poolId: {}", p)
                    ...
                    case hm: HealthMonitor =>
                        log.debug(s"updated health monitor for $poolId: {}", hm)
                    case lb: LoadBalancer =>
                        log.debug(s"updated load balancer for $poolId: {}", lb)
                    case other =>
                        log.debug(s"unexpected update for $poolId: $other")
            }
    ...
    var previous: PoolHealthMonitorMap = null

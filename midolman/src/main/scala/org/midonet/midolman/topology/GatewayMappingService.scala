...
object GatewayMappingService {
    ...
    private class GatewayState(networkId: UUID, vt: VirtualTopology, log: Logger)
        ...
        private val observer = new Observer[Update[UUID, AnyRef]] {
            override def onNext(t: Update[UUID, AnyRef]): Unit = {
        ...
        def start(): Unit = {
            table.start()
            table.observable.subscribe(observer)
        }
        ...
        def tryGet: UUID = {
            if (table.isReady) {
                val hostIds = table.localSnapshot.keySet.toArray
...
class GatewayMappingService(vt: VirtualTopology)
    ...
    @throws[NotYetException]
    private def tryGetGateway(portId: UUID): UUID = {

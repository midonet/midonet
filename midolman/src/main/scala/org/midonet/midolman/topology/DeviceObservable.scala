 ...
 */
abstract class DeviceObservable[D <: AnyRef](id: UUID, vt: VirtualTopology)
    ...
    private val tappedStream = BehaviorSubject.create[D]()

    private val tap = new Observer[D]() {
        override def onCompleted(): Unit = {
            log.debug("Device {}/{} deleted", tag, id)
            vt.devices.remove(id)
    ...
    protected def onNext(value: D): Unit = { }
}

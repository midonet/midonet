...
class TunnelZoneMapper (id: UUID, store: Storage, vt: VirtualTopology)
    ...
    private var simTunnel = emptySimTunnel
    private var hosts = mutable.HashMap[UUID, SimHost]()
    private var subscriptions = mutable.Map[UUID, Subscription]()
    private var tunnelObs = tunnelObserver
    private var hostObs = hostObserver
    private var outStream = BehaviorSubject.create[SimTunnelZone]()

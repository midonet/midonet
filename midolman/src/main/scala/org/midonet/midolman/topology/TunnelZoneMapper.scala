...
class TunnelZoneMapper (id: UUID, store: Storage, vt: VirtualTopology)
    ...
    private var simTunnel: SimTunnelZone = _
    private var hosts: mutable.Map[UUID, SimHost] = _
    private var subscriptions: mutable.Map[UUID, Subscription] = _
    private var tunnelObs: Observer[TopologyTunnelZone] = _
    private var hostObs: Observer[TopologyHost] = _
    private var outStream: BehaviorSubject[SimTunnelZone] = _
    ...
    private def init() = {
        simTunnel = emptySimTunnel
        hosts = mutable.HashMap[UUID, SimHost]()
        subscriptions =  mutable.HashMap[UUID, Subscription]()
        hostObs = hostObserver
        tunnelObs = tunnelObserver
        outStream = BehaviorSubject.create[SimTunnelZone]()

...
import org.midonet.cluster.state.StateStorage._
 ...
 *
 * It also allows the router the update the routing table with updates
...
private class RoutingTableBroker(vt: VirtualTopology, routerId: UUID)
    ...
    private val cacheWip = new AtomicInteger()
     ...
     * concurrent modification of the queue, however it prevents concurrent
     * updates to the cache to perform concurrent modifications of the
     * underlying table.
     */
    @ThreadSafe
    private def update(): Unit = {
        if (state == BrokerState.Started) {
            if (cacheWip.getAndIncrement == 0) {
                do {
                ...
                } while (cacheWip.decrementAndGet() > 0)
            }
        }
    }

...
class ContainerService(vt: VirtualTopology, hostId: UUID,
    ...
    private val provider = new ContainerHandlerProvider(vt, log)
    ...
    private def createContainer(cp: ContainerPort): Unit = {
        ...
        // Get the service container group for this container.
        val containerGroup = vt.store.get(classOf[ServiceContainerGroup],
                                          cp.groupId).await(StorageTimeout)

        // Create a new container handler for this container type.
        val handler = provider.getInstance(containerGroup.getServiceType)

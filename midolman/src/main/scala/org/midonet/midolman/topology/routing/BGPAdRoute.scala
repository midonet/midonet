...
object BGPAdRoute {
    /**
     * Creates a [[BGPAdRoute]] advertised route instance, corresponding to the
     ...
     */
    @Deprecated
    def from(route: AdRoute): BGPAdRoute = {
        new BGPAdRoute(route.getId,
...
/** A BGP route. */
final
class BGPAdRoute(@ScalaZoomField(name = "id", converter = classOf[UUIDConverter])
                 val id: UUID,

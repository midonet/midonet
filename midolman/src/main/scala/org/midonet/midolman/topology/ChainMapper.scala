...
sealed class ChainMapper(id: UUID, vt: VirtualTopology)
    ...
    private def unsubscribeFromJmpChain(jumpChainId: UUID) = {
        ...
        case rule: Rule =>
            ...
            if (rule.hasJumpData) {
                ...
                if (chainState.rules.contains(ruleId)) {
                    val prevJmpRule = chainState.rules(ruleId)
                        .asInstanceOf[JumpRule]
                    if (prevJmpRule.jumpToChainID != rule.getJumpData.getJumpTo)
                        unsubscribeFromJmpChain(prevJmpRule.jumpToChainID)
                }

                // Subscribe to the chain this jump rule points to.
                val jumpChainId = rule.getJumpData.getJumpTo

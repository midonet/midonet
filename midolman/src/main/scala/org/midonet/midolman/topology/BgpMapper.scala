    ...
    private def addPeer(peerState: PeerState): Unit = {
        peersById get peerState.peerId match {
            case None =>
                ...
                peersByAddress get peerState.neighbor.address match {
                    case None =>
                        // There is no other peer with the same address
                        log.debug("Adding neighbor {}", peerState.neighbor)
                        peersById += peerState.peerId -> peerState.neighbor
                        peersByAddress += peerState.neighbor.address ->
                                              peerState.neighbor
                    case Some(peer) =>
            ...
            case Some(peer)
                if peer.address == peerState.neighbor.address =>
                // The peer exists, but the address has not changed
                log.debug("Updating peer with same address {}",
                          peerState.neighbor)
                peersById(peerState.peerId) = peerState.neighbor
                peersByAddress(peer.address) = peerState.neighbor
            case Some(peer) =>

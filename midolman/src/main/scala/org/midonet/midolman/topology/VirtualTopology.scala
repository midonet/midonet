 ...
 * devices as notifications. A [[DeviceMapper]] is an abstract class
 ...
 * [[PortMapper]]) by overriding the observable() method, where it emits
 * simulation device updates for any update received from storage. See the
 * [[DeviceMapper]] ScalaDoc for how it managers subscriptions.
 *
    ...
    def tryGet[D <: Device](id: UUID)
        ...
        if (device eq null) {
            throw new NotYetException(
                guard { self.observableOf(id, m).asFuture },

...
class VirtualTopology(val backend: MidonetBackend,
    ...
    extends MidolmanLogging with DefaultRetriable {
    ...
    override def maxRetries: Int = 5 // Topology snapshot retries
    override def handleRetry[T](e: Throwable, retries: Int, log: Logger,
                                message: String): Unit = {}

    private val snapshot: TopologySnapshot = if (config.initialStorageCacheEnabled) {
        val client = backend.discovery
            .getClient[MidonetServiceURI](serviceName = "topology-cache")
        val discoverySelector = MidonetDiscoverySelector
            .roundRobin[MidonetServiceURI](client)
        val cacheClient = new TopologyCacheClientDiscovery(discoverySelector,

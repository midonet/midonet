...
class VirtualTopology(val backend: MidonetBackend,
    ...
    def store: Storage =
        new StorageWrapper(config.initialStorageCacheTtlMs, backend.store,
                           Map.empty /* TODO: pass real cache */)

    def stateStore: StateStorage =
        new StateStorageWrapper(config.initialStorageCacheTtlMs, backend.store,
                                backend.stateStore, Map.empty /* TODO: pass real cache */, Map.empty /* TODO: pass real cache */)

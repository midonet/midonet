...
import org.midonet.cluster.models.Topology.{Bgp => TopologyBGP, BgpAdRoute => TopologyRoute}
import org.midonet.midolman.topology.devices.{RouterPort, Port}
import org.midonet.midolman.topology.routing.{BGPAdRoute, BGP}
...
object BGPMapper {
    ...
    case class BGPRouteAdded(route: BGPAdRoute) extends BGPUpdate
    case class BGPRouteRemoved(route: BGPAdRoute) extends BGPUpdate
    ...
    private final class BGPState(bgpId: UUID, vt: VirtualTopology, log: Logger) {
        ...
        val removedRoutes = new mutable.HashSet[BGPAdRoute]
    ...
    private final class RouteState(routeId: UUID, vt: VirtualTopology,
        ...
        private var currentRoute: BGPAdRoute = null
        ...
        private val routeSubscription = vt.store
            ...
            .distinctUntilChanged()
            .map[BGPAdRoute](makeFunc1(ZoomConvert.fromProto(_, classOf[BGPAdRoute])))
            .observeOn(vt.vtScheduler)
        ...
        private def routeUpdated(route: BGPAdRoute): Observable[BGPUpdate] = {
        ...
        @inline
        private def routesAsObservable(added: BGPAdRoute, removed: BGPAdRoute)
...
final class BGPMapper(portId: UUID, vt: VirtualTopology = VirtualTopology.self)
    ...
    private def portUpdated(p: RouterPort): Unit = {
        ...
        for (bgpId <- bgpIds if !bgps.contains(bgpId)) {
            ...
            bgps += bgpId -> bgpState
            bgpSubject onNext bgpState.observable.doOnError(errorAction)
        }
    }
    ...
    private def initializationUpdates: util.List[BGPUpdate] = {
        vt.assertThread()
        val updates = new util.ArrayList[BGPUpdate](1 + 2 * bgps.size)
        if (port ne null) {

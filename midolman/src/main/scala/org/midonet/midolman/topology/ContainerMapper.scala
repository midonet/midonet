...
object ContainerMapper {
    ...
    private final class ContainerState(containerId: UUID, vt: VirtualTopology,
        ...
        val observable =
            vt.store.observable(classOf[ServiceContainer], containerId)
                .map[(String, UUID, UUID)](makeFunc1 { c =>
                    val serviceType = if (c.hasServiceType) c.getServiceType
                                      else null
                    val groupId = if (c.hasServiceGroupId) c.getServiceGroupId.asJava
                                  else null
                    val configId = if (c.hasConfigurationId) c.getConfigurationId.asJava
                                   else null
                    (serviceType, groupId, configId)
                })
        ...
        /** Indicates whether the state received the service container state.
          */
        def isReady = (currentType ne null) && (currentGroupId ne null) &&
                      (currentConfigurationId ne null)
        /** Returns the service type for the last received container.
          */
        @Nullable
        def serviceType = currentType
        /** Returns the service group for the last received container.
          */
        @Nullable
        def groupId = currentGroupId
        /** Returns the configuration for the last received container.
          */
        @Nullable
        def configurationId = currentConfigurationId

        private def containerUpdated(update: (String, UUID, UUID)): Boolean = {
            log debug s"Container $containerId updated with type ${update._1} " +
                      s"group ${update._2} configuration ${update._3}"
            val first = !isReady
            currentType = update._1
            currentGroupId = update._2
            currentConfigurationId = update._3
            first
        }
    }
    ...
    /**
      ...
      * monitors updates for the specified port identifier. Whenever the
    ...
    private final class BindingState(hostId: UUID, portId: UUID,
        ...
        /**
          * Returns an initial notification from this binding state, to be
          * published to new subscribers.
          */
        ...
        /**
          ...
          * current container and load the new container, or (c) if the
        ...
        private def cleanup: Observable[Notification] = {
            ...
            if (currentBinding.hasContainer && (currentContainer ne null) &&
                ...
                val serviceType = currentContainer.serviceType
...
final class ContainerMapper(hostId: UUID, vt: VirtualTopology)
    ...
    private val hostSubscriber = new Subscriber[Set[UUID]] {
        override def onNext(bindings: Set[UUID]): Unit = bindingsUpdated(bindings)
        override def onCompleted(): Unit = hostDeleted()
        override def onError(e: Throwable): Unit = hostError(e)
    }
    ...
    override def call(child: Subscriber[_ >: Notification]): Unit = {
        ...
        vt.executeVt {
            if (state.compareAndSet(MapperState.Unsubscribed,
                ...
                hostObservable subscribe hostSubscriber
            }

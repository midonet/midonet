...
final class HostMapper(hostId: UUID, vt: VirtualTopology)
    ...
    private def deviceUpdated(update: Any): SimulationHost = {
        ...
        val portBindings = for ((portId, state) <- ports)
            yield {
                log.debug("checking if state.device.interfaceName ({})", state.device.interfaceName)

                val interfaceName = if(state.device.interfaceName != null && !state.device.interfaceName.equals("")) {
                    log.debug("state.device.interfaceName is not null nor empty string")
                    state.device.interfaceName
                } else {
                    log.debug("state.device.interfaceName is null")
    ...
    private lazy val hostPortBindingsObservable =

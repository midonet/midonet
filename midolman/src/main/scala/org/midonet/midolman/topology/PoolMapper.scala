...
final class PoolMapper(poolId: UUID, vt: VirtualTopology)
    ...
    protected override val observable = Observable
        ...
        .filter(makeFunc1(isPoolDistinct))
    ...
    /** Indicates that the pool is distinct from the previous device. */
    private def isPoolDistinct(pool: SimulationPool): JBoolean = {
        assertThread()
        val distinct = pool != device

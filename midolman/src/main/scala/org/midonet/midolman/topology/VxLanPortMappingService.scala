...
object VxLanPortMappingService {
    ...
    case class TunnelInfo(tunnelIp: IPv4Addr, tunnelZoneId: UUID)
    ...
    /** Synchronous query method to retrieve the VTEP tunnel information for
      * the specified VTEP identifier. */
    def vtepOf(vtepId: UUID): Option[TunnelInfo] = {
        toVtepMappings get vtepId
    }
    ...
    private def updateMappings(): Unit = {
        ...
        VxLanPortMappingService.toVtepMappings =
            (for (vtep <- vteps.values)
                yield vtep.vtepId -> TunnelInfo(vtep.tunnelIp, vtep.tunnelZoneId))(breakOut)

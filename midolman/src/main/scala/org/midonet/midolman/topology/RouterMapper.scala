...
class RouterMapper(id: UUID, vt: VirtualTopology, dataClient: DataClient,
    ...
    private val routerMap = makeFunc1 { router: Router =>
        val routerCfg = new RouterConfig(router.getAdminStateUp,
                                         router.getInboundFilterId,
                                         router.getOutboundFilterId,
                                         router.getLoadBalancerId)
        // TODO(nicolas): Add a tag manager
        val simRouter = new SimRouter(id, routerCfg, null /*routing table*/,
                                      null /*tag manager*/, null /*arp table*/)

        //TODO: add/remove observables for routes to routesStream
        simRouter
    }
    ...
    private def triggerUpdate[D <: IPAddr](newCfg: RouterConfig, newArpCache: ArpCache,
                                           newRoutingTable: RoutingTableWrapper[D]): Unit = {
        ...
        //TODO: replace what this call does with calls to the VT
        prefetchTopology(loadBalancer(newCfg.loadBalancer))

        // Ask for the loadbalancer
        VirtualTopology.tryGet(routerBuilder.cfg.loadBalancer)
        // Ask for the chains of inbound and outbound filters
        VirtualTopology.tryGet(routerBuilder.cfg.inboundFilter)
        VirtualTopology.tryGet(routerBuilder.cfg.outboundFilter)
    }
    ...
    private def isRouterReady(router: SimRouter): Boolean =
        // TODO(nicolas): to complete
        router.arpTable != null && router.rTable != null

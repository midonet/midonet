...
object RouterMapper {
    ...
    private class PortState(val portId: UUID, vt: VirtualTopology) {
        ...
        def deletionRouteUpdate = RouteUpdate(None, Option(currentRoute))
        ...
        private def portUpdated(port: RouterPort): RouteUpdate = {
            ...
            val hasLocalRoute = port.isInterior ||
                                port.isExterior && port.isActive
            val route = if (hasLocalRoute) createLocalRoute(port) else null

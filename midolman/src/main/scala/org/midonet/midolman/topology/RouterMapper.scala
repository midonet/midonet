...
object RouterMapper {
    ...
    /** Represents a route update generated by a port update. */
    private[topology]
    case class PortRouteUpdate(added: Route, removed: Route) {
        def nonEmpty = (added ne null) || (removed ne null)
     ...
     * signal that port is no longer used by the router
     */
    private class PortState(val portId: UUID, vt: VirtualTopology) {

        private var currentPort: RouterPort = null
        private var currentRoute: Route = null
        private val mark = PublishSubject.create[PortRouteUpdate]

        val observable = VirtualTopology.observable[RouterPort](portId)
            .map[PortRouteUpdate](makeFunc1(portUpdated))
        ...
        def deletionRouteUpdates = PortRouteUpdate(null, currentRoute)
         ...
         */
        private def portUpdated(port: RouterPort): PortRouteUpdate = {
            ...
            val updates =
                if (route != currentRoute) PortRouteUpdate(route, currentRoute)
                else PortRouteUpdate(null, null)
...
final class RouterMapper(routerId: UUID, vt: VirtualTopology)
    ...
    private val localRoutes = new mutable.HashMap[UUID, Route]
    ...
    private lazy val portsSubject =
        PublishSubject.create[Observable[PortRouteUpdate]]
    ...
    private lazy val portsObservable = Observable
        .merge(portsSubject)
        .map[Config](makeFunc1(portUpdated))
    ...
    private def routerUpdated(router: TopologyRouter): Config = {
        ...
        val routes = router.getRoutesList.asScala
            .map(r => r.getId.asJava -> ZoomConvert.fromProto(r, classOf[Route]))
            .toMap
        ...
        val routesToAdd = new mutable.HashSet[Route]
        val routesToRemove = new mutable.HashSet[Route]
        // Remove any local routes no longer part of the router's configuration
        for ((routeId, route) <- localRoutes.toList if !routes.contains(routeId)) {
            localRoutes -= routeId
            routesToRemove += route
        }
        // Add any routes present in the router's configuration, and not part of
        // the local routes map.
        for ((routeId, route) <- routes if !localRoutes.contains(routeId)) {
            localRoutes += routeId -> route
            routesToAdd += route
        }
        routingTableBroker.update(routesToAdd.toSet, routesToRemove.toSet)
    ...
    /**
     * Handles route updates for ports, by adding them to the router's
     * replicated routing table via the routing table broker. The method returns
     * the router's current configuration to propagate the notifications to the
     * device observable.
     *                         +-------------+
     * Obs[PortRouteUpdate]--->| portUpdated |--->Obs[Config]
     *                         +-------------+
     */
    private def portUpdated(routeUpdate: PortRouteUpdate): Config = {
        log.debug("Port route update {}", routeUpdate)
        assertThread()

        if (routeUpdate.nonEmpty) {
            routingTableBroker.update(Set(routeUpdate.added),
                                      Set(routeUpdate.removed))
        }

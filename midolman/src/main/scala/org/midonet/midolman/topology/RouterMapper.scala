...
object RouterMapper {
    ...
    private class PortState(val portId: UUID, vt: VirtualTopology) {
        ...
        def complete(): RouteUpdates = {
            ...
            val publish = currentPort.isInterior ||
                          currentPort.isExterior && currentPort.isActive
            if (publish) RouteUpdates(EmptyRouteSet, currentRoutes.toSet)
        ...
        /**
         * A method called when the router port has changed. The method updates
         * the current port, and emits a set of route updates corresponding
         * to the port state change and its route set.
         * - If the port previously and currently publishes updates, then emit
         *   only the removed routes.
         * - If the port previously did not publish, but now publishes updates,
         *   add all routes.
         * - If the port previously published updates, but now does not, remove
         *   all routes.
         * - If the port previously and currently does not publish updates, do
         *   not emit any.
         */
        private def portUpdated(port: RouterPort): RouteUpdates = {
            vt.assertThread()

            val oldPublish = currentPort.isInterior ||
                             currentPort.isExterior && currentPort.isActive
            val newPublish = port.isInterior || port.isExterior && port.isActive
            ...
            if (oldPublish && newPublish) {
                ...
                RouteUpdates(currentRoutes.toSet, EmptyRouteSet)
            } else if (oldPublish && !newPublish) {
                // If the port did previously publish routes, but does not now,
                // remove all routes.
                RouteUpdates(EmptyRouteSet, currentRoutes.toSet)
            } else {
        ...
        private def routeUpdated(updates: RouteUpdates): RouteUpdates = {
            ...
            if (publish) updates else EmptyRouteUpdates
        }
    }
...
final class RouterMapper(routerId: UUID, vt: VirtualTopology)
    ...
    private def routerUpdated(router: TopologyRouter): Config = {
        ...
        for ((portId, portState) <- ports.toList if !portIds.contains(portId)) {
            ...
            //portsSubject onNext Observable.just(portState.deletionRouteUpdate)
            ports -= portId
        }
    ...
    private def routesUpdated(routeUpdates: RouteUpdates): Config = {
        log.debug("Port route updates {}", routeUpdates)
        assertThread()

...
object TunnelZone {
     ...
     */
    @Deprecated
    def toNewTunnelZoneType(oldType: OldTunnelZone.Type): TunnelZoneType =
        oldType match {

        case OldTunnelZone.Type.gre => TunnelZoneType.GRE
        case OldTunnelZone.Type.vtep => TunnelZoneType.VTEP
        case OldTunnelZone.Type.vxlan => TunnelZoneType.VXLAN
...
class TunnelZone extends ZoomObject with Device {
     ...
     */
    @Deprecated
    def diff(oldZone: ZoneMembers): ZoneChanged = {
        ...
        if (oldZoneSize > hosts.keySet.size) {
            ...
            new ZoneChanged(id, TunnelZone.toOldTunnelZoneType(zoneType), removedMember,
                            HostConfigOperation.Deleted)

        } else {
            val added = hosts.keySet.diff(oldZone.members.map(_.getId).toSet).toList.head
            val addedMember = new HostConfig(added).setIp(hosts.get(added).get.asInstanceOf[IPv4Addr])

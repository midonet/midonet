...
class DeviceHandlersManager[T <: AnyRef](handler: DeviceHandler,
    ...
    def subscriberStatus(deviceId: UUID, subscriber: ActorRef) = {
        val regular = deviceSubscribers.get(deviceId).map{ _.contains(subscriber) }
        val oneShot = deviceOneShotSubscribers.get(deviceId).map{ ! _.contains(subscriber) }
    ...
    def notifyClientOfError(id: UUID, e: Throwable) = {
        for (client <- deviceOneShotSubscribers.getOrElse(id, List.empty))
            client ! Status.Failure(e)
        for (client <- deviceSubscribers.getOrElse(id, List.empty))
            client ! Status.Failure(e)
    }
...
abstract class VirtualToPhysicalMapperBase extends VTPMRedirector {
    ...
    protected override def notifyClientOfError[D <: Device](deviceId: UUID,
                                                            e: Throwable)
                                                           (implicit t: ClassTag[D])
    : Unit = getDeviceHandler(t) match {
        case Some(deviceHandler) => deviceHandler.notifyClientOfError(deviceId, e)
        case None =>
            throw new IllegalArgumentException("The VTPM does not support"
                                               + s" devices of type $t")
    }

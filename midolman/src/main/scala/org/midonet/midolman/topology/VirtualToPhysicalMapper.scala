...
import org.midonet.midolman.topology.devices.{Host => DevicesHost, TunnelZone => NewTunnelZone}
...
import org.midonet.midolman.topology.VirtualToPhysicalMapper._
    ...
    protected override def updated(update: AnyRef): Unit = update match {
        ...
        case host: RcuHost =>
            hostsMgr.updateAndNotifySubscribers(host.id, DevicesHost.toDevicesHost(host),
                                                createHandlerIfNeeded=false)
        case host: DevicesHost =>
            hostsMgr.updateAndNotifySubscribers(host.id, host, createHandlerIfNeeded=true)
    ...
    protected override def hasSubscribers[T <: Device](id: UUID)
        ...
        else if (t.runtimeClass == classOf[DevicesHost])

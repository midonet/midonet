...
class VirtualToPhysicalMapper
     ...
     *  the request reschedules itself. */
    private def getPortConfig(vport: UUID, retries: Int = 3)
        ...
        VirtualTopologyActor.expiringAsk(
            ...
            case brPort: BridgePort[_] =>
                val req = BridgeRequest(brPort.deviceID, update=false)
                VirtualTopologyActor.expiringAsk(req).mapTo[Bridge]
                    .map { br => Some((brPort, br)) }
            case trPort: TrunkPort =>
                val req = VlanBridgeRequest(trPort.deviceID, update = false)
                VirtualTopologyActor.expiringAsk(req).mapTo[VlanAwareBridge]
                    .map { trunk => Some((trPort, trunk)) }

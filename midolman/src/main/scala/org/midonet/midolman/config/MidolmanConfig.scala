...
import org.midonet.services.agent_rest_api.AgentRestApiService
...
class MidolmanConfig(_conf: Config, val schema: Config = ConfigFactory.empty()) extends TypeFailureFallback {
    ...
    val agentRestApi = new AgentRestApiConfig(conf, schema)
}
...
class AgentRestApiConfig(val conf: Config, val schema: Config)
    extends TypeFailureFallback with MinionConfig[AgentRestApiService] {
    val prefix = "agent.minions.agent_rest_api"

...
class DatapathConfig(val conf: Config, val schema: Config) extends TypeFailureFallback {
    ...
    def recircCidr = IPv4Subnet.fromString(Try(getString(s"$PREFIX.recirc_cidr"))
...
class RecircConfig(recircCidr: IPv4Subnet) {
    ...
    def isValidAddr(addr: Int): Boolean =
        (addr ne network.toInt) &&
        (addr ne broadcast.toInt) &&
        (addr ne recircHostAddr.toInt) &&
        (addr ne recircMnAddr.toInt)
}

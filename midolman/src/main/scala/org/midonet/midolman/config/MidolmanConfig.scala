...
class DatapathConfig(val conf: Config, val schema: Config) extends TypeFailureFallback {
    ...
    def recircHostName = Try(getString(s"$PREFIX.recircExternalName"))
                             .getOrElse("recircVeth0")
    def recircMnName = Try(getString(s"$PREFIX.recircInternalName"))
                           .getOrElse("recircVeth1")
    def recircHostMac = MAC.fromString(Try(getString(s"$PREFIX.recircExternalMac"))
                                           .getOrElse("02:00:11:00:11:01"))
    def recircMnMac = MAC.fromString(Try(getString(s"$PREFIX.recircInternalMac"))
                                         .getOrElse("02:00:11:00:11:02"))

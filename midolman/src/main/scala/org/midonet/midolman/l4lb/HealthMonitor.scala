...
object HealthMonitor {
    ...
    private val log: Logger
        = LoggerFactory.getLogger("org.midonet.healthmonitor")
...
class HealthMonitor @Inject() (config: MidolmanConfig,
    ...
    override def doStart(): Unit = {
        ...
        executor = Executors.newSingleThreadScheduledExecutor(
            new NamedThreadFactory("HealthMonitor", isDaemon = true))
        scheduler = Schedulers.from(executor)
        watcher = new HealthMonitorConfigWatcher(
            config.healthMonitor.haproxyFileLoc, namespaceSuffix, executor)
    ...
    private def startHaProxy(poolId: UUID, config: PoolConfig,
                             routerId: UUID) = {

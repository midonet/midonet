...
class HaproxyHealthMonitor(var config: PoolConfig,
    ...
    /**
      ...
      *  transaction
      */
    protected def tryTx(populate: (Transaction) => Unit, lockNeeded: Boolean = true): Unit = {
        ...
        while (attempt <= StorageAttempts) {
            try {
                if (lockNeeded) {
                    HealthMonitor.zkLock(lockFactory)(executeOperations)
                } else {
                    executeOperations()
                }
                return
                ...
                if (conf.isConfigurable){
                    restartHaproxy(healthMonitorName,
                        ...
                        if (config.vip != conf.vip) {
                            tryTx(populateTransaction, lockNeeded = true)
                        }
                        setPoolMappingStatus(config.id, ACTIVE, lockNeeded = true,
    ...
    def unhookNamespaceFromRouter() = {
        if (routerPortId != null) {
            ...
            tryTx( { tx =>
                tx.delete(classOf[Port], routerPortId)
            }, lockNeeded = true)

...
object HaproxyHealthMonitor {
    def props(config: PoolConfig, manager: ActorRef, routerId: UUID,
              store: Storage, hostId: UUID, poolUpdater: BackendUtil):
...
class HaproxyHealthMonitor(var config: PoolConfig,
    ...
    override def preStart(): Unit = {
        try {
            ...
            backendUtil.setPoolMappingStatus(config.id, ACTIVE)
        } catch {
            case e: Exception =>
                log.error("Unable to create Health Monitor for " +
                          config.haproxyConfFileLoc + ": " + e.getMessage)
                e.printStackTrace()
                backendUtil.setPoolMappingStatus(config.id, ERROR)
    ...
    override def postStop(): Unit = {
        ...
        HealthMonitor.cleanAndDeleteNamespace(healthMonitorName,
                                              config.nsPostFix,
                                              config.l4lbFileLocs)
        backendUtil.setPoolMappingStatus(config.id, INACTIVE)
    }
        ...
        case ConfigUpdate(conf) =>
            try {
                ...
                if (config.vip != conf.vip) {
                    if (routeId != null) {
                        backendUtil.tryWrite({
                             store.delete(classOf[Route], routeId)
                        }, clazz = classOf[Route], id = routeId)
    ...
    private def setMembersStatus(memberIds: Set[UUID], status: LBStatus) = {
        ...
        members.foreach { m =>
            backendUtil.tryWrite({
                ...
                clazz = classOf[PoolMember], id = UUIDUtil.fromProto(m.getId))
        }
    }
    ...
    def addVipRoute(ip: String) = {
        ...
        val route = Route.newBuilder
            ...
            .build
        backendUtil.tryWrite({store.create(route)}, clazz = classOf[Route],
            id = routeId)
    }
    ...
    def hookNamespaceToRouter(): Unit = {
        ...
        ports.filter(_.getInterfaceName == namespaceName).foreach { p =>
            log.warn("deleting unused health monitor port " +
                     ...
                     " for pool " + config.id)
            backendUtil.tryWrite({store.delete(classOf[Port], p.getId)},
                                 clazz = classOf[Port],
                                 id = UUIDUtil.fromProto(p.getId))
        }
        ...
        backendUtil.tryWrite({store.create(hmPort)}, clazz = classOf[Port],
            id = UUIDUtil.fromProto(hmPort.getId))
    ...
    def unhookNamespaceFromRouter() = {
        if (routerPortId != null) {
            ...
            backendUtil.tryWrite({store.delete(classOf[Port], routerPortId)},
                clazz = classOf[Port], id = routerPortId)

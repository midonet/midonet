...
class HaproxyHealthMonitor(var config: PoolConfig,
    ...
    def addVipRoute(ip: String) = {
        ...
        val route = Route.newBuilder
            ...
            .build
        backendUtil.writeWithLock({store.create(route)})//, clazz = classOf[Route],
            //id = routeId)
    }
    ...
    def hookNamespaceToRouter(): Unit = {
        ...
        ports.filter(_.getInterfaceName == namespaceName).foreach { p =>
            log.warn("deleting unused health monitor port " +
                     ...
                     " for pool " + config.id)
            backendUtil.writeWithLock({store.delete(classOf[Port], p.getId)})
                                 //clazz = classOf[Port],
        ...
        backendUtil.writeWithLock({store.create(hmPort)}) //, clazz = classOf[Port],
    ...
    def unhookNamespaceFromRouter() = {
        if (routerPortId != null) {
            ...
            backendUtil.writeWithLock({store.delete(classOf[Port], routerPortId)})

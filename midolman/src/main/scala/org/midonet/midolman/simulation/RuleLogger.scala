...
abstract class RuleLogger extends VirtualDevice {
    ...
    def logReject(pktCtx: PacketContext, chain: Chain, rule: Rule): Unit = {
        if (logRejectEvents)
            logEvent(pktCtx, "REJECT", chain, rule)
    }
...
class FileRuleLogger(val id: UUID,
    ...
    override protected val eventLog: Logger = {
        ...
        fileAppender.setFile(s"$defaultLogDir/$fileName")

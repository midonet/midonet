...
trait FlowContext extends Clearable { this: PacketContext =>
    ...
    def calculateActionsFromMatchDiff(): Unit = {
        ...
        baseMatch.doNotTrackSeenFields()
        diffEthernet()
        diffIp()
        diffVlan()
        diffIcmp()
        ...
        baseMatch.doTrackSeenFields()
    }

    private def diffEthernet(): Unit =
        if (!baseMatch.getEthSrc.equals(wcmatch.getEthSrc) ||
            !baseMatch.getEthDst.equals(wcmatch.getEthDst)) {
    ...
    private def diffIp(): Unit =
        if (baseMatch.getNetworkSrcIP != wcmatch.getNetworkSrcIP ||
            baseMatch.getNetworkDstIP != wcmatch.getNetworkDstIP ||
            baseMatch.getNetworkTTL != wcmatch.getNetworkTTL) {
    ...
    private def diffVlan(): Unit =
        if (!baseMatch.getVlanIds.equals(wcmatch.getVlanIds)) {
            val vlansToRemove = baseMatch.getVlanIds.diff(wcmatch.getVlanIds)
            val vlansToAdd = wcmatch.getVlanIds.diff(baseMatch.getVlanIds)
    ...
    private def diffIcmp(): Unit = {
        ...
        if ((icmpData ne null) &&
            !Arrays.equals(icmpData, baseMatch.getIcmpData)) {
    ...
    private def diffL4(): Unit =
        if (baseMatch.getSrcPort != wcmatch.getSrcPort ||
            baseMatch.getDstPort != wcmatch.getDstPort) {
...
class PacketContext(val cookie: Int,
    ...
    val origMatch = baseMatch
    val wcmatch = baseMatch.clone()
    ...
    def isStateMessage = baseMatch.getTunnelKey == FlowStatePackets.TUNNEL_KEY

    def cookieStr = s"[cookie:$cookie]"

    protected def createPacketMatchCheckpoint(): Unit = {
        baseMatch.reset(wcmatch)
    }

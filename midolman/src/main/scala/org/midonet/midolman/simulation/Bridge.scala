...
class Bridge(val id: UUID,
    ...
    private def handleArp()(implicit context: PacketContext): Result = {
        ...
        context.ethernet.getPayload match {
            case arp: ARP =>
                arp.getOpCode match {
                    case ARP.OP_REQUEST => handleArpRequest(arp)
    ...
    /**
      ...
      */
    private def handleArpRequest(arp: ARP)(implicit context: PacketContext)
    : Result = {
        val tpa = IPv4Addr.fromBytes(arp.getTargetProtocolAddress)
        val spa = IPv4Addr.fromBytes(arp.getSenderProtocolAddress)

        context.log.debug(s"Handling ARP request from $spa for $tpa")

        val mac = ip4MacMap get tpa
        if (mac ne null) {
            context.log.debug("Known MAC {} reply to the ARP request", mac)
            processArpRequest(arp, mac, context.inPortId)
            NoOp
        } else {
            handleArpGeneric()
        }
        ...
        ipToMac get nwDst match {
            case Some(mac) if macToLogicalPortId.contains(mac) =>
                ...
                context.log.debug(s"ARP unicast to peer router port $portId " +
                                  s"(IP: $nwDst MAC: $mac)")
                unicastAction(portId)
            case _ if pMatch.getNetworkSrcIP.equals(pMatch.getNetworkDstIP) =>
                context.log.debug(

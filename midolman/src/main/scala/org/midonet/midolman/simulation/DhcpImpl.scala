...
class DhcpImpl(val dataClient: DataClient,
    ...
    private def makeDhcpReply(port: BridgePort,
        ...
        options ++= getExtraOptions(port, host)
    ...
    private
    def getExtraOptions(port: BridgePort,
                        host: Host): mutable.ListBuffer[DHCPOption] = {
        var extraOptions = mutable.ListBuffer[DHCPOption]()

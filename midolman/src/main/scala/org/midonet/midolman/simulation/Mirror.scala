...
trait MirroringDevice extends SimDevice {
    ...
    private def mirror(mirrors: JList[UUID], context: PacketContext,
        ...
        while (i < mirrors.size) {
            val mirror = tryAsk[Mirror](mirrors.get(i))
            i += 1
    ...
    def mirroringOutbound(context: PacketContext, next: SimulationResult,
                          as: ActorSystem): SimulationResult = {
        unwind(context, mirror(outboundMirrors, context, next, as))(as)
    }
}

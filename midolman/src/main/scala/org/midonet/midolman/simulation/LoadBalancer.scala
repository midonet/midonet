...
class LoadBalancer(val id: UUID, val adminStateUp: Boolean, val routerId: UUID,
    ...
    override def equals(obj: Any): Boolean = obj match {
        case loadBalancer: LoadBalancer =>
            ...
            util.Arrays.equals(pools.asInstanceOf[Array[AnyRef]],
                               loadBalancer.pools.asInstanceOf[Array[AnyRef]])

        case _ => false
    }

    override def hashCode: Int =
        Objects.hashCode(id, adminStateUp, routerId, vips, pools)

    override def toString =
        s"LoadBalancer [id=$id adminStateUp=$adminStateUp routerId=$routerId " +
        s"vips=${vips.toSeq} pools=${pools.toSeq}]"
}

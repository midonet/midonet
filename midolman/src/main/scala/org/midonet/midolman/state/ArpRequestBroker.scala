...
object ArpRequestBroker {
    ...
    type PacketEmitter = (EmitGeneratedPacket) => Unit
    type InvalidationSource = (FlowTag) => Unit
}
...
class SingleRouterArpRequestBroker(id: UUID,
    ...
    val EXPIRY_TIME = arpExpirySeconds * 1000
    val STALE_TIME = arpStaleSeconds * 1000
    val TIMEOUT = arpTimeoutSeconds * 1000
    val RETRY = arpRetrySeconds * 1000
    ...
    private val stalenessJitter: Long = {
        (STALE_TIME * STALE_JITTER * random.nextDouble()).toLong
    }
    ...
    private def arpLoopIsReady: Boolean =
        (!arpLoopQ.isEmpty) && arpLoopQ.peek().nextTry < clock.time
    ...
    private def arpForAddress(ip: IPv4Addr, port: RouterPort): Unit = {
        ...
        emitter(EmitGeneratedPacket(port.id, arp))
    ...
    def set(ip: IPv4Addr, mac: MAC): Unit = {
        ...
        if (upToDate(entry) && mac == entry.macAddr) {
            ...
            val now = clock.time
            val entry = new ArpCacheEntry(mac, now + EXPIRY_TIME,
                                               now + STALE_TIME, 0)
        ...
        while (arpLoopIsReady) {
            ...
            if (upToDate(entry)) {
                ...
                val arp = makeArpRequest(loop.port.portMac,
                                         ...
                                         loop.ip)
                emitter(EmitGeneratedPacket(loop.port.id, arp))
                loop.tick()
                arpLoopQ.add(loop)
            }
        }
    ...
    private def breakPromises(ip: IPv4Addr) {
        ...
        while (waiters.hasNext)
            waiters.next().tryFailure(ArpTimeoutException)
    }
    ...
    class ArpLoop(val ip: IPv4Addr, val port: RouterPort) {
        private val timeout = clock.time + TIMEOUT
        ...
        def tick() {
            retries += 1
            _nextTry = clock.time + (RETRY * jitter).toLong
        }

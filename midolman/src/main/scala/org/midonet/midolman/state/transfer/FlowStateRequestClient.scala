...
class FlowStateRequestClient(socketTimeout: Int) {
    ...
    def rawPipelinedFlowStateFrom(host: String, tcpPort: Int, portId: UUID,
                                  writer: ByteBufferBlockWriter[_]) = {
        retryClosing {
            ...
            response match {
                case StateAckRaw(port) =>
                    ...
                    pipelinedReadWriteRawState(writer)
                case StateError(code, description) =>
                    Log warn s"Ignoring response: $code error received from" +
                        s" previous owner: $description"
                case _ =>
    ...
    private def pipelinedReadWriteRawState(writer: ByteBufferBlockWriter[_]): Unit = {
        var next = dis.readInt()
        while (next > 0) {
    ...
    private def retryClosing(retriable: => Unit): Unit = {
        try {
    ...
    private def retry(retries: Int) (retriable: => Unit): Unit = {
        try {
            ...
            case NonFatal(e) if retries > 0 => retry (retries - 1) (retriable)

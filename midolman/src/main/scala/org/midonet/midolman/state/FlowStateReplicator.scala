...
abstract class BaseFlowStateReplicator(conntrackTable: FlowStateTable[ConnTrackKey, ConnTrackValue],
    ...
    def accumulateNewKeys(context: PacketContext): Unit = {
        ...
        if (!txPeers.isEmpty) {
            ...
            uuidToSbe(ingressPort, flowStateMessage.ingressPort)
    ...
    private def acceptNewState(msg: FlowStateSbe) {
        val conntrackIter = msg.conntrack.iterator
        ...
        val traceIter = msg.trace.iterator
        if (traceIter.hasNext()) {
            ...
            val reqIdsIter = msg.traceRequestIds.iterator
    ...
    def accept(p: Ethernet) = {
        ...
        if (data == null) {
            ...
            try {
                ...
                val ingressPort = uuidFromSbe(flowStateMessage.ingressPort)

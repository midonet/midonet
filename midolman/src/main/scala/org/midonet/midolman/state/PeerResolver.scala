...
class PeerResolver(
    ...
    private val hostId = hostIdProvider.hostId()
    private val fpHerald = new FloodingProxyHerald(backend, Some(hostId))
    ...
    private def collectPortGroupPeers(
        ...
        while (i < members.size()) {
            ...
            if (portId != skip) {
                ports.add(portId)
                addPeerFor(fetch(portId, portCtag), hosts, tags)
            }
            i += 1
        }
    }
    ...
    private def fetch[D <: Device](id: UUID, ctag: ClassTag[D]): D =
        virtualTopology.tryGet(id)(ctag)
}

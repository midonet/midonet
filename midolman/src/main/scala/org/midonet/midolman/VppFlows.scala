...
object VppFlows {
    ...
    /**
      ...
      *   |                              |
      * +--------+--------+--------+--------+
      * |****FFFF|FFFFFFFF|FFFFFFFF|FFFF****|
      * +--------+--------+--------+--------+
      */
    private val FlowIndexShift = FlowController.IndexShift
    private val FlowIndexBits = 8

    private val FlowIndexLowerBits = FlowIndexBits + FlowIndexShift - 32
    private val FlowIndexUpperBits = FlowIndexBits - FlowIndexLowerBits
    private val FlowIndexFilter = (1 << FlowIndexBits) - 1
    private val FlowIndexLowerMask = (1 << FlowIndexLowerBits) - 1
    private val FlowIndexMask = (0xffffffff >>> FlowIndexUpperBits) ^
                                ((1 << FlowIndexLowerBits) - 1)

    private val MaxFlows = 1 << FlowIndexBits
    ...
    /**
      * Convers the flow mark to the flow index in the interval [0 - MaxFlows].
      */
    @inline def flowMarkToIndex(mark: Int): Int = {
        (mark & FlowIndexFilter) | (mark >>> FlowIndexShift)
    }
...
trait VppFlows { this: Actor =>
    ...
    private def allocateFlow(context: PacketContext,
        ...
        flow.reset(context.origMatch, context.flowTags, context.flowRemovedCallbacks,
    ...
    private def indexFlow(flow: ManagedFlow): Unit = {
        ...
        while (indexToFlow(index) eq null) {
            currentIndex += 1
            index = currentIndex & FlowIndexFilter
        }

...
case class IPSecConfig(script: String,
    ...
    def ikeParamsToConfig(ikePolicy: IkePolicy): String = {
        val enc = ipsecEncryptionToConfig(ikePolicy.getEncryptionAlgorithm)
        val auth = ipsecAuthToConfig(ikePolicy.getAuthAlgorithm)
        val pfs = ipsecPfsToConfig(ikePolicy.getPfs)
        return enc + "-" + auth + ";" + pfs
    }

    def ipsecParamsToConfig(ipsecPolicy: IPSecPolicy): String = {
        val enc = ipsecEncryptionToConfig(ipsecPolicy.getEncryptionAlgorithm)
        val auth = ipsecAuthToConfig(ipsecPolicy.getAuthAlgorithm)
        val pfs = ipsecPfsToConfig(ipsecPolicy.getPfs)
        return enc + "-" + auth + ";" + pfs
    }

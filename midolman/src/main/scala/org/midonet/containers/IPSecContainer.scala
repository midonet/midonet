...
class IPSecContainer @Inject()(vt: VirtualTopology,
    ...
    private var mark: PublishSubject[VpnService] = _
    private var created: Promise[String] = _
    ...
    override def create(port: ContainerPort): Future[String] = {
        ...
        try {
            mark = PublishSubject.create[VpnService]
            created = Promise[String]
            vpnServiceObservable(port)
                .observeOn(scheduler)
                .takeUntil(mark)
    ...
    VpnServiceUpdatedEvent = {
        ...
        new VpnServiceUpdatedEvent(port, router, vpnService, routerPorts, connections)
    }
     ...
     * TODO: handle changes on ipsec site connections
     */
    private def vpnServiceUpdateEvent(updateEvent: VpnServiceUpdatedEvent): Unit = {
        updateEvent match {
            case VpnServiceUpdatedEvent(port, router, vpnService, routerPorts,
                                        connections) =>
                ...
                val externalMac = routerPorts
                    ...
                    .map(_.getPortMac)
                    .getOrElse {
                        created.tryFailure(IPSecException(
                ...
                if (!created.isCompleted)
                    // We need to complete the promise
                    created.success(config.ipsecService.name)
            case _ =>
        }
    }

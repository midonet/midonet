...
class HaProxyContainer @Inject()(
    ...
    private def subscribeRefs(portId: UUID): Unit = {
        assert(refsSub == null)
        refsSub = Observable
            .merge(poolHmMapTracker.refsObservable, portTracker.refsObservable)
            .doOnNext(makeAction1(obj => log.error("HaProxyContainer got update: " + obj)))
            ...
            .subscribe()
        poolHmMapTracker.requestRefs(PoolHealthMonitorMapKey)
        portTracker.requestRefs(portId)
    }

    private def areRefsReady(ignored: AnyRef): Boolean =
        portTracker.areRefsReady && portTracker.currentRefs.nonEmpty &&
        poolHmMapTracker.areRefsReady &&
        poolHmMapTracker.currentRefs.head._2.mappings.nonEmpty

    private def onRefsUpdated(ignored: AnyRef): Unit = {
        val lbCfg = toLbV2Config(poolHmMapTracker.currentRefs.head._2)
        if (createPromise != null) {
            ...
            createPromise.tryComplete(Try {
                haProxyHelper.deploy(
                    lbCfg, ifaceName, p.portMac.toString,

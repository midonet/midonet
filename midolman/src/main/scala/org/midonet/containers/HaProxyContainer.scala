...
class HaProxyContainer @Inject()(
    ...
    private val statusSubscription = Observable
        .interval(config.containers.haproxy.statusUpdateInterval.toMillis,
                  config.containers.haproxy.statusUpdateInterval.toMillis,
    ...
    private def areRefsReady(ignored: AnyRef): Boolean =
        ...
        poolHmTracker.areRefsReady &&
        poolHmTracker.currentRefs.head._2.mappings.nonEmpty
    ...
    private def publishStatus(tick: java.lang.Long): Unit = {
        ...
        statusSubject.onNext(ContainerHealth(code, namespaceName, msg))
    }

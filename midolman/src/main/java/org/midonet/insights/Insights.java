...
public final class Insights {
    ...
    public Insights(Reflections reflections, MetricRegistry metrics) {
        ...
        // Ignore inner or nested classes of this class, or classes that
        // do not have a suitable constructor.
        Iterator<Class<? extends Listener>> iterator = listeners.iterator();
        while (iterator.hasNext()) {
            Class<? extends Listener> c = iterator.next();
            if (isNestedClass(c)) {
                iterator.remove();
                continue;
            }
            try {
                c.getConstructor(MetricRegistry.class);
            } catch (NoSuchMethodException e) {
                iterator.remove();
            }
        }

        LOG.info("Insights listener classes: {}", listeners);
        ...
        if (listeners.size() == 0) {
            ...
            try {
                ...
                l = clazz.getConstructor(MetricRegistry.class)
                         .newInstance(metrics);
                LOG.info("Insights listener {} enabled: {}", clazz.getName(), l);
    ...
    /**
     * @return True if the argument class is an inner class.
     */
    private static boolean isNestedClass(Class<? extends Listener> clazz) {
        return clazz.getEnclosingClass() == Insights.class;
    }

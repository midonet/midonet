...
public class ZkNatBlockAllocator implements NatBlockAllocator {
    ...
    public ZkNatBlockAllocator(ZkConnection zk, PathBuilder paths,
        ...
        Runnable recycleBlocks = new Runnable() {
            ...
            public void run() {
                try {
                    int recycledBlocks = recycler.recycle();
                    log.debug("Recycled {} blocks", recycledBlocks);
                } catch (KeeperException e) {
                    log.error("Failed to GC unused blocks", e);
                ...
                ZkNatBlockAllocator.this.reactor.schedule(this, 1, TimeUnit.HOURS);
            }
        };
        int factor = ThreadLocalRandom.current().nextInt(10) + 1;
        reactor.schedule(recycleBlocks, 5 * factor, TimeUnit.MINUTES);
    }

...
/*
    ...
    private static boolean containsPathsStartsWith(Map<String, Op> ops,
                                                   String path) {
        for (Map.Entry<String, Op> op : ops.entrySet()) {
            if (path.startsWith(op.getKey())) {
                logger.warn("Found sub-path with Op: {}.",
                            getOpDesc(op.getValue()));
                return true;
            }
        }
        return false;

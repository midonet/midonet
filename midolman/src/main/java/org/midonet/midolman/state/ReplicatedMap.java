    ...
    public void put(final K key, final V value) {
        ...
        synchronized (this) {
            ...
            if (mapValue != null && mapValue.value != null &&
                mapValue.value.equals(value) && !deleteSet.contains(entry)) {
                log.debug("Entry {} -> {} already exists", key, value);
                return;
            }
            if (localMap.size() > MAX_SIZE) {
                log.warn("Replicated map exceeds maximum size: {}", MAX_SIZE);
                return;
            }
        }

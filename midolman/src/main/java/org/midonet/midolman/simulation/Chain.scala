...
object Chain {
     ...
     * @param isPortFilter whether the chain is being processed in a port filter
     *                     context
     */
    def apply(chain: Chain, context: PacketContext, ownerId: UUID,
        ...
        if (traversedChains.size > 25) {
            context.jlog.warn(s"Traversed ${traversedChains.size} chains " +
                s"when applying chain ${chain.id}.", context)
        }
        res
    }
}
...
class Chain(val id: UUID,
    ...
    private def apply(context: PacketContext, ownerId: UUID,
                      ...
                      checkpiontAction: CPAction = Coordinator.NO_CHECKPOINT): Unit = {
        context.jlog.debug(s"Processing chain with name $name and ID $id")
        if (depth > 10) {
        ...
        for (r <- rules if res.action == Action.CONTINUE) {
            ...
            if (res.action eq Action.JUMP) {
                ...
                if (jumpChain == null) {
                    context.jlog.error("ignoring jump to chain " +
                        s"${res.jumpToChain} : not found.", context)
                    res.action = Action.CONTINUE
                } else if (traversedChains.contains(jumpChain.id)) {
                    context.jlog.warn(s"cannot jump from chain $this to chain" +
                        s" $jumpChain -- already visited", context)

                    res.action = Action.CONTINUE
                } else {
                    jumpChain.`apply`(context, ownerId, isPortFilter, res,
                        depth + 1, traversedChains, checkpiontAction)
                    if (res.action == Action.RETURN) {
                        res.action = Action.CONTINUE
                    }
                }
            } else if (res.action eq Action.MIRROR) {
                val mirrorRule: MirrorRule = r.asInstanceOf[MirrorRule]
                val dstPortId: UUID = mirrorRule.getDstPortId
                if (dstPortId == null) {
                    context.jlog.error(
                        "ignoring mirror rule without the port ID")
                    res.action = Action.CONTINUE
                }
                checkpiontAction(context)
            }
        }

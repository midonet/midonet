...
public class Midolman {
    ...
    private void run(String[] args) throws Exception {
        ...
        injector.getInstance(MidonetBackend.class)
            .startAsync()
            .awaitRunning();
        long init15 = mark("start midonet backend service", init12);
        ...
        long init201 = mark("AFTERBOOT -> start flow expirator", init20);
        ...
        long init21 = mark("AFTERBOOT -> subscribe to logger levels and metrics", init201);
        ...
        log.info("Loaded configuration: {}",
                 configurator.dropSchema(conf, showConfigPasswords).root()
                             .render(renderOpts));
        long init22 = mark("AFTERBOOT -> render config options", init21);

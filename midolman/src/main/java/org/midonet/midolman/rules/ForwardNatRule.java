...
public class ForwardNatRule extends NatRule {
    ...
    public ForwardNatRule(Topology.Rule protoRule) {
        this(new Condition(protoRule),
             ...
             protoRule.getDnat(),
             fromTargetsProto(protoRule.getNatTargetsList()));

        if (protoRule.hasDnat() || protoRule.getNatTargetsCount() == 0) {
            throw new IllegalArgumentException(
    ...
    protected static boolean isForwardNatRule(Topology.Rule protoRule) {
        return protoRule.hasNwSrcIp() && protoRule.getOutPortIdsCount() > 0;
    }

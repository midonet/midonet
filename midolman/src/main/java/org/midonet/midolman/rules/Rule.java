...
// WARNING!
// At the moment the type field of the protocol buffer determines the subclass
// of Rule the pojo will be an instance of. As a consequence, calling toProto
// on an object of a subclass of Rule will not set the type field in the proto.
// This is not a problem since we use ZoomConvert to convert protos to pojos.
...
public abstract class Rule extends BaseConfig {
    ...
    private void validateProto(Topology.Rule protoRule) {
        if (!protoRule.hasAction() || !protoRule.hasChainId())
            throw new ZoomConvert.ConvertException("Rule " + protoRule.getId() +
                " has no chainId or no action set");
        ...
        switch (protoRule.getType()) {
            ...
            case NAT_RULE:
                ...
                // otherwise an exception would have been raised previously.
                if (!protoRule.getNatRuleData().hasDnat())
                    throw new ZoomConvert.ConvertException(
                        "NAT rule: " + protoRule.getId() + " must have its " +
                        "booleans dnat, and either match_forward_flow or "
                        + "match_return_flow set");

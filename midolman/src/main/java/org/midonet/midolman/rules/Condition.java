...
public class Condition extends BaseConfig {
    ...
    public Condition(Topology.Rule protoRule) {
        ...
        cnd.ethSrc = MAC.fromString(protoRule.getDlSrc());
        cnd.invDlSrc = protoRule.getInvDlSrc();
        cnd.ethDst = MAC.fromString(protoRule.getDlDst());
        ...
        cnd.nwSrcIp = IPSubnetUtil.fromProto(protoRule.getNwSrcIp());
        cnd.nwDstIp = IPSubnetUtil.fromProto(protoRule.getNwDstIp());
        ...
        cnd.fragmentPolicy = FragmentPolicy.valueOf(protoRule.getFragmentation().name());
    }

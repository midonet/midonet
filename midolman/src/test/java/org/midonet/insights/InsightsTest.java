...
public class InsightsTest {
    ...
    public void testDefaultListener() throws Exception {
        ...
        // And listener service methods are implemented.
        Assert.assertNull(insights.currentListener().state());
        Assert.assertNull(insights.currentListener().failureCause());

        // These methods are invoked for coverage.
        insights.currentListener().awaitRunning(0, TimeUnit.SECONDS);
        insights.currentListener().awaitTerminated(0, TimeUnit.SECONDS);
        insights.currentListener().addListener(
            Mockito.mock(Service.Listener.class),

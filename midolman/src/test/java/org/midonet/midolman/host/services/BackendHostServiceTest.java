...
public class BackendHostServiceTest extends HostServiceTest {
    ...
    public void cleanupServiceAfterStop() throws Throwable {
        ...
        assertNull(getInterfaces(hostId));
        assertEquals(hostService.shutdownCalls, 0);
    }
    ...
    public void hostServiceUpdatesHostInterfaces() throws Throwable {
        TestableHostService hostService = startService();
        Topology.HostInterfaces interfaces;

        interfaces = getInterfaces(hostId);
        assertNotNull(interfaces);
        assertTrue(interfaces.hasHostId());
        assertEquals(UUIDUtil.fromProto(interfaces.getHostId()), hostId);
        assertEquals(interfaces.getInterfacesCount(), 0);
        ...
        assertTrue(interfaces.hasHostId());
        assertEquals(UUIDUtil.fromProto(interfaces.getHostId()), hostId);
        assertEquals(interfaces.getInterfacesCount(), 1);
        assertEquals(interfaces.getInterfaces(0).getName(), "eth0");
    ...
    private Topology.HostInterfaces getInterfaces(UUID hostId) throws Exception {
        StateKey key = await(stateStore.getKey(Topology.Host.class, hostId,
        ...
        Topology.HostInterfaces.Builder builder =
            Topology.HostInterfaces.newBuilder();

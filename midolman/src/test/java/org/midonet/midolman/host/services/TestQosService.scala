...
import java.util.concurrent.{LinkedBlockingQueue => LBQ}

import org.midonet.cluster.data.storage.Storage
import org.midonet.cluster.models.Commons.UUID
import org.midonet.cluster.models.Topology.{Host, Network, QOSPolicy, QOSRuleBWLimit}
...
class TestQosService extends MidolmanSpec
    ...
    var qosPolicyRule: QOSRuleBWLimit = _
    var bridge: Network = _
    var channelFactory = new MockNetlinkChannelFactory()
    ...
    def makeQosService(scanner: InterfaceScanner): TestableTcRequestHandler = {
        ...
        val testReqHandler = new TestableTcRequestHandler(q)
        val testQosService = QosService.makeQosService(
            channelFactory, scanner, hostId, q, testReqHandler)
    ...
    feature("Multiple binds and unbinds") {
        scenario("bind and unbind") {
            ...
            val p1 = makePort("eth1")
            req += TR(add, 2)

            scanner.removeInterface("eth2")

            val p3 = makePort("eth3")

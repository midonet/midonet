...
import com.midokura.midonet.cluster.data.ports.{LogicalRouterPort, MaterializedBridgePort, MaterializedRouterPort}
...
class FloatingIpTestCase extends VirtualConfigurationBuilders with RouterHelper {
    ...
    def test() {
		...
		// Note that a reply to this packet would never make it back to
		// VM2 since it'd be SNAT'ed and not match the request

...
class FlowTracingTest extends MidolmanSpec {
    ...
    feature("Ingress/Egress matching") {
        scenario("Trace generates flow state messages") {
            ...
            val key = new TraceKey(pktCtx.origMatch)
        ...
        scenario("Trace gets enabled on egress host (no tunnel key hint)") {
            ...
            val key = new TraceKey(
                new FlowMatch(FlowKeys.fromEthernetPacket(frame)))
            ...
            val key2 = new TraceKey(pktCtx.origMatch)
    ...
    class TestableDDAWithWorkflow(val workflow2: PacketHandler)
            extends DeduplicationActor(
        injector.getInstance(classOf[MidolmanConfig]),
        new CookieGenerator(1, 1), mockDpChannel, clusterDataClient,
        new ShardedFlowStateTable[ConnTrackKey, ConnTrackValue](),

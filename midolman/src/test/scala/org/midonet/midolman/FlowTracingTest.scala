...
import org.midonet.packets.{SbeEncoder, Ethernet, IPv4Addr}
    ...
    feature("Ingress/Egress matching") {
        ...
        scenario("Trace requests should be different for 2 identical packets, " +
            ...
            def parse(p: Ethernet): FlowState = {
                ...
                discardExtendedFields(fs)
                fs
            }

            def discardExtendedFields(fs: FlowState) = {
                FlowStatePackets.uuidFromSbe(fs.ingressPortId) should not be null
                val iterEgress = fs.egressPortIds.iterator()
                while (iterEgress.hasNext)
                    FlowStatePackets.egressPortIdFromSbe(iterEgress.next) should not be null
            }

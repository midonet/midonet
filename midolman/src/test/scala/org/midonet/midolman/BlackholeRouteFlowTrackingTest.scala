...
import org.midonet.midolman.simulation.Coordinator.{ToPortAction, DropAction}
...
class BlackholeRouteFlowTrackingTest extends SingleActorTestCase
        with VirtualConfigurationBuilders {

    implicit def _actorSystem = actorSystem
    implicit def _executionContext = actorSystem.dispatcher

    var fc: MessageAccumulator = null
    ...
    var leftPort: MaterializedRouterPort = null
    var rightPort: MaterializedRouterPort = null
    ...
    lazy val vta = VirtualTopologyActor.getRef(actorSystem)
    ...
    private def buildTopology() {
        ...
        leftPort = newExteriorRouterPort(clusterRouter,
        ...
        rightPort = newExteriorRouterPort(clusterRouter,
        ...
        simRouter = ask(vta, RouterRequest(clusterRouter.getId, false))
    ...
    override def actorSpecs: Map[String, Props] =
        Map(VirtualTopologyActor.Name ->
            new Props(() => injectedInstance(() => new VirtualTopologyActor())))

    override def beforeTest() {
        fc = actorsService.actor(FlowController.Name) getOrElse null
        fc should not be null
    ...
    def packetContextFor(frame: Ethernet, inPort: UUID): PacketContext = {
        ...
        context.inPortId = ask(vta, PortRequest(inPort, false))
        context
    }

    def refreshRouter: Router = ask(vta, RouterRequest(clusterRouter.getId, false))
    ...
    feature("RouterManager tracks permanent flows but not temporary ones") {
        scenario("blackhole route") {
            ...
            action should be === DropAction(true)
        ...
        scenario("to-port route") {
            ...
            fc.get should be === List(InvalidateFlowsByTag(tag))
        }
    }
}

...
import org.apache.zookeeper.{CreateMode, Op}
...
class VppDownlinkTest extends MidolmanSpec with TopologyBuilder {
    ...
    private def createDownlink(port: Port, rule: Rule, vrf: Int)
    : CreateDownlink = {
        CreateDownlink(
            portId = port.getId,
            vrfTable = vrf,
            ip4Address = fromV4Proto(port.getPortSubnet),
            ip6Address = fromV6Proto(rule.getNat64RuleData.getPortAddress),

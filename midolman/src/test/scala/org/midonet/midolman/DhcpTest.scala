...
class DhcpTest extends MidolmanSpec {
    ...
    def getOpt121FromBytes(bytes: Array[Byte]): mutable.ListBuffer[Opt121] = {
        ...
        while (bb.hasRemaining) {
            ...
            opt121.setGateway(new IPv4Addr(bb.getInt))
            opt121Routes += opt121
        }
        opt121Routes
    }
    ...
    scenario("Classless Routes should get default route") {
        ...
        val expectedRoute = new Opt121
        // Defined as routerIp1 above, but the full IP is used
        // (192.168.11.1/24) we compare only the sub.
        val opt121sub = IPv4Subnet.fromCidr("192.168.11.0/24")
        expectedRoute.setRtDstSubnet(opt121sub)
        expectedRoute.setGateway(routerIp2.getAddress)

        var matched = false
        val classlessRoutes = DHCPOption.Code.CLASSLESS_ROUTES.value
        val opt121 = dhcpReply.getOptions
            .filter(option => option.getCode == classlessRoutes)

        opt121.size shouldBe 1
        val routes = getOpt121FromBytes(opt121.head.getData)

        val expectedRoutes = List(defaultRoute, expectedRoute)
        routes should contain theSameElementsAs expectedRoutes
    }

...
class DhcpTest extends MidolmanSpec {
    ...
    def injectDhcpDiscover(port: UUID, portNumber: Int, srcMac : MAC): Unit = {
    ...
    def injectDhcpDiscoverWithReply(port: UUID, portNumber: Int,
                                    srcMac : MAC): Ethernet = {
        injectDhcpDiscover(port, portNumber, srcMac)
    ...
    scenario("Subnet DHCP is disabled") {
        injectDhcpDiscover(bridgePort1, bridgePortNumber1, vm4Mac)
        simBackChannel.find[GeneratedLogicalPacket]() shouldBe null
    }
}

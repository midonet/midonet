...
class NatTest extends MidolmanSpec {
    ...
    scenario("destination NAT") {
        ...
        val mapping = new Mapping(newMappings.head,
                                  ...
                                  packet.getEthernet,
                                  applyOutPacketActions(packet.getEthernet,
                                                        actions))
        ...
        mapping.matchReturnOutPacket(
            applyOutPacketActions(packet2.getEthernet, actions2))
        ...
        mapping.matchForwardOutPacket(
            applyOutPacketActions(packet3.getEthernet, actions3))
    ...
    def testSnat() {
        ...
        val mapping = new Mapping(newMappings.head,
            ...
            packet.getEthernet,
            applyOutPacketActions(packet.getEthernet, actions))
    ...
    private def pingExpectDnated(inPort: UUID,
        ...
        val eth = applyOutPacketActions(packet.getEthernet, actions)
        val mapping = new Mapping(newMappings.head,
    ...
    private def expectICMPError(port: UUID, expectIcmpData: IPv4): Unit = {
        ...
        val ethApplied = applyOutPacketActions(packet.getEthernet, actions)
    ...
    private def verifySnatICMPErrorAfterPacket(userspace: Boolean) {
        ...
        val ethApplied = applyOutPacketActions(eth, actions)
    ...
    def applyOutPacketActions(packet: Ethernet,
                              actions: ju.List[FlowAction]): Ethernet = {

...
import org.midonet.cluster.data.{Router, Bridge}
...
class BridgeInvalidationTest extends MidolmanTestCase
    ...
    var uplinkPort: MaterializedRouterPort = null
    ...
    override def beforeTest() {
        ...
        uplinkPort = newExteriorRouterPort(router, MAC.fromString(uplinkPortMac),
        ...
        val simRouter: RCURouter = ask(vta, RouterRequest(router.getId, false))
        simRouter.arpTable.set(IPv4Addr(ipUplink), MAC.fromString(macUplink))(actors())
    ...
    def testLearnMAC2() {
        ...
        val add2 = ackWCAdded()
        val outports2 = actionsToOutputPorts(add2.actions)
        ...
        val add3 = ackWCAdded()
        val outports3 = actionsToOutputPorts(add3.actions)

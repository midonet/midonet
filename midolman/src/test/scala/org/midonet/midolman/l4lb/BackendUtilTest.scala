...
class BackendUtilTest extends FlatSpec with BeforeAndAfter {
    ...
    "writeWithLock" should "acquire and release the lock" in {
        ...
        backendUtil.writeWithLock(someFunction)
    ...
    "setPoolMappingStatus" should "update the pool in storage" in {
        val poolId = UUID.randomUUID()
        val pool = Pool.newBuilder().setId(UUIDUtil.toProto(poolId)).build()
        val status = PoolHealthMonitorMappingStatus.ACTIVE
        getMutexMockAndSetReturnValues(lockFactory, acquireSuccessful = true)
        Mockito.when(backend.get(classOf[Pool], poolId))
            .thenReturn(Future.successful(pool))

        backendUtil.setPoolMappingStatus(poolId, status)

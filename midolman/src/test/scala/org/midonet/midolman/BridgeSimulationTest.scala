...
class BridgeSimulationTest extends MidolmanSpec {
    ...
    private var portOnHost2: BridgePort = null
    private var portOnHost3: BridgePort = null
    ...
    val port2OnHost1Mac = MAC.fromString("0a:fe:88:70:33:ab")
    ...
    scenario("ethernet broadcast on bridge") {
        val srcMac = MAC.fromString("0a:fe:88:70:44:55")
        val ethPkt = Packets.udp(
                srcMac,
                MAC.fromString("ff:ff:ff:ff:ff:ff"),
                IPv4Addr.fromString("10.10.10.10"),
    ...
    scenario("multicast destination ethernet on bridge") {
        ...
        val ethPkt = Packets.udp(
                ...
                IPv4Addr.fromString("10.10.10.11"),
                IPv4Addr.fromString("10.11.11.10"), 10, 12,
                "Test UDP Packet".getBytes)
        ethPkt.setVlanIDs(networkVlans)
    ...
    scenario("mac migration on bridge") {
        val srcMac = MAC.fromString("02:13:66:77:88:99")
     ...
     */
    private def verifyMacLearned(learnedMac: MAC, expectedPort: UUID) {
        val ethPkt = Packets.udp(
            port2OnHost1Mac,
            learnedMac,
            IPv4Addr.fromString("10.10.10.10"),
    ...
    private def verifyFloodAction(action: SimulationResult)
            : Unit = action match {
        ...
        case _ => fail("Not FloodBridgeAction, instead: " + action.toString)
    }
}

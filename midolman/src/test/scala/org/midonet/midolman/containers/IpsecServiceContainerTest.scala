...
class IpsecServiceContainerTest extends FeatureSpecLike
    ...
    val vpnService = new IpsecServiceDef("name", "/opt/stack/stuff",
                                         ...
                                         "09:08:07:06:05:04")
    val vpnConn = new IpsecConnectionDef("test_conn",
                                         IPv4Addr.fromString("200.200.200.2"),
                                         "start", List(IPv4Subnet.fromCidr("10.1.0.0/24")),
                                         List(IPv4Subnet.fromCidr("10.2.0.0/24")), 1500,
                                         "hold", 30, 120,
                                         "never", "pfs",
                                         "esp", "aes128", "modp1536",
                                         3600, "tunnel",
    ...
    feature("VpnServiceContainer writes contents of config files") {
        ...
        scenario("VpnServiceContainer creates config for multiple conns") {
            ...
            secondConn.remotePeerIp = IPv4Addr.fromString("230.230.230.230")
    ...
    feature("Vpn script starts and stops") {
        scenario("commands are correctly executred") {
            var conf = new IpsecServiceConfig("vpn-helper", vpnService, List(vpnConn))

...
class MSSClampTest extends FlatSpec with Matchers {
    ...
    "clampMss" should "do nothing for unencasulated TCP packets" ignore {
    ...
    it should "Reduce MSS for encapsulated TCP SYN packet" ignore {
    ...
    it should "Reduce MSS for encapsulated TCP SYN-ACK packet" ignore {
        val pkt = { eth src srcMac2 dst dstMac2 } <<
                  { ip4 src srcIp2 dst dstIp2 } <<
                  { udp src srcPort2 dst dstPort2 } <<
                  { vxlan vni 5 } <<
    ...
    it should "Do nothing for encapsulated TCP ACK packet" ignore {
        val pkt = { eth src srcMac2 dst dstMac2 } <<
                  { ip4 src srcIp2 dst dstIp2 } <<
                  { udp src srcPort2 dst dstPort2 } <<
                  { vxlan vni 5 } <<
    ...
    it should "ignore options other than MSS" ignore {
        val pkt = { eth src srcMac2 dst dstMac2 } <<
                  { ip4 src srcIp2 dst dstIp2 } <<
                  { udp src srcPort2 dst dstPort2 } <<
                  { vxlan vni 5 } <<
    ...
    it should "Do nothing for ill-formed packet" ignore {
        val pkt = { eth src srcMac2 dst dstMac2 } <<
                  { ip4 src srcIp2 dst dstIp2 } <<
                  { udp src srcPort2 dst dstPort2 } <<
                  { vxlan vni 5 } <<

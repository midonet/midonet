...
class NatLeaserExpirationTest extends MidolmanSpec {
    ...
    override def beforeTest(): Unit = {
        ...
        materializePort(vmPort, host.getId, vmPortName)
        portMap += portIdCounter -> vmPort.getId
        portIdCounter += 1
        ...
        clusterDataClient.portsLink(failRtrPort.getId, failUplinkPort.getId)
        materializePort(failSubnetPort, host.getId, "failPort")

        newRoute(router, "0.0.0.0", 0,
                 failUplinkPortAddr.toNetworkAddress.toString,
                 failUplinkPortAddr.getPrefixLen,
                 NextHop.PORT, failUplinkPort.getId,
                 new IPv4Addr(Route.NO_GATEWAY).toString, 10)
        newRoute(router, "0.0.0.0", 0,
        ...
        newRoute(failRouter, "0.0.0.0", 0,
                 failSubnetPortAddr.toNetworkAddress.toString,
                 failSubnetPortAddr.getPrefixLen,
                 NextHop.PORT, failSubnetPort.getId,
                 new IPv4Addr(Route.NO_GATEWAY).toString, 10)
        newRoute(failRouter, "0.0.0.0", 0,
                 failUplinkTargetSubnet.toNetworkAddress.toString,
                 failUplinkTargetSubnet.getPrefixLen,
        ...
        newRoute(failRouter, "0.0.0.0", 0,
                 loopSubnet.toNetworkAddress.toString,
                 loopSubnet.getPrefixLen,
                 NextHop.PORT, failRtrPort.getId,
        ...
        feedArpTable(simRouter, failUplinkGatewayAddr.getAddress.addr, failUplinkGatewayMac)

        // feed the router's arp cache with each of the vm's addresses
        feedArpTable(simRouter, vmIp, vmMac)
        simBridge.vlanMacTableMap
            .getOrElse(Bridge.UNTAGGED_VLAN_ID, null).add(vmMac, vmPort.getId)

        fetchTopology(router, bridge, rtrPort, rtrInChain,
                      rtrOutChain, uplinkPort,
                      failUplinkPort, failRouter, failRtrPort,
                      failSubnetPort, vmPort)
    ...
    feature("NAT leases are freed") {
        scenario("after flow state expires") {
            ...
            injectTcp(vmPort.getId, vmMac, vmIp, 30501,

...
import org.midonet.midolman.FlowController.{AddWildcardFlow, WildcardFlowAdded, WildcardFlowRemoved}
...
class PingTestCase extends VirtualConfigurationBuilders with RouterHelper {
    ...
    def testFlood() {
        ...
        expect[WildcardFlowAdded].on(wflowAddedProbe)
            .f.getMatch.getIcmpIdentifier should be === icmpId

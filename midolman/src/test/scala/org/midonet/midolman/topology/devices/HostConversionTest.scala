...
class HostConversionTest extends FeatureSpec with Matchers {
    ...
    private def assertEquals(proto: Topology.Host, zoomObj: Host) = {
        ...
        for (portMapping <- proto.getPortInterfaceMappingList) {
            ...
            val zoomInterface = zoomObj.portInterfaceMapping.get(portMapping.getPortId.asJava)
    ...
    private def portInterfaceMapping(interface: String): Topology.PortToInterface = {
        Topology.PortToInterface.newBuilder

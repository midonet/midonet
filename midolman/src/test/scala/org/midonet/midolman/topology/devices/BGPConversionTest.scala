...
import org.midonet.midolman.routingprotocols.BGP
...
class BGPConversionTest extends FlatSpec with Matchers
    ...
    "BGP" should "convert to Protocol Buffers" in {
        val bgp = new BGP(UUID.randomUUID, random.nextInt(), random.nextInt(),
                          IPv4Addr.random, UUID.randomUUID,
                          Set(UUID.randomUUID, UUID.randomUUID))
        val proto = ZoomConvert.toProto(bgp, classOf[Topology.Bgp])

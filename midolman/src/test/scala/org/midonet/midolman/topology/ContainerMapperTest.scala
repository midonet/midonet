...
class ContainerMapperTest extends MidolmanSpec with TopologyBuilder
    ...
    feature("Test mapper lifecyle") {
        scenario("Mapper subscribes and unsubscribes") {
            ...
            val container = createServiceContainer(serviceType = Some("type"),
                                                   groupId = Some(group.getId),
                                                   configurationId = Some(randomUUID))
    ...
    feature("Test mapper container updates") {
        scenario("Updating the container from a binding") {
            ...
            val bridge = createBridge()
            val group = createServiceContainerGroup()
            val container1 = createServiceContainer(serviceType = Some("type"),
                                                    groupId = Some(group.getId),
                                                    configurationId = Some(randomUUID))
            val port1 = createBridgePort(bridgeId = Some(bridge.getId),
                                         hostId = Some(host.getId),
            ...
            val container2 = createServiceContainer(serviceType = Some("type"),
                                                    groupId = Some(group.getId),
                                                    configurationId = Some(randomUUID))
        ...
        scenario("Container without a container group") {
            Given("A port bound to a host with a container")
            val host = createHost()
            val bridge = createBridge()
            val container1 = createServiceContainer(serviceType = Some("type"),
            ...
            When("Adding the container to a group")
            val group = createServiceContainerGroup()
            val container2 = container1.setGroupId(group.getId)
                                       .setPortId(port.getId)
            store.multi(Seq(CreateOp(group), UpdateOp(container2)))

            Then("The observer should receive a Created notification")
            obs.getOnNextEvents should have size 1
            obs.getOnNextEvents.get(0) shouldBe createdFor(host, port, container2)
        }

        scenario("Container without a type") {
            Given("A port bound to a host with a container")
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup()
            ...
            When("Setting the container type")
            val container2 = container1.setServiceType("type").setPortId(port.getId)
            store.update(container2)
        ...
        scenario("Container without a configuration") {
            Given("A port bound to a host with a container")
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup()

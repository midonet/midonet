...
class PoolHealthMonitorMapperTest extends MidolmanSpec
    ...
    feature("The mapper emits pool health monitor maps") {
        ...
        scenario("The mapper emits a map on load balancer update") {
            ...
            val lbUpdated = lb.toBuilder.setAdminStateUp(true).build()
        ...
        scenario("The mapper emits a map on load balancer change") {
            ...
            val savedPool =
                Await.result(store.get(classOf[Proto.Pool], poolId), timeout)
            // Change the health monitor
            val lb2Id = UUID.randomUUID()
            val vip2 = createTopologyObject(classOf[Proto.VIP], Map(
                "pool_id" -> poolId,
                "load_balancer_id" -> lb2Id))
            val lb2 = createTopologyObject(classOf[Proto.LoadBalancer], Map(
                "id" -> lb2Id,
                "vip_ids" -> Set(vip2.getId)))
            val poolUpdated = savedPool.toBuilder.setLoadBalancerId(lb2.getId).build()
            store.create(vip2)
            store.create(lb2)
            store.update(poolUpdated)
            ...
            val data = map(poolId)
            data.healthMonitor.id shouldBe fromProto(hm.getId)
            data.loadBalancer.id shouldBe lb2Id
            data.poolMembers.map(_.id).toSet shouldBe Set(fromProto(pm1.getId))
        ...
        scenario("The map is cleared if no load balancer is associated") {
            ...
            val poolUpdated = pool.toBuilder.clearLoadBalancerId().build()
            store.update(poolUpdated)
        ...
        scenario("The mapper emits a map on vip update") {
            ...
            // Update the vip
            val vipUpdated = vip.toBuilder.setAddress(IPAddressUtil.toProto("10.0.0.1")).build()
        ...
        scenario("The mapper emits a map on vip addition") {
            ...
            val vip2 = createTopologyObject(classOf[Proto.VIP], Map(
                "pool_id" -> UUID.randomUUID(),
                "load_balancer_id" -> lbId))
            val vip3 = createTopologyObject(classOf[Proto.VIP], Map(
                "pool_id" -> poolId,
                "load_balancer_id" -> lbId))
            val lbUpdated = lb.toBuilder.addAllVipIds(
                Set(vip.getId, vip2.getId, vip3.getId)).build
            store.create(vip2)
            store.create(vip3)
            store.update(lbUpdated)

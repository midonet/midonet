...
class ContainerHandlerProviderTest extends FlatSpec with Matchers
    ...
    "Container provider" should "load all containers" in {
        ...
        val provider = new ContainerHandlerProvider(log, vt)
        ...
        provider.get("test-a").getClass shouldBe classOf[ContainerA2]
        provider.get("test-b").getClass shouldBe classOf[ContainerB]

        And("The provider fails on non-existing classes")
        intercept[NoSuchElementException] {
            provider.get("test-none")
        }
    }

...
import org.midonet.cluster.models.Topology.{Port => TopologyPort, Rule => TopologyRule, Chain => TopologyChain}
    ...
    feature("Test supported devices") {
        scenario("Test that chains are supported") {
            val literalRule = createLiteralRuleBuilder(id = UUID.randomUUID(),
                ...
                .build()
            val chain = createChain(UUID.randomUUID(), Some("test-chain"),
                                    List(literalRule.getId))
            store.multi(List(CreateOp(literalRule), CreateOp(chain)))
            ready(store.get(classOf[TopologyRule], literalRule.getId), timeout)
            ready(store.get(classOf[TopologyChain], chain.getId), timeout)

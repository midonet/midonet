...
class VirtualTopologyRedirectorTest extends MidolmanSpec with TopologyBuilder {
    ...
    feature("Test tryAsk") {
        ...
        scenario("Port exists and is not cached") {
            ...
            val portId = UUID.randomUUID
            val port = createBridgePort(id = portId)
            store.create(port)
            ready(store.get(classOf[TopologyPort], portId), 5 seconds)
        ...
        scenario("The VT returns the latest port version") {
            Given("A network port")
            val portId = UUID.randomUUID
            val port1 = createBridgePort(id = portId, tunnelKey = 1)
            store.create(port1)
            ready(store.get(classOf[TopologyPort], portId), 5 seconds)
            ...
            val port2 = createBridgePort(id = portId, tunnelKey = 2)
            store.update(port2)
            ready(store.get(classOf[TopologyPort], portId), 1 second)
        ...
        scenario("The VT removes the port from cache on deletion") {
            ...
            ready(store.exists(classOf[TopologyPort], portId), 5 seconds)
    ...
    feature("Test request with update false") {
        scenario("The port does not exist") {
            ...
            VirtualTopologyActor ! PortRequest(portId, false)
            sender.await(5 second)

            Then("The sender receives a failure message")
            expectLast { case Status.Failure(e: NotFoundException)=>
    ...
    feature("Test request with update true") {
        scenario("The sender receives all updates") {
            ...
            When("Sending a port request to the VTA and waiting for reply")
            VirtualTopologyActor ! PortRequest(portId, true)
            sender.await(5 seconds)
        ...
        scenario("The sender should not receive device deleted") {
            Given("A network port")
            val portId = UUID.randomUUID
            val port = createBridgePort(id = portId, tunnelKey = 1)
            store.create(port)
            ready(store.get(classOf[TopologyPort], portId), 5 seconds)

...
class VTPMRedirectorTest extends TestKit(ActorSystem("VTPMRedirectorTest"))
    ...
    private def addHostToTunnel(tunnel: ProtoTunnelZone, hostId: UUID,
        ...
        val updatedTunnel = createTunnelZoneBuilder(tunnel.getId, "tunnel",
                                                    hosts.toMap)
    ...
    feature("Obtaining Hosts from the VTPMRedirector") {
        ...
        scenario("Deleting the host after sending a host request") {
            ...
            //TODO(nicolas): We should probably fix the current behavior of the VTPM
            //               when a device is deleted. The VTPM should clear its
            //               cache and perhaps notify subscribers of the deletion.
            try {
                Then("No exceptions should be raised")
                val hostRequest = HostRequest(protoHost.getId, update=false)
                VirtualToPhysicalMapper.tryAsk[SimHost](hostRequest)(actorSystem)                                                                       } catch {
                case e: Exception => fail("Sending a host request for a deleted" +
                                          s"host should not throw exception: $e")
            }
        }

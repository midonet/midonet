...
class RouterMapperTest extends MidolmanSpec with TopologyBuilder
    ...
    feature("Test port route updates") {
        ...
        scenario("Port route updated") {
            ...
            // There are two distinct updates from the replicated set for
            // removing the old route and adding the new one.
            Then("The observer should receive two router updates with new route")
            obs.await(timeout) shouldBe true
    ...
    feature("Test flow invalidation") {
        scenario("For added route corresponding to destination IP") {
            val obs = createObserver(1)
            val mapper = testRouterCreated(obs, count = 2)
            val tagManager = obs.getOnNextEvents.get(0).routerMgrTagger
            ...
            fc.messages should contain theSameElementsInOrderAs List(
                InvalidateFlowsByTag(tagForDestinationIp(mapper.id, "2.0.0.1"))
            )
        }
        ...
        scenario("For removed route") {
            ...
            And("Waiting for the router update")
            obs.await(timeout) shouldBe true

            Then("The flow controller should receive a route invalidation")
            fc.messages should contain theSameElementsInOrderAs List(
                InvalidateFlowsByTag(tagForRoute(route.asJava))
            )
        }
        ...
        scenario("Invalidation via the ARP table") {
            ...
            fc.messages should contain theSameElementsInOrderAs List(
                InvalidateFlowsByTag(tagForDestinationIp(router.getId, "10.0.0.1"))
            )
        }
    }
}

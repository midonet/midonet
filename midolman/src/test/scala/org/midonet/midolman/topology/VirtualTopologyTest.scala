    ...
    feature("The topology allows subscription for port updates") {
        scenario("The port does not exist") {
            ...
            val e = intercept[NotFoundException] {
                VirtualTopology.subscribe(id, observer)
            }

            Then("Subscribe throws a NotFoundException")
            e.clazz should be (classOf[TopologyPort])
            e.id should be (id)
        }
        ...
        scenario("The observer should not receive updated after unsubscribe") {
            ...
            When("Subscribing to the port")
            val subscription = VirtualTopology.subscribe(id, observer)
        ...
        scenario("The observer unsubscribing should not affect other observers") {
            ...
            val subscription = VirtualTopology.subscribe(id, observer1)
            ...
            VirtualTopology.subscribe(id, observer2)

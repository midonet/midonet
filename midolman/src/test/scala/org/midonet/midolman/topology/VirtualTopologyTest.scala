...
class VirtualTopologyTest extends MidolmanSpec with TopologyBuilder {
    ...
    feature("The topology returns a port with tryGet()") {
        scenario("The port does not exist") {
            ...
            e2.clazz should be (classOf[TopologyPort])
            e2.id should be (id)
        }
        ...
        scenario("The port exists in the topology and is not cached") {
            ...
            val id = UUID.randomUUID
            val port = createNetworkPort(id = id)
            store.create(port)
            store.get(classOf[TopologyPort], id).await(1.second)
            ...
            Then("The request throws a NotYetException with a future")
            // The exception should contain a future
        ...
        scenario("The VT returns the latest port version") {
            Given("A network port")
            val id = UUID.randomUUID
            val port1 = createNetworkPort(id = id, tunnelKey = 1)
            store.create(port1)
            store.get(classOf[TopologyPort], id).await(1.second)
            ...
            val future = intercept[NotYetException] {
                VirtualTopology.tryGet[SimulationPort](id)
            }.waitFor.asInstanceOf[Future[SimulationPort]].await(1.second)

            Then("The thrown future completes successfully with the port")
            future.isCompleted should be (true)
            future.value should not be null
            future.value.get.get.id should be (id)
            future.value.get.get.tunnelKey should be (1)

            When("The port is updated")
            val port2 = createNetworkPort(id = id, tunnelKey = 2)
            store.update(port2)
            store.get(classOf[TopologyPort], id).await(1.second)
    ...
    feature("The topology allows subscription for port updates") {
        scenario("The port does not exist") {
            ...
            observer.notifications.size should be (1)
            observer.notifications(0).isInstanceOf[OnError] should be (true)

            And("The exception should be a NotFoundException")
            val e = observer.notifications(0).asInstanceOf[OnError].e
            e.isInstanceOf[NotFoundException] should be (true)
            e.asInstanceOf[NotFoundException].clazz should be (classOf[TopologyPort])
            e.asInstanceOf[NotFoundException].id should be (id)
        }
        ...
        scenario("The port exists in the topology and is not cached") {
            ...
            observer.notifications should contain only
                OnNext(VirtualTopology.get(id).value.get.get)

            And("The simulation device should have the same identifier")
            observer.notifications(0).asInstanceOf[OnNext[SimulationPort]]
        ...
        scenario("The port exists in the topology and is cached") {
            ...
            Then("The observer should received one onNext notification")
            observer.await(1.second)
            observer.notifications should contain only OnNext(device)

            And("The simulation device should have the same identifier")
            observer.notifications(0).asInstanceOf[OnNext[SimulationPort]]
                .value should be (device)
        }
        ...
        scenario("The observer receives update notifications") {
            ...
            observer.notifications should contain only OnNext(device1)
            ...
            observer.notifications should contain inOrder
                (OnNext(device1), OnNext(device2))
        }
        ...
        scenario("The observer unsubscribing should not affect other observers") {
            ...
            val device1 = VirtualTopology.get[SimulationPort](id).value.get.get
            observer1.notifications should contain only OnNext(device1)
            ...
            observer2.notifications should contain only OnNext(device1)
            ...
            observer2.await(1.second)
            val device2 = VirtualTopology.get[SimulationPort](id).value.get.get
            observer2.notifications should contain inOrder
                (OnNext(device1), OnNext(device2))
        }

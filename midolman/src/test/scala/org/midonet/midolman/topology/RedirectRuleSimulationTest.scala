...
class RedirectRuleSimulationTest extends MidolmanSpec with TopologyBuilder {
    ...
    feature("Test redirect rules") {
        scenario("Test redirect without vlans") {
            ...
            val leftPort = makePort(host, "left_if",
        ...
        scenario("Test redirect with vlans") {
            ...
            val vm1Port = makePort(host, "if_vm1", vmBridge,
                                   Some(vm1In.getId), Some(vm1Out.getId))
            val vm2Port = makePort(host, "if_vm2", vmBridge,
                                   Some(vm2In.getId), Some(vm2Out.getId))
            var svc1Port = makePort(host, "if_svc1", svcBridge,
                                    Some(svc1In.getId), Some(svc1Out.getId))
            val svc2Port = makePort(host, "if_svc2", svcBridge,
                                    Some(svc2In.getId), Some(svc2Out.getId))

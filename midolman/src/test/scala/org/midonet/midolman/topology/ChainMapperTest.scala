...
class ChainMapperTest extends TestKit(ActorSystem("ChainMapperTest"))
    ...
    feature("Obtaining a chain with its observable") {
        scenario("A chain with one literal rule") {
            ...
            val rule1 = buildAndStoreLiteralRule(ProtoRule.Action.ACCEPT)
            ...
            val rule2 = buildAndStoreLiteralRule(ProtoRule.Action.REJECT)
            var updatedChain = addRuleToChain(rule2, chain)

            Then("We receive the chain with the two rules")
            obs.await(timeoutOneSecond, 2) shouldBe true
            ...
            obs.await(timeoutOneSecond, 2) shouldBe true
            // A deletion of a rule results in two updates:
            // Zoom first removes the chainId from the rule and then the ruleId
            // from the chain. Hence, the chain mapper emits the chain with
            // the rule with no chainId and then the chain with just one rule.
        ...
        scenario("Obtaining a chain with get") {
            Given("A topology with one chain containing one rule")
            val chainId = UUID.randomUUID()
            val rule = buildAndStoreLiteralRule(ProtoRule.Action.ACCEPT)
            val chain = buildAndStoreChain(chainId, "test-chain",
                                           List(rule.getId))
        ...
        scenario("Obtaining a chain with a jump rule") {
            ...
            val jumpChain = buildAndStoreChain(jumpChainId, "jump-chain",
                                               List.empty)
            val jumpRule = buildAndStoreJumpRule(jumpChainId)
            val chain = buildAndStoreChain(chainId, "test-chain",
                                           List(jumpRule.getId))
    ...
    private def buildAndStoreLiteralRule(action: ProtoRule.Action)
    : ProtoRule = {
        val rule = createLiteralRuleBuilder(UUID.randomUUID(),
                                            action = Some(action))
            .build()
        store.create(rule)
        Await.ready(store.get(classOf[ProtoRule], rule.getId), timeoutOneSecond)
        rule
    }

    private def buildAndStoreJumpRule(jumpChainId: UUID)
    : ProtoRule = {
        val jumpRule = createJumpRuleBuilder(UUID.randomUUID(),
                                             jumpChainId = Some(jumpChainId))
            .build()
        store.create(jumpRule)
        Await.ready(store.get(classOf[ProtoRule], jumpRule.getId), timeoutOneSecond)
        jumpRule
    }
    ...
    : ProtoChain = {
        ...
        store.create(chain)
        Await.ready(store.get(classOf[ProtoRule], chain.getId),
                    timeoutOneSecond)
        chain
    }
}

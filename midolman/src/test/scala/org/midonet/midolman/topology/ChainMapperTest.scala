...
//        }
    ...
    private def assertChainHasRule(rule: ProtoRule, simRules: List[SimRule]) = {
        if (rule.getType == ProtoRule.Type.JUMP_TYPE) {
        ...
        } else if (rule.getType == ProtoRule.Type.LITERAL_TYPE) {
    ...
    : Unit = {
        ...
        rules.foreach(rule => {
            ...
            if (rule.getType == Type.JUMP_TYPE) {
    ...
    : ProtoRule = {
        val rule = createLiteralRuleBuilder(UUID.randomUUID(), chainId, action)
            .build()
        store.create(rule)
        rule
    }

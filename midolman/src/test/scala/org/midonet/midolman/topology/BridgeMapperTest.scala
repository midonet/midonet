...
import org.midonet.midolman.topology.BridgeMapper.MacTableReplicatedMap
...
class BridgeMapperTest extends MidolmanSpec with TopologyBuilder
    ...
    feature("Test flow invalidation") {
        ...
        scenario("For MAC added to port") {
            ...
            val map = if (vt.backend.isEnabled) {
                new BridgeMapper.MacTableMergedMap(vt, bridgeId, vlanId)
            } else {

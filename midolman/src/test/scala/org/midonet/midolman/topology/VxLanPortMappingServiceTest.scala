...
import org.midonet.midolman.topology.VxLanPortMappingService.{TunnelInfo, portOf, vtepOf}
...
class VxLanPortMappingServiceTest extends MidolmanSpec
    ...
    feature("Mapper returns None for non-existing keys") {
        scenario("Non-existing VNI") {
            ...
            vtepOf(vtepId) shouldBe Some(TunnelInfo(tunnelIp, tzoneId))
        }
    ...
    feature("Mapper processes VTEP updates") {
        ...
        scenario("Updating the VTEP tunnel IP") {
            ...
            vtepOf(vtepId) shouldBe Some(TunnelInfo(tunnelIp1, tzoneId))
            ...
            And("The mapper returns the VTEP information for the VTEP")
            vtepOf(vtepId) shouldBe Some(TunnelInfo(tunnelIp2, tzoneId))
        }
        ...
        scenario("Adding a VTEP with binding to the same network") {
            ...
            Then("The mapper returns the VTEP information for the VTEP")
            vtepOf(vtepId1) shouldBe Some(TunnelInfo(tunnelIp1, tzoneId1))
            ...
            And("The mapper returns the VTEP information for the VTEP")
            vtepOf(vtepId1) shouldBe Some(TunnelInfo(tunnelIp1, tzoneId1))
            vtepOf(vtepId2) shouldBe Some(TunnelInfo(tunnelIp2, tzoneId2))
            ...
            vtepOf(vtepId1) shouldBe Some(TunnelInfo(tunnelIp1, tzoneId1))
            vtepOf(vtepId2) shouldBe Some(TunnelInfo(tunnelIp2, tzoneId2))
            vtepOf(vtepId3) shouldBe Some(TunnelInfo(tunnelIp3, tzoneId3))
        }
        ...
        scenario("VTEP error does not prevent other VTEP update bound to the " +
            ...
            And("The mapper returns the VTEP information for the VTEP")
            vtepOf(vtepId1) shouldBe None
            vtepOf(vtepId2) shouldBe Some(TunnelInfo(tunnelIp3, tzoneId2))
        }

...
import org.midonet.cluster.models.Topology.{PortToInterface, Host, TunnelZone, Port}
...
trait TopologyBuilder {
    ...
    protected def createTunnelZoneBuilder(id: UUID,
                                        name: String,
                                        hostIds: Set[UUID]): TunnelZone.Builder = {
        val builder = TunnelZone.newBuilder
            .setId(id.asProto)
            .setName(name)
        hostIds foreach { hostId => builder.addHostIds(hostId.asProto) }
        builder
    }
    ...
    protected def createHostBuilder(id: UUID,
        ...
        converter.asInstanceOf[MapConverter[UUID, String, PortToInterface]]

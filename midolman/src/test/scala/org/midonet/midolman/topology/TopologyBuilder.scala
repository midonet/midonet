...
trait TopologyBuilder {
    ...
    protected def createRouter(id: UUID = UUID.randomUUID,
                               ...
                               bgpPeerIds: Set[UUID] = Set.empty): Router = {
...
object TopologyBuilder {
    ...
    class RichRouter(val router: Router) extends AnyVal {
        ...
        def addBgpNetworkId(networkId: UUID): Router =
            router.toBuilder.addBgpNetworkIds(networkId.asProto).build()
        def addBgpPeerId(peerId: UUID): Router =
            router.toBuilder.addBgpPeerIds(peerId.asProto).build()

...
trait TopologyBuilder {
    ...
    protected def createVip(adminStateUp: Option[Boolean] = None,
        ...
        if (isStickySourceIp.isDefined && isStickySourceIp.get) {
            val sessionPersistenceBldr = VIP.SessionPersistence.newBuilder
                .setType(Type.SOURCE_IP)
            builder.setSessionPersistence(sessionPersistenceBldr)
        }

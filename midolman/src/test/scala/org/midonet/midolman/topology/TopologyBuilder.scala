...
import org.midonet.midolman.topology.devices.{HostIpConverter, PortInterfaceConverter}
...
trait TopologyBuilder {
    ...
    protected def createTunnelZoneBuilder(id: UUID, name: String,
        ...
        val hostList = converter.asInstanceOf[MapConverter[UUID, IPAddr, HostToIp]]
            .toProto(hosts, classOf[HostToIp])
        builder.addAllHosts(hostList)
        builder
    }

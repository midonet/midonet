...
import org.midonet.cluster.models.Topology.{BgpNetwork, BgpPeer, Port, Router}
...
class BgpPortMapperTest extends MidolmanSpec with TopologyBuilder
    ...
    private def bgpPort(port: Port, router: Router,
                        peers: Set[BgpPeer] = Set.empty,
                        networks: Set[BgpNetwork] = Set.empty): BgpPort = {
        BgpPort(ZoomConvert.fromProto(port, classOf[RouterPort]),
                BgpRouter(
                    ...
                    neighborIds = peers.map(_.getId.asJava),
                    networks = networks
    ...
    feature("Test mapper lifecycle") {
        scenario("Mapper subscribes and unsubscribes") {
            ...
            eventually { mapper.mapperState shouldBe MapperState.Unsubscribed }
            ...
            eventually { mapper.mapperState shouldBe MapperState.Subscribed }
            ...
            eventually { mapper.mapperState shouldBe MapperState.Closed }
            eventually { mapper.hasObservers shouldBe false }

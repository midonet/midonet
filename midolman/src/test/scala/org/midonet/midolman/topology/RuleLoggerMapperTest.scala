...
class RuleLoggerMapperTest extends MidolmanSpec with TopologyBuilder
    ...
    feature("RuleLoggerMapper") {
        scenario("Publishes existing RuleLogger on startup.") {
            ...
            val obs = createMapperAndObserver(rl.getId.asJava)
    ...
    private def createMapperAndObserver(rlId: UUID)
    : TestAwaitableObserver[SimRuleLogger] = {
        val mapper = new RuleLoggerMapper(rlId, vt)
        val obs = new TestAwaitableObserver[SimRuleLogger]

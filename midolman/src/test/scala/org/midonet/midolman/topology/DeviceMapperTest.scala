...
class DeviceMapperTest extends MidolmanSpec {
    ...
    class TestableMapper(id: UUID, obs: Observable[TestableDevice],
      metricsRegistry: MetricRegistry = null) (implicit vt: VirtualTopology)
    ...
    object MetricableObservable {
        def apply(mapper: TestableMapper) =
            Observable.create(mapper)
    }
  ...
  feature("Test metrics") {
        scenario("A used observable subscribes to storage") {
            ...
            val observable = MetricableObservable(mapper)

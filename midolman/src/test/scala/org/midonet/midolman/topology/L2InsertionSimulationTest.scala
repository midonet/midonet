    ...
    feature("Test redirect without vlans") {
        scenario("Test") {
            ...
            eventually (timeout(Span(2, Seconds))) {
                val c1inS = VirtualTopology.tryGet[SimChain](vm1Port.getInboundChain(0))
                c1inS.getRules.size should be(3)
            }
            val c1outS = VirtualTopology.tryGet[SimChain](vm1Port.getOutboundChain(0))
            val srv1inS = VirtualTopology.tryGet[SimChain](srv1Port.getInboundChain(0))
            val srv1outS = VirtualTopology.tryGet[SimChain](srv1Port.getOutboundChain(0))



            When("A packet ingresses vm1's port")
            val packetContext = packetContextFor(packet, vm1Port.getId)

...
import org.midonet.cluster.models.Topology.{ServiceContainer, ServiceContainerGroup, Host, Port}
...
import org.midonet.midolman.topology.containers.ContainerServiceTest.TestContainer
...
class ContainerServiceTest extends MidolmanSpec with TopologyBuilder {
    ...
    def containerPort(host: Host, port: Port, container: ServiceContainer,
        ...
        ContainerPort(port.getId, host.getId, port.getInterfaceName,
                      container.getId, group.getId, container.getConfigurationId)
    }
    ...
    feature("Test service processes container notifications") {
        scenario("Start and stop unknown container") {
            ...
            val group = createServiceContainerGroup(serviceType = Some("unknown"))
        ...
        scenario("Start and stop a good container") {
            ...
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup(serviceType = Some("test-good"))
        ...
        scenario("Start and stop a bad container for create") {
            ...
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup(serviceType = Some("test-error-create"))
            val container = createServiceContainer(configurationId = Some(randomUUID),
                                                   groupId = Some(group.getId))
            val port = createBridgePort(bridgeId = Some(bridge.getId),
                                        hostId = Some(host.getId),
        ...
        scenario("Start and stop a bad container for update") {
            ...
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup(serviceType = Some("test-error-update"))
            val container1 = createServiceContainer(configurationId = Some(randomUUID),
                                                    groupId = Some(group.getId))
            val port = createBridgePort(bridgeId = Some(bridge.getId),
                                        hostId = Some(host.getId),
        ...
        scenario("Start and stop a bad container for delete") {
            ...
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup(serviceType = Some("test-error-delete"))
            val container1 = createServiceContainer(configurationId = Some(randomUUID),
                                                    groupId = Some(group.getId))
            val port = createBridgePort(bridgeId = Some(bridge.getId),
                                        hostId = Some(host.getId),
    ...
    feature("Service handles exceptions thrown by the container handlers") {
        scenario("Exception thrown on create") {
            ...
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup(serviceType = Some("test-throws-create"))
            val container = createServiceContainer(configurationId = Some(randomUUID),
                                                   groupId = Some(group.getId))
            val port = createBridgePort(bridgeId = Some(bridge.getId),
                                        hostId = Some(host.getId),
        ...
        scenario("Exception thrown on update") {
            ...
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup(serviceType = Some("test-throws-update"))
            val container1 = createServiceContainer(configurationId = Some(randomUUID),
                                                    groupId = Some(group.getId))
            val port = createBridgePort(bridgeId = Some(bridge.getId),
                                        hostId = Some(host.getId),
        ...
        scenario("Exception thrown on delete") {
            ...
            val host = createHost()
            val bridge = createBridge()
            val group = createServiceContainerGroup(serviceType = Some("test-throws-delete"))
            val container1 = createServiceContainer(configurationId = Some(randomUUID),
                                                    groupId = Some(group.getId))
            val port = createBridgePort(bridgeId = Some(bridge.getId),
                                        hostId = Some(host.getId),

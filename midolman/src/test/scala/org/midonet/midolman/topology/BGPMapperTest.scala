...
import org.midonet.cluster.data.storage.{NotFoundException, Storage, TestAwaitableObserver}
...
import org.midonet.util.reactivex.AssertableObserver
...
class BGPMapperTest extends MidolmanSpec with TopologyBuilder
    ...
    private def createObserver() = {
        new TestAwaitableObserver[BGPUpdate] with AssertableObserver[BGPUpdate] {
            override def assert() =

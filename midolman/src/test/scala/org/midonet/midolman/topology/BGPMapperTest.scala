...
import org.midonet.cluster.models.Topology.{Bgp, BgpAdRoute, Port}
...
class BGPMapperTest extends MidolmanSpec with TopologyBuilder
    ...
    feature("Test BGP peering updates for exterior ports") {
        ...
        scenario("When adding/removing multiple BGP peerings") {
            ...
            When("Adding a second BGP peering")
            val bgp2 = createBGP(portId = Some(port.getId))
            store.create(bgp2)

            Then("The observer should receive the second BGP peering")
            obs.awaitOnNext(4, timeout) shouldBe true
            obs.getOnNextEvents.get(3) match {
                case BGPAdded(b) => b shouldBeDeviceOf bgp2
                case u => fail(s"Unexpected update $u")
            }

            When("Adding a third BGP peering")
            val bgp3 = createBGP(portId = Some(port.getId))
            store.create(bgp3)


            Then("The observer should receive the third BGP peering")
            obs.awaitOnNext(6, timeout) shouldBe true
            obs.getOnNextEvents.get(5) match {
                case BGPAdded(b) => b shouldBeDeviceOf bgp3
                case u => fail(s"Unexpected update $u")
            }

            When("The second BGP peering is updated")
            val bgp4 = bgp2.setLocalAs(10)
            store.update(bgp4)

            Then("The observer should receive the BGP update")
            obs.awaitOnNext(7, timeout) shouldBe true
            obs.getOnNextEvents.get(6) match {
                case BGPUpdated(b) => b shouldBeDeviceOf bgp4
                case u => fail(s"Unexpected update $u")
            }

            When("Deleting the first BGP peering")
            store.delete(classOf[Bgp], bgp1.getId)

            Then("The observer should receive a BGP removal update")
            obs.awaitOnNext(9, timeout) shouldBe true
            obs.getOnNextEvents.get(7) match {
                case BGPRemoved(id) => id shouldBe bgp1.getId.asJava
                case u => fail(s"Unexpected update $u")
            }

            When("Deleting the second BGP peering")
            store.delete(classOf[Bgp], bgp2.getId)

            Then("The observer should receive a BGP removal update")
            obs.awaitOnNext(11, timeout) shouldBe true
            obs.getOnNextEvents.get(9) match {
                case BGPRemoved(id) => id shouldBe bgp2.getId.asJava
                case u => fail(s"Unexpected update $u")
            }

            When("Deleting the third BGP peering")
            store.delete(classOf[Bgp], bgp3.getId)

            Then("The observer should receive a BGP removal update")
            obs.awaitOnNext(13, timeout) shouldBe true
            obs.getOnNextEvents.get(11) match {
                case BGPRemoved(id) => id shouldBe bgp3.getId.asJava
                case u => fail(s"Unexpected update $u")
            }
            obs.getOnNextEvents.get(12) match {
                case BGPPort(p) => p shouldBeDeviceOf port
                case u => fail(s"Unexpected update $u")
            }
        }
    ...
    feature("Test BGP peering are not advertised for interior ports") {
        ...
        scenario("An exterior port becomes interior and adds BGP") {
            ...
            obs.awaitOnNext(3, timeout) shouldBe true
            obs.getOnNextEvents.get(2) match {
                case BGPPort(p) => p shouldBeDeviceOf port2.addPortBgpId(bgp.getId)
                case u => fail(s"Unexpected update $u")
            }
        ...
        scenario("An interior port with BGP becomes exterior") {
            ...
            val port2 = makePortExterior(port1).addPortBgpId(bgp.getId)
            store.update(port2)
    ...
    feature("Test BGP route updates") {
        ...
        scenario("For removed BGP route") {
            ...
            store.delete(classOf[BgpAdRoute], route.getId)
    ...
    feature("Test subscriber receives initial state") {
        ...
        scenario("With updates from a previous subscriber") {
            ...
            store.delete(classOf[BgpAdRoute], route1.getId)

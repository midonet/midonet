...
import org.midonet.cluster.storage.Fip64StateTable.{DefaultValue, Fip64Encoder}
...
class Fip64StateTableTest extends FlatSpec with GivenWhenThen with Matchers
    ...
    private def hasNode(id: UUID, version: Int): Boolean = {
        val stat = new Stat
        curator.getData.storingStatIn(stat).forPath(getPath(id, version))
        stat.getEphemeralOwner != 0
    }

    private def hasPersistentNode(id: UUID): Boolean = {
        val stat = new Stat
        curator.getData.storingStatIn(stat).forPath(getPersistentPath(id))
        stat.getEphemeralOwner == 0
    }
    ...
    "Encoder" should "decode key and values" in {
        ...
        val key = KeyValue.newBuilder().setDataVariable(
            ByteString.copyFrom(buffer.array())).build()

        Then("The decoder decodes the key")
        encoder.keyOf(key) shouldBe id

...
class WildcardFlowTest extends Suite with Matchers {
    ...
    def testCombineExpirationTimes() {
        val wf1 = WildcardFlow.apply(new FlowMatch(), List(), 0, 0, 0)
        val wf2 = WildcardFlow.apply(new FlowMatch(), List(), 0, 0, 0)

        val wf3 = WildcardFlow.apply(new FlowMatch(), List(), 10, 0, 0)
        val wf4 = WildcardFlow.apply(new FlowMatch(), List(), 0, 10, 0)

        val wf5 = WildcardFlow.apply(new FlowMatch(), List(), 2, 0, 0)
        val wf6 = WildcardFlow.apply(new FlowMatch(), List(), 0, 2, 0)

        val wf7 = WildcardFlow.apply(new FlowMatch(), List(), 1, 1, 0)

...
import org.midonet.cluster.services.Context
...
class FlowStateServiceTest extends FeatureSpec with GivenWhenThen with Matchers
    ...
    before {
        ...
        midolmanConfig = new MidolmanConfig(ConfigFactory.parseString(
            s"""
               |zookeeper.zookeeper_hosts = "${zk.getConnectString}"
               |agent.flow_state.enabled : true
               |agent.flow_state.port : 1234

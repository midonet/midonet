...
class FlowStateServiceTest extends FlowStateBaseTest
    ...
    private class FlowStateServiceTest(nodeContext: Context, curator: CuratorFramework,
        ...
        override def startServerFrontEnd(): Unit = {
            port shouldBe 1234
            super.startServerFrontEnd()
        }

        def getCassandraSession: Session = cassandraSession

        def getMessageHandler: FlowStateMessageHandler = messageHandler
    ...
    private class TestableStorageHandler(session: Session)
        ...
        def getStorageProvider: ThreadLocal[FlowStateStorageWriter] = storageProvider

        override def getStorage: FlowStateStorageWriter = {
            if (storage eq null) {

...
import org.midonet.midolman.state.transfer.PipelinedSbeEncoderTranslator
...
class FlowStateServiceTest extends FlowStateBaseTest
    ...
    private val tcpClient = new FlowStateRequestClient(3000)
    ...
    before {
        ...
        val flowStateConfig = ConfigFactory.parseString(
            s"""
               ...
               |agent.minions.flow_state.tcp_timeout : 5s
    ...
    feature("Flow state read message handling") {
        scenario("Service read handler receives valid raw state request") {
            ...
            And("The response sent is an AckRaw")
            val response = parseMockedTransferResponse(mockedCtx)
            assert(response.getClass == classOf[StateAckRaw])
        }
        ...
        scenario("Service read handler receives valid remote state request") {
            ...
            And("The response sent is an AckRemote")
            val response = parseMockedTransferResponse(mockedCtx)
            assert(response.getClass == classOf[StateAckRemote])
        }
        ...
        scenario("Service read handler receives valid internal state request") {
            ...
            And("The response sent is an AckInternal")
            val response = parseMockedTransferResponse(mockedCtx)
            assert(response.getClass == classOf[StateAckInternal])
        }
    ...
    feature("Client-Server interaction between agents/flow state minions") {
        scenario("A raw flow state request between minions") {
            ...
            val server = ServerFrontEnd.tcp(handler, midolmanConfig.flowState.tcpPort)
            ...
            try {
                ...
                tcpClient.rawPipelinedFlowStateFrom("127.0.0.1",
                    midolmanConfig.flowState.tcpPort, portId, writer)
        ...
        scenario("A internal flow state request between minion and agent") {
            ...
            try {
                ...
                When("The flow state is requested by the TCP client")
                val translator = mock(classOf[PipelinedSbeEncoderTranslator])
                tcpClient.internalFlowStateFrom(midolmanConfig.flowState.tcpPort,
                    portId, translator)

                Then("The flow state for the given portId was received")
                verify(translator, times(1)).pushToPipeline(mockito.any())
            } finally {
                server.stopAsync().awaitTerminated(20, TimeUnit.SECONDS)
            }
        }
        ...
        scenario("A remote flow state request between minion and agent") {
            ...
            val newTcpPort = midolmanConfig.flowState.tcpPort + 1
            ...
            val remoteServer = ServerFrontEnd.tcp(remoteHandler, newTcpPort)
            ...
            try {
                ...
                When("The flow state is requested by the TCP client")
                val translator = mock(classOf[PipelinedSbeEncoderTranslator])
                tcpClient.remoteFlowStateFrom("127.0.0.1", newTcpPort, portId,
                    translator)

                Then("The flow state for the given portId was received")
                // It's 2 because our 'remote' is on the same host, so it's overwritten
                verify(translator, times(2)).pushToPipeline(mockito.any())
            } finally {
                server.stopAsync().awaitTerminated(20, TimeUnit.SECONDS)
            }
        }
        ...
        scenario("An invalid flow state transfer between agents") {
            ...
            try {
                ...
                verify(translator, times(0)).pushToPipeline(mockito.any())
            } finally {
                server.stopAsync().awaitTerminated(20, TimeUnit.SECONDS)
            }
        }

...
trait FlowStateBaseTest extends FeatureSpec
    ...
    : (DatagramPacket, FlowStateProtos, SbeEncoder) = {
        ...
        // Decode message so the decoded length is available in the decoder
        encoder.decodeFrom(encoder.flowStateBuffer.byteArray())
        val conntrack = encoder.flowStateMessageDecoder.conntrack()
        while (conntrack.hasNext) conntrack.next()
        val nat = encoder.flowStateMessageDecoder.nat()
        while (nat.hasNext) nat.next()
        val trace = encoder.flowStateMessageDecoder.trace()
        while (trace.hasNext) trace.next()
        val traceIds = encoder.flowStateMessageDecoder.traceRequestIds()
        while (traceIds.hasNext) traceIds.next()
        val portIds = encoder.flowStateMessageDecoder.portIds()
        while (portIds.hasNext) portIds.next()

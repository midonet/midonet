...
trait FlowStateBaseTest extends FeatureSpec
    ...
    protected def createValidFlowStatePorts(flowStateConfig: FlowStateConfig) = {
        val validPorts = (1 to 3) map { _ => UUID.randomUUID() }
    ...
    protected def rawStateRequest(port: UUID = UUID.randomUUID()) = {
        val request = buildStateRequestRaw(port)
        Unpooled.wrappedBuffer(request.toByteArray)
    }

    protected def internalStateRequest(port: UUID = UUID.randomUUID()) = {
        val request = buildStateRequestInternal(port)
        Unpooled.wrappedBuffer(request.toByteArray)
    }

    protected def remoteStateRequest(port: UUID = UUID.randomUUID()) = {

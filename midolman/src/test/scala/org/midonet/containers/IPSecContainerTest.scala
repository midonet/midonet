...
class IPSecContainerTest extends MidolmanSpec with Matchers with TopologyBuilder {
    ...
    feature("IPSecConfig") {
        scenario("Sanitize connection name") {
            Given("A valid connection name")
            var name = "asd876asd"
            val id = UUID.randomUUID()

            Then("The sanitized name should be the same as the original")
            IPSecConfig.sanitizeName(id, name) shouldBe "ipsec-" + name

            When("The connection name contains invalid characters")
            name = "sdf%876!@%$}/\\"

            Then("The sanitized name should not contain the invalid characters")
            IPSecConfig.sanitizeName(id, name) shouldBe "ipsec-sdf876"

            When("The name contains only invalid characters")
            name ="~!$#%^*(\\}]["

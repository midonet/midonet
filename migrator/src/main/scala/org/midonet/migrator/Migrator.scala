...
//            migratePortGroups()
//            migratePorts()
            migrateRoutes(routers.map(_.id))
    ...
    private def migrateBridges(): Unit = {
        for (b <- legacyImporter.listBridges) {
            ...
            val resp = resources.bridges.create(b, APPLICATION_BRIDGE_JSON_V3)
            handleResponse(resp)
    ...
    private def migrateChains(): Unit = {
        for (chain <- legacyImporter.listChains) {
            ...
            chain.ruleIds = null
            resources.chains.create(chain, APPLICATION_CHAIN_JSON)
            val chainRuleResource = resources.chains.rules(chain.id)
    ...
    /** Migrates port groups. No prerequisites. */

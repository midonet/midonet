...
class QOSPolicy(resource_base.ResourceBase):
    ...
    def get_bwlimit_rules(self):
        query = {}
        headers = {'Accept': self.bwlimit_coll_mtype}

        return self.get_children(
            self.dto['bwLimitRules'], query, headers,
            qos_rule_bwlimit.QOSRuleBWLimit)

    def add_bwlimit_rule(self):
        return qos_rule_bwlimit.QOSRuleBWLimit(

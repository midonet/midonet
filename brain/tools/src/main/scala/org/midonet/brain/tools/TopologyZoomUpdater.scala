 ...
 *    - Fixed Router
 *       - Fixed Network
 *          * Contains ports for bindings to the vteps (fixed and temporary)
 *          * Contains ports bound to hosts (fixed and temporary)
 *          - fixed PortGroup
 *             * Contains temporary ports
...
class TopologyZoomUpdater @Inject()(implicit val storage: Storage,
    ...
    /** updatable vteps */
    private var vtepIds: Set[Commons.UUID] = Set()

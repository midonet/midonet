...
class TopologyZoomUpdater @Inject()(val backend: MidonetBackend,
    ...
    implicit val storage = backend.store;
    ...
    /** updatable Vteps */
    private var tmpVtepIds: Set[Commons.UUID] = Set()
    def tmpVteps: Iterable[Vtep] = tmpVtepIds flatMap {Vtep.get(_)}

    /** updatable Hosts */
    private var tmpHostIds: Set[Commons.UUID] = Set()
    ...
    private def addVtep() = {
        ...
        tmpVtepIds = tmpVtepIds + vtep.getId
    ...
    private def addHost() = {
        ...
        tmpHostIds = tmpHostIds + host.getId
    ...
    private def rmVtep(vt: Vtep) = {
        tmpVtepIds = tmpVtepIds - vt.getId
        vt.delete()
    }

    private def rmHost(h: Host) = {
        tmpHostIds = tmpHostIds - h.getId
...
class TopologyEntity(protected var proto: Message)
    ...
    protected def getRepeatedField[T](f: String, k: Class[T]): Iterable[T] = {
        val field = proto.getDescriptorForType.findFieldByName(f)
        if (field != null) {
            for {idx <- 0 to proto.getRepeatedFieldCount(field) - 1}
 ...
 */
abstract class VSwitch(p: Message)(implicit storage: Storage)
...
object Vtep {
    def apply(tzName: String)(implicit storage: Storage): Vtep =
        new Vtep(tzName)
    def get(id: Commons.UUID)(implicit storage: Storage): Option[Vtep] =

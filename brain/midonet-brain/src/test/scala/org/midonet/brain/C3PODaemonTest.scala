...
class C3PODaemonTest extends FlatSpec with BeforeAndAfter with Matchers {
    ...
    before {
        ...
        try {
            ...
            // If an exception is thrown inside the before clause, the after
            // clause will not be run, therefore need to delete the Task DB file
            // if it exists.
            case th: Throwable =>

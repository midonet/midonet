...
class FloatingIpTranslatorTestBase extends FlatSpec with BeforeAndAfter
    ...
    protected val prvRouterGatewayPort = mPortFromTxt(s"""
        id { $prvRouterGatewayPortId }
        router_id { ${RouterTranslator.providerRouterId} }
        peer_id { $fipRouterGatewayPortId }
        """)
    protected val fipRouterInternalPort = mPortFromTxt(s"""
        id { $fipRouterInternalPortId }
        network_id { ${randomUuidProto} }
        """)
    protected val fipRouterGatewayPort = mPortFromTxt(s"""
        id { $fipRouterGatewayPortId }
        router_id { $fipRouterId }
        peer_id { $prvRouterGatewayPortId }
        """)
...
class FloatingIpTranslatorCreateTest extends FloatingIpTranslatorTestBase {
    before {
        ...
        when(storage.get(classOf[Port], fipRouterInternalPortId))
            .thenReturn(Promise.successful(fipRouterInternalPort).future)
        when(storage.get(classOf[Port], fipRouterGatewayPortId))
            .thenReturn(Promise.successful(fipRouterGatewayPort).future)
        when(storage.get(classOf[Port], prvRouterGatewayPortId))
            .thenReturn(Promise.successful(prvRouterGatewayPort).future)
    }

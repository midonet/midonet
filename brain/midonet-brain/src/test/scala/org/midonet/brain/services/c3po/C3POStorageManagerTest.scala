    ...
    def startsWith(pOps: PersistenceOp*) = {
        argThat(new ArgumentMatcher[Seq[PersistenceOp]] {
            override def matches(list: Object) = {
                val ops = list.asInstanceOf[Seq[PersistenceOp]]
                var matches = ops.length >= pOps.length
                if (matches) {
                   for (i <- 0 to (pOps.length - 1) if (!pOps(i).equals(ops(i)))) {
                       matches = false
                   }
                }
                matches
...
class C3POStorageManagerTest extends FlatSpec with BeforeAndAfterEach {
    ...
    override def beforeEach() = {
        ...
        stateMgrDataExists.success(true)
    ...
    "Mido Network" in {
        ...
        storageManager.interpretAndExec(c3poUpdate(2, updatedNetwork))

...
class C3POStorageManagerTest extends FlatSpec with BeforeAndAfterEach {
    ...
    val portId = UUIDUtil.randomUuidProto
    ...
    "Mido Network" in {
        ...
        storageManager.interpretAndExec(C3POCreate(neutronNetwork))
    ...
    "Mido Network" in {
        ...
        storageManager.interpretAndExec(C3POUpdate(
    ...
    "Mido Network" in {
        ...
        storageManager.interpretAndExec(
                C3PODelete(classOf[NeutronNetwork], networkId))

        verify(storage).multi(List(
                DeleteOp(classOf[Network], networkId)))
    }

    "Neutron transaction" should "call make a single equivalent ZOOM.multi " +
    "call." in {
        ...
        storageManager.interpretAndExecTxn("txn1",
                List(C3POCreate(neutronNetwork),
                     C3POCreate(neutronNetworkPort)))

        verify(storage).multi(List(CreateOp(midoNetwork), CreateOp(midoPort)))
    ...
    "Model translation failure" should "throw C3PODataManagerException" in {
        ...
        intercept[C3PODataManagerException] {
            storageManager.interpretAndExecTxn(
                    "txn1", List(C3POCreate(neutronNetwork)))
        }
    }

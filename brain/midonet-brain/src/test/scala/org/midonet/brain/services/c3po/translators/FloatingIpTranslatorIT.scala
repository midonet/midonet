...
class FloatingIpTranslatorIT extends C3POMinionTestBase {
    ...
    "C3PO" should "set a route to GW for the floating IP." in {
        ...
        // Tests that the tenant Router has been hook up with Provider Router
        ...
        fipRoute.getRouterId shouldBe RouterTranslator.providerRouterId

        // Provider Router should have routes for the Floating IP set up now.
        eventually {
            prvRtr = storage.get(
                classOf[Router], RouterTranslator.providerRouterId).await()
            prvRtr.getRouteIdsList.contains(fipRouteId) shouldBe true
        }
        ...
        prvRtr = storage.get(
            classOf[Router], RouterTranslator.providerRouterId).await()
        prvRtr.getRouteIdsList.contains(fipRouteId) shouldBe false
    }
}
...
import org.midonet.brain.services.c3po.translators.{RouterTranslator, PortManager, RouteManager}
...
class C3POMinionTest extends FlatSpec with BeforeAndAfter
    ...
    it should "handle router CRUD" in {
        ...
        eventually {
            ...
            tx2Futures(0).await().getTenantId shouldBe "new-tenant"
            tx2Futures(1).await().getName shouldBe "router2"
        }
    ...
    it should "handle router gateway CRUD" in {
        ...
        val prGwPortJson = portJson("pr-r1-gw-port", prGwPortId, nwId,
                                    fixedIps = List(gwIpAlloc),
                                    deviceId = prGwPortId,
                                    deviceOwner = DeviceOwner.ROUTER_GATEWAY)

        val trId = UUID.randomUUID()
        val trJson = routerJson("router1", trId, gwPortId = prGwPortId)
        ...
        val r1RenamedJson = routerJson("router1-renamed", trId).toString
        executeSqlStmts(insertTaskSql(6, Update, RouterType,
                                      r1RenamedJson, trId, "tx5"))
        val r1Renamed = eventually {
            val r1Renamed = storage.get(classOf[Router], trId).await()
            r1Renamed.getName shouldBe "router1-renamed"
            r1Renamed
        }
        tr.getPortIdsCount shouldBe 1
        validateGateway(r1Renamed, prGwPortId, gwIpAddr)
        ...
        val List(prNoGw, trNoGw) =
            storage.getAll(classOf[Router], List(prId, trId)).map(_.await())
        prNoGw.getPortIdsCount shouldBe 0
        trNoGw.getPortIdsCount shouldBe 0
    ...
    def validateGateway(tr: Router, prGwPortId: UUID,
        ...
        prGwRt.getDstSubnet shouldBe
            IPSubnetUtil.toProto(trGwPort.getPortAddress)
    ...
    private def validateLocalRoute(rt: Route, nextHopPort: Port): Unit = {
        ...
        rt.getDstSubnet shouldBe
            IPSubnetUtil.toProto(nextHopPort.getPortAddress)

...
class C3POMinionTest extends FlatSpec with BeforeAndAfter
    ...
    it should "handle Config / AgentMembership Create" in {
        ...
        Thread.sleep(1000)  // This is necessary otherwise get below may
                            // below return the old value.
        val tz1 = eventually(storage.get(classOf[TunnelZone], cId).await())
        tz1.getHostsList.size shouldBe(1)
        ...
        executeSqlStmts(insertMidoNetTaskSql(4, Delete, AgentMembershipType,
                                             "", hostId, "tx3"))
        Thread.sleep(1000)  // Likewise.
        val tz2 = eventually(storage.get(classOf[TunnelZone], cId).await())

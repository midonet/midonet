...
import org.midonet.cluster.data.storage.{Storage, ObjectReferencedException}
...
class C3POMinionTest extends FlatSpec with BeforeAndAfter
    ...
    override protected def beforeAll() {
        try {
            ...
            val zkCfg = new ZookeeperConfig {
                override def getZkRootPath: String = "/test"
                override def getClusterStorageEnabled: Boolean = true
                override def getCuratorEnabled: Boolean = true
                // These below are not used
                override def getZkHosts: String = ???
                override def getZkSessionTimeout: Int = ???
                override def getZkGraceTime: Int = ???
            }

            backend = new MidonetBackendService(zkCfg, curator)
            backend.startAsync().awaitRunning()

            // curator is started inside the MidonetBackendService

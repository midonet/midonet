...
class C3POMinionTestBase extends FlatSpec with BeforeAndAfter
    ...
    private val DROP_STATE_TABLE = "DROP TABLE IF EXISTS midonet_task_state"
    private val EMPTY_STATE_TABLE = "DELETE FROM midonet_task_state"
    private val CREATE_STATE_TABLE =
        "CREATE TABLE midonet_task_state (" +
        "    id int(11) NOT NULL," +
    ...
    private val INIT_STATE_ROW =
        "INSERT INTO midonet_task_state values(1, NULL, datetime('now'))"
    private val LAST_PROCESSED_ID =
        "SELECT last_processed_id FROM midonet_task_state WHERE id = 1"
    ...
    private def createStateTable() = {
        ...
        log.info("Created and initialized the midonet_task_state table.")
    }

...
class AgentMembershipTranslatorTest extends FlatSpec with BeforeAndAfter
    ...
    private val mVtepTunnelZone = mTunnelZoneFromTxt(s"""
        id { $randomUuidProto }
        name: "tz1"
        type: VTEP
        """)
    ...
    "with a corresponding host-IP address mapping." in {
        when(storage.getAll(classOf[TunnelZone]))
            .thenReturn(Promise.successful(Seq(mVtepTunnelZone, mTunnelZone))
                               .future)
    ...
    "with a corresponding host-IP address mapping removed." in {
        when(storage.getAll(classOf[TunnelZone]))
            .thenReturn(Promise.successful(Seq(mVtepTunnelZone,
                                               mTunnelZoneWith2Hosts)).future)

...
class PortBindingTranslatorIT extends C3POMinionTestBase {
    ...
    it should "handle Port Binding" in {
        // Need to sleep to make sure that the initial flush completes before
        // creating a Host via Zoom directly.
        Thread.sleep(1000)

        // Set up the host. Need to do this directly via Zoom as the Host info

...
public class Condition extends UriResource {
    ...
    @ZoomField(name = "tpSrc", converter = RangeUtil.Converter.class)
    public Range<Integer> tpSrc;
    @ZoomField(name = "tpDst", converter = RangeUtil.Converter.class)
    public Range<Integer> tpDst;

    @ZoomField(name = "invTpSrc")
    public boolean invTpSrc;
    @ZoomField(name = "invTpDst")
    public boolean invTpDst;

    @Override
    public void afterFromProto(MessageOrBuilder proto) {
        nwDstAddress = nwDst.getAddress().toString();
        nwDstLength = nwDst.getPrefixLen();
        nwSrcAddress = nwSrc.getAddress().toString();
        nwSrcLength = nwSrc.getPrefixLen();
    }

    @Override
    public void beforeToProto() {
        nwDst = IPv4Subnet.fromCidr(nwDstAddress + "/" + nwDstLength);
        nwSrc = IPv4Subnet.fromCidr(nwSrcAddress + "/" + nwSrcLength);
    }

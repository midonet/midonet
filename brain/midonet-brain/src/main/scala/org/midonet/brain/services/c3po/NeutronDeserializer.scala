...
object NeutronDeserializer {
    ...
    private def toMessage[M <: Message](node: JsonNode, clazz: Class[M]): M = {
        ...
        for (field <- node.fields.asScala if !field.getValue.isNull) {
            // Some field has a name / value in the form "project:field_or_val".
            // We ignore the project name part.
    ...
    /* Some field has a name / value in the form "project:field_or_val". We just
     * ignore the project name part. */

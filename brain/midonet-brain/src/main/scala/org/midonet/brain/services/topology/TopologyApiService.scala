...
import org.midonet.util.netty.{ServerFrontEnd, ProtoBufWebSocketAdapter, ProtoBufSocketAdapter}

/**
 * Topology api service skeleton
 */
class TopologyApiService @Inject()(val storage: Storage,
    ...
    def doStart(): Unit = {
        ...
        SessionInventory.SESSION_BUFFER_SIZE = cfg.getSessionBufferSize
        SessionInventory.SESSION_GRACETIME = cfg.getSessionGracetime
        ...
        if (cfg.getSocketEnabled) ntSrv = new ServerFrontEnd(
            new ProtoBufSocketAdapter(srvHandler,
...
trait TopologyApiServiceConfig extends MinionConfig[TopologyApiService] {
    ...
    /** Session grace time before expiration on disconnect */
    @ConfigLong(key = "session_gracetime",
                defaultValue = DEFAULT_SESSION_GRACETIME)
    def getSessionGracetime: Long
...
object TopologyApiServiceConfig {
    ...
    final val DEFAULT_SESSION_GRACETIME = 120000

...
class Aggregator[KEY,TYPE] {
    ...
    private val stream: Observable[TYPE] = Observable.merge(collector)

    /* The index of subscriptions for each Observable key */
    type SourceControl = ConnectableObservable[Null]
    ...
    def add(what: KEY, o: Observable[_ <: TYPE]): Unit = {
        lazy val ctl: SourceControl = Observable.just(null).publish()
        lazy val src = o.asInstanceOf[Observable[TYPE]].takeUntil(ctl)

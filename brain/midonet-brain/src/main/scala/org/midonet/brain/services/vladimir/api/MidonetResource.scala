...
class MidonetResource {
    ...
    : util.List[UriResource] = {
        ...
        val mainMediaType = MidonetMediaTypes.resourceNames.inverse.get(mainRes).orNull
        val mainClass = MidonetMediaTypes.resourceOf(mainMediaType)
        val mainProtoClass = MidonetMediaTypes.zoomFor.get(mainClass).orNull
    ...
    def create(@PathParam("id") id: UUID,
        ...
        val mainMediaType = MidonetMediaTypes.resourceNames.inverse.get(mainRes).orNull
        val mainClass = MidonetMediaTypes.resourceOf(mainMediaType)
        val mainZoomClass = MidonetMediaTypes.zoomFor.get(mainClass).orNull
        try {
        ...
        // TODO: factor this out and share with the create above
        create(UUID.randomUUID(), dto, zoomClass) map { newId =>
            Response.created(uriFor(subRes, newId))
        } match {
            case Success(r) => r.build()
    ...
    def create(@HeaderParam("Content-Type")contentType: String,
        ...
        create(UUID.randomUUID(), dto, zoomClass) map { id =>
            Response.created(uriFor(resource, id))
        } match {
            case Success(r) => r.build()
            case Failure(e) => log.error("Failed: ", e)
                Response.serverError().build()
    ...
    @Consumes(Array(
        ...
        APPLICATION_TRACE_REQUEST_JSON,
        MediaType.APPLICATION_JSON))
    def update(@HeaderParam("Content-Type")contentType: String,
    ...
    private def create(id: UUID, dto: UriResource,
                       zoomClass: Class[_ <: Message]): Try[UUID] = Try {
        ...
        val zoomWithId = zoom.toBuilder
                             .setField(idField, toProto(id)).build()

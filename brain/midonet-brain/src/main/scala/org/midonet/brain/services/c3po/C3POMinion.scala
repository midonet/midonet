...
import org.midonet.cluster.data.neutron.{SqlNeutronImporter, TaskStateUpdater, importer}
...
class C3POMinion @Inject()(nodeContext: ClusterNode.Context,
    ...
    private val taskStateUpdater = new TaskStateUpdater(dataSrc)
    ...
    protected override val runnable = new Runnable {
        override def run(): Unit = try {
            ...
            taskStateUpdater.updateLastProcessedId(dataMgr.lastProcessedTaskId)
        } catch {
            case ex: Throwable =>

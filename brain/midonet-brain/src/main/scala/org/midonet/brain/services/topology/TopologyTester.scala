...
import org.midonet.brain.{ScheduledMinionConfig, ScheduledClusterMinion}
 ...
 */
class TopologyTester @Inject()(val storage: Storage, val cfg: TopologyTesterConfig)
    extends ScheduledClusterMinion(cfg) {
    override protected val log = LoggerFactory.getLogger(classOf[TopologyTester])
...
@ConfigGroup("topology_tester")
trait TopologyTesterConfig extends ScheduledMinionConfig[TopologyTester] {
    import TopologyTesterConfig._

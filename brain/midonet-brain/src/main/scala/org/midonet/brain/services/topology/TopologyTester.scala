...
class TopologyTester @Inject()(val storage: Storage) extends ClusterMinion {
    ...
    private val runnable: Runnable = makeRunnable(doSomething())
    ...
    def doStart(): Unit = {
        ...
        try {
            buildLayout()
            pool.scheduleAtFixedRate(runnable, INTERVAL, INTERVAL,
    ...
    private def rmNetwork(br: Network) = {
        ...
        storage.delete(classOf[Port], info._2)
        storage.delete(classOf[Port], info._3)
    ...
    private def rmRouter(rt: Router) = {
        ...
        storage.delete(classOf[Port], info._1)
        storage.delete(classOf[Port], info._2)
        storage.delete(classOf[Router], rt)

...
import org.midonet.brain.services.vxgw.TunnelZoneState.{FloodingProxyEvent, FloodingProxyOp}
...
import org.midonet.brain.southbound.vtep.{VtepMAC, VtepNotConnectedException}
...
class VxlanGatewayManager(networkId: UUID,
    ...
    /** Takes care of the flooding proxy */
    private val tzObserver = new Observer[FloodingProxyEvent] {
        override def onCompleted(): Unit = {
            log.warn("Tunnel zone watcher terminated")
        }
        override def onError(e: Throwable): Unit = {
            log.warn("Error on tunnel zone watcher", e)
        }
        override def onNext(t: FloodingProxyEvent): Unit = {
            t.operation match {
                case FloodingProxyOp.CLEAR =>
                    log.info("Flooding proxy removed")
                case FloodingProxyOp.SET =>
                    log.info("Flooding proxy added")
                    if (vxgw != null) {
                        midoFloodLocation foreach vxgw.asObserver.onNext
                    }
            }
        }
    }
    ...
    private def midoFloodLocation: Seq[MacLocation] = {
        ...
        if (fpIp == null) {
            ...
            Seq(MacLocation(VtepMAC.UNKNOWN_DST, null, lsName, fpIp))
        }
    }
    ...
    private def vtepFloodLocations: Seq[MacLocation] = {
        vxlanPorts.values().foldLeft(Seq.empty[MacLocation])((mls, vxPort) => {
            if (vxPort.getTunnelIp == null) {
                ...
                mls :+ MacLocation(VtepMAC.UNKNOWN_DST, null, lsName,
                                   vxPort.getTunnelIp)
            }
        }
    )}
    ...
    private def ensureInitialized(vxPortIds: Seq[UUID]): Boolean = {
        ...
        if (macPortMap == null) {
            ...
            tzSubscription = tzState.get(vxgw.tzId).getFloodingProxyObservable
    ...
    def vxlanPort(id: UUID) = dataClient.portsGet(id).asInstanceOf[VxLanPort]

...
class TopologyZoomUpdater @Inject()(val nodeContext: ClusterNode.Context,
    ...
    private def buildLayout() = {
        ...
        val leftPort = createPort(pNet)
        val rightPort = createPort(pNet)

        log.debug("Binding interfaces leftdp and rightdtp to their respective " +
                  "ports on the bridge")
        // We are assuming that there are two hosts in the system
        val futureSeq: Future[Seq[Future[Host]]] = storage.getAll(classOf[Host])
        val hostFutures = Await.result(futureSeq, 1 second)
        log.debug(s"The midonet virtual network contains ${hostFutures.length} host(s)")

        val dpList = List((leftPort, "leftdp"), (rightPort, "rightdp"))
        for (hostFuture <- hostFutures;
             portInterface <- dpList) {

            val host = Await.result(hostFuture, 1 second)
            bindPortToHostInterface(host, portInterface._1.getId, portInterface._2)
        }
    }

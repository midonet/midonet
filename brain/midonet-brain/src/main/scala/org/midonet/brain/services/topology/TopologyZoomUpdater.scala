...
class TopologyZoomUpdater @Inject()(val nodeContext: ClusterNode.Context,
    ...
    private def bindPortToHostInterface(hostId: UUID, portId: UUID, interface: String) = {
        val host = Await.result(storage.get(classOf[Host], hostId), 1 second)
        val updatedHost = Host.newBuilder
            .setId(hostId)
            .addAllTunnelZoneIds(host.getTunnelZoneIdsList)

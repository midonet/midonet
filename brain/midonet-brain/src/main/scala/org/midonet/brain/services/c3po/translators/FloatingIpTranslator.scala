...
class FloatingIpTranslator(protected val readOnlyStorage: ReadOnlyStorage)
    ...
    : Option[Create[Route]] = {
        ...
        val floatingIpSubnet = IPSubnetUtil.fromAddr(fip.getFloatingIpAddress)
        provRtrPeer.map { p =>
            Create(newNextHopPortRoute(p.peer.getId,
                                       id = fipGatewayRouteId(fip.getId),
                                       dstSubnet = floatingIpSubnet,
...
object FloatingIpTranslator {
    ...
    def findPeerRouterPort(routerId: UUID, peerRouterId: UUID)
        ...
        for (routerPortId <- router.getPortIdsList.asScala) {
            val routerPort = storage.get(classOf[Port], routerPortId).await()
            if (routerPort.hasPeerId) {
                val peerPortId = routerPort.getPeerId

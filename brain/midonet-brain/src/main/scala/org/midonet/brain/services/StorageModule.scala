...
import org.midonet.config.{ConfigProvider, ConfigInt, ConfigString, ConfigGroup}

class StorageModule(cfgProvider: ConfigProvider) extends AbstractModule {

    val curatorCfg = cfgProvider.getConfig(classOf[CuratorConfig])
    val curator = CuratorFrameworkFactory.newClient(
        curatorCfg.zkHosts, new ExponentialBackoffRetry(curatorCfg.baseRetryMs,
                                                        curatorCfg.maxRetries))
    curator.start()

    val storage = new ZookeeperObjectMapper(curatorCfg.basePath, curator)
    storage.registerClass(classOf[StorageManagerState])
    storage.build()

    val latchPath = "/midonet/c3po/latch"

...
import org.midonet.brain.{BrainConfig, ClusterMinion, ClusterNode, FlowTracingConfig}
...
class FlowTracingMinion @Inject()(nodeCtx: ClusterNode.Context,
    ...
    val cass = new CassandraClient(
        brainConfig.cassandra.servers,
        brainConfig.cassandra.cluster,
        FlowTracingSchema.KEYSPACE_NAME,
        brainConfig.cassandra.replication_factor,
        FlowTracingSchema.SCHEMA, reactor);

    val service = new FlowTracingService(
        brainConfig.flowTracing, new CassandraFlowTracingStorage(cass))

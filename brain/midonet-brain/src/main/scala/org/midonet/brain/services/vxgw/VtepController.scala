...
import org.midonet.brain.southbound.vtep.VtepMAC.UNKNOWN_DST
...
class VtepController(vtepOvsdb: VtepConfig, midoDb: DataClient,
    ...
    private class FloodingProxyWatcher extends Observer[FloodingProxyEvent] {
        ...
        override def onNext(e: FloodingProxyEvent): Unit = {
            ...
            if (!tzId.equals(vtepConf.getTunnelZoneId)) {
                return // not ours
            }
            val currTz = tzStatePublisher.get(tzId)
            if (currTz == null) {
                log.info(s"Unable to load tunnel zone $tzId")
                return
            }
            val currFp = currTz.getFloodingProxy
            if (currFp == null || currFp.ipAddr == null) {
                log.info(s"No host in $tzId designated as flooding proxy")
                return
            }
            log.info(s"Flooding proxy is now ${currFp.ipAddr}")
            subscriptions.keySet.asScala.foreach { ls =>
                vtepOvsdb.macRemoteUpdater.onNext(
                    MacLocation(UNKNOWN_DST, null, ls.name, currFp.ipAddr)
                )
            }
    ...
    : Unit = {
        ...
        consolidate(vxgw) map { lsUUID =>
            ...
            floodingProxy.foreach { fp =>
                val ml = MacLocation(UNKNOWN_DST, null, vxgw.name, fp)
                vtepOvsdb.macRemoteUpdater.onNext(ml)
            }

            // Ask all peers to send us their flooded traffic
            log.info(s"Advertise unknown-dst to receive flooded traffic")
            vxgw.asObserver.onNext(
                MacLocation(UNKNOWN_DST, null, vxgw.name, tunIp)
            )

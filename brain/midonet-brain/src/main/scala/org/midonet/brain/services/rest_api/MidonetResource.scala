...
class MidonetResource @Inject()(implicit backend: MidonetBackend,
    ...
    def create(@HeaderParam("Content-Type") contentType: String,
        ...
        create(UUID.randomUUID.toString, dto, resource, dtoAttr.zoom.clazz)
    ...
    def create(@PathParam("id") id: String,
        ...
        val mainDtoAttr = MidonetMediaTypes.ResourceToDtoAttributes(Seq(resource))

        try {
            Await.result(backend.store.get(mainDtoAttr.zoom.clazz, id), timeout)
        } catch {
            case e: NotFoundException =>
                return Response.status(HttpStatus.NOT_FOUND_404)
                               .`type`(resource).build()
            case _: Throwable =>
                return Response.serverError().`type`(resource).build()
        }

        val subDtoAttr = MidonetMediaTypes.ResourceToDtoAttributes(Seq(resource, subresource))
        val dto = deserialize(data, subDtoAttr.clazz)
        dto.setParentId(id)

        create(UUID.randomUUID.toString, dto, subresource, subDtoAttr.zoom.clazz)
    ...
    : Seq[MessageOrBuilder] = {
        val fieldAttr = dtoAttr.subresources(path.last)
        val fieldDescriptor =
        ...
        if (fieldDescriptor.isRepeated) {
            if (fieldDescriptor.getMessageType == Commons.UUID.getDescriptor) {
                val subAttr = MidonetMediaTypes.ResourceToDtoAttributes(path)
                ...
                if (id eq null) {
                    ...
                    Seq(Await.result(backend.store.get(subAttr.zoom.clazz, id),
                                     timeout))
            } else {
    ...
    private def tryWithResponse(f: => Response)
    : Response = {
        try {
            f
        } catch {

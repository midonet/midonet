...
object C3POStorageManager {
    def storageManagerState(managerId: JUUID, lastProcessed: Int) =
 ...
 */
class C3POStorageManager(val storageManagerId: JUUID, val storage: Storage)
        extends C3PODataManager {
    ...
    def init() {
        try {
            storage.get(classOf[StorageManagerState], toProto(storageManagerId))
                   .value.get.get
        } catch {
            case se: StorageException =>
                try {
                    storage.create(storageManagerState(storageManagerId, 0))
                } catch {
                    case se: StorageException =>
                        throw new C3PODataManagerException(
                                "Failure initializing C3PODataManager state " +
                                s"for Manager with ID = $storageManagerId.", se)
                }
        }
        initialized = true
    }
    ...
    override def interpretAndExecTxn(txnId: String,
        ...
        assert(initialized)
        try {
            val newState = storageManagerState(storageManagerId,
                                               tasks.last.taskId)

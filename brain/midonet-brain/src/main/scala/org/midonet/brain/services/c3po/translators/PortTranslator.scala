...
class PortTranslator(protected val storage: ReadOnlyStorage,
    ...
    override protected def translateCreate(nPort: NeutronPort): MidoOpList = {
        ...
        if (isOnProviderNetwork(nPort)) return List()
    ...
    override protected def translateDelete(id: UUID): MidoOpList = {
        ...
        if (!isFloatingIpPort(nPort) && !isOnProviderNetwork(nPort))
    ...
    private def isOnProviderNetwork(np: NeutronPort) = {
        NetworkTranslator.isProviderNetwork(

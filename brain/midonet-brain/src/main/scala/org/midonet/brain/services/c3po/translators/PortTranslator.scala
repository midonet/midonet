...
class PortTranslator(private val storage: ReadOnlyStorage)
    ...
    : List[MidoOp[_ <: Message]] = {
        ...
        if (isVifPort(nPort)) {
            ...
            val vifPortInfo = constructVifPortInfo(nPort, null, midoPortBldr)
    ...
    : List[MidoOp[_ <: Message]] = {
        ...
        if (isVifPort(nPort)) { // It's a VIF port.
            ...
            val vifPortInfo = constructVifPortInfo(null, nPort, mPort)
            addMidoOps(vifPortInfo, midoOps)
        }
    ...
    : List[MidoOp[_ <: Message]] = {
        ...
        if (isVifPort(nPort)) { // It's a VIF port.
            ...
            val vifPortInfo = constructVifPortInfo(nPort, oldNPort, mPort)
            addMidoOps(vifPortInfo, midoOps)
        }
        midoOps.toList
    }
    ...
    case class VifPortInfo(midoNetworks: mutable.Map[UUID, Network.Builder],
                           ...
                           updatedIpAddressGs: ListBuffer[MidoOp[IpAddrGroup]])

    def addMidoOps(portInfo: VifPortInfo,
                   midoOps: ListBuffer[MidoOp[_ <: Message]]) {
            midoOps ++= portInfo.midoNetworks.values.map(n => Update(n.build))
            midoOps ++= portInfo.inRules ++ portInfo.outRules
            midoOps ++= portInfo.chains
            midoOps ++= portInfo.updatedIpAddressGs
    }

    private def constructVifPortInfo(nPort: NeutronPort,
                                     nPortOld: NeutronPort,
                                     mPortOld: PortOrBuilder) = {
        ...
        val portInfo = VifPortInfo(mutable.Map[UUID, Network.Builder](),
        ...
        if (nPortOld != null) { // UPDATE or DELETE
            processDhcpEntries(nPortOld,
                               portInfo.midoNetworks,
                               mutable.Set(), // Not interested in old cache.
                               (network: Network.Builder,
                                ipSubnet: IPSubnet,
                                mac: String,
                                ipAddr: IPAddress) =>
                               deleteDhcpHost(network, ipSubnet, mac, ipAddr))
        }

        if (nPort != null) {
            processDhcpEntries(nPort,
                               portInfo.midoNetworks,
                               portInfo.neutronSubnet,
                               (network: Network.Builder,
                                ipSubnet: IPSubnet,
                                mac: String,
                                ipAddress: IPAddress) =>
                               addDhcpHost(network, ipSubnet, mac, ipAddress))

            updateSecurityBindings(
                    nPort, nPortOld, inChainId, outChainId, portInfo)
        } else { // Port deletion
            deleteSecurityBindings(nPortOld, inChainId, outChainId, portInfo)
        }

        portInfo
    }

    /* process host entries in the Network's DHCP settings. */
    private def processDhcpEntries(
            nPort: NeutronPort,
            networkCache: mutable.Map[UUID, Network.Builder],
            subnetCache: mutable.Set[NeutronSubnet],
            processHost: (Network.Builder, IPSubnet, String, IPAddress) => Unit) {
        for (ipAlloc <- nPort.getFixedIpsList.asScala) {
            ...
            val network = networkCache.getOrElse(
                    subnet.getNetworkId, {
                        val newNetwork = storage.get(classOf[Network],
                                                     subnet.getNetworkId)
                                                .await().toBuilder
                        networkCache(subnet.getNetworkId) = newNetwork
                        newNetwork
                    })

            val mac = nPort.getMacAddress
            val ipAddress = ipAlloc.getIpAddress
            processHost(network, ipSubnet, mac, ipAddress)
        }
    }
        ...
        for (sgId <- nPort.getSecurityGroupsList.asScala) {
            ...
            for (newIp <- added)
                updatedIpAddrG.addIpAddrPortsBuilder()
                              .setIpAddress(newIp)
                              .addPortId(portId)
            portInfo.updatedIpAddressGs += (Update(updatedIpAddrG.build))
        }

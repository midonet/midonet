...
class PortTranslator(val storage: ReadOnlyStorage)
    ...
    private def updateSecurityBindings(nPort: NeutronPort,
        ...
        for (subnet <- portInfo.neutronSubnet.values) {
            ...
            portInfo.inRules += Create(ipSpoofRuleBldr)
        ...
        val macSpoofRuleBldr = dropRuleBuilder(inChainId)
            ...
            .setInvDlSrc(true)
            .build()
        portInfo.inRules += Create(macSpoofRuleBldr)
        ...
        val arpDropInRuleBldr = dropRuleBuilder(inChainId)
            ...
            .setInvDlType(true)
            .build()
        portInfo.inRules += Create(arpDropInRuleBldr)

        val arpDropOutRuleBldr = dropRuleBuilder(outChainId)
            .setDlType(ARP.ETHERTYPE)
            .setInvDlType(true)
            .build()
        portInfo.outRules += Create(arpDropOutRuleBldr)

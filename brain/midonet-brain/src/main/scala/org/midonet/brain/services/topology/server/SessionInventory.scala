 ...
 */
class StorageTransformer(val nackWith: Response)
     ...
     */
    class StorageOnSubscribe(val s: Observable[_ <: Message],
        ...
        override def call(client: Subscriber[_ >: Response.Builder]): Unit = {
            sub = s.subscribe(new StorageWrapper(
    ...
    /** Convenient wrapper to convert zoom observable completions into
      ...
      */
    class StorageWrapper(val observer: Observer[Response.Builder],
                         val nackWith: Response)
 ...
 */
class MsgSequencer(val minCapacity: Int)(implicit executor: ExecutorService)
    extends Observer[Response.Builder] {
    private val log = LoggerFactory.getLogger(classOf[MsgSequencer])
...
class SessionInventory(private val store: Storage) {
    ...
    private def makeSession(sessionId: UUID): Session = new Session {
        ...
        private val buffer = new MsgSequencer(SESSION_BUFFER_SIZE)

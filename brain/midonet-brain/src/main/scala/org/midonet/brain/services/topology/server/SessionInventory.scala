...
import org.midonet.brain.services.topology.server.SessionInventory
...
class SessionInventory(private val store: Storage) {
    ...
    private def makeSession(): Session = new Session {
        ...
        class DataProvider[T] extends Observer[T] {
            ...
            private val event = new ReentrantLock()
            private val pending = event.newCondition()
            private var thread: JavaFuture[Null] = null
            ...
            private val client = new Callable[Null] {
                override def call(): Null = {
                    log.debug("starting client thread")
                    var terminated = false
                    while (!terminated) {
                        log.debug("looping client thread")
                        if (subscriber.get() != null &&
                            ready.get() == 0 && !done.get()) {
                            event.lock()
                            try {
                                while (subscriber.get() != null &&
                                       ready.get() == 0 && !done.get()) {
                                    log.debug("blocking client thread")
                                    pending.await()
                                    log.debug("awaiting client thread")
                                }
                            } catch {
                        ...
                        if (!terminated) {
                            var item = queue.peek()
                            var dest = subscriber.get()
                            if (dest != null) {
                                if (item != null) {
                                    log.debug("client push: " + item)
                                    dest.onNext(item)
                                    queue.poll()
            ...
            def subscribe(s: Subscriber[_ >: T]): Subscription = {
                ...
                thread = sessionExecutor.submit(client)
        ...
        object DataObservable {
            implicit def create[T](p: DataProvider[T]): Observable[T] = {
        ...
        private val buffer = new DataProvider[Response]

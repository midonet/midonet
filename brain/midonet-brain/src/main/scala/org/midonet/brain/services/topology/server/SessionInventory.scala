...
import org.midonet.util.concurrent.{OneToOneRwdRingBuffer, Locks}
...
class ZoomSequencer(val minCapacity: Int)(implicit executor: ExecutorService)
    ...
    private val ring = new OneToOneRwdRingBuffer[Response.Builder](minCapacity)
...
class SessionTimeout(private val session: Session, ms: Long) {
    ...
    SessionInventory.sessionTimer.schedule(expire, ms, TimeUnit.MILLISECONDS)
...
class SessionInventory(private val store: Storage) {
    ...
    private def makeSession(sessionId: UUID): Session = new Session {
        ...
        private val timeout = new AtomicReference[SessionTimeout](
            new SessionTimeout(this, SESSION_TIMEOUT))
            ...
            Observable.create(new OnSubscribe[Response] {
                override def call(s: Subscriber[_ >: Response]): Unit = {
                    ...
                    try {
                        ...
                        case exc: Throwable =>
                            timeout.set(new SessionTimeout(session,
                                                           SESSION_TIMEOUT))
                    }
                }
            }).doOnTerminate(
                ...
                {timeout.set(new SessionTimeout(session, SESSION_TIMEOUT))}
            )
        }

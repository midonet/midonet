...
object SessionInventory {
    ...
    def extractId(m: Message): ObservableId = m match {
        case t: Chain =>
            ObservableId(Uuid(t.getId), classOf[Chain])
        case t: Host =>
            ObservableId(Uuid(t.getId), classOf[Host])
        case t: IpAddrGroup =>
            ObservableId(Uuid(t.getId), classOf[IpAddrGroup])
        case t: Network =>
            ObservableId(Uuid(t.getId), classOf[Network])
        case t: Port =>
            ObservableId(Uuid(t.getId), classOf[Port])
        case t: PortGroup =>
            ObservableId(Uuid(t.getId), classOf[PortGroup])
        case t: Route =>
            ObservableId(Uuid(t.getId), classOf[Route])
        case t: Router =>
            ObservableId(Uuid(t.getId), classOf[Router])
        case t: Rule =>
            ObservableId(Uuid(t.getId), classOf[Rule])
        case t: TunnelZone =>
            ObservableId(Uuid(t.getId), classOf[TunnelZone])
        case t: Vtep =>
            ObservableId(StrId(t.getId), classOf[Vtep])
        case t: VtepBinding =>
            ObservableId(Uuid(t.getId), classOf[VtepBinding])
        case _ => null
    }
    ...
    var SESSION_BUFFER_SIZE: Int = 1 << 12

    /* Expiration time for non connected sessions, in milliseconds */
    var SESSION_GRACETIME: Long = 120000
 ...
 */
protected[SessionInventory] class StorageTransformer(val nackWith: Response)
 ...
 */
protected[SessionInventory] class Buffer(minCapacity: Int)
                                        (implicit executor: ExecutorService)
    extends Observer[Response.Builder] {
    private val log = LoggerFactory.getLogger(classOf[Buffer])
...
class SessionInventory(private val store: Storage) {
    ...
    private def makeSession(sessionId: UUID): Session = new Session {
        ...
        private val buffer = new Buffer(SESSION_BUFFER_SIZE)
        ...
        setExpiration(SESSION_GRACETIME)
        ...
        override def observable(seqno: Long): Observable[Response] = {
            val destroyAction = makeAction0 {terminate()}
            val unsubscribeAction = makeAction0 {
                log.debug("session unsubscribed: "+ sessionId)
                session.setExpiration(SESSION_GRACETIME)
            }
            ...
            Observable.create(subscribeAction)
                      .doOnTerminate(destroyAction)
                      .doOnUnsubscribe(unsubscribeAction)
        }

...
import org.midonet.brain.services.topology.common.TopologyApi
...
object SessionInventory {
    ...
    private def updateBuilder(m: Message): Response = {
        val u: Update = m match {
            ...
            case h: PortSet => update.setPortSet(h).build()
    ...
    def extractClass(m: Message): Class[_ <: Message] = m match {
        ...
        case h: PortSet => classOf[PortSet]
    ...
    def extractId(m: Message): ID = m match {
        ...
        case h: PortSet => idBuilder.setUuid(h.getId).build()

...
class ZoomSequencer(val minCapacity: Int)(implicit executor: ExecutorService)
    ...
    private val log = LoggerFactory.getLogger(classOf[ZoomSequencer])
...
class SessionInventory(private val store: Storage) {
    ...
    private def makeSession(sessionId: UUID): Session = new Session {
        ...
        // Keep a reference to the current subscriber.
        // This is a trick to make sure we are able to forward the termination
        // ack before the funnel collapses (see terminate) and without waiting
        // to emit the messages in the buffer
        // TODO: we should consider not sending acks for successful termination
        private val currentSubscriber =
        ...
        override def terminate(ackWith: Response): Unit = {
            log.debug("enter session terminate")
            if (ackWith != null) {
                val s = currentSubscriber.get()
                if (s != null)
                    s.onNext(ackWith)
            }
            destroy()
        ...
        override def unwatch[M <: Message](id: Id, klass: Class[M],
                                           ackWith: Response,
                                           nackWith: Response): Unit = {
            log.debug("enter session unwatch")
        ...
        override def watchAll[M <: Message](ofType: Class[M],
                                            ackWith: Response,
                                            nackWith: Response): Unit = {
            log.debug("enter session watchAll")
        ...
        override def unwatchAll[M <: Message](ofType: Class[M],
                                              ackWith: Response,
                                              nackWith: Response): Unit = {
            log.debug("enter session unwatchAll")

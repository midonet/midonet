...
object SessionInventory {
    ...
    def extractId(m: Message): ObservableId = m match {
        case t: Chain =>
            ObservableId(ProtoUuid(t.getId), classOf[Chain])
        case t: Host =>
            ObservableId(ProtoUuid(t.getId), classOf[Host])
        case t: IpAddrGroup =>
            ObservableId(ProtoUuid(t.getId), classOf[IpAddrGroup])
        case t: Network =>
            ObservableId(ProtoUuid(t.getId), classOf[Network])
        case t: Port =>
            ObservableId(ProtoUuid(t.getId), classOf[Port])
        case t: PortGroup =>
            ObservableId(ProtoUuid(t.getId), classOf[PortGroup])
        case t: Route =>
            ObservableId(ProtoUuid(t.getId), classOf[Route])
        case t: Router =>
            ObservableId(ProtoUuid(t.getId), classOf[Router])
        case t: Rule =>
            ObservableId(ProtoUuid(t.getId), classOf[Rule])
        case t: TunnelZone =>
            ObservableId(ProtoUuid(t.getId), classOf[TunnelZone])
        case t: Vtep =>
            ObservableId(StrId(t.getId), classOf[Vtep])
        case t: VtepBinding =>
            ObservableId(ProtoUuid(t.getId), classOf[VtepBinding])
        case _ => null
    }
...
class SessionInventory(private val store: Storage,
    ...
    private def makeSession(sessionId: UUID): Session = new Session {
        ...
        funnel.observable().subscribeOn(scheduler).subscribe(buffer)

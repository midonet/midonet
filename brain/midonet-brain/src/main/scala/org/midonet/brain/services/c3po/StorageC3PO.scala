...
import org.midonet.cluster.services.c3po.OpType.{Create, Delete, OpType, Update}
...
class StorageC3PO(val storage: Storage) extends C3PO {
    ...
    override def interpretAndExec[T](op: OpType, inputModel: T) {
        val modelClass: Class[_ <: T] = inputModel.getClass
        if (!apiTranslators.containsKey(modelClass)) {
            throw new TranslationException(
                    op, modelClass, s"No translator for ${modelClass}.")
        }

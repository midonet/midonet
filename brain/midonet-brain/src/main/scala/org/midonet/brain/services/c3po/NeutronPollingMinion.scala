...
import org.midonet.cluster.data.neutron.NeutronDeserializer.toMessage
...
class NeutronPollingMinion @Inject()(config: NeutronPollingConfig,
                                     storage: Storage,
    ...
    val neutron = new RemoteNeutronService(config.connectionString,
    ...
    protected override val runnable = new Runnable {
        override def run(): Unit = try {
            ...
            log.debug("Getting last processed task ID.")
            ...
            log.debug("Got {} transactions: {}", txns.size, txns)
...
@ConfigGroup("neutron-poller")
trait NeutronPollingConfig extends ScheduledMinionConfig[NeutronPollingMinion] {

...
class NetworkTranslator(storage: ReadOnlyStorage, pathBldr: PathBuilder)
    ...
    override protected def translateCreate(nn: NeutronNetwork): MidoOpList = {
        ...
        val id = nn.getId.asJava
        ops ++= ensureProviderRouterExists(nn)
        ops += Create(translate(nn))
        ops ++= replMapPaths(nn.getId.asJava).map(CreateNode(_, null))
        ops.toList
    }
    ...
    override protected def translateDelete(id: UUID): MidoOpList = {
        ...
        ops ++= replMapPaths(id.asJava).map(DeleteNode)
        ops.toList
    }
    ...
    private def replMapPaths(networkId: util.UUID): Iterable[String] =
        Iterable(pathBldr.getBridgeIP4MacMapPath(networkId),
                 pathBldr.getBridgeMacPortsPath(networkId),
                 pathBldr.getBridgeVlansPath(networkId))

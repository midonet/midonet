...
import org.midonet.brain.services.c3po.midonet.{Create, MidoOp}
...
import org.midonet.cluster.models.Neutron.{NeutronNetwork, NeutronPort, NeutronRouterInterface}
import org.midonet.cluster.models.Topology.{Route, Dhcp, Network, Port}
...
class RouterInterfaceTranslator(val storage: ReadOnlyStorage)
    ...
    import NetworkTranslator.isProviderNetwork
    ...
    : MidoOpList = {
        ...
        // If the port is on a provider network, then there will be no
        // corresponding Midonet port, because provider networks don't exist in
        // Midonet. Otherwise, there will be a corresponding network port in the
        // Midonet topology with the same port ID.
        val nNetwork =
            storage.get(classOf[NeutronNetwork], nPort.getNetworkId).await()
        ...
        // router with ID nPort.getRouterId. If nNetwork is a provider network,
        // then, again, there is no corresponding Midonet network, and the
        // router is called an edge router.
        val isEdgeRouter = isProviderNetwork(nNetwork)

        // The router ID is given as nPort's device ID.
        ...
        val subnet = storage.get(classOf[Dhcp], ri.getSubnetId).await()
        routerPortBldr.setPortSubnet(subnet.getSubnetAddress)
        ...
        if (nPort.getFixedIpsCount > 0) {
            ...
            routerPortBldr.setPortAddress(subnet.getDefaultGateway)
        }
        ...
        midoOps += Create(routerPortBldr.build())
        midoOps += Create(rifRoute)

        if (isEdgeRouter) {
            midoOps ++= bindPortOps(nPort.getHostId, routerPortId,
                                    nPort.getProfile.getInterfaceName)
        } else {
            midoOps ++= createMetadataServiceRoutes(
                routerPortId, nNetwork.getId, subnet.getSubnetAddress)
        }

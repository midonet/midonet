...
public class OvsdbVtepBackend {
    ...
    public LogicalSwitch createLogicalSwitch(LogicalSwitch ls)
        throws Exception {
        Insert<GenericTableSchema> op = lsTable.insertLogicalSwitch(ls);
        try {
     ...
     */
    public PhysicalLocator createPhysicalLocator(PhysicalLocator loc)
        throws Exception {
        Insert<GenericTableSchema> op = plTable.insertPhysicalLocator(loc);
        try {
            UUID newId = modify(client, plTable, op).result(Duration.Inf());
            if (newId == null)
                throw new VtepException(endPoint,
                    "cannot create locator for tunnel endpoint ip: " + loc);
            Set<PhysicalLocator> locator = retrievePhysicalLocator(
                plTable.getUuidMatcher(newId));
            if (locator.isEmpty())
                throw new VtepRetrievalException("locator for ip " + loc);
    ...
    public UUID storeMacEntry(MacsTable table, MacEntry entry)
        throws Exception {
        Insert<GenericTableSchema> op = table.insertMacEntry(entry);
        try {
     ...
     */
    private Observable<? extends MacUpdate> macUpdates(
        final MacsTable table) {
        return Observable.create(
            new Observable.OnSubscribe<MacUpdate>() {
                @Override
                public void call(
                    final Subscriber<? super MacUpdate> subscriber) {
                    ...
                    Observer<TableUpdate<GenericTableSchema>> forwarder =
                        new Observer<TableUpdate<GenericTableSchema>>() {
                            ...
                            @Override public void onNext(
                                ...
                                for (org.opendaylight.ovsdb.lib.notation.UUID
                                    rowId : update.getRows().keySet()) {
                                    MacEntry oldData = table.parseMacEntry(
                                        update.getOld(rowId));
                                    MacEntry newData = table.parseMacEntry(
                                        update.getNew(rowId));
                                    if (table.isUcastTable())
                                        subscriber.onNext(
                                            UcastMacUpdate.apply(
                                                (UcastMac) oldData,
                                                (UcastMac) newData));
                                    else
                                        subscriber.onNext(
                                            McastMacUpdate.apply(
                                                (McastMac) oldData,
                                                (McastMac) newData));
                                }
                            }
     ...
     */
    public Observable<UcastMacUpdate> localUcastMacUpdates() {
        return (Observable<UcastMacUpdate>)macUpdates(uMacsLocalTable);
    }

    /**
     * Get an observable with remote ucast macs table updates
     */
    public Observable<UcastMacUpdate> remoteUcastMacUpdates() {
        return (Observable<UcastMacUpdate>)macUpdates(uMacsRemoteTable);
    }

    /**
     * Get an observable with local ucast macs table updates
     */
    public Observable<McastMacUpdate> localMcastMacUpdates() {
        return (Observable<McastMacUpdate>)macUpdates(mMacsLocalTable);
    }

    /**
     * Get an observable with remote ucast macs table updates
     */
    public Observable<McastMacUpdate> remoteMcastMacUpdates() {
        return (Observable<McastMacUpdate>)macUpdates(mMacsRemoteTable);
    }
}

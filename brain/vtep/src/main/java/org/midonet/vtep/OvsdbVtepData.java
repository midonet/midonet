...
public class OvsdbVtepData {
        ...
        opResult.onComplete(new Expectation.OnComplete<OperationResult>() {
            ...
            public void onSuccess(OperationResult r) {
                ...
                if (rows.isEmpty())
                    ...
                    LogicalSwitch ls = logicalSwitchTable.parseLogicalSwitch(
                        rows.iterator().next());
                    result.success(Option.apply(ls));
                    logicalSwitchesByName.putIfAbsent(ls.name(), ls);
                }
            }
            @Override
            public void onFailure(Throwable exc) {
    ...
    private void retrieveLogicalSwitchByName(
        final String lsName, final Expectation<Option<LogicalSwitch>> result) {
        Select<GenericTableSchema> op = logicalSwitchTable.selectAll();
        op.addCondition(logicalSwitchTable.getNameMatcher(lsName));
        Expectation<OperationResult> opResult =
            singleOp(client, databaseSchema, op, executor);
        opResult.onComplete(new Expectation.OnComplete<OperationResult>() {
            @Override
    ...
    public Future<Option<LogicalSwitch>> getLogicalSwitchByName(String lsName) {
        Expectation<Option<LogicalSwitch>> lsFuture = new Expectation<>();
        LogicalSwitch ls = logicalSwitchesByName.get(lsName);
        if (ls != null)
            lsFuture.success(Option.apply(ls));
        else
            retrieveLogicalSwitchByName(lsName, lsFuture);
        return lsFuture.future();
    }
    ...
    private void applyRemoteUcastAddition(MacLocation macLocation) {
        Option<LogicalSwitch> lsOption;
        UUID locatorId = null; //FIXME: get the physical locator
        try {
            lsOption = Await.result(
                getLogicalSwitchByName(macLocation.logicalSwitchName()),
                Duration.Inf());
        } catch (Throwable exc) {
            lsOption = Option.apply((LogicalSwitch)null);
        }
        if (lsOption.isEmpty()) {
            log.warn("unknown logical switch: {}",
                     macLocation.logicalSwitchName());
            return;
        }
        Insert<GenericTableSchema> op = ucastMacsRemoteTable.insert(
            macLocation.mac(), locatorId, lsOption.get().uuid(),
            macLocation.ipAddr());
    }

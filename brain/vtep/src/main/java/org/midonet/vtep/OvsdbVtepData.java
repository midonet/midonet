...
public class OvsdbVtepData {
    ...
    public OvsdbVtepData(OvsdbClient client, ExecutionContext executionCtx) {
        ...
        this.dbReady = PromiseFactory.get(DatabaseSchema.class);
        this.tunnelIps = (new PromiseMaker<List<IPv4Addr>>()).get();
        initialize();
    ...
    private Future<OperationResult> doSingleOp(DatabaseSchema dbs, Operation op) {
        final Promise<OperationResult> result =
            PromiseFactory.get(OperationResult.class);
    ...
    public Future<IPv4Addr> getTunnelIp() {
        final Promise<IPv4Addr> result = PromiseFactory.get(IPv4Addr.class);

...
import org.midonet.cluster.data.vtep.model.{McastMac, UcastMac, MacEntry, VtepMAC}

/**
 * A container for Mac table entry updates
 */
abstract class MacUpdate(val oldEntryValue: MacEntry,
...
case class UcastMacUpdate(old: UcastMac, cur: UcastMac)
    extends MacUpdate(old, cur)
case class McastMacUpdate(old: McastMac, cur: McastMac)
    extends MacUpdate(old, cur)

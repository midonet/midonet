...
class Expectation[T] {
    ...
    def awaitReady(timeOut: Duration = Duration.Inf): this.type = {
        Await.ready(promise.future, timeOut)
        this
    }
    @throws[Exception]
    def awaitResult(timeOut: Duration = Duration.Inf): T =
...
/*
 * FIXME: to be removed when the patch adding this to midonet-utils is merged
 */
object SameThreadExecutor extends Executor {
    override def execute(runnable: Runnable): Unit = runnable.run()
    def getInstance: Executor = this

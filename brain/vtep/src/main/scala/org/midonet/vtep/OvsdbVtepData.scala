...
class OvsdbVtepData(val endPoint: VtepEndPoint, val client: OvsdbClient,
    ...
    private def applyRemoteMacAddition(macLocation: MacLocation): Unit = {
        lazy val entries = macLocationToMacEntries(macLocation)
        if (macLocation.mac.isUcast)
            for (e <- entries)
                uRemoteTable.insert(e.asInstanceOf[UcastMac])
                    .onFailure {case exc: Throwable =>
                        log.warn("failed to insert entry " + e, exc)
                } (CallingThreadExecutionContext)
        else
            for (e <- entries)
                mRemoteTable.insert(e.asInstanceOf[McastMac])
                    .onFailure {case exc: Throwable =>
                    log.warn("failed to insert entry " + e, exc)
                } (CallingThreadExecutionContext)
    }
    ...
    private def applyRemoteMacDeletion(macLocation: MacLocation): Unit = {
        ...
        if (macLocation.mac.isUcast)
            for (e <- entries)
                OvsdbUtil.delete(client, uRemoteTable.table,
                                 uRemoteTable.table.delete(e))
                    .future.onFailure {case exc: Throwable =>
                    log.warn("failed to remove entry " + e, exc)
                } (CallingThreadExecutionContext)
        else
            for (e <- entries)
                OvsdbUtil.delete(client, mRemoteTable.table,
                                 mRemoteTable.table.delete(e))
                    .future.onFailure {case exc: Throwable =>
                    log.warn("failed to remove entry " + e, exc)
                } (CallingThreadExecutionContext)
    }
    ...
    private def setBindings(lsName: String, bindings: Iterable[(String, Short)])
        :Future[Unit] = ready.collect {case true =>
            (psSwitch, lsSwitch(lsName)) match {
                ...
                case (Some(ps), Some(ls)) =>
                    ...
                    for (b <- bindings) ports.get(b._1) match {
                        ...
                        case Some(p) =>
                            val updatedPort = PhysicalPort(
                                p.uuid, p.name, p.description,
                                p.vlanBindings updated (b._2.toInt, ls.uuid),
                                p.vlanStats, p.portFaultStatus)
                            ports = ports updated (p.name, updatedPort)
                    }
                    ports.values.foreach(p => {
                        portTable.insert(p).onFailure {case e: Throwable =>
                            log.warn("cannot update port bindings for " + p, e)
                        } (CallingThreadExecutionContext)
                    })
            }
        }(vtepContext)
    ...
    private def deleteLogicalSwitch(name: String): Future[Unit] =
        ready.collect {case true =>
            lsSwitch(name) match {
                ...
                case Some(ls) =>
                    // clear mac tables
                    for (t <- List(uLocalTable, uRemoteTable,
                                   mLocalTable, mRemoteTable)) {
                        t.getAll.values.filter(_.logicalSwitchId == ls.uuid)
                            .foreach(e => {
                                OvsdbUtil.delete(client, t.table, t.table.delete(e))
                        })
                    }
                    // clear bindings
                    portTable.getAll.values
                        .filter(p => {p.vlanBindings.values.toSet.contains(ls)})
                        .foreach(p => {
                            val updatedPort = PhysicalPort(
                                p.uuid, p.name, p.description,
                                p.vlanBindings.filterNot(_._2 == ls.uuid),
                                p.vlanStats, p.portFaultStatus)
                            portTable.insert(updatedPort)
                        })
                    // delete the switch
                    OvsdbUtil.delete(client, lsTable.table,
                                     lsTable.table.deleteByName(ls.name))
            }
        }

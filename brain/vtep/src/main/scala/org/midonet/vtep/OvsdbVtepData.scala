...
class OvsdbVtepData(val endPoint: VtepEndPoint, val client: OvsdbClient,
    ...
    private def applyRemoteMacDeletion(macLocation: MacLocation): Unit = {
        ...
        if (macLocation.mac.isUcast)
            for (e <- entries)
                OvsdbTools.delete(client, uRemoteTable.table,
            ...
            for (e <- entries)
                OvsdbTools.delete(client, mRemoteTable.table,
        ...
        :Future[Unit] = ready.collect {case true =>
            (psSwitch, lsSwitch(lsName)) match {
                ...
                case (Some(ps), Some(ls)) =>
                    // FIXME: the following code should be inside a transaction
        ...
        ready.flatMap {case true =>
            lsSwitch(name) match {
                ...
                case Some(ls) =>
                    ...
                    OvsdbTools.multiOp(client, dbSchema, ops).future.map(r => {})
            }
        } (vtepContext)
 }

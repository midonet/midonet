...
abstract class MockOvsdbVtep {
    ...
    private abstract class MockTable(val schema: MockTableSchema)

    private case class LSTable(data: TrieMap[util.UUID, LogicalSwitch],
                               sch: MockTableSchema) extends MockTable(sch)

    private case class MLocalTable(data: TrieMap[util.UUID, McastMac],
                                   sch: MockTableSchema) extends MockTable(sch)

    private case class MRemoteTable(data: TrieMap[util.UUID, McastMac],
                                    sch: MockTableSchema) extends MockTable(sch)

    private case class LocSetTable(data: TrieMap[util.UUID, PhysicalLocatorSet],
                                   sch: MockTableSchema) extends MockTable(sch)

    private case class LocTable(data: TrieMap[util.UUID, PhysicalLocator],
                                sch: MockTableSchema) extends MockTable(sch)

    private case class PortTable(data: TrieMap[util.UUID, PortTable],
                                 sch: MockTableSchema) extends MockTable(sch)

    private case class PSTable(data: TrieMap[util.UUID, PhysicalSwitch],
                               sch: MockTableSchema) extends MockTable(sch)

    private case class ULocalTable(data: TrieMap[util.UUID, UcastMac],
                                   sch: MockTableSchema) extends MockTable(sch)

    private case class URemoteTable(data: TrieMap[util.UUID, UcastMac],
                                    sch: MockTableSchema) extends MockTable(sch)

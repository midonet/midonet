    ...
    override def connect(user: UUID) = {
        ...
        try {
            ...
            if (state == DISCONNECTED) {
                ...
                vtepData = new OvsdbVtepData(client, executionContext)
                state = CONNECTED
            }
            users.add(user)
        } catch {
...
class OnFutureResult[T, T1](ret: T1) extends Function1[Try[T], T1] {
    override final def apply(value: Try[T]): T1 = value match {
        case Success(result) => onSuccess(result); ret
        case Failure(exception) => onFailure(exception); ret
    }
...
object PromiseFactory {
    def get[T](k: Class[T]): Promise[T] = Promise[T]()
    def obtain[T](): Promise[T] = Promise[T]()
}

class PromiseMaker[T] {
    def get: Promise[T] = Promise[T]()
}

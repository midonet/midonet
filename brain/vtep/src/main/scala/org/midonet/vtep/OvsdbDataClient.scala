...
import org.midonet.cluster.data.vtep.{VtepException, VtepStateException, VtepConnection, VtepDataClient}
...
class OvsdbDataClient(val endPoint: VtepEndPoint) extends VtepDataClient {
    ...
    // Connection status
    private val state = new AtomicReference[State.Value](State.DISCONNECTED)
    ...
    override def disconnect(user: UUID): Unit = {
        ...
        val disconnector = makeRunnable {
            state.get() match {
                case State.CONNECTED if users.remove(user) && users.isEmpty => try {
    ...
    override def getState: State.Value = state.get()

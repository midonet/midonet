...
import org.midonet.vtep.model.{PhysicalLocator, LogicalSwitch, MacLocation}
...
class OvsdbDataClient(val endPoint: VtepEndPoint)
    ...
    // Connection status
    private val state = new AtomicReference[VtepConnection.State.Value](
        ...
        val connector = makeRunnable {
            state.get() match {
                ...
                case DISCONNECTED => try {
                    ...
                    ovsdbData.set(
                        OvsdbVtepData.get(client, SameThreadExecutor).future)
    ...
    override def getState: VtepConnection.State.Value = state.get()
    ...
    override def macLocalUpdates: Observable[MacLocation] =
        Await.result(ovsdbData.get(), Duration.Inf).ucastMacLocalUpdates()
    ...
    override def macRemoteUpdates: Observer[MacLocation] =
        new Observer[MacLocation] {
            ...
            override def onNext(t: MacLocation): Unit = ???
        }
    ...
    def getLogicalSwitches: Set[LogicalSwitch] =
        Await.result(ovsdbData.get(), Duration.Inf).getLogicalSwitches.toSet

    def getPhysicalLocators: Set[PhysicalLocator] =
        Await.result(ovsdbData.get(), Duration.Inf).getPhysicalLocators.toSet

    def getPhysicalLocatorSets: Map[UUID, java.util.Set[UUID]] =
        Await.result(ovsdbData.get(), Duration.Inf).getPhysicalLocatorSets.toMap

    def getLocalUcastMacs: Set[MacLocation] =
        Await.result(ovsdbData.get(), Duration.Inf).getUcastMacLocalEntries.toSet

    def getRemoteUcastMacs: Set[MacLocation] =
        Await.result(ovsdbData.get(), Duration.Inf).getUcastMacRemoteEntries.toSet

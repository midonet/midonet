...
import org.midonet.cluster.data.vtep.model.{VtepMAC, UcastMac, LogicalSwitch}
...
class OvsdbToolsTest extends FeatureSpec with Matchers with BeforeAndAfter {
    ...
    feature("insert") {
        scenario("vtep processes an insert") {
            ...
            val t = new UcastMacsLocalTable(db)
            val mac = UcastMac(ls = UUID.randomUUID(),
                               mac = VtepMAC.fromMac(MAC.random()),
                               ip = IPv4Addr.random,
                               loc = UUID.randomUUID())

            val op = OvsdbTools.insert(client, t, t.insert(mac))
    ...
    feature("query") {
        ...
        scenario("query on a table with a single row") {
            ...
            val ins = OvsdbTools.insert(client, t, t.insert(mac))
        ...
        scenario("query on a table with multiple rows") {
            ...
            val t = new UcastMacsLocalTable(db)
            val mac1 = UcastMac(ls = UUID.randomUUID(),
                                mac = VtepMAC.fromMac(MAC.random()),
                                ip = IPv4Addr.random,
                                loc = UUID.randomUUID())
            val mac2 = UcastMac(ls = UUID.randomUUID(),
                                mac = VtepMAC.fromMac(MAC.random()),
                                ip = IPv4Addr.random,
                                loc = UUID.randomUUID())

            val ins1 = OvsdbTools.insert(client, t, t.insert(mac1))
            val ins2 = OvsdbTools.insert(client, t, t.insert(mac2))
    ...
    feature("monitor") {
        scenario("subscription on an empty table") {
            ...
            data.head.getRows.size() shouldBe 1
        ...
        scenario("subscription on a non-empty table") {
            ...
            // no events received
            monitor.getOnNextEvents.length shouldBe 0
            monitor.awaitOnNext(1, shortTimeout) shouldBe false
        }
        scenario("row replacement") {
            ...
            val ls1 = LogicalSwitch(id = UUID.randomUUID(),
                                    ...
                                    desc = "lsDesc")
            val ins1 = OvsdbTools.insert(client, t, t.insert(ls1))
            ...
            val ls2 = LogicalSwitch(id = ls1.uuid,
                                    ...
                                    vni = random.nextInt(4096),
                                    desc = "lsChanged")
            val ins2 = OvsdbTools.insert(client, t, t.insert(ls2))

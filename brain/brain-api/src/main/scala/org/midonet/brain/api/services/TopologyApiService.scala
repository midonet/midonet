...
class TopologyApiService extends AbstractService {
    ...
    private val connMgr = new ConnectionManager
    private val reqHandler = new RequestHandler(connMgr)

    private val ntSrv = new ServerFrontEnd(
        new PlainAdapter(reqHandler.subject), NETTY_PORT)
    private val wsSrv = new ServerFrontEnd(
        new WebSocketAdapter(reqHandler.subject), WEBSK_PORT)

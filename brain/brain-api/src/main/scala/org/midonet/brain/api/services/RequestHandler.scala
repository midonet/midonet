...
case class Proto(ctx: ChannelHandlerContext, proto: Commons.UUID) extends Request
...
class RequestHandler(private val connMgr: ConnectionManager) {
    ...
    subject.subscribe(new Action1[Request] {
        override def call(req: Request): Unit = req match {
            ...
            case Disconnect(ctx) =>
                connMgr.unregister(ctx)
                log.info("api connection terminated")
            case Error(ctx, exc) =>
                connMgr.register(ctx).error(exc)
                log.error("api connection error")
            case Proto(ctx, pb) =>
                connMgr.register(ctx).msg(pb)

 ...
 */
class Session(id: Commons.UUID)(implicit val mgr: SessionManagerBase)
    extends SessionBase {
    private val log = LoggerFactory.getLogger(classOf[Session])
    private val outgoing =
        new AtomicReference[Observer[Message]](null)
    ...
    def attach(out: Observer[Message], onSuccess: Message,
               onReject: Message): Option[Session] = {

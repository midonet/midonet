...
class DefaultProtocolFactory(val sMgr: SessionManagerBase)
    extends ProtocolFactory {
    override def start(output: Observer[Message]): State = {
        Ready((id, success, reject) => {sMgr.get(id, output, success, reject)})
    }
}
 ...
 */
object State {
    type ClientFactory = (Commons.UUID, Message, Message) => Option[SessionBase]

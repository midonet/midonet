...
class MonitoringTest extends MidolmanTestCase {
    ...
    def testActualMonitoring() {
        ...
        val jvmNames = immutable.Set(
            ...
            "TotalPhysicalMemorySize", "ProcessCPUTime", "TotalSwapSpaceSize",
            "MaxHeapMemory", "UsedHeapMemory", "CommittedHeapMemory", "AvailableProcessors",
            "FreeSwapSpaceSize", "SystemLoadAverage", "txBytes", "rxBytes",
            "rxPackets", "txPackets")
        ...
        probeByName(MonitoringActor.Name).expectMsgType[DatapathController.PortStats]
            .portID.toString should be(port.getId.toString)


        waitFor("Wait for the MidoReporter to write to Cassandra.", 10000, 500,
        ...
        assertThat("The registry contains all the expected metrics.",
            (names.diff(jvmNames)).isEmpty)

        // clean stuff.
        registry.allMetrics().keys.foreach((arg: MetricName) =>
            registry.removeMetric(arg))
    }

...
class JGroupsZkClient {
    private val log = LoggerFactory.getLogger("org.midonet.cluster.storage.jgroups")
    private val jgroupsConf = JGroupsZkClient.jgroupsConf
    private val backendConf = JGroupsZkClient.backendConf
     ...
     */
    private[jgroups] def readBrokersFromZK: List[IpAddress] = {
        ...
        log.error("Reading brokers in ZK: {}", children.fold("")(_+", "+_))

        children.map(child => new IpAddress(child)).toList
    }
     ...
     * This list is retrieved from ZooKeeper.
     */
    def readBrokerConnectionStringsFromZK: List[String] = {
        val path = brokersRootPath
        val children = zkClient.getZookeeperClient
            .getZooKeeper
            .getChildren(path, false /*watch*/)
    ...
    /**
     * Returns a randomly-selected broker server, ignoring the passed broker
      * address.
     */
    private[jgroups] def randomBroker(brokerToDiscard: Option[Address])
    : IpAddress = {
        val brokers = brokerToDiscard match {
            case Some(ipPort) =>
                readBrokersFromZK.filterNot(addr => addr == ipPort)
            case None =>
                readBrokersFromZK
        }

...
public class ZkDirectory implements Directory {
    ...
    public ZkDirectory(ZooKeeper zk, String basePath, Reactor reactor) {
        this.zk = zk;
        this.basePath = basePath;
        this.reactor = reactor;
    }

    public ZkDirectory(ZkConnection connection, String basePath,
                       Reactor reactor) {
        this(connection.getZooKeeper(), basePath, reactor);
    }
    ...
    public String add(String relativePath, byte[] data, CreateMode mode)
        ...
        String path = zk.create(absPath, data, acl, mode);
    ...
    public void update(String relativePath, byte[] data, int version)
            throws KeeperException, InterruptedException {
        String absPath = getAbsolutePath(relativePath);
        zk.setData(absPath, data, version);
    }
    ...
    public byte[] get(String relativePath, Runnable watcher)
        throws KeeperException, InterruptedException {
        String absPath = getAbsolutePath(relativePath);
        return zk.getData(absPath, wrapCallback(watcher), null);
    }
    ...
    public Set<String> getChildren(String relativePath, Runnable watcher)
        ...
        return
            new HashSet<>(zk.getChildren(absPath, wrapCallback(watcher)));
    }
    ...
    public boolean exists(String path, Watcher watcher)
        ...
        return zk.exists(absPath, watcher) != null;
    }

...
public class CassandraClient {
    ...
    private void _connect(int retries) {
        ...
        try {
            ...
            if (this.session == null) {
                ...
                log.info("Connection to Cassandra key space {} ESTABLISHED",
                         keyspaceName);
            }
    ...
    private void scheduleReconnect(final int retries) {
        if (retries > 0) {
            log.info("Scheduling Cassandra reconnection retry to key space " +
                     keyspaceName + " (" + retries + ") retries left).",
                     retries);
            ...
            log.warn("Unable to connect to cassandra key space {} after "
                     + "10 retries, giving up", keyspaceName);

...
package object snapshot {
    ...
    class TopologySnapshotSerializer {
        ...
        private def encodeStateOwner(encoder: StateOwnerEncoder,
            ...
            while (entries.hasNext) {
                ...
                encoder.next()
                val ownerId = UUID.fromString(entry.getKey)
        ...
        private def encodeStateKey(encoder: StateKeyEncoder,
            ...
            while (entries.hasNext) {
                ...
                val values = encoder.multiValueCount(
                    state.multiData().length)
    ...
    class TopologySnapshotDeserializer {
        ...
        private def decodeObjectClass(decoder: ObjectClassDecoder,
            ...
            Log.debug(s"Dentro de decodeObjectClass con ${objects.count()} objects.")
            ...
            Log.debug(s"Class to decode $clazz")
        ...
        private def decodeObject(decoder: ObjectDecoder,
            ...
            val data = new String(objData, "UTF-8")
            Log.debug(s"Decoded object: $objId $data")
        ...
        private def decodeStateOwner(decoder: StateOwnerDecoder,
            ...
            snapshot.putIfAbsent(ownerId.toString, stateClassGroup)
        }

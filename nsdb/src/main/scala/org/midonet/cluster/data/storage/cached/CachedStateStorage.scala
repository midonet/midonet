...
class CachedStateStorage(private val store: Storage,
                         ...
                         private val cache: Map[Class[_], Map[ObjId, Map[String, StateKey]]])
    ...
    override def getKey(clazz: Class[_], id: ObjId,
        ...
        cache.get(clazz).flatMap(_ get id).flatMap(_ get key) match {
            case Some(stateKey) =>
    ...
    override def getKey(namespace: String, clazz: Class[_], id: ObjId,
                        key: String): Observable[StateKey] = {
        val cached: StateKey = null
        stateStore.getKey(namespace, clazz, id, key).startWith(cached)
    }
    ...
    override def keyObservable(clazz: Class[_], id: ObjId,
                               key: String): Observable[StateKey] = {
        val cached: StateKey = null
        stateStore.keyObservable(clazz, id, key).startWith(cached)
    }
    ...
    override def keyObservable(namespace: String, clazz: Class[_], id: ObjId,
                               key: String): Observable[StateKey] = {
        val cached: StateKey = null
        stateStore.keyObservable(namespace, clazz, id, key).startWith(cached)
    }
    ...
    override def keyObservable(namespaces: Observable[String], clazz: Class[_],
        ...
        val cached: StateKey = null
        stateStore.keyObservable(namespaces, clazz, id, key).startWith(cached)
    }
    ...
    override protected def namespace: String = notImplemented
}

    ...
    : StateTable[K, V] = {
        ...
        val injector = Guice.createInjector(new AbstractModule {
            override def configure(): Unit = {
                bind(classOf[Directory]).toInstance(directory)
                bind(classOf[ZkConnectionAwareWatcher]).toInstance(connectionWatcher)
            }
        })
        injector.getInstance(provider.clazz).asInstanceOf[StateTable[K, V]]
    }

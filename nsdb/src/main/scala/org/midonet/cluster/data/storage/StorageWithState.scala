...
private[storage] final class StateInfo {
    val keys = new ConcurrentHashMap[String, WritePolicy]
}
...
trait StorageWithState {
    ...
    protected[this] val stateInfo = new ConcurrentHashMap[Class[_], StateInfo]
    ...
    : WritePolicy = {
        val info = stateInfo.get(clazz)
        if (info eq null) {
            throw new IllegalArgumentException(
                s"Class ${clazz.getSimpleName} is not registered")
        }
        val policy = info.keys.get(key)
        if (policy eq null) {
            throw new IllegalArgumentException(

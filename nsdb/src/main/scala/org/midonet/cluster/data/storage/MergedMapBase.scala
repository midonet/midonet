 ...
 * The observable and observer arguments are there to interact with the
 * middleware used to propagate opinions across the various participants.
 * Opinions coming from merged maps originating from other hosts are emitted on
...
class MergedMapBase[K, V >: Null <: AnyRef]
    (mapId: MapId, owner: String, executor: ExecutorService,
     opinionObservable: Observable[(K, V, String)],
     opinionObserver: Observer[(K, V, String)])

...
private[storage] class ConsumerService(config: KafkaConfig, owner: String) {
    ...
    /**
     * An observer that deserializes kafka messages received in arrays of bytes,
...
/**
 * This object keeps a reference to the consumer service and counts the
 * number of subscriptions to the service. The service is stopped when
 * the number of subscriptions reaches zero and it is started when needed.
 *
 * All calls to the service are synchronized, effectively allowing
...
object KafkaBus {
    ...
    private[storage] def subscribe[K, V >: Null <: AnyRef]
        ...
        this.synchronized {
            ...
            service.startIfStopped()
        }
    }

    /**
     * Unsubscribes from the given topic. This method should not be called
     * if the consumer service is not subscribed to this topic.
     */
    private[storage] def unsubscribe(topic: String): Unit = {
        this.synchronized {
    ...
    @VisibleForTesting
    private[storage] def isServiceStopped(): Boolean =
        consumerService.isStopped()
}

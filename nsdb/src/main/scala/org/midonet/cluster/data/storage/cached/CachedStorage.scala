...
class CachedStorage(private val store: Storage,
    ...
    protected def notImplemented = throw new NotImplementedException(
    ...
    override def observable[T](clazz: Class[T], id: ObjId): Observable[T] = {
        val cached = cache.get(clazz).flatMap(_ get id)
        store.observable(clazz, id).startWith(cached.get.asInstanceOf[T])
    }
    ...
    override def observable[T](clazz: Class[T]): Observable[Observable[T]] = {
        val cached = cache.get(clazz).map(_.values.map(_.asInstanceOf[T])).get
    ...
    override def getAll[T](clazz: Class[T],
        ...
        val allCached = cache.get(clazz).filter(ids contains _)
            .map(_.values).getOrElse(Seq.empty)
        Future.successful(allCached.asInstanceOf[Seq[T]])
    }
    ...
    override def getAll[T](clazz: Class[T]): Future[Seq[T]] = {
        val allCached = cache.get(clazz).map(_.values).getOrElse(Seq.empty)
        Future.successful(allCached.asInstanceOf[Seq[T]])
    }

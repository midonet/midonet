    ...
    : Array[Byte] = {
        ...
        if (count > 0) {
            ...
            if (provenanceBuilder.getChangeOwner == owner.id) {
                if (provenanceBuilder.getProductVersion == Storage.ProductVersion &&
                    provenanceBuilder.getProductCommit == Storage.ProductCommit &&
                    (provenanceBuilder.getChangeType & change) != 0) {
                    // If the provenance data has not changed, return null.
                   return null
                } else {
                    // If the provenance has changed, update the product
                    // version, commit and change type.
                    provenanceBuilder
                        .setProductVersion(Storage.ProductVersion)
                        .setProductCommit(Storage.ProductCommit)

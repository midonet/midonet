...
import org.midonet.cluster.util.{DirectoryObservableClosedException, NodeObservableClosedException, PathDirectoryObservable, NodeObservable}
 ...
 * [[Storage]] trait (e.g. [[ZookeeperObjectMapper]]), and which must implement
 * the trait's abstract methods below specifying the root path, Apache Curator
 * instance and version.
 *
 * The trait provides an implementation, where the state for each object is
 * stored in a ZooKeeper path as:
 *
 ...
 * This must be done for every new object added to the storage. It also the
...
trait ZookeeperObjectState extends StorageWithState with Storage {
    ...
    /** Returns an observable for the given object. */

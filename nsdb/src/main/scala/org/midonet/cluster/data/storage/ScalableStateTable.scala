...
import org.midonet.cluster.data.storage.ScalableStateTable.{KeyValue, Log, PersistentVersion, TableEntry}
...
trait ScalableStateTable[K, V] extends StateTable[K, V] with StateTableEncoder[K, V] {
    ...
    private class State extends SubscriptionList[Update[K, V]] {
            ...
            if (storageConnectedFlag.get()) {
                this.synchronized {
                    ...
                    if (adding.contains(keyValue)) {
                        Log debug s"($key) Already adding $key -> $value"
                        return
                    }
                    val entry = cache.get(key)
                    if ((entry ne null) && entry.value == value &&
                         !removing.contains(keyValue)) {
                        Log debug s"($key) Entry $key -> $value exists"
                        return
                    }
                    adding.add(keyValue)
                }
        ...
        def addComplete(entry: TableEntry[K, V], keyValue: KeyValue[K, V]): Unit = {
            Log trace s"($key) Add ${entry.key} -> ${entry.value} " +
                      s"completed"

            def publish(update: Update[K, V]): Unit = {
                val sub = subscribers
                var index = 0
            ...
            val removeEntry = this.synchronized {
                ...
                val removeEntry = if (oldEntry eq null) {
                    ...
                    Log trace s"($key) Added ${entry.key} -> ${entry.value} " +
                    ...
                    Log trace s"($key) Updated ${entry.key} -> ${entry.value} " +

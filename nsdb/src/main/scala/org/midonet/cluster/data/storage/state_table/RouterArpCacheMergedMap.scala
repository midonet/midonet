...
object RouterArpCacheMergedMap {
    ...
    case class ArpCacheUpdate(ipV4Addr: IPv4Addr, oldEntry: ArpCacheEntry,
        ...
        private def toStringOrNull(entry: ArpCacheEntry): String =
            if (entry eq null) "null" else entry.toString

        override val toString =
            s"[ip=$ipV4Addr oldEntry=${toStringOrNull(oldEntry)} " +
            s"newEntry=${toStringOrNull(newEntry)}]"
    }

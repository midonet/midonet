...
class ZookeeperObjectMapper(protected override val rootPath: String,
    ...
    override implicit private[storage] val zoomMetrics = metricsRegistry match {
        case null => new EmptyZoomMetrics
    ...
    @VisibleForTesting
    private[storage] def objectWatcherTriggered: Long =
        zoomMetrics.objectWatcherTriggerCount.get
    @VisibleForTesting
    private[storage] def classWatcherTriggered: Long =
        zoomMetrics.classWatcherTriggerCount.get
    ...
    override def getAll[T](clazz: Class[T]): Future[Seq[T]] = {
        ...
        val cb = new BackgroundCallback {
            override def processResult(client: CuratorFramework,
                ...
                zoomMetrics.addReadChildrenLatency(end-start)
    ...
    override def multi(ops: Seq[PersistenceOp]): Unit = {
        ...
        val end = System.nanoTime()
        zoomMetrics.addMultiLatency(end-start)
    }
    ...
    override def observable[T](clazz: Class[T], id: ObjId): Observable[T] = {
        ...
        .doOnEach(zoomMetrics.incrementObjectWatcherTriggeredObs[T]())
        .asInstanceOf[Observable[T]]
    }
    ...
    override def observable[T](clazz: Class[T]): Observable[Observable[T]] = {
        ...
        classCaches.getOrElse(clazz, {
          ...
          .map(makeFunc1((obs: Observable[T]) => {
              zoomMetrics.classWatcherTriggerCount.incrementAndGet()
              obs.doOnEach(zoomMetrics.incrementClassWatchersTriggeredObs[T]())
          }))
    }

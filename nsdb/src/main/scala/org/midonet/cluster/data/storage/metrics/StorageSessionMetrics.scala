...
trait StorageSessionMetrics {

    def connectionStateListener(): ConnectionStateListener
}

object StorageSessionMetrics {

    val Nil = new StorageSessionMetrics {
        override def connectionStateListener() =
            StorageMetrics.nilStateListener()
    }
}

class JmxStorageSessionMetrics(override val registry: MetricRegistry)
    extends StorageSessionMetrics with MetricRegister {

    private val timeConnectedHistogram = histogram("timeConnectedHistogram")
    private val timeDisconnectedHistogram = histogram("timeDisconnectedHistogram")

...
class StorageWrapper(private val cacheTtlMs: Long,
    ...
    private val initialTimestamp = NANOSECONDS.toMillis(System.nanoTime)
    private var cacheInvalidated = false

    // On demand invalidation check to avoid using an extra thread
    protected def cacheValid: Boolean = if (cacheInvalidated) false else {
        val actual = NANOSECONDS.toMillis(System.nanoTime)
        cacheInvalidated = (actual - initialTimestamp) > cacheTtlMs
        !cacheInvalidated
    }

    protected def validStore: Storage = if (cacheValid) { println("cachedStore"); cachedStore } else { println("regularStore"); store }

...
trait JGroupsMessageSerializer[K, V] {
    ...
    def decodeMessage(triple: KeyOwnerPayloadTriple): (K, V, String)

    def encodeMessage(opinion: (K, V, String)): KeyOwnerPayloadTriple
...
class JGroupsBus[K, V](mapName: String,
    ...
    private def mapToOpinion(kopTriple: KeyOwnerPayloadTriple): Opinion =
        serializer.decodeMessage(kopTriple)

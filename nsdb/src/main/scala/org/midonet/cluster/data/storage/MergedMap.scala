...
class MergedMap[K, V >: Null <: AnyRef](opinionBus: MergedMapBus[K, V])
    ...
    private val log =
        Logger(LoggerFactory.getLogger(s"MergedMap-${opinionBus.mapId}"))
    ...
    private def onError(e: Throwable): Unit = {
        log.warn("Exception in opinion observable, completing winning and " +
                 "opinion observer subjects", e)
        winningSubject.onCompleted()
        opinionBus.opinionObserver.onCompleted()
    }

    private def onCompleted(): Unit = {
        log.warn("Opinion observable completed, completing winning and " +
                 "opinion observer subjects")
        winningSubject.onCompleted()
        opinionBus.opinionObserver.onCompleted()
    }
    ...
    private def onNewOpinion(opinion: (K, V, String)): Unit = opinion match {
        case (null, _, owner) =>
            log.warn("Opinion with a null key on the opinion bus from owner {}",
                     owner)
        case (_, value, null) =>
            log.warn("Opinion with a null owner on the opinion bus for value {}",
                     value)
    ...
    private def onOpinionAddition(key: K, value: V, owner: String): Unit = {
        if (!cache.contains(key)) {
            cache.put(key, new mutable.HashMap[String, V]())
        }
        ...
        val newWinner = winner(cache(key))
    ...
    private def onOpinionRemoval(key: K, owner: String): Unit = {
        cache.get(key) match {
            case Some(opinions) =>
                val prevWinner = winner(opinions)
                opinions -= owner
                ...
                if (prevWinner != null && !prevWinner.equals(newWinner)) {
                    if (newWinner != null) {

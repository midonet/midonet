...
class MergedMap[K, V >: Null <: AnyRef](opinionBus: MergedMapBus[K, V])
    ...
    private val log =
        Logger(LoggerFactory.getLogger(s"getClass.getName-${opinionBus.mapId}"))
    ...
    private def onError(e: Throwable): Unit = {
        log.warn("Exception in opinion bus, completing winning subject", e)
        winningSubject.onCompleted()
    }

    private def onCompleted(): Unit = {
        log.warn("Opinion bus completed, completing winning subject")
        winningSubject.onCompleted()
    }
    ...
    private def onOpinionAddition(key: K, value: V, owner: String): Unit = {
        ...
        if (!prevWinner.isDefined || !prevWinner.get.equals(newWinner)) {
    ...
    private def onOpinionRemoval(key: K, owner: String): Unit = {
        cache.get(key) match {
            case Some(opinions) =>
                ...
                if (opinions.isEmpty && prevWinner != null) {
                ...
                if (!prevWinner.equals(newWinner)) {
                    if (newWinner != null) {

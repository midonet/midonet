...
trait SharedMap[K, V] {
    /**
     * An update to the map. If the second element of a map update is null,
     * it means that the entry was deleted.
     */
    type MapUpdate[K, V] = (K, V)
}
 ...
 * trait below.
 */
trait PrivateMap[K, D <: Any, V <: AnyRef] extends SharedMap[K, V] {
     ...
     */
    def observable: Observable[MapUpdate[K, V]]
 ...
 */
trait MergedMap[K, D <: Any, V <: AnyRef] extends SharedMap[K, V] {

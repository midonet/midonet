...
object JGroupsBroker {
    ...
    type TopicLog = mutable.Map[(String, String), (Long, Array[Byte])]
    ...
    private class State(messages: MessageLog = mutable.HashMap.empty) {
        ...
        /**
          ...
          * otherwise.
          */
        def addMessage(topic: String, key: String, owner: String, stamp: Long, bytes: Array[Byte])
        : Boolean = {
            this.synchronized {
                if (messages.get(topic).isEmpty) {
                    val map: TopicLog = new mutable.HashMap[(String, String), (Long, Array[Byte])]
                    messages += topic -> map
                }

                val topicLog = messages(topic)
                if (!topicLog.contains( (owner, key) )) {
                    topicLog += (owner, key) -> (stamp, bytes)
                    true
                } else {
...
class JGroupsBroker(port: Int) extends ReceiverAdapter with ClientReceiver
    ...
    override def receive(msg: Message): Unit = scheduler.submit(makeRunnable {
        JGroupsBroker.toPubSubMessage(msg.getBuffer) match {
            case PublishMessage(topic, key, owner, stamp, payload) =>
                if (state.addMessage(topic, key, owner, stamp, msg.getBuffer)) {
                    log.debug("Delivered msg with key: {} and payload: {} " +
                        "for topic: {}, owner: {}, stamp: {}",
                        Array(key, payload, topic, owner, stamp.toString):_*)

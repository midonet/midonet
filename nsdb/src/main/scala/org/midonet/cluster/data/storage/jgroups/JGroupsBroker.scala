...
object JGroupsBroker {
    ...
    case class PublishMessage(topic: String,
        ...
        override def toByteArray(): Array[Byte] = {
            ...
            val msg = s"$topic#$key#$owner#$timestampString#$payload"
            msg.getBytes(defaultCharset)
        }
    }

    /* (Owner, Key) -> (Timestamp, Message) */
    type TopicLog = mutable.Map[(JGroupsClient.Owner, JGroupsClient.Key), (Long, PublishMessage)]
    ...
    private class State(messages: MessageLog = mutable.HashMap.empty) {
        ...
        : Boolean = addMessage(msg.topic, msg.key, msg.owner, msg.timestamp, msg)
                ...
                scheduler.submit(makeRunnable {
                    if (state.addMessage(pm)) {
                        log.debug("Delivered msg with key: {} and payload: {} " +
                            "for topic: {}, owner: {}, timestamp: {}",
                            Array(pm.key, pm.payload, pm.topic, pm.owner, pm.timestamp.toString): _*)

 ...
 * thread-safe.
 */
class JGroupsClient() extends ReceiverAdapter with ConnectionListener {
    ...
    private val server = Utils.randomBroker
    log.info("Selected broker server: {}", server)
    private val client = new TcpClient(InetAddress.getLocalHost, 0, server.addr,
                                       server.port)

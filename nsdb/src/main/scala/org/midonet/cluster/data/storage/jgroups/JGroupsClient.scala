...
class JGroupsClient extends ReceiverAdapter
    ...
    /* Map of messages to be acknowledged by a broker. This is a mapping
       ...
       MessageIdentifier = (Topic, Key, Stamp)
       */
    type MessageIdentifier = (String, String, Long)
    private val msgsToAck = new mutable.HashMap[MessageIdentifier, Array[Byte]]()
        ...
        try {
            JGroupsBroker.toPubSubMessage(buf.array()) match {
                case msg: PublishMessage =>
                    if (msgsToAck.remove((msg.topic, msg.key, msg.stamp)).isEmpty) {
    ...
    private def publish(msg: PublishMessage): Unit =
        scheduler.submit(makeRunnable {
            ...
            try {
                ...
                msgsToAck.put((msg.topic, msg.key, msg.stamp), msgInBytes)
    ...
    private def publish(id: MessageIdentifier, msg: Array[Byte]): Unit = {
        try {
            client.send(ByteBuffer.wrap(msg))
            msgsToAck.put(id, msg)
        } catch {
            case NonFatal(e) => log.warn("Unable to publish msg: " + msg, e)
        }
    }
    ...
    def topicObservable(topic: String): Observable[(String, String)] = {

        Observable.create(new OnSubscribe[(String, String)] {
            override def call(subscriber: Subscriber[_ >: (String, String)]): Unit = {

                def extractKeyAndPayload(msg: PublishMessage): (String, String) =
                    (msg.key, msg.payload)

                val topicObs: Observable[(String,String)] = messageSubject
                    .filter(makeFunc1(_.topic == topic))
                    .map(makeFunc1(extractKeyAndPayload))

...
object JGroupsClient {
    ...
    class NotSubscribedToTopicException(topic: String)
      extends Exception(s"You are not subscribed to the requested topic: $topic") {
    }
}
...
class JGroupsClient(bindHost: String = "", initialBroker: Option[IpAddress] = None) extends PubSubMessageReceiver
    ...
    def this(bindHost: String, brokerHostPort: String) = {
    ...
    /* Map of messages to be acknowledged by a broker. This is a mapping
       ...
       */
    type MessageIdentifier = (String, JGroupsClient.Key, JGroupsClient.Owner)
    ...
    : Boolean = {
        ...
        bootstrap.connect(broker.getIpAddress, broker.getPort).sync().isSuccess
    }
     ...
     * if it was
     */
    private def handleMessage(pm: PublishMessage): Boolean =
        msgsToAck.get((pm.topic, pm.key, pm.owner)) match {
            case Some((stamp, msg)) =>
                if (pm.timestamp >= stamp) {

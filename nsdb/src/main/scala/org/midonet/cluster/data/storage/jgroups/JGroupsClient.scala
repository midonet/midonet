...
import org.midonet.cluster.data.storage.jgroups.JGroupsClient.NotSubscribedToTopicException
...
object JGroupsClient {
    class StringStringMapJGroupsSerializer() extends JGroupsMessageSerializer[String, String] {
        override def decodeMessage(keyValuePair: (String, String)): (String, String, String) = {
            val parts = keyValuePair._1.split("-")
            (parts(0), keyValuePair._2, parts(1))
        }

        override def encodeMessage(opinion: (String, String, String)): (String, String) = {
            (opinion._1 + "-" + opinion._3, opinion._2)
        }
    }

    def main(args: Array[String]): Unit = {
        val client = new JGroupsClient()
        val serializer = new StringStringMapJGroupsSerializer()
...
class JGroupsClient extends ReceiverAdapter
        ...
        try {
            JGroupsBroker.toPubSubMessage(buf.array()) match {
                case message:PublishMessage  =>
                    if (msgsToAck.remove(message.key).isEmpty) {
                        log.debug("Received msg for topic: {} with key: {} " +
                                  "and payload: {} from broker: {}",
                                  Array(message.topic, message.key, message.payload, sender.toString):_*)
                    } else {
    ...
    def isSubscribedTo(topic: String): Boolean = subscriptions.contains(topic)
    ...
    def publish(topic: String, key: String, payload: String): Unit =
        publish(PublishMessage(topic, key, payload))

    def publish(msg: PublishMessage): Unit =
        scheduler.submit(makeRunnable {
            ...
            try {
                ...
                msgsToAck.put(msg.key, msgInBytes)
    ...
    def topicKeyValuePairObservable(topic: String): Observable[(String, String)] = {
        messageSubject
            .filter(makeFunc1(_.topic == topic))
            .map(makeFunc1((msg) => (msg.key, msg.payload)))
    }

...
class StateClient(settings: StateClientSettings)
    ...
    private class Builder {
        private val msg: State.Message.Builder
            = State.Message.newBuilder().setRequestId(nextRequestId())

        def send(flush: Boolean = true): Try[RequestId] = Try {
            connection.get.write(msg.build(), flush)
            msg.getRequestId
        }
    }

    private object Builder {
        def fromSubscribe(t: State.Message.Subscribe): Builder = {
            val b = new Builder()
    ...
    private def sendSubscribeTo(params: SubscriptionParams): Try[RequestId] = {
        val msg = State.Message.Subscribe.newBuilder()
            .setObjectClass(params.objectClass)
            .setObjectId(params.objectId)
            .setTableName(params.tableName)
            .addAllTableArguments(params.tableArguments.asJava)

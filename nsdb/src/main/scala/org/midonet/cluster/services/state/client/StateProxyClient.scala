...
/**
  ...
  * Use  [[observable]] to obtain an observable to a [[StateTable]] that can
  * be subscribed to.
...
class StateProxyClient(settings: StateProxyClientSettings,
    ...
    /**
      ...
      * @param table the [[StateTable]] description.
      * @return an observable to a state table.
      */
    override def observable(table: StateTable): Observable[Update] = {

        def subscribe(observer: StateObserver, table: StateTable): Boolean = {
        ...
        def unsubscribe(observer: StateObserver, table: StateTable): Unit = {
            state.get match {
                case s: Connected =>
                ...
                if (state.compareAndSet(s, Connected(obs,
                    ...
                    if (obs.nonEmpty) {
                        Future {
                            ...
                            log.debug(s"$this - sending $total subscriptions")
            ...
            case Some(observer) =>
                msg.getNotificationCase match {
                    ...
                    case ProxyResponse.Notify.NotificationCase.UPDATE =>
                        observer.onNext(Update(msg.getUpdate))
    ...
    private def sendSubscribeRequest(observer: StateObserver,
        ...
        sendAndAddToPending(msg,observer,isSubscribe=true,flush)
    }
...
object StateProxyClient {
    ...
    private def errorToString(msg: ProxyResponse.Error): String = {
        ...
        def codeToString(code: ProxyResponse.Error.Code) = code match {
            ...
            case ProxyResponse.Error.Code.NO_TABLE         => "No table"
        }

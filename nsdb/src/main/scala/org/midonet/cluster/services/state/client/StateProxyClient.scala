...
class StateProxyClient(conf: StateProxyClientConfig,
                ...
                case s: Waiting =>
                    if (s.retryCount < conf.maxSoftReconnectAttempts) {
                        ...
                        if (state.compareAndSet(s, Waiting(count,
                                                           s.subscribers))) {
                            log.debug(s"$this Connection attempt $count failed",
                                      cause)
    ...
    private def onAckReceived(rid: RequestId,
        ...
        state.removeTransaction(rid) match {
            ...
            case None =>
                log debug s"$this Received acknowledge for unknown request:$rid"
        }
    }
    ...
    private def onErrorReceived(rid: RequestId,
        ...
        state.removeTransaction(rid) match {
            case Some(transaction) =>
                ...
                if (isSubscribe)
                    log debug s"$this Subscribe failed request:$rid " +
                              s"reason:$description"
                else
                    log debug s"$this Unsubscribe failed request:$rid " +
                              s"reason:$description"
    ...
    private def onPongReceived(rid: RequestId, msg: ProxyResponse.Pong): Unit = {
        ...
        if (reference._1 == rid) {
            ...
            log debug s"$this Unexpected ping response with seq:$rid"
        }
    }
    ...
    private def sendRequest(msg: ProxyRequest,
        ...
        if (isSubscribe)
            log debug s"$this Subscribing transaction $rid"
        else
            log debug s"$this Unsubscribing transaction $rid"
    ...
    def ping(): Boolean = {
        ...
        if (sent) {
            log debug s"$this Sent ping request: $rid"
        } else {

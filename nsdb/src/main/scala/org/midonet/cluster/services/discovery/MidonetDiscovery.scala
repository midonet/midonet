...
class MidonetDiscovery @Inject()(curator: CuratorFramework,
    ...
    : MidonetDiscoveryClient[S] = {
        new MidonetDiscoveryClient[S](serviceName, discoveryService, executor)
    }
    ...
    : MidonetServiceHandler = {
        new MidonetServiceHandler(serviceName, Option(address), Option(port),
    ...
    : MidonetServiceHandler = {
        new MidonetServiceHandler(serviceName, Option.empty, Option.empty,
...
/**
  ...
  * to use it any more.
  */
final class MidonetDiscoveryClient[S] (serviceName: String,
                                       serviceDiscovery: ServiceDiscovery[Void],
                                       executor: ExecutorService)
                                      (implicit val tag: TypeTag[S]) {
    ...
    def instances: Seq[S] = {
        cache.getInstances.asScala.filter(_.isDefined).map(_.get)
    }

    implicit def asMidonetService(instance: ServiceInstance[Void]): Option[S] = {
        (tag, instance.getAddress, instance.getPort, instance.getUriSpec) match {
            case (_, address: String, port: Integer, _)
                if typeOf[S] =:= typeOf[MidonetServiceHostAndPort] =>
                Option(MidonetServiceHostAndPort(address, port).asInstanceOf[S])
            case (_, _, _, uri: UriSpec)
...
/**
  ...
  * expiration.
  */
final class MidonetServiceHandler (name: String, address: Option[String],
                                   port: Option[Int], uri: Option[URI],
                                   discovery: ServiceDiscovery[Void]) {
    ...
    def unregister(): Unit = discovery.unregisterService(instance)
}

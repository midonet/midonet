...
/**
  ...
  * be the same.
  *
  * Service registration example for a service on bind_ip:2674
  * ...
  * val discoveryService = new MidonetDiscovery(curator, executor)
  * val service = discoveryService.registerServiceInstance("service_name", bind_ip, 2674)
  * ...
  ...
  * ...
  * val discoveryService = new MidonetDiscovery(curator, executor)
  * val client = discovery.getClient("service_name", classOf[Void])
  ...
  * ...
  * val serviceObserver: Observer[List[ServiceInstance[HostPortInfo]]] = ...
...
class MidonetDiscovery[P] @Inject()(curator: CuratorFramework,
    ...
    val discoveryService = ServiceDiscoveryBuilder
    ...
    /** Returns a [[MidonetServiceInstance]] handle necessary to registers the
      ...
      * @return
      */
    def createServiceInstance(serviceName: String,
                              server: Option[String],
                              port: Option[Int], info: Option[P])
...
/**
  ...
  */
final class MidonetDiscoveryClient[P: ClassTag] protected[discovery](
    val serviceName: String, serviceDiscovery: ServiceDiscovery[P],
    executor: ExecutorService) {

    private val log = Logger(LoggerFactory.getLogger("service-discovery"))
    ...
    private def updateServiceInstances(): Unit = {
        updates.onNext(getAllInstances)
    }
    ...
    /** Get the current set of registered [[ServiceInstance]]. It's updated
      * upon notification from the storage layer (zk) so it could be outdated
      * for a limited amount of time. */
    def getAllInstances: List[ServiceInstance[P]] = {
        cache.getInstances.asScala.toList
    }

    /** Get a random service provider instance */
    def getRandomInstance: Option[ServiceInstance[P]] = {
...
/**
  ...
  */
final class MidonetServiceInstance[P: ClassTag] protected[discovery](
    serviceName: String, server: Option[String], port: Option[Int],
    info: Option[P], serviceDiscovery: ServiceDiscovery[P]) {

    var serviceInstance: ServiceInstance[P] = _
     ...
     */
    def register() = {
        val serviceInstanceBldr = ServiceInstance.builder[P]()
        ...
        serviceInstance = serviceInstanceBldr.build()

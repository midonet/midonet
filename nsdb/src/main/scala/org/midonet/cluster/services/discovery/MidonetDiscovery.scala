...
class MidonetDiscovery[P] (@Inject curator: CuratorFramework,
                           @Inject executor: ExecutorService,
                           @Inject config: MidonetBackendConfig)
    ...
    : MidonetServiceInstance[P] = {
        ...
        new MidonetServiceInstance[P](serviceName, server, port, info,
                                      discoveryService)
    }
}
...
final class MidonetServiceInstance[P: ClassTag] (
    ...
    private val serviceInstanceBldr = ServiceInstance.builder[P]()
        ...
        .name(serviceName)
    if (server.isDefined) serviceInstanceBldr.address(server.get)
    if (port.isDefined) serviceInstanceBldr.port(port.get)
    if (info.isDefined) serviceInstanceBldr.payload(info.get)

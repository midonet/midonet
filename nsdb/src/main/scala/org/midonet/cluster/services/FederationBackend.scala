...
abstract class FederationBackend extends AbstractService with Backend {
    ...
    final def setupBindings(): Unit = {
        List(classOf[MidonetRouterBinding],
             classOf[MidonetVtep],
             classOf[OvsdbRouterBinding],
             classOf[OvsdbVtep],
             classOf[VxlanSegment]
        ).foreach(store.registerClass)

        store.declareBinding(classOf[VxlanSegment], "midonet_binding", CASCADE,
                             classOf[MidonetRouterBinding], "segment_id", CLEAR)
        store.declareBinding(classOf[VxlanSegment], "ovsdb_binding", CASCADE,
                             classOf[OvsdbRouterBinding], "segment_id", CLEAR)

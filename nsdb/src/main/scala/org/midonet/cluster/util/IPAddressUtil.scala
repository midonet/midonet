...
object IPAddressUtil {
    ...
    final class RichAddresString(str: String) extends AnyVal {
        def asProtoIPAddress: Commons.IPAddress = str
    }

    final class RichIPAddress(addr:IPAddr) extends AnyVal {
        def asProto: Commons.IPAddress = addr
    }

    final class RichInetAddress(addr: InetAddress) extends AnyVal {
        def asProto: Commons.IPAddress = IPAddressUtil.toProto(addr)
    }

    final class RichProtoIPAddress(addr: Commons.IPAddress) extends AnyVal {

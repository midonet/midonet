...
object ObservablePathDirectoryCache {
     ...
     * [[DirectoryCacheDisconnectedException]] error.
...
class OnSubscribeToDirectory(curator: CuratorFramework, path: String)
    ...
    private def refresh(): Unit = try {
        ...
        case e: Exception =>
            ...
            subject.onError(new DirectoryCacheDisconnectedException(path, e))
            close()
    }
    ...
    private def processChildren(event: CuratorEvent): Unit = {
        if (event.getResultCode == Code.OK.intValue) {
            ...
            subject.onError(new DirectoryCacheDisconnectedException(path))
            close()
        } else  {
...
class DirectoryCacheDisconnectedException(val path: String, e: Throwable)

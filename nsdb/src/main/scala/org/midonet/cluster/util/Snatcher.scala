...
/** The Snatcher will watch the observable that emits state keys for a given
  ...
  *
  * You should avoid using two instances of the Snatcher in the same JVM
  * using the same nodeId, but nothing will prevent you from doing it.
  *
...
sealed class Snatcher[D](val targetId: UUID,
    ...
    override def onNext(t: StateKey): Unit = t match {
        case SingleValueKey(k, Some(value), ownerId) =>
            log.debug(s"$typeName $targetId already taken by node $ownerId")
            val wasOwner = isOwner
            isOwner = false
            if (wasOwner) {
                onLost()
        ...
        case SingleValueKey(k, None, _) =>
            ...
            try {
                ...
                case e: UnmodifiableStateException =>  // race lost
                case e: ConcurrentStateModificationException => // race lost
                case t: Throwable =>
                    log.warn("Failure", t)
            }
        case wh00t =>
            log.warn(s"Unexpected value on state key $stateKey: $wh00t")
    }

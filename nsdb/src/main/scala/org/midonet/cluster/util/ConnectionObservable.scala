...
object ConnectionObservable {
     ...
     * observable has not yet been subscribed to, and a connection listener has
     * not yet been set, [[State.Started]] after the first subscriber subscribed
...
class OnSubscribeToConnection(curator: CuratorFramework)
    ...
    private val log = getLogger(s"org.midonet.nsdb.zk-connection")

...
object NodeObservable {
     ...
     * [[NodeObservableDisconnectedException]]. When the cache is closed by
     * calling the `close()` method, the observable emits a
     * [[NodeObservableClosedException]].
     *
     * The `nullOnDelete` argument specifies whether the observable should
     * terminate with a [[NoNodeException]] if the node does not exist. When set
     * to `false`, the observable will install an exists watcher on the
    ...
    def create(curator: CuratorFramework, path: String,
               nullOnDelete: Boolean = false): NodeObservable = {
        new NodeObservable(new OnSubscribeToNode(curator, path, nullOnDelete))
    }
}
...
class OnSubscribeToNode(curator: CuratorFramework, path: String,
    ...
    private def processExistsCallback(event: CuratorEvent): Unit = {
        ...
        if (event.getResultCode == Code.OK.intValue) {
            ...
            log.debug("Node does not exist: wait for creation")

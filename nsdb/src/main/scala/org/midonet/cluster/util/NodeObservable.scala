...
object NodeObservable {
     ...
     *
     * When set to `true`, the observable completes when the node is deleted. If
     * the node does not exist, then the observable emits a [[NoNodeException]]
     * error.
     *
...
/** Signals that the [[Observable]] is no longer able to emit notifications from
  * ZK because the [[NodeObservable]] was closed. */

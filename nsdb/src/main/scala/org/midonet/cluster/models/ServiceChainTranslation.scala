...
object ServiceChainTranslation {
    ...
    def updateElements(store: Storage, op: PersistenceOp): Boolean = {
        ...
        var (inChain, outChain) = L2InsertionTranslation.ensureRedirectChains(store, portId)
        var (inSrvChain, _) = L2InsertionTranslation.ensureRedirectChains(store, srvPortId)

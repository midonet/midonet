...
object L2InsertionTranslation {
    ...
    def updateInsertions(store: Storage, op: PersistenceOp): Boolean = {
        ...
        if (added.isDefined) {
            var ruleIn = Rule.newBuilder().setId(UUID.randomUUID.asProto)
                ...
                .build
            var ruleOut = Rule.newBuilder().setId(UUID.randomUUID.asProto)
                .setChainId(inSrvChain.getId)
                .setType(Rule.Type.JUMP_RULE)
                .setAction(Rule.Action.JUMP)
                .setJumpRuleData(
            ...
            added.get.getVlan match {
                case 0 =>
                case v =>
                    ruleIn = ruleIn.toBuilder.setVlan(v | (1 << 13)).build
                    ruleOut = ruleOut.toBuilder.setVlan(v | (2 << 13)).build
            }
        ...
        if (previous.isDefined) {
            ...
            val ruleIn2 = Rule.newBuilder().setId(UUID.randomUUID.asProto)
                .setChainId(inChain.getId)
                .setDlSrc(last.getMac)
                .addInPortIds(portId)
                .setVlan(1<<15)

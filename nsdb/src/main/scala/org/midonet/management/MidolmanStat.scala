...
object MidolmanMetricCatalog {
    ...
    class AllMetrics(val mbsc: MBeanServerConnection) {
        ...
        private def buildLatency(worker: Int): List[StatColumn] = {
            ...
            List(new StatColumn(s"95th(w${worker})", 10, latency.get95th, 1),
                 new StatColumn(s"99th(w${worker})", 10, latency.get99th, 1),
                 new StatColumn(s"999th(w${worker})", 10, latency.get999th, 1))
        }

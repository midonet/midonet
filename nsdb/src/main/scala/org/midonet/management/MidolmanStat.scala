...
object MidolmanMetricCatalog {
    ...
    class AllMetrics(val mbsc: MBeanServerConnection) {
        ...
        def run(delaySecs: Int = 0, count: Int = 1): Unit = {
            ...
            var stat = new StatSet(getColumns)
            stat.printHeader()
            var curWorkers = getNumWorkers
            do {
                stat.printValues()
                iterations -= 1
                ...
                val nextWorkers = getNumWorkers
                if (nextWorkers != curWorkers) {
                    curWorkers = nextWorkers
                    stat = new StatSet(getColumns)
                    stat.printHeader()
                }
            } while (iterations > 0)
        }

        private val latencyColumnGroupName = "latency (microsecs)"
        private def buildLatency(worker: Int): List[StatColumn] = {
            ...
            List(new StatColumn(s"95th(w${worker})", 10, latency.get95th, 1000),
                 new StatColumn(s"99th(w${worker})", 10, latency.get99th, 1000),
                 new StatColumn(s"999th(w${worker})", 10, latency.get999th, 1000))
        }
        ...
        private def buildLatencies = {
            ...
            val allColumns = if (numWorkers > 0) {
                buildLatenciesHelper(0, numWorkers)
            } else {
                List(new StatColumn("NoWorkers!",
                                    latencyColumnGroupName.size,
                                    0.0))
            }
            new ColumnGroup(latencyColumnGroupName, allColumns)
        }

        private def getNumWorkers = {
            var beanName = SIM_LATENCY + "*packetsProcessed"
            val objectPattern = new ObjectName(beanName)
            mbsc.queryNames(objectPattern, null).size()
        }
    }

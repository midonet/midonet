...
object NatState {
    ...
    case class NatKeyStore(keyType: KeyType,
                           networkSrc: IPv4Addr,
                           transportSrc: Int,
                           networkDst: IPv4Addr,
                           transportDst: Int,
                           networkProtocol: Byte,
                           deviceId: UUID) extends IdleExpiration {
        override def toString = s"nat:$keyType:$networkSrc:$transportSrc:" +
                                s"$networkDst:$transportDst:$networkProtocol"

    }

    case class NatBindingStore(networkAddress: IPv4Addr, transportPort: Int)

}

object ConnTrackState {
    case class ConnTrackKeyStore(networkSrc: IPAddr,

        ...
        cfg.getString("zookeeper.root_key") match {
            case legacyPath if legacyPath == "/midonet/v1" =>
                log.warn("!!WARNING!! Detected legacy ZooKeeper root key {} " +
                         "in local configuration files, falling back to the " +
                         "default root key {}. To fix this, remove key " +
                         "'root_key' from these files: {}.",
                         Array(legacyPath, defaultZkRootKey,
                               MIDONET_CONF_LOCATIONS.mkString(", ")): _*)
                defaultZkRootKey
            case key => key
        }
    }

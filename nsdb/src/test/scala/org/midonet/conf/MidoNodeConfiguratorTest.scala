...
class MidoNodeConfiguratorTest extends FeatureSpecLike
    ...
    feature("MidonetBackendService should set the root path correctly") {
        scenario("Default root path with use_my_own_zk_root_path set to false") {
            ...
            MidoNodeConfigurator.zkRootPath(conf) shouldBe defaultZkRootPath
        }
        ...
        scenario("Default root path with use_my_own_zk_root_path set to true") {
            val overrides =
                ...
                """.stripMargin
            val conf = MidoTestConfigurator.forAgents(overrides)
            MidoNodeConfigurator.zkRootPath(conf) shouldBe "/midonet"
        }

        scenario("Default root path with use_my_own_zk_root_path not set") {
        ...
        scenario("random root path with use_my_own_zk_root_path set to true") {
            ...
            val overrides =
                ...
                """.stripMargin
            val conf = MidoTestConfigurator.forAgents(overrides)
            MidoNodeConfigurator.zkRootPath(conf) shouldBe rndPath
        }

        scenario("random root path with use_my_own_zk_root_path not set") {
            val rndPath = RandomStringUtils.random(10 /* string length */)
            val overrides =
                s"""
                  |zookeeper.root_key = ${rndPath}

...
class InMemoryMergedMapTest extends FeatureSpec with BeforeAndAfter
    ...
    /* For all tests below, we rely on observables because updates made to the
       private maps are not merged instantaneously into the merged map. */
    feature("containsKey/get/size") {
        ...
        scenario("One opinion or more exist") {
            ...
            When("We remove the opinion from the 1st private map and " +
                 "insert a dummy opinion")
            privateMap.removeOpinion("myKey")
            privateMap.putOpinion("dummy", 1, "dummyValue")
            ...
            privateMap2.putOpinion("dummy", 2, "dummyValue")
            ...
            privateMap.putOpinion("dummy", 3, "dummyValue")
    ...
    feature("snapshot/size") {
        scenario("A merged map with one private map") {
            ...
            privateMap.putOpinion("myKey1", 1, "myNewValue2")
            privateMap.putOpinion("dummy", 1, "dummyValue")
            obs.awaitOnNext(3, timeout) shouldBe true
    ...
    feature("join/leave") {
        scenario("A merged map with one private map") {
            ...
            And("Putting an opinion in the 1st private map has no effect")
            val privateMap2 = map.join("owner2")
            privateMap2.putOpinion("myKey2", 1, "myValue2")
            privateMap.putOpinion("myKey", 1, "myValue")
            obs.awaitOnNext(4, timeout) shouldBe true
            map.snapshot should contain only ("myKey2" -> "myValue2")
        }
        ...
        scenario("A merged map with 2 private maps-2nd private map leaves") {
        ...
        scenario("A merged map with 2 private maps-1st private map leaves") {
            Given("A merged map with two private maps")
            val privateMap1 = map.join("owner1")
            val privateMap2 = map.join("owner2")
            val obs = createObserverAndSubscribe()
            ...
            map.leave("owner1")
            privateMap2.putOpinion("dummy", 1, "dummyValye")
            obs.awaitOnNext(3, timeout) shouldBe true
        ...
        scenario("A merged map with 2 private maps-remove non-winning opinion") {
            Given("A merged map with two private maps")
            val privateMap1 = map.join("owner1")
            val privateMap2 = map.join("owner2")
            val obs = createObserverAndSubscribe()
            ...
            And("When the 1st private map remove its opinion and " +
                "we insert a dummy opinion")
            privateMap1.removeOpinion("myKey")
            privateMap1.putOpinion("dummy", 1, "dummyValue")
            obs.awaitOnNext(3, timeout) shouldBe true
    ...
    feature("Observable") {
        ...
        scenario("The observable emits an update when the merged map changes") {
            ...
            And("We insert a dummy opinion in the 2nd private map")
            privateMap2.putOpinion("dummy", 1, "dummyValue")

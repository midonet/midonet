...
import org.midonet.cluster.data.storage.KafkaSerialization
import org.midonet.cluster.data.storage.state_table.BridgeArpTableMergedMap.{ArpMergedMapSerialization, MacTS}
import org.midonet.packets.{IPv4Addr, MAC}

@RunWith(classOf[JUnitRunner])
class ArpTableMergedMapSerializationTest extends FeatureSpec with BeforeAndAfter
                                         with Matchers {

    var serialization: KafkaSerialization[IPv4Addr, MacTS] = _

    before {
        serialization = new ArpMergedMapSerialization()
    }
    ...
    feature("Serialization") {
        scenario("serializing opinion") {
            ...
            val arpOpinion = (ip, MacTS(mac, ts), "owner")

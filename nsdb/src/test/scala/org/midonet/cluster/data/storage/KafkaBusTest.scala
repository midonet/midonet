...
class KafkaBusTest extends KafkaTest[Opinion] {
    ...
    feature("PutOpinion/RemoveOpinion") {
        ...
        scenario("Closing the bus and creating a new one") {
            ...
            And("The consumer service should be stopped")
            KafkaBus.isServiceStopped() shouldBe true

            When("We create a new bus and map")
            val (_, map2) = newBusAndMap("Arp2", "owner2")

            And("We create a producer")
            val producer = createProducer(config,
                                          classOf[StringSerializer].getName,
                                          classOf[OpinionEncoder].getName)
            ...
            And("We publish a message on the 3rd map's topic")
            ...
            And("The service should not be stopped")
            KafkaBus.isServiceStopped() shouldBe false
    ...
    //TODO: Provide more thorough testing for the ConsumerService?

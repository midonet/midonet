...
class KafkaBusTest extends KafkaTest[Opinion] {
    ...
    : (KafkaBus[String, String], MergedMap[String, String]) = {
        val storage = KafkaBus.newBus[String, String](mapId, owner,
    ...
    feature("newBus") {
        scenario("newBus returns the same bus given the same id")  {
            val bus1 = KafkaBus.newBus("id", "owner", fillConfig(),
                                       kafkaServer.zkClient,
                                       new MySerialization())
            val bus2 = KafkaBus.newBus("id", "owner", fillConfig(),
                                       kafkaServer.zkClient,
                                       new MySerialization())

            bus1 eq bus2 shouldBe true
        }

        scenario("newBus returns different buses given different ids") {
            val bus1 = KafkaBus.newBus("id1", "owner", fillConfig(),
                                       kafkaServer.zkClient,
                                       new MySerialization())
            val bus2 = KafkaBus.newBus("id2", "owner", fillConfig(),
                                       kafkaServer.zkClient,
                                       new MySerialization())

            bus1 ne bus2 shouldBe true
        }
    }

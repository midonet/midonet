...
class MergedMapBaseTest extends FeatureSpec with BeforeAndAfter
    ...
    private var map: MergedMapBase[String, String] = _
    ...
    private class MyConcreteMergedMap(mapId: MapId, owner: String,
                                      executor: ExecutorService,
                                      inputSubj: PublishSubject[Opinion],
                                      outputSubj: PublishSubject[Opinion])
        extends MergedMapBase[String, String](mapId, owner, executor) {

        override protected def opinionObservable: Observable[Opinion] = inputSubj
        override protected def opinionObserver: Observer[Opinion] = outputSubj
    }
    ...
    before {
        ...
        map = new MyConcreteMergedMap(new MyMapId("myId"), "owner", executor,
                                      inputSubj, outputSubj)
    }

...
class MergedMapTest extends FeatureSpec with BeforeAndAfter
    ...
    feature("putOpinion/removeOpinion") {
        scenario("Invalid opinions are ignored") {
            Given("One observer")
            val obs = createObserverAndSubscribe()
            ...
            putOpinion("dummy2", "dummy2", "owner1")
    ...
    feature("Error handling") {
        scenario("The opinion observable completes") {
            ...
            And("So does the opinion observer")
            opinionObs.awaitCompletion(timeout)
            opinionObs.getOnCompletedEvents should have size 1
        }

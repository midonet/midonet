...
class MergedMapTest extends FeatureSpec with BeforeAndAfter
    ...
    private class MyMergedMapStorage(inputSubj: PublishSubject[Opinion],
                                     outputSubj: PublishSubject[Opinion])
        extends MergedMapStorage[String, String] {
    ...
    before {
        ...
        val myStorage = new MyMergedMapStorage(inputSubj, outputSubj)
        map = new MergedMap[String, String](new MyMapId("myId"), "owner",
                                            executor, myStorage)
    }

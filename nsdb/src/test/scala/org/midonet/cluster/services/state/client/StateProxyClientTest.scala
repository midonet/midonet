...
class StateProxyClientTest extends FeatureSpec
    ...
    feature("observers completed after after retry limit reached") {
        ...
        scenario("reach attempt limit") { for (i <- 1 to 1) {
            ...
            val ratio = (System.currentTimeMillis() - startTime).toDouble /
                        (t.softReconnectDelay.toMillis * t.maxAttempts)

            ratio>=0.8 shouldBe true
    ...
    feature("connection observable behavior") {
        ...
        scenario("keeps connected during transient failures") { for (i <- 1 to 1) {
        ...
        scenario("disconnected after retry limit") { for (i <- 1 to 1) {

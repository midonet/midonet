...
class MidonetDiscoveryTest extends FeatureSpec with Matchers
    ...
    feature("server") {
        scenario("server registration - server and port") {
            ...
            val handler = discovery.registerServiceInstance("testService", "10.0.0.1", 1234)
            Then("It's handler should contain a non-null instance")
            handler.instance shouldNot be (null)
            handler.unregister()
            discovery.stop()
        }
        ...
        scenario("server registration - uri") {
            ...
            val discovery = new MidonetDiscovery(curator, executor, config)
            When("Registering the service")
            val handler = discovery.registerServiceInstance("testService", new URI("udp://10.0.0.1:1234"))
            Then("It's handler should contain a non-null instance")
            handler.instance shouldNot be (null)
            handler.unregister()
            discovery.stop()
        ...
        scenario("server disconnection - server and port") {
            ...
            val handler1 = discovery.registerServiceInstance("testService", address1, 1234)
            val handler2 = discovery.registerServiceInstance("testService", address2, 1234)
            val client = discovery.getClient[MidonetServiceHostAndPort]("testService")
            ...
            if (address == address1) {
                ...
                Then("Is not present on the instances returned by the client")
                eventually {

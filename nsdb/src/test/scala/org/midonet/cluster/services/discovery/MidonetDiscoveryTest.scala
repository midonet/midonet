...
class MidonetDiscoveryTest extends FeatureSpec with Matchers
    ...
    feature("server") {
        scenario("server registration - scala") {
            ...
            val provider = discovery.registerServiceInstance("testService", Option(server1), Option(port), Option(info))
    ...
    feature("URI as payload without server/port") {
        scenario("service discovery") {
            ...
            val provider = discovery.registerServiceInstance("test_service", Option.empty, Option.empty, Option(info))

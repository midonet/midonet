...
class MidonetDiscoveryTest extends FeatureSpec with Matchers
    ...
    feature("server") {
        ...
        scenario("client handle - scala") {
            ...
            client.start()
            eventually {
                client.getServerInfo.isDefined shouldBe true
                client.getServerInfo.get shouldBe info
            }
        ...
        scenario("server disconnection") {
            ...
            if (info == info1) {
                ...
                eventually {
                    client.getServerInfo.isDefined shouldBe true
                    client.getServerInfo.get shouldBe info2
                }
                provider2.stop()
            } else {
                provider2.stop()
                eventually {
                    client.getServerInfo.isDefined shouldBe true
                    client.getServerInfo.get shouldBe info1
                }
                provider1.stop()
            }

            eventually {
                client.getServerInfo.isEmpty shouldBe true
            }
...
object MidonetDiscoveryTest {
     ...
     * paramers (note: setting a default value in the main constructor is
     * not enough).

...
class MidonetDiscoveryTest extends FeatureSpec with Matchers
    ...
    private val config = new MidonetBackendConfig(ConfigFactory.parseString(
        """
          |zookeeper.root_key : "/test_root_key"
        """.stripMargin
    ))

    feature("discovery basics") {
        scenario("instantiation") {
            val discovery = new MidonetDiscovery(curator, executor, config)
    ...
    feature("server") {
        scenario("server registration - scala") {
            ...
            val provider = discovery.registerServiceInstance(
                "testService", Option(server1), Option(port), Option(info))
        ...
        scenario("client handle - scala") {
            ...
            eventually {
                client.randomInstance.isDefined shouldBe true
                val service = client.randomInstance.get
                service.getAddress shouldBe server1
                service.getPort shouldBe port
                service.getPayload shouldBe info
                client.allInstances.toSet should have size 1
            }
        ...
        scenario("server disconnection") {
            ...
            val provider1 = discovery.registerServiceInstance(
                "testService", Option(server1), Option(port), Option(info1))
            val provider2 = discovery.registerServiceInstance(
            ...
            eventually {
                client.allInstances should have length 2
                val server = client.randomInstance
            ...
            if (info == info1) {
                ...
                eventually {
                    client.allInstances should have length 1
                    val server = client.randomInstance
                    server.isDefined shouldBe true
            ...
            eventually {
                client.allInstances should have length 0
                client.randomInstance.isEmpty shouldBe true
            }
    ...
    feature("URI as payload without server/port") {
        scenario("service discovery") {
            ...
            val provider = discovery.registerServiceInstance(
                "test_service", Option.empty, Option.empty, Option(info))

            val client = discovery.getClient("test_service")

            eventually {
                client.randomInstance.isDefined shouldBe true
                client.randomInstance.get.getPayload shouldBe info
            }
        ...
        scenario("service with only one parameter specified") {
            ...
            intercept[InvalidParameterException] {
                val provider = discovery
                    .registerServiceInstance("test_service", Option(server1),

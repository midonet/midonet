...
class PersistentConnectionTest  extends FeatureSpec with Matchers {
    ...
    feature("started behavior") {
        ...
        scenario("reconnection should be disabled") {
            while(server.hasClient())
                Thread.sleep(100)
            val c = new TestConnection(server.port)
            Await.ready(c.start(),1 second).isCompleted shouldBe true
            Await.ready(server.clientConnected(),1 second).isCompleted shouldBe true
            server.close()
            Thread.sleep(2 * reconnectTimeout.toMillis)
            c.stop()
            Thread.sleep(5 * reconnectTimeout.toMillis)
            c.numDisconnects shouldBe 1
            c.numConnects shouldBe 2
        }

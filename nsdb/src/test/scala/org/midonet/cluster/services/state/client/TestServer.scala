...
class TestServer(decoder: MessageLite) extends Connection.Observer {

    import TestServer._

    var attachedObserver: Connection.Observer = null
    ...
    private def unregister(c: Channel): Unit = {
        state.get match {
            ...
            case Inactive => throw new IllegalStateException("unregister while inactive")
            case _ => throw new IllegalStateException("out of place unregister")
        }
    }
    ...
    def clientConnected()(implicit ec: ExecutionContext): Future[Unit] = {
        val promise = Promise[Unit]
        Future {
            //(0 to 50).takeWhile(_ => !hasClient()).foreach(_ => blocking {
            while (!hasClient()) {
            ...
            if (hasClient()) {
                ...
                promise.failure(new Exception("Timeout expired while waiting for connection"))
            }
        }
        promise.future
    }
...
object TestServer {
    ...
    private class ClientHandler(observer: Connection.Observer,

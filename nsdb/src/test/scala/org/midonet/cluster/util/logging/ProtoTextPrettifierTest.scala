...
class ProtoTextPrettifierTest extends FunSuite with ShouldMatchers {
    ...
    test("UUID") {
        val uuid = randomUUID()
        val serialized = pimpUp(UUIDUtil.toProto(uuid))
        uuid.toString shouldBe serialized
    }

    test("IPv4") {
        val ipv4 = IPv4Addr.random
        val serialized = pimpUp(IPAddressUtil.toProto(ipv4))
        ipv4.toString shouldBe serialized
    }

    test("IPv6") {
        val ipv6 = IPv6Addr.random
        val serialized = pimpUp(IPAddressUtil.toProto(ipv6))
    ...
    test("Repeated fields that are messages") {
        ...
        println(pimpUp(r.build()))

        val serialized = pimpUp(r.build())

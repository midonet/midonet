...
public class FlowManager {
    ...
    */
     ...
     * If the wildcard flow has not been added, the method will return an empty set.
     *
     * @param wildFlow
    ...
    public Set<FlowMatch> remove(WildcardFlow wildFlow) {
        ...
        if(wcMap != null)
            ...
            log.debug("WildcardFlow map didn't contain flow {}", wildFlow.getMatch().toString());
         ...
         for (FlowMatch match : flowMatches) {
             ...
             if( kernelFlow != null && kernelFlow.getLastUsedTime() != null){
                 if (System.currentTimeMillis() - kernelFlow.getLastUsedTime()
                     ...
                     flowManagerHelper.removeFlow(kernelFlow);
                     dpFlowToWildFlow.remove(match);
                     wildFlowToDpFlows.get(flowToExpire).remove(match);
                 }
             }
             else{
    ...
    private void manageDPFlowTableSpace() {
        if (isFlowTableFull()) {
            ...
            while (removedFlowsCount < dpFlowRemoveBatchSize && it.hasNext()) {
                flowManagerHelper.removeFlow(new Flow().setMatch(it.next()));
                removedFlowsCount++;
            }
        }
    }
    ...
    public Callback<Flow> getFlowDeleteCallback(final Flow flowToRemove){
        return new FlowManagerCallback(flowToRemove) {

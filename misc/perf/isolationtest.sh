...
verify_isolation() {
    ...
    ip netns exec $VICTIM_DEST_NETNS tcpdump udp -i $VICTIM_DEST_DEV \
        >$tcpdump_out 2>&1 &
    tcpdump_pid=$!
    ...
    ip netns exec $VICTIM_SOURCE_NETNS $LOADGEN 1 $VICTIM_SOURCE_DEV \
        $VICTIM_DEST_MAC $PACKETS_TO_SEND $SEND_RATE
    ...
    if [ $packets -ne $PACKETS_TO_SEND ]; then
        echo "[FAIL] - Only received $packets packets out of $PACKETS_TO_SEND"
    else
        echo '[OK]'
    fi
}

...
We reserve 3 bits for operation field to accomplish possible new operations.
The UUID of downlink port will be provided by "fip64 add" command so all VPPs
 share the same set of UUIDs. The fixed ipv4 address is included to allow
 remote VPP figure out which fip6 is accessed by this ipv6 client.

...
addresses. To allow configuration of such addresses from neutron, translation
of the following primitives was extended to handle IPv6 addresses:
subnet, port, router, router interface, floatingip. The midonet implements
statefull NAT64.
...
Note that in MidoNet a router port could only be assigned at most one IPv4
address. Though, several IPv6 addresses can be assigned. This is a current
limitation of MidoNet.  Also, MidoNet does not store IP addresses on the
 ...
 * This port is on an external network, and the matching MidoNet network must
   ...
   'external_gateway_info'. Port can have at most one IPv4 subnet, and any
 ...
 * If subnet is IPv6:
     ...
     1. Find an available 169.254.x.y/30 subnet within the
        169.254.0.0/16 range. Use the router port IPv4 addres of this
...
before coming back to the network. Notice that VMs with only FIP6 cannot

 ...
 * If BGP is configured on the router, BGP Network is created for the route to
   the subnet attached to the router.
...
and BGP speaker ID.  Only MD5 auth type is suppored.  Set the password to the
provided password valued in the MidoNet BGP peer object.
...
'add_network_id' field contains the ID of the network which you want BGP to
...
'del_network_id' field contains the ID of the network which you want BGP to
stop advertising.  If this field is set, then the corresponding MidoNet BGP
Network is deleted.
...
For each corresponding MidoNet Peer object on the MidoNet BGP Speakers
specified in 'bgp_speaker_ids' field:

 * If 'delete' field is set to True, it is deleted
 * Otherwise, update the 'password' field if changed

...
A gateway device type can be either 'router_vtep' or 'vlan_trunk'.  If it is
 ...
 * type = 'router_vtep' -> VTEP Router GW
 * type = 'vlan_trunk' -> VLAN GW

For both types, only one item is expected in 'devices' field of 'l2_gateway'
object, which is passed in as a sub-resource of 'l2_gateway_connection'.  Raise
an error if that is not the case.

VLAN L2GW:

VLAN L2 gateway connection is implemented using the VAB created when the
gateway device associated with 'l2_gateway' was created.  Since each L2 gateway
...
The translation of 'l2_gateway_connection':
...
('segmentation_id') of 'l2_gateway_connection'.  Make sure to delete the peer
port as well.

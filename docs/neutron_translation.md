 ...
 * qos_policy_id => qosDefaultPolicyId
...
Note: The agent should check this value only if the port object has no
policy ID set.  If this is also not set, no QoS policy is to be used.
...
If a QOS policy ID is specified, set the policy ID on the port object:

 * qos_policy_id => qosPolicyId

Note: The agent should check this value first, and if it is unset, check
the policy ID value on the MidoNet bridge object this port is linked to.
...
If a QOS policy ID is specified, update the port object with the policy ID:
...
This is a list of QOSRuleBWLimit objects defined as:
...
For each QOSRuleBWLimit, the following fields are copied over
directly:
...
This is a list of QOSRuleDSCP objects defined as:
...
For each QOSRuleDSCP, the following fields are copied over
directly:

 * dscp_mark => dscpMark

As each QOS object contains its own set of rules, there is no sharing of
rule objects from one QOS object to another.  They are entirely contained
as children to a parent QOS object.

The basic check for the policy ID in the Agent should be:

 1. Check the port, and if a qosPolicyId is set, use that policy's
 information.
 2. If the port value is null, check the port's parent network (bridge)
 object, and if that qosPolicyId is set, use that policy's information.
 3. If neither the port nor the bridge's qosPolicyId is set, do not use
 any QoS policy for the simulation on the port.
...
Clear the rule list and delete the object.  Any ports or networks which
have a link to the deleted object should have their qosPolicyId field cleared.

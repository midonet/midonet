...
import org.midonet.cluster.{TopologyZoomUpdaterConfig, ClusterConfig}
import org.midonet.cluster.tools.TopologyEntity._
import org.midonet.cluster.tools.TopologyZoomUpdater._
...
class TopologyZoomUpdater @Inject()(val backend: MidonetBackend,
                                    val clusterConf: ClusterConfig)
    extends AbstractService {
    private val cfg: TopologyZoomUpdaterConfig = clusterConf.topologyUpdater
...
class Port(p: Topology.Port)(implicit storage: StorageWithOwnership)
    ...
    def linkTo(p: Port): Port = {
        if (model.hasPeerId) {
            // need to clear the peer id to avoid a loop
            val peerId = model.getPeerId
            clearField("peer_id").update()

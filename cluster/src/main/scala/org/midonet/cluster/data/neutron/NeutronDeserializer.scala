...
/**
 * Converts Neutron JSON to corresponding protobuf classes defined in
 * neutron.proto.
 */
object NeutronDeserializer {
    ...
    private def descriptorFor[M <: Message](clazz: Class[M]): Descriptor = {
        descriptors.getOrElse(clazz, {
            val getter = clazz.getMethod("getDescriptor")
            val descriptor = getter.invoke(null).asInstanceOf[Descriptor]
            descriptors.getOrElseUpdate(clazz, descriptor)
        })
    }

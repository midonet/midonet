...
class ZookeeperObjectMapper(
    ...
    private class ZoomTransactionManager(val version: Long)
            ...
            for (op <- ops) txn = {
                op._2 match {
                    case TxCreate(obj, ownerOps) =>
                        txn.create()
                            .forPath(getPath(op._1.clazz, op._1.id),
                                     serialize(obj)).and()
                    ...
                    case TxDelete(ver, ownerOps) =>
                        txn.delete().withVersion(ver)
                            .forPath(getPath(op._1.clazz, op._1.id)).and()
                    case TxCreateOwner(owner) =>
                        txn.create().withMode(CreateMode.EPHEMERAL)
                            .forPath(getOwnerPath(op._1.clazz, op._1.id, owner))
                            .and()
        ...
        private def rethrowException(ops: Seq[(Key, TxOp)],
            ...
            val op = opForException(ops, e)
            op._2 match {
                case oop: TxOwnerOp =>
                    throw new OwnershipConflictException(
                        op._1.clazz.getSimpleName, op._1.id.toString,
                        Set.empty[String], oop.owner)
                case _ =>
                    throw new ObjectExistsException(op._1.clazz, op._1.id)
            }
        }
    }

...
class ZookeeperObjectMapper(
    ...
    private val curator: CuratorFramework) extends Storage with Ownership {
    ...
    private class TransactionManager(val version: Long) {
        ...
        def delete(clazz: Class[_], id: ObjId, ignoresNeo: Boolean,
            ...
            if (classInfo(clazz).ownershipType.isSupported) {
                ...
                owner match {
                    ...
                    case None =>
                        throw new OwnershipConflictException(
                            clazz.getSimpleName, id.toString, thisOwners.keySet,
                            null, s"Delete not supported because owner is " +
                                  s"not specified")
                    case _ =>
                }
            }
            ...
            if (!classInfo(clazz).ownershipType.isSupported &&
                owner.isDefined) {
                throw new UnsupportedOperationException(
                    "Delete not supported because an owner is not expected")
            }
...
object ZookeeperObjectMapper {
    ...
    private case class TxDeleteOwner(owner: String,
                                     version: Int) extends TxOwnerOp

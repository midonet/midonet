...
class ZookeeperObjectMapper(
    ...
    private class TransactionManager(val version: Long) {
        ...
        def update(obj: Obj, owner: String, rewrite: Boolean,
                   validator: UpdateValidator[Obj]): Unit = {
            update(obj, Some(owner), rewrite, validator)
        }

        private def update(obj: Obj, owner: Option[String], rewrite: Boolean,
                           validator: UpdateValidator[Obj]): Unit = {
            ...
            validateOwner(clazz, thisId, snapshot.owners, owner, rewrite)
        ...
        def updateOwner(clazz: Class[_], id: ObjId, owner: String,
                        rewrite: Boolean): Unit = {
            assert(isRegistered(clazz))
            val snapshot = cacheGet(clazz, id).getOrElse(
                    throw new NotFoundException(clazz, id))

            validateOwner(clazz, id, snapshot.owners, Some(owner), rewrite)
        ...
        private def updateCacheAndOp(clazz: Class[_], id: ObjId,
            ...
            ops.get(key) match {
                ...
                case Some(TxCreate(_, o)) =>
                    ...
                    val ownerOps = o ++ updateOwnerOps(snapshot.owners, owner)
                    ops(key) = TxCreate(obj, ownerOps)
         ...
         * @param rewrite Indicates whether the current owners is rewritten by
         *                the new client session, if the node exists.
         */
        private def validateOwner(clazz: Class[_], id: ObjId,
            ...
            if (owner.isDefined && !rewrite && owners.contains(owner.get)) {
                throw new OwnershipConflictException(
                    clazz.getSimpleName, id.toString, owners, owner.get,
    ...
    @throws[ReferenceConflictException]
    @throws[OwnershipConflictException]
    override def update[T <: Obj](obj: T, owner: ObjId,rewriteOwner: Boolean,
                         validator: UpdateValidator[T]) =
        multi(List(UpdateWithOwnerOp(obj, owner.toString, rewriteOwner,
                                     validator)))

    @throws[NotFoundException]
    @throws[OwnershipConflictException]
    def updateOwner(clazz: Class[_], id: ObjId, owner: ObjId,
                    rewriteOwner: Boolean): Unit =
        multi(List(UpdateOwnerOp(clazz, id, owner.toString, rewriteOwner)))
    ...
    override def multi(ops: Seq[PersistenceOp]): Unit = {
        ...
        ops.foreach {
            ...
            case UpdateWithOwnerOp(obj, owner, rewrite, validator) =>
                manager.update(obj, owner, rewrite, validator)
            case UpdateOwnerOp(clazz, id, owner, rewrite) =>
                manager.updateOwner(clazz, id, owner, rewrite)

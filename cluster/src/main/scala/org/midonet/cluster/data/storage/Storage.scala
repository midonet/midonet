...
trait StorageWithOwnership extends Storage {
     ...
     * object, or if the object has been orphaned and it has no owner. The
     * method rewrites the current ownership node in storage, if the update
     * has been successful.
     */
    @throws[NotFoundException]
    @throws[ReferenceConflictException]
     ...
     * [[update()]], except that only the object ownership is modified and not
     * the object data.
     * @param rewriteOwner The update rewrites the current ownership node in
     *                     storage, if otherwise the conditions for an update
     *                     are met: the update is validated, and the ownership
     *                     allows the update for the object's ownership type.
     *                     This deletes and recreates the ownership node in
     *                     storage, allowing a new client session to reclaim an
     *                     existing ownership.
     */
    @throws[NotFoundException]
    @throws[OwnershipConflictException]

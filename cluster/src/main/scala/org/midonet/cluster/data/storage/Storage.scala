...
trait StorageWithOwnership extends Storage {
     ...
     *                       update are met: the update is validated, and the
     *                       ownership allows the update for the object's
     *                       ownership type. This deletes and recreates the
     ...
     * [[update()]], except that the only the object ownership is modified and
     * not the object data.
     * @param overwriteOwner The update overwrites the current ownership node
     *                       in storage, if otherwise the conditions for an
     *                       update are met: the update is validated, and the

 ...
 */
class RemoteNeutronService(cnxnStr: String, user: String, password: String)
    ...
    private def getTasksSince(lastTaskId: Int,
        ...
        val rslt = queryTasksSince(lastTaskId)
    ...
    private def queryTasksSince(lastTaskId: Int): ResultSet = {
        val con = dataSrc.getConnection
    ...
    override def deleteTask(taskId: Int): Unit = {
        val con = dataSrc.getConnection
        val stmt = con.prepareStatement("delete from task where id = ?")

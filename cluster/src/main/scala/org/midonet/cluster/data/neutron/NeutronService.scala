 ...
 */
protected[cluster] sealed trait Task {
    val taskId: Int
}
protected[cluster] case class Create(taskId: Int,
                                     rsrcType: NeutronResourceType[_ <: Message],
                                     json: String) extends Task
protected[cluster] case class Delete(taskId: Int,
                                     rsrcType: NeutronResourceType[_ <: Message],
                                     objId: UUID) extends Task
protected[cluster] case class Update(taskId: Int,
                                     rsrcType: NeutronResourceType[_ <: Message],
                                     json: String) extends Task
protected[cluster] case class Flush(taskId: Int) extends Task

protected[cluster] class Transaction(val id: String, val tasks: List[Task]) {
 ...
 */
protected[cluster] trait NeutronService {
    /**
 ...
 */
protected[cluster] class RemoteNeutronService(conn: Connection)

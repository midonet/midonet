...
abstract class TransactionManager(classes: ClassesMap, bindings: BindingsMap) {
    ...
    // nodes.
    //
    // For example, CreateNode("/a/b/c", "val1") will set nodeOps("/a/b/c") to
    // TxCreateNode("val1"), and if the operation UpdateNode("/a/b/c", "val2")
    // appears later in the transaction, then TxCreateNode("val1") with be

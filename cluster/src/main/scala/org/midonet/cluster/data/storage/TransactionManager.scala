...
abstract class TransactionManager(classes: ClassesMap, bindings: BindingsMap) {
    ...
    // list of operations in this list, such that there only would be a single
     ...
     * @param rewrite Indicates whether the current owners is rewritten by the
     *                new client session, if the node exists.
     */

 ...
 */
final class VtepMAC private (val mac: MAC) {
    import VtepMAC._
...
object VtepMAC {
    ...
    final val UNKNOWN_DST = new VtepMAC(null)

    def fromMac(m: MAC): VtepMAC =
        if (m == null) throw new IllegalArgumentException("null mac value")
        else new VtepMAC(m)

    def fromString(s: String): VtepMAC =
        if (s == null) throw new IllegalArgumentException("null mac string")
        else if (S_UNKNOWN_DST.equals(s)) UNKNOWN_DST
        else new VtepMAC(MAC.fromString(s))

    def unknownDst(): VtepMAC = UNKNOWN_DST

    def apply(m: MAC): VtepMAC = fromMac(m)
    def apply(s: String): VtepMAC = fromString(s)
}

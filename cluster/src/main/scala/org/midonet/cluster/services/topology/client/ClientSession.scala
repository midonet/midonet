...
class ClientSession(val host: String, val port: Int, val wspath: String,
                    ...
                    val startAt: Long = 0,
                    val senderFactory: ConnectionSenderFactory
                    = ConnectionSender)
    ...
    private val connected = Promise[ConnectionSender]()
    ...
    override protected def onNext(ev: CommEvent): Unit = ev match {
        case Connect(ctx) =>
            connected.success(senderFactory.get(ctx, handshake.future))
        case Disconnect(ctx) =>
        ...
        case Response(ctx, proto) => parse(proto) match {
            case Some((id, true)) =>
                ...
                pending.remove(id).map{_.trySuccess(true)}
                    .foreach(v => {log.debug("command acknowledged: " + id)})
            case Some((id, false)) =>
                ...
                pending.remove(id).map{_.tryFailure(
                    ...
                    .foreach(v => {log.debug("command rejected: " + id)})
            case _ =>
                log.debug("received update: " + proto)

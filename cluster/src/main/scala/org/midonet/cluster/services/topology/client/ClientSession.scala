...
/**
 * A client session for the Topology service. It publishes an observable where
 * the updates and deletions from the Midonet topology are pushed (according
 * to the client requests). If the connection is lost (i.e. the observable is
 * closed) unexpectedly, the client can try to recover the same session in the
 * server (mantaining existing subscriptions) by creating a new ClientSession
 * and specifying the same sessionId as the interrupted one, and the
 * lastEventSeqno seen as a 'startAt' position.
 *
 * The life-cycle uses 'promises' to indicate states; it goes as follows:
 * - Initially: the netty client service is stopped
 * - on observable subscribe: the netty client service is started and a plain
 *                            socket connection will be created, generating
 *                            a Connect(ctx) event
 * - on Connect(ctx) event: wait for low-level protocol (e.g. websocket)
 *                          handshake, then satisfy the 'connected' promise
 * - on 'connected' promise: send the topology API service protobuf handshake,
 *                           and wait for ack.
 * - on protobuf handshake ack: satisfy the 'handshake' promise and emit any
 ...
 *
 * - on network disconnection: satisfy the 'closed' promise.
 * - on 'closed' promise: complete update observable, clean-up pending requests
 * - on update observable completion: stop netty service
    ...
    override protected def onNext(ev: CommEvent): Unit = ev match {
        case Connect(ctx) =>
            channelReady(ctx.channel()).onComplete({
                case Success(ch) =>
                    log.info("underlaying protocol handshake completed")
                    connected.trySuccess(senderFactory.get(ctx, handshake.future))
                case Failure(err) =>
                    log.warn("underlaying protocol handshake failed")

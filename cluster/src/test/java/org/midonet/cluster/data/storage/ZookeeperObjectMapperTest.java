...
import org.midonet.util.reactivex.ClassAwaitableObserver;
...
public class ZookeeperObjectMapperTest {
    ...
    public void testClassSubscriberGetsNewObject() throws Exception {
        PojoChain chain1 = new PojoChain("chain1");
        zom.create(chain1);
        ClassAwaitableObserver<PojoChain> sub = subscribe(PojoChain.class, 1);
        sub.await(ONE_SECOND, 1);
        assertEquals(1, sub.list().size());
        AwaitableObserver<PojoChain> chainObs = sub.list().get(0).get();
    ...
    public void testSecondClassSubscriberGetsCurrentList() throws Exception {
        ...
        ClassAwaitableObserver<PojoChain> sub2 = subscribe(PojoChain.class, 2);
        sub2.await(ONE_SECOND, 0);
        assertEquals(2, sub2.list().size());
    }
    ...
    public void testClassObservableIgnoresDeletedInstances() throws Exception {
        ...
        assertEquals(2, classObs1.list().size());

        AwaitableObserver<PojoChain> obs1 = classObs1.list().get(0).get();
        AwaitableObserver<PojoChain> obs2 = classObs1.list().get(1).get();
        ...
        ClassAwaitableObserver<PojoChain> sub2 = subscribe(PojoChain.class, 1);
        sub2.await(ONE_SECOND, 0);
        assertEquals(1, sub2.list().size());
        AwaitableObserver<PojoChain> chainObs = sub2.list().get(0).get();

...
class ObservablePathChildrenCacheTest extends Suite
     ...
     * state of the node. */
     def testChildObservablesArePrimed() {
        ...
        opcc close()
        collector.children.foreach { _.getOnCompletedEvents should have size 1 }
    }
    ...
    def testCloseCompletesObservable() {
        ...
        val ts = new TestObserver[Observable[ChildData]] {
        ...
        ts.getOnNextEvents.map ( o => {
            ...
            o.subscribe(new TestObserver[ChildData] {
                override def onCompleted(): Unit = {
                    super.onCompleted()
                    latch.countDown()
                }
                override def onError(e: Throwable): Unit = {
                    super.onError(e)
                    fail("Unexpected onError")
                }
                override def onNext(t: ChildData): Unit = {
                    super.onNext(t)
                    fail("Unexpected onNext")
                }
            })
            latch
        }).foreach {

...
class InMemoryStorage(reactor: Reactor) extends Storage {
    ...
    private class ClassNode[T](val clazz: Class[T]) {
        ...
        private val instances = new ConcurrentHashMap[ObjId, InstanceNode[T]]()
        ...
        def exists(id: ObjId): Future[Boolean] =
            Promise().success(instances.containsKey(id)).future

        def delete(id: ObjId): T = instances.remove(id) match {

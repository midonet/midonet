...
import org.midonet.cluster.data.storage.InMemoryStorage.{copyObj, getIdString, Key}
...
class InMemoryStorage(reactor: Reactor) extends Storage {
    ...
    override def multi(ops: Seq[PersistenceOp]): Unit = withWriteLock(lock) {
        ...
        ops.foreach {
            ...
            case DeleteOp(clazz, id, false) => tr.delete(clazz, id)
        }
    }
...
object InMemoryStorage {
    ...
    private def getIdString(clazz: Class[_], id: ObjId): String = {

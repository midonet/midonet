...
import rx.{Scheduler, Observable, Subscription, Observer}

import org.midonet.cluster.data.storage.FieldBinding.DeleteAction
import org.midonet.cluster.data.storage.InMemoryStorage.{copyObj, Key}
import org.midonet.cluster.data.storage.ZookeeperObjectMapper.{deserialize, serialize}
...
class InMemoryStorage(reactor: Reactor) extends Storage {
    ...
    private class ClassNode[T](val clazz: Class[T]) {
        ...
        private val instances = new ConcurrentHashMap[String, InstanceNode[T]]()
        ...
        def create(id: ObjId, obj: Obj): Unit = withWriteLock(lock) {
            ...
            instances.putIfAbsent(id.toString, node) match {
                case null => stream.onNext(node.observable)
                case _ => throw new ObjectExistsException(clazz, id)
            }
        }

        def apply(id: ObjId): Option[T] = instances.get(id.toString) match {
            case node: InstanceNode[T] => Some(node.value)
            case _ => None
        }

        def update(id: ObjId, obj: T): Unit = instances.get(id.toString) match {
            case node: InstanceNode[T] => node.update(obj)
            case _ => throw new NotFoundException(clazz, id)
        }

        def get(id: ObjId): Future[T] = instances.get(id.toString) match {
            case node: InstanceNode[T] => node.get
            case _ => throw new NotFoundException(clazz, id)
        }

        def getAll(ids: Seq[_ <: ObjId]): Seq[Future[T]] =
            ids.map { id => get(id) }

        def getAll: Future[Seq[Future[T]]] =
            Promise().success(instances.values.map { node => node.get}.to[Seq])
                     .future

        def exists(id: ObjId): Future[Boolean] =
            Promise().success(instances.containsKey(id.toString)).future

        def delete(id: ObjId): T = instances.remove(id.toString) match {
            case node: InstanceNode[T] => node.delete()
            case _ => throw new NotFoundException(clazz, id)
        }

        def subscribe(id: ObjId, obs: Observer[_ >: T]): Subscription = {
            instances.get(id.toString) match {
                case node: InstanceNode[T] => node.subscribe(obs)
                case _ => Observable.error(new NotFoundException(clazz, id))
                                    .subscribe(obs)
            }
        }
    ...
    private class MessageIdGetter(clazz: Class[_]) extends IdGetter {
        val idField = ProtoFieldBinding.getMessageField(clazz,
                                                        FieldBinding.ID_FIELD)

        def idOf(obj: Obj) = obj.asInstanceOf[Message].getField(idField)
    }
        ...
        def delete(clazz: Class[_], id: ObjId): Unit = {
            ...
            for (bdg <- bindings.get(clazz).asScala
                 ...
                 if !objsToDelete.contains(
                     Key(bdg.getReferencedClass, thatId.toString))) {

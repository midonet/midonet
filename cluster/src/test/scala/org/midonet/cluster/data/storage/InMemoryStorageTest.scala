...
class InMemoryStorageTest extends FeatureSpec with BeforeAndAfter
    ...
    def testUpdateOwnerExclusiveDifferentOwner(rewriteOwner: Boolean): Unit = {
    ...
    feature("Test ownership") {
        ...
        scenario("Test update owner exclusive same owner no rewrite") {
            val state = new ExclusiveState
            val owner = UUID.randomUUID
            storage.create(state, owner)
            await(storage.exists(classOf[ExclusiveState], state.id)) shouldBe true
        ...
        scenario("Test update owner exlcusive same owner with rewrite") {
            val state = new ExclusiveState
            val owner = UUID.randomUUID
            storage.create(state, owner)
            await(storage.exists(classOf[ExclusiveState], state.id)) shouldBe true
        ...
        scenario("Test update owner exclusive different owner no rewrite") {
            testUpdateOwnerExclusiveDifferentOwner(false)
        }

        scenario("Test update owner exclusive different owner with rewrite") {
        ...
        scenario("Test update shared existing owner with rewrite") {
            val state = new SharedState
            val owner = UUID.randomUUID
            storage.create(state, owner)
            await(storage.getOwners(classOf[SharedState], state.id)) shouldBe Set(
        ...
        scenario("Test update owner shared same owner no rewrite") {
            val state = new SharedState
            val owner = UUID.randomUUID
            storage.create(state, owner)
            await(storage.exists(classOf[SharedState], state.id)) shouldBe true
        ...
        scenario("Test update owner shared same owner with rewrite") {
            val state = new SharedState
            val owner = UUID.randomUUID
            storage.create(state, owner)
            await(storage.exists(classOf[SharedState], state.id)) shouldBe true

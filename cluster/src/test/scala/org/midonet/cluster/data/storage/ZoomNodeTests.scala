...
class ZoomNodeTests extends Suite with CuratorTestFramework with Matchers {
    ...
    def testCreateNode(): Unit = {
        zom.multi(List(CreateNode(Bridge1Path, null)))
    ...
    def testUpdateNodeValues(): Unit = {
        createNodes(Map(Bridge1Path -> null,
                        Bridge1Path + "/key1" -> "val1",
                        Bridge1Path + "/key2" -> null))
        zom.multi(List(UpdateNode(Bridge1Path + "/key1", null),
                       UpdateNode(Bridge1Path + "/key2", "val2")))
    ...
    def testDeleteEntries(): Unit = {
        createNodes(Map(Bridge1Path -> null,
                        Bridge1Path + "/key1" -> "val1",
                        Bridge1Path + "/key2" -> null))
        zom.multi(List(DeleteNode(Bridge1Path + "/key1")))
    ...
    def testRecursiveDelete(): Unit = {
        createNodes(Map(Bridge1Path -> null,
                        Bridge1Path + "/key1" -> "val1",
                        Bridge1Path + "/key2" -> null))
        zom.multi(List(DeleteNode(Bridge1Path)))
    ...
    def testUpdateNonExistingNode(): Unit = {
        ...
        val ex = the [StorageNodeNotFoundException] thrownBy
                 zom.multi(List(UpdateNode(Bridge1Path + "/key2", "val2")))
    ...
    def testDeleteNonExistingNode(): Unit = {
        val ex = the [StorageNodeNotFoundException] thrownBy
                 zom.multi(List(DeleteNode(Bridge1Path + "/key")))
    ...
    def testBridgeMaps(): Unit = {
        ...
        zom.multi(List(DeleteNode(Bridge1Path + "/ARP/1.2.3.5,mac2"),
                       CreateNode(Bridge1Path + "/ARP/1.2.3.5,mac3", null)))
        zom.getNodeChildren(Bridge1Path + "/ARP") should contain only
            ("1.2.3.4,mac1", "1.2.3.5,mac3")

        zom.multi(List(DeleteNode(Bridge1Path + "/mac_ports/vlans/1")))
    ...
    private def createNodes(nodes: Map[String, String]): Unit = {
        zom.multi(nodes.toList.map{case (k, v) => CreateNode(k, v)})
    }

        ...
        tableData.synchronized {
            ...
            tableData.get(t.getName) match {
                case Some(MockData(data, _)) => data.put(e.uuid, e) match {
                    case Some(old) =>
                        update.addRow(toOvsdb(e.uuid), t.generateRow(old, k),
                                      t.generateRow(e, k))
                    case None =>
            ...
            if (update.getRows.size() > 0) {
                tableMonitors.getOrElse(t.getName, List()).foreach(_.update(
                    OvsdbTools.newTableUpdates(t.getName, update),
                    databaseSchema))
            }
    }
        ...
        tableData.synchronized {
            ...
            tableData.get(t.getName) match {
                case Some(MockData(data, _)) => data.remove(id) match {
                    case Some(old) =>
                        update.addRow(toOvsdb(id), t.generateRow(old, k), null)
        ...
        tableData.get(op.getTable) match {
            ...
            case Some(MockData(data, clazz)) => op match {
                ...
                case del: Delete[GenericTableSchema] =>
                    ...
                    data.values
                        .map(e => (e, t.generateRow(e, clazz))).toSeq

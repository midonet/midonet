...
public class OvsdbTools {
    ...
    static public class OvsdbOpException extends OvsdbException {
        ...
        public OvsdbOpException(OvsdbClient client, Integer idx,
                                Operation op, OperationResult r) {
            super(client, (op == null)?
                  String.format("%s: %s", r.getError(), r.getDetails()):
                  String.format("%s on %s: %s: %s", op.getOp(), op.getTable(),
                                r.getError(), r.getDetails())
            );
            this.opIndex = idx;
            this.op = op;
            this.result = r;
            ...
            new FutureCallback<List<OperationResult>>() {
                @Override public void onSuccess(List<OperationResult> rlist) {
                    ...
                    for (OperationResult r: rlist) {
                        if (exc == null &&r.getError() != null &&
                            !r.getError().trim().isEmpty()) {
                            exc = new OvsdbOpException(client, idx,
                                (idx >= ops.size())? null: ops.get(idx).op, r);
                        }
                        idx++;
                    }
                    if (exc != null)
    ...
    // Methods to circumvent ovsdb typing issues when interacting with Scala
    ...
    static public TableUpdates newTableUpdates(String tableName,
                                               TableUpdate<?> update) {
        Map<String, TableUpdate<?>> map = new HashMap<>();
        map.put(tableName, update);

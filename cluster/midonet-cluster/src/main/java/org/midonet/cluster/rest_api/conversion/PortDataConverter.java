...
public class PortDataConverter {
    public static Port fromData(org.midonet.cluster.data.Port<?, ?> port,
        ...
        if (port instanceof BridgePort) {
            ...
            typedDto.vlanId = typedPort.getVlanId();
            dto = typedDto;
            ...
            typedDto.mgmtIpAddr = toStringOrNull(typedPort.getMgmtIpAddr());
            typedDto.mgmtPort = typedPort.getMgmtPort();
            typedDto.networkId = typedPort.getDeviceId();
            typedDto.vni = typedPort.getVni();
            dto = typedDto;
        } else {

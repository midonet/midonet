...
public class MidonetVtep extends UriResource {
    ...
    public UUID vtepRouter;

    @Size(min = 1)
    //@Pattern(regexp = IPv4.regex, message = MessageProperty.IP_ADDR_INVALID)
    @ZoomField(name = "api_ip", converter = IPAddressUtil.Converter.class)
    public List<String> networkApiEndpoint;

    @Min(1)
    @Max(65535)
    @ZoomField(name = "api_port")
    public int apiPort;
    ...
    @Override
    public URI getUri() {
        return absoluteUri(Application.MIDONET_VTEPS, id);
    }

...
public class VtepBroker implements VxLanPeer {
    ...
    public UUID ensureLogicalSwitchExists(String lsName, int vni)
        ...
        assert(lsName != null);
        return vtepDataClient.ensureLogicalSwitch(lsName, vni).get().uuid();
    }
    ...
    public void pruneUnwantedLogicalSwitches(
        ...
        for (LogicalSwitch ls : setAsJavaSet(vtepDataClient.listLogicalSwitches())) {
            ...
            Try<?> st = vtepDataClient.removeLogicalSwitch(ls.name());
            if (st.isSuccess()) {
    ...
    public void renewBindings(UUID ls, Collection<VtepBinding> bindings)
        throws VxLanPeerConsolidationException, VtepNotConnectedException {
        vtepDataClient.ensureBindings(
            LogicalSwitch.networkIdToLogicalSwitchName(ls),
            collectionAsScalaIterable(bindings)).get();
    }
}

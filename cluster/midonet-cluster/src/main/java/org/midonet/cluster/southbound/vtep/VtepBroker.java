...
import static org.midonet.cluster.southbound.vtep.model.VtepModelTranslator.toMido;
...
public class VtepBroker implements VxLanPeer {
    ...
    public void advertiseMacs() throws VtepNotConnectedException {
        ...
        for (UcastMac ucastMac : macs) {
            LogicalSwitch ls =
                vtepDataClient.getLogicalSwitch(ucastMac.logicalSwitchId());
            if (ls == null) {
                log.warn("Unknown logical switch {}", ucastMac.logicalSwitchId());
                continue;
            }
            try {
                VtepMAC mac = ucastMac.mac();
                IPv4Addr ip = ucastMac.ip();
    ...
    private void applyUcastAddition(MacLocation ml) {
        ...
        for (UcastMac uc : ucasts) {
            if (ml.mac().equals(uc.mac())) { // NPE safe
                String mlIp = ml.ipAddr() == null ? "" : ml.ipAddr().toString();
                String ucIp = Strings.nullToEmpty(uc.ipString());
    ...
    public static class RowParser {
        ...
        public static UUID logicalSwitch(Ucast_Macs_Local row) {
            ...
            OvsDBSet<org.opendaylight.ovsdb.lib.notation.UUID> ls =
                row.getLogical_switch();
            return (ls == null || ls.isEmpty()) ? null : toMido(
                ls.iterator().next());
        }

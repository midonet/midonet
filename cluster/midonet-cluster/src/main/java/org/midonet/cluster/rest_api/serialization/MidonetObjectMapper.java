...
public class MidonetObjectMapper extends ObjectMapper {
    ...
    static JsonMappingException wrapException(JsonMappingException e,
                                              DeserializationConfig config,
                                              JavaType valueType) {
        if (e.getPath().size() == 0) return e;
        JsonMappingException.Reference ref = e.getPath().get(0);
        if (null == ref || null == ref.getFrom()) return e;

        JsonError error = getError(config, valueType, ref.getFieldName());
        if (null == error) return e;

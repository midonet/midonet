...
import org.midonet.cluster.data.{ZoomClass, ZoomConvert, ZoomField}
import org.midonet.cluster.models.{Commons, Topology}
import org.midonet.cluster.services.rest_api.annotation._
import org.midonet.cluster.services.rest_api.models.{Chain, DhcpHost, DhcpSubnet, Rule, _}
...
object MidonetTypes {
    ...
    case class RequestPath(items: Seq[RequestItem]) {
        ...
        def isSubresourceMergeable(index: Int): Boolean = {
            if (index == 0) return false
            items(index - 1).attr.subresources(items(index).name)
                .subsresource.merge()
        }
    }
    ...
    final class RichMessage(val msg: Message) extends AnyVal {
        ...
        : Message = {
            ...
            if (ids.contains(protoId)) {
                ids = ids - protoId
                msg.toBuilder.setField(fieldDescriptor, ids.asJava).build()
            } else null
        }

...
class AbstractPortResource[P >: Null <: Port] (resContext: ResourceContext)
    ...
    extends WithChainsResource[P](resContext)(tag) {
    ...
    protected final override def idOf(port: P): UUID =
        port.id
    protected final override def inboundFilterOf(port: P): UUID =
        port.inboundFilterId
    protected final override def outboundFilterOf(port: P): UUID =
        port.outboundFilterId
...
class PortResource @Inject()(resContext: ResourceContext)
    ...
    protected override def deleteFilter(portId: String): Ops = {
        getResource(classOf[Port], portId) flatMap { port =>
            updateChainsOnDelete(port)
        }
    }

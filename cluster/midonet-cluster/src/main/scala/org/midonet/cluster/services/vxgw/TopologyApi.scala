...
trait TopologyApi {
    ...
    /** Deletes the association between the given MAC and Port */
    def unlearn(networkId: UUID, mac: MAC, portId: UUID): Unit
...
class Topology(@Nonnull dataClient: DataClient) extends TopologyApi {
    ...
    override def unlearn(networkId: UUID, mac: MAC, portId: UUID): Unit =

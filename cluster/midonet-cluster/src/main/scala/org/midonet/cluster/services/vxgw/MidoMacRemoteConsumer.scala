...
import org.midonet.cluster.services.vxgw.VtepSynchronizer.NetworkCard
...
/** This class is used by the VtepSynchronizer when updates are detected
  ...
  */
class MidoMacRemoteConsumer(nwStates: util.Map[UUID, NetworkCard])
    ...
    private def macRemoval(nwId: UUID, nwState: NetworkCard,
                           ml: MacLocation): Unit = {
        nwState.macTable.removeIfOwnerAndValue(ml.mac.IEEE802, nwState.vxPort)
        removeIpsOnMac(nwState, ml.mac.IEEE802)
    }

    private def macUpdate(nwId: UUID, nwState: NetworkCard, ml: MacLocation)
    : Unit = {
    ...
    private def removeIpsOnMac(nwState: NetworkCard, mac: MAC): Unit = {

...
class LoadBalancerPoolMemberTranslator(protected val storage: ReadOnlyStorage)
    ...
    : MidoOpList = {
        // Load Balancer Pool's status is set to ACTIVE by default, and it is
        // to be updated only by Health Monitor. Therefore when we update Pool,
        // we need to look up the current status and explicitly set it.

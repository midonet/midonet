...
object VtepStorage {
    ...
    implicit def asVtepStorage(storage: StateStorage): VtepStorage = {
        new VtepStorage(storage)
    }

    /** Serializes the VTEP information for storing the in the state storage. */
    @inline private def serializeInfo(info: Information): String = {
        info.toString
    }

    /** Deserializes the VTEP information from the format used in the state
      * storage. */
    @inline private def deserializeInfo(string: String): Information = {
        val builder = Information.newBuilder()
 ...
 */
class VtepStorage(val store: StateStorage) extends AnyVal {
    ...
    def getVtepInfo(vtepId: UUID): Observable[Information] = {
        store.getKey(classOf[Vtep], vtepId, VtepInfo) map makeFunc1 {
            case SingleValueKey(_, Some(info), _) => deserializeInfo(info)
            case SingleValueKey(_, None, _) => Information.getDefaultInstance
    ...
    def getVtepConnectionState(vtepId: UUID): Observable[ConnectionState] = {
        store.getKey(classOf[Vtep], vtepId, VtepConnectionState) map makeFunc1 {
            case SingleValueKey(_, Some(state), _) =>
                ConnectionState.valueOf(state)
            case SingleValueKey(_, None, _) => ConnectionState.DISCONNECTED
            case _ => ConnectionState.ERROR
        }
    }
    ...
    def setVtepInfo(vtepId: UUID, info: Information): Observable[StateResult] = {
    ...
    /** Sets the VTEP connection state. The method returns an observable, which
      * when subscribed to writes the VTEP information to the state store and
      * emits a single notification with the result of the operation. */
    def setVtepConnectionState(vtepId: UUID, state: ConnectionState)
    : Observable[StateResult] = {
        store.addValue(classOf[Vtep], vtepId, VtepConnectionState,
                       state.toString)
    }

    /** An observable that emits notifications when the information of the
      * specified VTEP changes. */
    def vtepInfoObservable(vtepId: UUID): Observable[Information] = {
    ...
    /** An observable that emits notifications when the connection state of the
      * specified VTEP changes. */
    def vtepConnectionStateObservable(vtepId: UUID): Observable[ConnectionState] = {
        store.keyObservable(classOf[Vtep], vtepId, VtepConnectionState) map makeFunc1 {
            case SingleValueKey(_, Some(state), _) =>
                ConnectionState.valueOf(state)
            case SingleValueKey(_, None, _) => ConnectionState.DISCONNECTED
            case _ => ConnectionState.ERROR

...
import org.midonet.cluster.data.storage.{NotFoundException, PersistenceOp}
import org.midonet.cluster.data.{ZoomClass, ZoomObject}
import org.midonet.cluster.models.Commons
import org.midonet.cluster.rest_api.NotFoundHttpException
import org.midonet.cluster.services.MidonetBackend
import org.midonet.cluster.services.c3po.{C3POMinion, C3POStorageManager, neutron}
...
class NeutronZoomPlugin @Inject()(backend: MidonetBackend,
    ...
    private val c3po = new C3POStorageManager(backend.store)

    C3POMinion.supportedTranslators(backend.store,
                                 new PathBuilder(cfg.rootKey)) foreach { pair =>
        c3po.registerTranslator(pair._1, pair._2)
    }
    c3po.init()
    c3po

    def create[T >: Null <: ZoomObject](dto: T)(implicit ct: ClassTag[T]): T = {
        log.info(s"A create: $dto " + backend.store)
    ...
    def get[T >: Null <: ZoomObject](id: UUID)(implicit ct: ClassTag[T]): T = {
        ...
        try {
            val proto = backend.store.get(protoClassOf(dtoClass), id).await(timeout)
    ...
    def list[T >: Null <: ZoomObject](ids: util.List[UUID])
        ...
        Future.sequence (
            ids map {
                backend.store.get(protoClass, _).map(fromProto(_, dtoClass))
            }
    ...
    def listAll[T >: Null <: ZoomObject](dtoClass: Class[T]): util.List[T] = {
        ...
        log.info(s"A list of all $dtoClass for proto: $protoClass " +
                 backend.store)
        backend.store.getAll(protoClass).await(timeout)
                     .map { fromProto(_, dtoClass.asInstanceOf[Class[T]]) }
                     .toList
    }
    ...
    /* TODO: this is not in ZOOM and will require custom impl */

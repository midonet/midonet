...
import org.midonet.cluster.models.Neutron.{NeutronSubnet, NeutronNetwork, NeutronPort, NeutronRouterInterface}
...
import org.midonet.cluster.services.c3po.translators.NetworkTranslator.isUplinkNetwork
...
class RouterInterfaceTranslator(val storage: ReadOnlyStorage)
    ...
    : MidoOpList = {
        ...
        val isEdgeRouter = isUplinkNetwork(nNetwork)
        ...
        if (isEdgeRouter) {
            ...
            midoOps ++= createMetadataServiceRoutes(
                routerPortId, nNetwork.getId, portSubnet)
        }

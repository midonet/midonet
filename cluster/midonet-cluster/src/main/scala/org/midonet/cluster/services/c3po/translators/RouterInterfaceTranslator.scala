...
import org.midonet.cluster.models.Neutron.{NeutronNetwork, NeutronPort, NeutronRouterInterface, NeutronSubnet}
...
class RouterInterfaceTranslator(val storage: ReadOnlyStorage)
    ...
    : MidoOpList = {
        ...
        // If the port is on an uplink network, then there will be no
        // corresponding Midonet port, because uplink networks don't exist in
        ...
        // router with ID nPort.getRouterId. If nNetwork is a uplink network,

...
import com.google.inject.{AbstractModule, Guice, Singleton, _}
...
object ClusterNode extends App {
    ...
    private var bootstrap: ClusterBootstrap = _

    ClusterBootstrap.fromCommandLine(args) match {
        case Success(b) =>
            b.startAndWait(nodeContext)
            bootstrap = b
        case Failure(t) =>
            log.error("MidoNet Cluster failed to bootstrap", t)
            System.exit(-1)
    }

    val clusterConf = new ClusterConfig(bootstrap.runtimeConfig)

...
class FirewallTranslator(protected val storage: ReadOnlyStorage)
    ...
    private def preRoutingRules(fw: NeutronFirewall): List[Rule] = {
        ...
        if (!fw.getAdminStateUp) {
            rules += returnRuleWithId(inChainFwAdminStateRuleId(chainId),
                                      chainId)
        }

        // Match on return
        rules += returnFlowRuleWithId(inChainFwReturnRuleId(chainId), chainId)
        ...
        rules += dropRuleWithId(inChainFwDropRuleId(chainId), chainId)
        rules.toList
    }
    ...
    private def postRoutingRules(fw: NeutronFirewall): List[Rule] = {
        ...
        // If admin state is down, add a RETURN rule
        if (!fw.getAdminStateUp) {
            rules += returnRuleWithId(outChainFwAdminStateRuleId(chainId),
                                      chainId)
        }

        // Just match forward flow
        rules += forwardFlowRuleWithId(outChainFwForwardRuleId(chainId),
                                       chainId)
        rules.toList
    }
...
object FirewallTranslator {
    ...
    def inChainFwAdminStateRuleId(chainId: UUID): UUID =
        chainId.xorWith(0x46c4206a41b011e5L, 0x878b0242ac110002L)

    def outChainFwAdminStateRuleId(chainId: UUID): UUID =
    ...
    def inChainFwDropRuleId(chainId: UUID): UUID =
        chainId.xorWith(0xae3a0312402611e5L, 0xab660242ac110002L)

    def inChainFwReturnRuleId(chainId: UUID): UUID =
        chainId.xorWith(0xdac28d1e402611e5L, 0xa59c0242ac110002L)

    def outChainFwForwardRuleId(chainId: UUID): UUID =

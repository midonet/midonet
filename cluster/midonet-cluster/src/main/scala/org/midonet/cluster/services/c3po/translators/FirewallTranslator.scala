...
class FirewallTranslator(protected val storage: ReadOnlyStorage)
    ...
    private def translateIngressRules(fw: NeutronFirewall): List[Rule] = {
        ...
        // Copy over the rules
        rules ++= fw.getFirewallRuleListList.asScala.map(
            r => toPreRoutingRule(r, chainId))
    ...
    override protected def translateCreate(fw: NeutronFirewall): MidoOpList = {
        ...
        // Pre-routing chain
        ops ++= translateIngressRules(fw).map(Create(_))
    ...
    override protected def translateUpdate(fw: NeutronFirewall): MidoOpList = {
        ...
        translateIngressRules(fw).foreach(

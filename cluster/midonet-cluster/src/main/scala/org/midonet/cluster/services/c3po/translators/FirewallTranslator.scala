...
class FirewallTranslator(protected val storage: ReadOnlyStorage)
    ...
    private def ingressRules(fw: NeutronFirewall): List[Rule] = {
        ...
        if (!fw.getAdminStateUp) {
            rules += returnRule(inChainFwAdminStateRule(chainId), chainId)
        }
    ...
    private def egressRules(fw: NeutronFirewall): List[Rule] = {
        ...
        // If admin state is down, add a RETURN rule
        if (!fw.getAdminStateUp) {
            rules += returnRule(outChainFwAdminStateRule(chainId), chainId)
        }
    ...
    private def translateRules(fw: NeutronFirewall,
                               chainId: UUID => UUID,
    ...
    private def translateRouterAssocs(fw: NeutronFirewall): MidoOpList = {
        ...
        // Neutron guarantees that add and del routers are mutually exclusive.
        ...
        fw.getDelRouterIdsList.asScala foreach { rId =>
            ...
            val inRuleId = inChainFwJumpRule(rId)
            if (storage.exists(classOf[Rule], inRuleId).await()) {
                ops += Delete(classOf[Rule], inRuleId)
            }

            val outRuleId = outChainFwJumpRule(rId)
            if (storage.exists(classOf[Rule], outRuleId).await()) {
                ops += Delete(classOf[Rule], outRuleId)
            }
        }
        ...
        fw.getAddRouterIdsList.asScala foreach { rId =>
            ...
            val inRuleId = inChainFwJumpRule(rId)
            if (!storage.exists(classOf[Rule], inRuleId).await()) {
                ops += Create(jumpRule(inRuleId, inChainId(rId),
                                       inChainId(fw.getId)))
            }

            val outRuleId = outChainFwJumpRule(rId)
            if (!storage.exists(classOf[Rule], outRuleId).await()) {
                ops += Create(jumpRule(outRuleId, outChainId(rId),
                                       outChainId(fw.getId)))
            }
        }
    ...
    override protected def translateUpdate(fw: NeutronFirewall) =
        translateRules(fw, inChainId, ingressRules) ++
        translateRules(fw, outChainId, egressRules) ++
        translateRouterAssocs(fw)
}
...
object FirewallTranslator {
    ...
    def inChainFwAdminStateRule(chainId: UUID): UUID =
        chainId.xorWith(0x46c4206a41b011e5L, 0x878b0242ac110002L)

    def outChainFwAdminStateRule(chainId: UUID): UUID =

...
class FirewallTranslator(protected val storage: ReadOnlyStorage)
    ...
    def translateIngressRules(nRules: Seq[NeutronFirewallRule],
                              chainId: UUID): List[Rule] = {
        ...
        // Copy over the rules
        rules ++ nRules.map(r => toPreRoutingRule(r, chainId))
    ...
    override protected def translateCreate(fw: NeutronFirewall): MidoOpList = {
        ...
        // Pre-routing chain
        ops ++ translateIngressRules(fw.getFirewallRuleListList.asScala,
                                     inFwChain.getId).map(Create(_))

        // Post routing chain
        ops += Create(forwardFlowRule(outChainFwForwardRule(outFwChain.getId),
                                      outFwChain.getId))
    ...
    override protected def translateUpdate(fw: NeutronFirewall): MidoOpList = {
        ...
        val chainBuilder = inFwChain.toBuilder.clearRuleIds()
        translateIngressRules(
            fw.getFirewallRuleListList.asScala, inFwChain.getId).foreach(
                r => chainBuilder.addRuleIds(r.getId))
        ops += Update(chainBuilder.build())

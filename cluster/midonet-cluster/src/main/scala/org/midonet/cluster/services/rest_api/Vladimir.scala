...
class Vladimir @Inject()(nodeContext: ClusterNode.Context,
    ...
    override def doStart(): Unit = {
        log.info(s"Starting REST API service at ports: " +
                 s"PLAIN ${config.restApi.httpPort} SSL: " +
    ...
    private def httpConfiguration(): HttpConfiguration = {
    ...
    /**
     * Builds and configure the HTTPS connector, based on the keystore path
    ...
    private def httpsConnector(): ServerConnector = {
        ...
        val sslConnFactory = new SslConnectionFactory(sslContextFactory,
                                                      HTTP_1_1.asString())
        val httpsConfig = httpConfiguration()
        httpsConfig.addCustomizer(new SecureRequestCustomizer())
        val httpConnFactory = new HttpConnectionFactory(httpsConfig)
        val https = new ServerConnector(server, sslConnFactory, httpConnFactory)

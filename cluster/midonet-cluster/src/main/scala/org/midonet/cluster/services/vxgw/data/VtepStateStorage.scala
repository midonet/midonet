...
class VtepStateStorage(val store: StateStorage) extends AnyVal {
    ...
    def setVtepName(vtepId: UUID, name: String): Observable[StateResult] = {
        getVtepConfig(vtepId) flatMap makeFunc1 { config =>
            setVtepConfig(vtepId, config.toBuilder.setName(name).build())
        }
    }
    ...
    : Observable[StateResult] = {
        getVtepConfig(vtepId) flatMap makeFunc1 { config =>
            setVtepConfig(vtepId,
                          config.toBuilder.setDescription(description).build())
        }
    }

...
class VtepSynchronizer(vtepId: UUID,
        ...
        ovsdb.connect().flatMap {
            ...
            case Success(obs) =>
                ...
                macRemoteConsumer = new VtepMacRemoteConsumer(nsdbVtep,
                                  boundNetworks, store, vtepMacRemoteConsumer)
                whenOvsdbCnxnStatusChanges.onNext(Connected)
                watchVtepLocalMacs()
    ...
    private def removeLogicalSwitchWithRetry(id: UUID): Unit = {
        ...
        ovsdb.logicalSwitch(lsName) map {
    ...
    private def watchNetwork(nwId: UUID, retries: Int = 10): Unit = {
        val nwCard = new NetworkCard
        boundNetworks.getOrElseUpdate(nwId, { nwCard })
        loadVxlanPortFor(nwId, nwCard)
            .flatMap { _ => ensureLogicalSwitch(nwId) }
            .flatMap { lsId =>
                log.debug(s"Network $nwId mapped to Logical Switch $lsId")
                ensureBindings(lsId, nwId, nsdbVtep.getBindingsList)
            }
            .map { _ => watchMacPortMap(nwId, macRemoteConsumer) }
            .map { _ => watchArpTable(nwId, macRemoteConsumer) } onComplete {
                case Success(_) =>
    ...
    /** The NetworkCard requires having the VxLAN port that links to the VTEP
      * managed in this synchronizer.  This method finds this port, and
      * updates the network card with it.  Note that this is a one-off in the
      * lifetime of the VxGW with this network, as the VxLAN port never changes.
      */
    private def loadVxlanPortFor(nwId: UUID, nwCard: NetworkCard)
    : Future[Unit] = {
        store.get(classOf[Network], nwId).flatMap { nw =>
            val pIds = nw.getVxlanPortIdsList.map { fromProto }
                ...
                ports.find(p => fromProto(p.getVtepId) == vtepId) match {
                    case Some(p) =>
                        nwCard.vxPort = fromProto(p.getId)
    ...
    private def ensureLogicalSwitch(nwId: UUID): Future[UUID] = {
        ...
        store.get(classOf[Network], nwId).flatMap { network =>
            ovsdb.createLogicalSwitch(bridgeIdToLogicalSwitchName
                                                   (nwId), network.getVni)
        }
    }
        ...
        if (!boundNetworks.containsKey(nwId)) {
            ...
            Future.sequence(
                bindings.groupBy(_.getNetworkId).map { nwAndBdgs =>
                    ...
                    val portVlanPairs = bdgs.map { proto =>

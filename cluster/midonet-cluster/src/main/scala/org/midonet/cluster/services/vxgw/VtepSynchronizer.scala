...
class VtepSynchronizer(nodeId: UUID,
    ...
    private var vtepUpdater: VtepUpdater = null
    ...
    // Initialized when connected to the VTEP
    private var vtepMacRemoteUpdater: Observer[MacLocation] = _
    ...
    override def onNext(vtep: NsdbVtep): Unit = executor submit makeRunnable {
        ...
        if (oldState == null) {
            ...
            ovsdb.connect().flatMap { case _ =>
                ovsdb.macRemoteUpdater
            } onComplete {
                case Success(obs) =>
                    vtepMacRemoteUpdater = obs
                    subscription.add(
                        ovsdb.macLocalUpdates
                             .observeOn(scheduler)
                             .subscribe(new MidoNetUpdater(nwStates))
                    )
                    vtepUpdater = new VtepUpdater(nsdbVtep, nwStates, fpHerald,
                                                  backend.store,
                                                  vtepMacRemoteUpdater)
    ...
    /** Ensure that the logical switches fo the given networks exist in this
      * VTEP.
      */
    def ensureLogicalSwitch(nwId: UUID): Future[LogicalSwitch] = {
        if (!nwStates.contains(nwId)) {
            return Future.failed(new IllegalStateException())
        }

        backend.store.get(classOf[Network], nwId).flatMap { network =>

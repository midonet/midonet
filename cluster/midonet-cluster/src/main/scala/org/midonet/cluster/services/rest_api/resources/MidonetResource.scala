...
import org.midonet.cluster.services.rest_api.models
import org.midonet.cluster.services.rest_api.models.{ResourceUris, UriResource}
...
class MidonetResource @Inject()(implicit backend: MidonetBackend) {
    ...
    : Message = {
        ...
        if (path.isSubresourceReferenced(path.length - 1)) {
            if (path.isSubresourceMergeable(path.length - 1)) {
                // The subresource is mergeable with an existing resource:
                // merge with update the existing resource.
                var current = storeGet(obj.getClass, obj.getId)
                current = current.mergeWith(obj)
                backend.store.update(current)
                null
            } else {
    ...
    private def updateResource(data: String, args: String*): Response = {
        ...
        val current = storeGet(path.last)
        var obj: Message = ZoomConvert.toProto(dto, path.last.attr.zoom.clazz)
        obj = current.toBuilder.mergeFrom(obj).build()
    ...
    private def deleteSubresource(path: RequestPath): Message = {
        ...
        if (path.isSubresourceReferenced(path.length - 1)) {
            if (path.isSubresourceMergeable(path.length - 1)) {
                // The subresource is mergeable with an existing resource:
                // diff with and update the existing resource.

                // TODO : Find a better way
                var current = storeGet(path.last.attr.zoom.clazz,
                                       path.last.id.protoId)
                current = current match {
                    case port: Topology.Port =>
                        port.toBuilder.clearHostId().clearInterfaceName().build()
                }
                backend.store.update(current)
                // TODO
    ...
    @inline
    @throws[NotFoundException]
    private def storeGet(clazz: Class[_ <: Message], id: Any)
    : Message = {

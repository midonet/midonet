...
class FloatingIpTranslator(protected val readOnlyStorage: ReadOnlyStorage,
    ...
    private def addNatRules(fip: FloatingIp, gwPortId: UUID): MidoOpList = {
        ...
        // When an update changes the FIP's association is changed from one port
        // to another behind the same router, the DNAT and SNAT rules' IDs will
        // already be in the chains' rule ID lists, because the changes that
        // disassociated the FIP from the old port are part of the same Neutron
        // operation (see removeNatRules()) and haven't been persisted yet. We
        // need to avoid duplicate entries in the chains' rule ID lists.
        //
    ...
    private def getRouterGwPortId(routerId: UUID)
    : UUID = {

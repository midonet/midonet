...
class MidoNetUpdater(nwStates: util.Map[UUID, NetworkState])
    ...
    override def onNext(ml: MacLocation): Unit = {
        ...
        nwState.macTable.get(ml.mac.IEEE802) match {
            case portId if portId != null &&
                           ml.vxlanTunnelEndpoint == null =>
                macRemoval(nwId, nwState, ml)
            case portId if ml.vxlanTunnelEndpoint != null =>
                macUpdate(nwId, nwState, ml, portId)
        }
    }

    private def macRemoval(nwId: UUID, nwState: NetworkState,
                           ml: MacLocation): Unit = {
        nwState.macTable.removeIfOwner(ml.mac.IEEE802)
        if (ml.ipAddr != null) {
            nwState.arpTable.removeIfOwner(ml.ipAddr)
        }
    }

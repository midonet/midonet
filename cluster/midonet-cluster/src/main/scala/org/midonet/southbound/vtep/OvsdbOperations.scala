...
import org.opendaylight.ovsdb.lib.notation.{Row, Condition}
import org.opendaylight.ovsdb.lib.{MonitorCallBack, OvsdbClient}
import org.opendaylight.ovsdb.lib.message._
import org.opendaylight.ovsdb.lib.operations.{Select, OperationResult}
import org.opendaylight.ovsdb.lib.schema.{ColumnSchema, GenericTableSchema, DatabaseSchema}
...
object OvsdbOperations {
     ...
     */
    def getDbSchema(client: OvsdbClient, dbName: String, executor: Executor,
    ...
    def singleOp(client: OvsdbClient, schema: DatabaseSchema,
                 op: Table.OvsdbOperation, executor: Executor,
                 closeFuture: ChannelFuture = VoidCloseFuture,
                 closeException: Exception = null)
    : Future[OperationResult] = {
        implicit val ec = ExecutionContext.fromExecutor(executor)

        multiOp(client, schema, Seq(op), executor, closeFuture,
                closeException) map { results => results.head }
    }
    ...
    def multiOp(client: OvsdbClient, schema: DatabaseSchema,
                ops: Seq[Table.OvsdbOperation], executor: Executor,
                closeFuture: ChannelFuture = VoidCloseFuture,
                closeException: Exception = null)
    ...
    def tableEntries(client: OvsdbClient,
                     ...
                     condition: Condition,
                     executor: Executor): Future[Seq[Row[GenericTableSchema]]] = {
        ...
        singleOp(client, schema, new OvsdbSelect(op), executor) map { result =>

...
import org.midonet.southbound.vtep.schema.Table.{OvsdbUpdate, OvsdbOperation}
...
class OvsdbVtepData(val client: OvsdbClient, val dbSchema: DatabaseSchema,
    ...
    override def currentMacLocal: Future[Seq[MacLocation]] = {
        vxlanTunnelIp map { someTunnelIp =>
            uLocalTable.getAll.toSeq.flatMap {
                m => macUpdateToMacLocations(VtepEntryUpdate.addition(m),
                                             someTunnelIp)
            } ++
            mLocalTable.getAll.toSeq.flatMap {
                m => macUpdateToMacLocations(VtepEntryUpdate.addition(m),
                                             someTunnelIp)
            }
        }
        ...
        private def applyRemoteMacAddition(macLocation: MacLocation): Unit = {
            macLocationToMacEntries(macLocation) onComplete {
                case Success(entries) =>
                    ...
                    entries foreach {
                        case u: UcastMac => ops += uRemoteTable.table.delete(u.id)
                        case m: McastMac => ops += mRemoteTable.table.delete(m.id)
                    }

...
import org.midonet.cluster.models.Commons.{IPSubnet, IPVersion, Int32Range, Protocol}
...
class FirewallTranslatorTest extends TranslatorTestBase  {
    ...
    private def neutronFwRule(action: FirewallRuleAction =
    ...
    private def assertRuleMatches(rule: Rule, nRule: NeutronFirewallRule,
        ...
        val action = toTopologyRuleAction(nRule.getAction)

        // Otherwise defaults to TCP
        val protocol = if (nRule.hasProtocol)
            nRule.getProtocol.getNumber else 0

        val emptyRange = Int32Range.newBuilder().build()
        val srcPort = if (nRule.hasSourcePort)
            toInt32Range(nRule.getSourcePort) else emptyRange
        val dstPort = if (nRule.hasDestinationPort)
            toInt32Range(nRule.getDestinationPort) else emptyRange
        ...
        rule.getCondition.getTpSrc shouldBe srcPort
        rule.getCondition.getTpDst shouldBe dstPort
    ...
    it should "produce IllegalArgumentException for IPv6" in {
        intercept[IllegalArgumentException] {
            toRule(neutronFwRule(ipVersion = IPVersion.V6),
                   UUIDUtil.toProto(UUID.randomUUID()))                                          }
    }
}

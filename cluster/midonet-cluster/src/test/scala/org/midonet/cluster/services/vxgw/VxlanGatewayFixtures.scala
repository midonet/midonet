...
import org.midonet.cluster.data.vtep.VtepConnection.ConnectionState.Disconnected
...
import org.midonet.midolman.state.{MockDirectory, Ip4ToMacReplicatedMap, MacPortMap}
...
trait VxlanGatewayFixtures extends TopologyBuilder with MockitoSugar
    ...
    class VtepFixture(store: Storage, val tzId: UUID = UUID.randomUUID()) {
        ...
        val ovsdb: VtepDataClient = mock[VtepDataClient]
        ...
        def verifyBindingsWrittenToOvsdb(): Unit = {
            // And the relevant bindings
            vtep.getBindingsList.groupBy { bdg =>
                bdg.getNetworkId
            } foreach { nwAndBdgs =>
                val nwId = UUIDUtil.fromProto(nwAndBdgs._1)
                ...
                verify(ovsdb, Mockito.atLeast(1))
                    .ensureBindings(Eq(lsName), Eq(expectBindings))
            }
        }
    ...
    class VxlanGatewayFixture(store: Storage, vtepFix: VtepFixture) {
        ...
        val nwId = randomUUID()
        store.create(createBridge(nwId))
        ...
        def createVxPortForVtep(vtep: Vtep): Unit = {
            ...
            store.update(nw.toBuilder.addVxlanPortIds(UUIDUtil.toProto
                                                          (vxPortId)).build())
            Mockito.when(vtepFix.ovsdb.ensureLogicalSwitch(lsName, nw.getVni))
                .thenReturn(successful(new LogicalSwitch(UUID.randomUUID(),
                                                         lsName, nw.getVni,
                                                         "some-txt")))
            Mockito.when(vtepFix.ovsdb.ensureBindings(Eq(lsName), anyObject()))

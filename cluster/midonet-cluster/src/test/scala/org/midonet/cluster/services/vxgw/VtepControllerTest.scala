...
class VtepControllerTest extends FlatSpec with Matchers
    ...
    "A VTEP controller" should "manage flooding proxies properly" in {
        ...
        tzStateObserver.awaitOnNext(1, 5 seconds) shouldBe true
        ...
        val (primary, failback) = try {
            tzStateObserver.awaitOnNext(next + 1, 5 seconds)
        ...
        tzStateObserver.awaitOnNext(next + 1, 5 seconds) shouldBe true

...
class LoadBalancerIT extends C3POMinionTestBase {
    ...
    "C3PO" should "be able to set up Load Balancer." in {
        ...
        val danglingMember2 =
            storage.get(classOf[PoolMember], member2Id).await()
        danglingMember2.hasPoolId shouldBe false
        val lbWithNoPool = storage.get(classOf[LoadBalancer], routerId).await()

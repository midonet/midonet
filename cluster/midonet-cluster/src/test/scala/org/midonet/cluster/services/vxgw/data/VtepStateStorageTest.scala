...
class VtepStateStorageTest extends FlatSpec with CuratorTestFramework
    ...
    "Store" should "return default disconnected when VTEP does not exist" in {
        ...
        storage.getVtepConnectionState(vtepId)
               .await(timeout) shouldBe VtepConnectionState.DISCONNECTED
    }
    ...
    "Store" should "fail setting VTEP connection state when VTEP does not exist" in {
        val vtepId = UUID.randomUUID
        intercept[UnmodifiableStateException] {
            storage.setVtepConnectionState(vtepId, VtepConnectionState.CONNECTED)
                   .await(timeout)
        }
    }
    ...
    "Store" should "set and get the VTEP connection state" in {
        ...
        storage.setVtepConnectionState(vtep.getId, VtepConnectionState.CONNECTED)
               .await(timeout)
        storage.getVtepConnectionState(vtep.getId)
               .await(timeout) shouldBe VtepConnectionState.CONNECTED
        storage.setVtepConnectionState(vtep.getId, VtepConnectionState.ERROR)
               .await(timeout)
        storage.getVtepConnectionState(vtep.getId)
               .await(timeout) shouldBe VtepConnectionState.ERROR
    }

    "Store observable" should "emit notifications on information updates" in {
        val vtep = createVtep()
        val obs = new TestObserver[VtepInformation]
                      with AwaitableObserver[VtepInformation]
    ...
    "Store observable" should "emit notifications on connection state update" in {
        ...
        obs.awaitOnNext(1, timeout) shouldBe true
        obs.getOnNextEvents.get(0) shouldBe VtepConnectionState.DISCONNECTED

        storage.setVtepConnectionState(vtep.getId, VtepConnectionState.CONNECTED)
               .await(timeout)
        obs.awaitOnNext(2, timeout) shouldBe true
        obs.getOnNextEvents.get(1) shouldBe VtepConnectionState.CONNECTED


        storage.setVtepConnectionState(vtep.getId, VtepConnectionState.ERROR)
               .await(timeout)
        obs.awaitOnNext(3, timeout) shouldBe true
        obs.getOnNextEvents.get(2) shouldBe VtepConnectionState.ERROR

...
import org.midonet.cluster.services.conf.ZookeeperTestSuite
...
class VladimirTest extends FeatureSpecLike
    ...
    override def beforeAll(): Unit = {
        super.beforeAll()
        val clusterConfig = new ClusterConfig(config)
        val context = ClusterNode.Context(HostIdGenerator.getHostId, embed = true)
        backend = new MidonetBackendService(clusterConfig.backend, zkClient)
        backend.setupBindings()
        confMinion = new Vladimir(context, backend, clusterConfig)

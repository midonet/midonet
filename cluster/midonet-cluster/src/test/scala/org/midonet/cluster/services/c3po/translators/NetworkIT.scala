...
class NetworkIT extends C3POMinionTestBase {
    ...
    override protected def maybeInjectLegacyDataClient() {
        injectLegacyDataClient()
    }
    ...
    "table node children" in {
        ...
        // Test the replicated table nodes, etc.
        eventually(checkReplTables(network1Uuid, shouldExist = true))
        // Add an ARP entry node under the ARP replicated table node by
        // directly calling CreateNodeOp on Storage.

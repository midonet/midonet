...
class FirewallTranslatorIT extends C3POMinionTestBase with ChainManager {
    ...
    private def validateIngressRuleOrder(chain: Chain, ruleIds: List[UUID],
        ...
        if (!adminStateUp) {
            chain.getRuleIds(index) shouldBe
                inChainFwAdminStateRuleId(chain.getId)
            index += 1
        }

        chain.getRuleIds(index) shouldBe inChainFwReturnRuleId(chain.getId)
        index += 1
        ...
        chain.getRuleIds(index) shouldBe inChainFwDropRuleId(chain.getId)
    }
    ...
    private def validateEgressRuleOrder(chain: Chain,
        ...
        if (!adminStateUp) {
            chain.getRuleIds(index) shouldBe
                outChainFwAdminStateRuleId(chain.getId)
            index += 1
        }

        chain.getRuleIds(index) shouldBe outChainFwForwardRuleId(chain.getId)
    }

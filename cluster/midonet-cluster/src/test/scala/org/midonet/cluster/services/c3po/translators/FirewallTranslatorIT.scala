...
class FirewallTranslatorIT extends C3POMinionTestBase with ChainManager {
    ...
    private def validateFirewall(fwId: UUID, inRuleIds: List[UUID],
    ...
    private def validateIngressRuleOrder(chain: Chain, ruleIds: List[UUID],
        ...
        if (!adminStateUp) {
            chain.getRuleIds(index) shouldBe
                FirewallTranslator.inChainAdminStateRule(chain.getId)
            index += 1
        }

        chain.getRuleIds(index) shouldBe
            FirewallTranslator.inChainFwReturnRule(chain.getId)
        index += 1
        ...
        chain.getRuleIds(index) shouldBe
            FirewallTranslator.inChainFwDropRule(chain.getId)
    }
    ...
    private def validateEgressRuleOrder(chain: Chain,
        ...
        if (!adminStateUp) {
            chain.getRuleIds(index) shouldBe
                FirewallTranslator.outChainAdminStateRule(chain.getId)
            index += 1
        }

        chain.getRuleIds(index) shouldBe
            FirewallTranslator.outChainFwForwardRule(chain.getId)
    }
}

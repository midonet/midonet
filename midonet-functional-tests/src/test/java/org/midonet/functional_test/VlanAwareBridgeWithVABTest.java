...
/**
 * This extends the VlanAwareBridgeTest with a topology based on the legacy
 * VAB bridge.
 */
public class VlanAwareBridgeWithVABTest extends VlanAwareBridgeTest {
    ...
    public void setup() {
        ...
        VlanBridge vlanBr = apiClient.addVlanBridge()
                          .tenantId(tenantName)
                          .name("vlanBr1")
                          .create();

        // Create ports linking vlan-bridge with each bridge, and assign vlan id
        VlanBridgeInteriorPort vlanPort1 = vlanBr.addInteriorPort()
                                                 .setVlanId(vlanId1)
                                                 .create();
        vlanPort1.link(br1IntPort.getId());

        VlanBridgeInteriorPort vlanPort2 = vlanBr.addInteriorPort()
                                                 .setVlanId(vlanId2)
                                                 .create();
        vlanPort2.link(br2IntPort.getId());

        // Create the trunk ports
        VlanBridgeTrunkPort trunk1 = vlanBr.addTrunkPort().create();
        VlanBridgeTrunkPort trunk2 = vlanBr.addTrunkPort().create();

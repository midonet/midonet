...
public class TestRule extends JerseyTest {
	...
	public void before() {
		...
		assertTrue(testTenantUri.toString().endsWith(
				"tenants/" + testTenantName));
		...
		UUID testRouterPortId = FuncTest.getUuidFromLocation(response
				.getLocation());
		log.debug("Got portId {}", testRouterPortId);
		...
		response = resource().uri(routerChainUri).type(APPLICATION_CHAIN_JSON)
	...
	public void testCreateGetListDelete() {
		...
		// Create up
		response = resource().uri(rulesUri).type(APPLICATION_RULE_JSON)
		...
		// Get the rule
		response = resource().uri(ruleUri).accept(APPLICATION_RULE_JSON)
		...
		// List rules
		response = resource()
				.uri(URI.create(ruleChainUri.toString() + "/rules"))
		...
		response = resource().uri(newRuleUri).type(APPLICATION_RULE_JSON)

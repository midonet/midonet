...
public class TestVifOpService {

    private VifZkDao zkDaoMock = null;
    private VifOpBuilder opBuilderMock = null;
    private PortOpService portOpServiceMock = null;
    private VifOpService service = null;
    ...
    public void setUp() throws Exception {
        ...
        this.opBuilderMock = mock(VifOpBuilder.class);
        this.portOpServiceMock = mock(PortOpService.class);
        this.service = new VifOpService(this.opBuilderMock,
    ...
    public void testBuildCreatePortSuccess() throws Exception {
        ...
        when(opBuilderMock.getVifCreateOp(id, config)).thenReturn(
                dummyCreateOp0);
        when(portOpServiceMock.buildPlug(config.portId, id)).thenReturn(
                dummyCreateOps);

        List<Op> ops = service.buildCreate(id, config);
    ...
    public void testBuildDeleteSuccess() throws Exception {
        ...
        when(portOpServiceMock.buildPlug(config.portId, null)).thenReturn(
                dummyDeleteOps);
        when(opBuilderMock.getVifDeleteOp(id)).thenReturn(dummyDeleteOp0);

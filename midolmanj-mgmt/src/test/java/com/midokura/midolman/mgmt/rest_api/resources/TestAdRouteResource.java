...
public class TestAdRouteResource {
    ...
    @Mock(answer = Answers.RETURNS_SMART_NULLS)
    private AdRouteDao dao;

    @Before
    public void setUp() throws Exception {
        testObject = new AdRouteResource();
        doReturn(dao).when(factory).getAdRouteDao();
    }
    ...
    public void testDeleteUnauthorized() throws Exception {
        ...
        doReturn(false).when(auth).adRouteAuthorized(context, AuthAction.WRITE,
                id);
    ...
    public void testDeleteNonExistentData() throws Exception {
        // Set up
        UUID id = UUID.randomUUID();
        doReturn(true).when(auth).adRouteAuthorized(context, AuthAction.WRITE,
                id);
    ...
    public void testGetUnauthorized() throws Exception {
        // Set up
        UUID id = UUID.randomUUID();
        doReturn(false).when(auth).adRouteAuthorized(context, AuthAction.READ,
                id);

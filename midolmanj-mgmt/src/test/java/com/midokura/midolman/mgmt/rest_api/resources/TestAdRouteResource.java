/*
 * @(#)TestAdRouteResource        1.6 12/01/16
 *
...
public class TestAdRouteResource {
    ...
    private AdRouteResource resource = null;
    private AdRouteDao daoMock = null;
    ...
    public void setUp() throws Exception {
        ...
        this.daoMock = mock(AdRouteDao.class);
        this.uriInfoMock = mock(UriInfo.class);
        this.resource = new AdRouteResource();

        doReturn(daoMock).when(factoryMock).getAdRouteDao();
    }

    @Test
    public void testDeleteSuccess() throws Exception {
        UUID id = UUID.randomUUID();
        doReturn(true).when(authMock).adRouteAuthorized(contextMock,
                AuthAction.WRITE, id);
    ...
    public void testDeleteUnauthorized() throws Exception {
        UUID id = UUID.randomUUID();
        doReturn(false).when(authMock).adRouteAuthorized(contextMock,
                AuthAction.WRITE, id);
        resource.delete(id, contextMock, factoryMock, authMock);
    }
    ...
    private AdRoute getAdRoute(URI baseUri) {
    ...
    public void testGetSuccess() throws Exception {
        ...
        AdRoute adRoute = getAdRoute(uri);
        doReturn(true).when(authMock).adRouteAuthorized(contextMock,
                AuthAction.READ, id);
        doReturn(uri).when(uriInfoMock).getBaseUri();
        doReturn(adRoute).when(daoMock).get(id);

        AdRoute result = resource.get(id, contextMock, uriInfoMock,
                factoryMock, authMock);

        Assert.assertEquals(adRoute, result);

...
add_if() {
    ...
    if [[ "$(mm_dpctl --list-dps | grep -- "${DATAPATH}")" = "" ]]; then
        if ! mm_dpctl --add-dp "${DATAPATH}" ; then
            echo "Failed to create datapath: ${DATAPATH}"
            exit 1
        fi
    fi
    ...
    ip link add "${IFNAME}" type veth peer name "${IFNAME}_c"

    # Add one end of veth to the datapath.
    if ! mm_dpctl --add-if "${IFNAME}" "${DATAPATH}" 2>&1 > /dev/null; then
        echo >&2 "Failed to add interface ${IFNAME} to the datapath ${DATAPATH}"

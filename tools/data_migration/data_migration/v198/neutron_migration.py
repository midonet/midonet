...
# Limitations:
# * MN - Do not modify auto-generated chains
# * MN - Do not create a chain that starts with OS_PRE_ROUTING,
#        OS_POST_ROUTING, OS_PORT, or OS_SG
# * MN - Do not create ports on a router peered to already-existing
#        neutron-based bridge ports
# * MN - Do not add DHCP hosts through MN API/CLI as these won't be migrated.
# * MN - Do not modify existing neutron-based objects, especially routers,
#        bridges, ports, LB pools.
...
def _fill_object_from_type(topo, obj, root_type, skip_depends=False):
    ...
    for key, default in object_update_attr:
        print "looking up attr: " + key + "==" + str(obj[key])
        if obj[key] != default:
            print "setting update attr: " + key + "==" + str(obj[key])
            ret_object_map[key] = obj[key]
...
neutron_queries = [
    ...
    ('health-monitors', cfg.lb_client.get_health_monitors,
     [migration_funcs.MinLengthFilter(
         field='pools',
         min_len=1)]),
]
...
def _prepare():
    ...
    for key, func, filter_list in neutron_queries:
        topology_map.update(
            migration_funcs.get_neutron_objects(
                key=key, func=func, context=cfg.ctx, log=log,

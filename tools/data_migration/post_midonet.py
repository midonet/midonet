...
api_log = logging.getLogger("midonetclient.api_lib")
api_log.setLevel(logging.FATAL)
...
def _mn_get_port(_, port_id):
    try:
        return mc.mn_api.get_port(port_id)
    except exc.HTTPNotFound:
        return None
...
def _get_object_to_update_create(old_obj_id, obj, obj_type,
    ...
    if operation == 'update':
        ...
        if not dry_run and get_func:
            ...
            if not ret_obj:
                raise exceptions.UpgradeScriptException(
                    "ERROR: Object not found for updating: " + old_obj_id)
        ...
        if update_id_field:
            new_id = ret_obj.get_id() if not dry_run else "<NEW ID>"

    log.info((" "*(indent*2)) + "MAP old (MN) " +
             obj_type + "(" + old_obj_id +
             ") ID to: " + new_id)
    new_id_map[old_obj_id] = new_id
    return ret_obj
        ...
        if dry_run:
            ...
            try:
                new_tz = (mc.mn_api.add_tunnel_zone()
                          ...
                          .name(tz_obj['name'])
                          .create())
                new_id_map[tz_obj['id']] = new_tz.get_id()
            except exc.HTTPClientError as e:
                if e.code == 409:
...
def _run_midonet_post(mn_map, new_id_map):
    ...
    for bridge_id, bridge in (iter(mn_map['bridges'].items())
        ...
        for port_id, port in (iter(bridge['ports'].items())
            ...
            if 'peerId' in port and port['peerId']:
                must_peer_ports.append((port_id, port, port_obj))
    ...
    for lb_id, lb in (iter(mn_map['load_balancers'].items())
        ...
        router_obj = _mn_get_router(None, old_router_id)
        if not router_obj:
            if old_router_id not in new_id_map:
                raise exceptions.UpgradeScriptException(
                    "Supposedly newly-created Load Balancer with router ID: " +
                    old_router_id + " doesn't have a new ID.  Has it been "
                    "created yet?")
            if not dry_run:
                router_obj = _mn_get_router(None, new_id_map[old_router_id])
                if not router_obj:
                    raise exceptions.UpgradeScriptException(
                        "ERROR: Load-balancer not found with either old ID: " +
                        old_router_id + ", or newly-created ID: " +
                        new_id_map[old_router_id])
        ...
        for pool_id, pool in (iter(lb['pools'].items())
            ...
            for member_id, member in iter(pool['poolMembers'].items()):
                _get_object_to_update_create(
    ...
    peered_ports = set()
    for port_id, port, port_obj in must_peer_ports:
        if dry_run:
            new_port_id = port_id
            ...
            if port['peerId'] not in new_id_map:
                translated_peer_port = _mn_get_port(None, port['peerId'])
                new_peer_id = translated_peer_port.get_id()
                if not translated_peer_port:
                    raise exceptions.UpgradeScriptException(
                        "Port with old ID: " + port_id + " and new ID: " +
                        new_port_id + " tried to link to old peer ID: " +
                        port['peerId'] + ", but did not find a new peer ID. "
                        "Has the peer port been created/translated yet?")
            else:
                new_peer_id = port['peerId']
...
def _setup_bgp(mn_map, bgp_info_map):
    # BGP
    uplink_router = (
        None if dry_run
        else next(
    ...
    ur_id = "<UPLINK_ROUTER_ID>" if dry_run else uplink_router.get_id()
    uplink_bgp = mn_map['uplink_router_bgp']
    bgp_info_map[ur_id] = []
    for bgp_info in uplink_bgp:
        for bgp_id, bgp in iter(bgp_info.items()):
    ...
    for router_id, bgp_list in iter(bgp_info_map.items()):
        ...
        router_obj = (
            None if dry_run
            else mc.mn_api.get_router(router_id))
        for bgp in bgp_list:
            for ad_id, ad_route in (iter(bgp['adRoutes'].items())
                                    if 'adRoutes' in bgp else {}):
                cidr = (ad_route['nwPrefix'] + '/' +
                        str(ad_route['prefixLength']))
                bgp_net_map[cidr] = ad_route
            bgp_peer_unique_id = str(bgp['peerAS']) + '/' + bgp['peerAddr']
            bgp_peer_map[bgp_peer_unique_id] = bgp

        for cidr, ad_route in iter(bgp_net_map.items()):
            log.info("BGP (MN) " + router_id + " ADD NET (" +
                     str(ad_route) + ")")
            if not dry_run:
                _mn_create_router_bgp_net(router_obj, ad_route)

        for peer_id, peer in iter(bgp_peer_map.items()):
...
with open(migration_defs.MIDONET_POST_COMMAND_FILE) as f:
    ...
    midonet_topo = json.load(f)

    log.info("Running post-migration configuration")

new_ids = {}
_create_mn_tz_hosts_uplink_router(midonet_topo, new_ids)
#bgp_map = _run_midonet_post(midonet_topo, new_ids)
#_setup_bgp(midonet_topo, bgp_map)

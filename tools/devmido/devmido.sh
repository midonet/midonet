...
# Keep track of the devmido(current) directory
...
# Check to see if we are already running devmido
if is_screen_running $SCREEN_NAME ; then
    echo "You are already running a devmido.sh session."
    echo "To rejoin this session type 'screen -x $SCREEN_NAME'."
    echo "To destroy this session, type './undevmido.sh'."
    exit 1
fi
...
sudo ./gradlew clean
sudo chown $MIDOSTACK_USER .gradle/ ~/.gradle -R

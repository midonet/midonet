...
import org.midonet.cluster.services.state.{StateProxyProtocolDecoder, StateProxyProtocolEncoder}
...
class StateProxyServerTest extends FeatureSpec with Matchers
    ...
    feature("Test server handles protocol message") {
        scenario("Server handles dummy request") {
            Given("A state proxy server")
            val config = newConfig()
            val server = newServer(config)
            ...
            val request = Message.newBuilder().setRequestId(requestId).build()
            client.channel.writeAndFlush(request)

            Then("The client should receive a response")
            client.observer.awaitOnNext(1, timeout)
            val response = client.observer.getOnNextEvents.get(0).asInstanceOf[Message]
            response.getRequestId shouldBe requestId
            response.hasError shouldBe true
            response.getError.getCode shouldBe Message.Error.Code.UNKNWON_MESSAGE
        ...
        scenario("Server handles unknown data") {
            ...
            response.hasError shouldBe true
            response.getError.getCode shouldBe Message.Error.Code.UNKNWON_PROTOCOL

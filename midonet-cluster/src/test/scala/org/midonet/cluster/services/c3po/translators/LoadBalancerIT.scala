...
class LoadBalancerIT extends C3POMinionTestBase with LoadBalancerManager {
    ...
    "The delete of all Pools" should "also delete the load balancers" in {
        val rId = UUID.randomUUID
        val p1Id = UUID.randomUUID
        val p2Id = UUID.randomUUID
        val rJson = routerJson(rId)
        val p1Json = lbPoolJson(id = p1Id, routerId = rId) // neutron pools
        val p2Json = lbPoolJson(id = p2Id, routerId = rId)

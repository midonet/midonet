...
class BgpTranslationIT extends C3POMinionTestBase {
    ...
    "BgpSpeakerTranslator" should "create associated objects" in {
        ...
        val rifPortId = createRouterInterfacePort(
            30, netId, subId, routerId, "10.0.0.1", MAC.random().toString)
        createRouterInterface(40, routerId, rifPortId, subId)
        ...
        eventually {
            ...
            storage.exists(classOf[NeutronBgpSpeaker], speakerId).await() shouldBe true
            storage.exists(classOf[Port], quaggaPortId(routerId)).await() shouldBe true
            storage.exists(classOf[ServiceContainer], quaggaContainerId(routerId)).await() shouldBe true
        }
    }
    ...
    "BgpSpeakerTranslator" should "delete associated objects" in {
        ...
        val speakerId = createBgpSpeaker(50, routerId = routerId)
        insertDeleteTask(60, BgpSpeakerType, speakerId)
        ...
        eventually {
            ...
            storage.exists(classOf[NeutronBgpSpeaker], speakerId).await() shouldBe false
            storage.exists(classOf[Port], quaggaPortId(routerId)).await() shouldBe false
            storage.exists(classOf[ServiceContainer], quaggaContainerId(routerId)).await() shouldBe false
        }
    }
    ...
    "BgpPeerTranslator" should "create associated objects" in {
        ...
        val netId = createTenantNetwork(1)
        createSubnet(10, netId, s"$cidrAddr/$prefixLen")
        val routerId = createRouter(20)
    ...
    "RouterInterfaceTranslator" should "should create and delete associated bgp networks" in {
        ...
        val subId2 = createSubnet(12, netId, s"$cidrAddr2/$prefixLen2")

        val routerId = createRouter(20)

        val p1Id = createRouterInterfacePort(30, netId, subId, routerId,
                                             "10.0.0.1", MAC.random().toString)
        createRouterInterface(40, routerId, p1Id, subId)

        val speakerId = createBgpSpeaker(50, routerId = routerId)

        val p2Id = createRouterInterfacePort(60, netId2, subId2, routerId,
                                             "10.0.1.1", MAC.random().toString)
        createRouterInterface(61, routerId, p2Id, subId2)

        createBgpPeer(70, peerIp = "10.0.0.4", speakerId = speakerId)
        ...
        createRouterInterface(93, routerId, p3Id, subId3)
    ...
    "Bgp Translators" should "manage peers and networks" in {
        ...
        eventually {
            val r = storage.get(classOf[Router], routerId).await()
            r.hasLocalRedirectChainId shouldBe true
        ...
        eventually {
            ...
            storage.exists(classOf[NeutronBgpPeer], peerId).await() shouldBe true
            storage.exists(classOf[BgpPeer], peerId).await() shouldBe true
            storage.exists(classOf[NeutronBgpPeer], peerId2).await() shouldBe true
            storage.exists(classOf[BgpPeer], peerId2).await() shouldBe true

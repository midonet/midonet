...
class RouterInterfacePortTranslationTest extends PortTranslatorTest {
    ...
    protected val mRouterPortWithPeer = mPortFromTxt(s"""
        ...
        router_id: { $routerId }
        """)
    protected val snatRuleId = sameSubnetSnatRuleId(postRouteChainId, peerPortId)
    ...
    """)
    protected val revSnatRuleId = sameSubnetSnatRuleId(preRouteChainId, peerPortId)
...
class RouterInterfacePortUpdateDeleteTranslationTest
    ...
    before {
        ...
        bind(snatRuleId, snatRule)
        bind(revSnatRuleId, revSnatRule)
        bindAll(Seq(), Seq(), classOf[IPAddrGroup])
    }
    ...
    ", Next Hop Routes and SNAT rules" in {
        ...
        val target = Rule.NatTarget.newBuilder
            .setNwStart(newIfPortIp).setNwEnd(newIfPortIp)
            .setTpStart(1024).setTpEnd(65535).build()
        val natRuleData = Rule.NatRuleData.newBuilder
            .setDnat(false)
            .addNatTargets(target).build()
        val snatRuleBldr = snatRule.toBuilder.setNatRuleData(natRuleData)
        midoOps should contain (UpdateOp(snatRuleBldr.build))

        val cond = Condition.newBuilder()
            .addInPortIds(peerPortId)

...
class VPNaaSTranslatorIT extends C3POMinionTestBase {
    ...
    "VpnServiceTranslator" should "handle VPN CRUD" in {
        ...
        verifyNoContainerAndRedirectRoute(router1Id)
        eventually (storage.get(classOf[Router], router1Id).await()
                        .getVpnServiceIdsCount shouldBe 0)
        eventually (storage.getAll(classOf[VpnService]).await().size shouldBe 0)
    }

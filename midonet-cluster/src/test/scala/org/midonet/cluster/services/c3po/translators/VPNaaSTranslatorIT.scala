...
import org.midonet.cluster.services.c3po.translators.IPSecSiteConnectionTranslator.vpnContainerRouteId
...
class VPNaaSTranslatorIT extends C3POMinionTestBase {
    ...
    "VpnServiceTranslator" should "handle VPN CRUD" in {
        ...
        eventually(verifyVpnServices(router1Id, Seq(vpn1Id), router1GatewayIp))
        verifyNoContainerAndRedirectRoute(router1Id)
        ...
        eventually(verifyVpnServices(router1Id, Seq(vpn1Id, vpn2Id), router1GatewayIp))
        verifyNoContainerAndRedirectRoute(router1Id)
        var router = storage.get(classOf[Router], router1Id).await(60 seconds)
        router.getVpnServiceIdsCount shouldBe 2

        // delete the first vpn service
        insertDeleteTask(50, VpnServiceType, vpn1Id)
        eventually(verifyVpnServices(router1Id, Seq(vpn2Id), router1GatewayIp))
        verifyNoContainerAndRedirectRoute(router1Id)
        ...
        verifyNoContainerAndRedirectRoute(router1Id)
        router = storage.get(classOf[Router], router1Id).await()
        router.getVpnServiceIdsCount shouldBe 0
        eventually (storage.exists(classOf[VpnService], vpn1Id).await() shouldBe false)
        eventually (storage.exists(classOf[VpnService], vpn2Id).await() shouldBe false)
    }
    ...
    "IPSecSiteConnectionTranslator" should "handle IPSecSiteConnection CRUD" in {
        ...
        // We create two routes when we create the VpnService container, hence
        // numOtherRoutes = 2
        val cnxn1 = createAndVerifyConnection(vpn1Id, router1Id, cnxn1LocalCidrs,
                                              cnxn1PeerCidrs,
                                              expectedRoutes = 2)
        val cnxn1Id = cnxn1.getId

        var cnxn2LocalCidrs = Seq("10.0.1.0/24", "10.0.2.0/24")
        var cnxn2PeerCidrs = Seq("40.0.1.0/24", "40.0.2.0/24", "40.0.3.0/24")
        // Create a second connection on the same VPN and verify that they
        // coexist.
        val cnxn2 = createAndVerifyConnection(vpn1Id, router1Id,
                                              cnxn2LocalCidrs,
                                              cnxn2PeerCidrs,
                                              2 + cnxn1.getRouteIdsCount)
        val cnxn2Id = cnxn2.getId()
        ...
        val cnxn1V2Json = ipSecSiteConnectionJson(
            cnxn1.getId, vpn1Id, cnxn1LocalCidrs, cnxn1PeerCidrs,
            ikePolicyJson(), ipSecPolicyJson(), adminStateUp = false,
        ...
        eventually(verifyNoContainerAndRedirectRoute(router1Id))
    }
    ...
    "IPSecSiteConnectionTranslator" should "not delete preexisting local redirect chain" in {
        ...
        val rule = Rule.newBuilder()
            ...
            .build()
        storage.create(rule)
        storage.update(router.toBuilder.setLocalRedirectChainId(
            chain.getId).build)
        ...
        var cnxn1LocalCidrs = Seq("10.0.1.0/24")
        var cnxn1PeerCidrs = Seq("20.0.1.0/24", "20.0.2.0/24")
        var cnxn1 = createAndVerifyConnection(vpn1Id, router1Id, cnxn1LocalCidrs,
                                              cnxn1PeerCidrs,
                                              expectedRoutes = 2)


        insertDeleteTask(50, IpSecSiteConType, cnxn1.getId)
        storage.exists(classOf[Chain], chain.getId).await() shouldBe true

        storage.delete(classOf[Rule], rule.getId)
        storage.exists(classOf[Chain], chain.getId).await() shouldBe true

        cnxn1 = createAndVerifyConnection(vpn1Id, router1Id, cnxn1LocalCidrs,
                                          cnxn1PeerCidrs,
                                          expectedRoutes = 2)
        insertDeleteTask(60, IpSecSiteConType, cnxn1.getId)
        eventually {
    ...
    private def verifyNoContainer(routerId: UUID): Router = {
        ...
        val rules = storage.getAll(classOf[Rule]).await()
            .filter(IPSecSiteConnectionTranslator.isRedirectRule)
        rules.count(IPSecSiteConnectionTranslator.isESPRule) shouldBe 0
        rules.count(IPSecSiteConnectionTranslator.isUDP500Rule) shouldBe 0
        rules.count(IPSecSiteConnectionTranslator.isUDP4500Rule) shouldBe 0
        router
    }

    private def verifyNoContainerAndRedirectRoute(routerId: UUID): Unit = {

        val router = verifyNoContainer(routerId)
    ...
    private def verifyContainer(rtrId: UUID, vpnIds: Seq[UUID],
        ...
        val rules = storage.getAll(classOf[Rule],
            ...
            .filter((r: Rule) => {
                IPSecSiteConnectionTranslator.isRedirectForEndpointRule(r, localEndpointIp) &&
                            r.getTransformRuleData.getTargetPortId == rtrPort.getId
                    })
        rules.count(IPSecSiteConnectionTranslator.isESPRule) shouldBe 1
        rules.count(IPSecSiteConnectionTranslator.isUDP500Rule) shouldBe 1
        rules.count(IPSecSiteConnectionTranslator.isUDP4500Rule) shouldBe 1
        ...
        router.getVpnServiceIdsList should
    ...
    private def verifyVpnServices(rtrId: UUID, vpnIds: Seq[UUID],
                                  rtrGwIp: String): Unit = {
        val vpnServices = storage.getAll(classOf[VpnService], vpnIds)
            .await(60 seconds)
        vpnServices.size shouldBe vpnIds.size

        val router = storage.get(classOf[Router], rtrId).await()
        router.hasLocalRedirectChainId shouldBe false

        val containerIds = vpnServices.map(_.hasContainerId)
        all (containerIds) shouldBe false

        all (vpnServices.map(_.getExternalIp.getAddress)) shouldBe rtrGwIp
        storage.exists(classOf[Port], rtrId).await(60 seconds) shouldBe false

        storage.exists(classOf[Chain], router.getLocalRedirectChainId)
            .await() shouldBe false

        val localEndpointIp = IPSubnetUtil.fromAddress(rtrGwIp)
    }

    private def createAndVerifyConnection(vpnServiceId: UUID, routerId: UUID,
                                          localCidrs: Seq[String],
                                          peerCidrs: Seq[String],
                                          expectedRoutes: Int): IPSecSiteConnection = {
        ...
        val cnxn1Json = ipSecSiteConnectionJson(
            cnxn1Id, vpnServiceId, localCidrs, peerCidrs,
            cnxn1IkeJson, cnxn1IpSecJson)
        insertCreateTask(40, IpSecSiteConType, cnxn1Json, cnxn1Id)
        eventually {
            verifyCnxn(cnxn1Id, router1Id, localCidrs, peerCidrs,
                       expectedRoutes)
        }
    }
}

...
class RouterTranslatorIT extends C3POMinionTestBase with ChainManager {
    ...
    it should "handle router multiple IP gateway CRUD" in {
        ...
        createRouterGatewayPort(
            ...
            "04:04:04:04:04:04",
            id = gwPortId)
        insertUpdateTask(9, RouterType, routerJson(
            tenantRouterId, name = "tenant", gwPortId = gwPortId), tenantRouterId)

        verifyRouterPortCreated()

        // Remove the gateway port from the router without deleting the port.
        insertUpdateTask(10, RouterType, routerJson(
            tenantRouterId, name = "tenant"), tenantRouterId)

        verifyRouterPortDeleted()

        // Re-add the gateway port.
        insertUpdateTask(11, RouterType, routerJson(
            tenantRouterId, name = "tenant", gwPortId = gwPortId), tenantRouterId)

        verifyRouterPortCreated()

        // Delete the gateway port.
        insertDeleteTask(12, PortType, gwPortId)
        ...
        createRouterGatewayPort(
            ...
            "04:04:04:04:04:04",
            id = gwPortId)
        insertUpdateTask(14, RouterType, routerJson(
            tenantRouterId, name = "tenant", gwPortId = gwPortId), tenantRouterId)
        ...
        // Delete gateway and router.
        insertDeleteTask(15, PortType, gwPortId)
        insertDeleteTask(16, RouterType, tenantRouterId)
        verifyRouterPortDeleted()
    }

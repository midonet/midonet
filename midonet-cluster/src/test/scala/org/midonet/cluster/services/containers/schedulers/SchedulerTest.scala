...
import org.midonet.cluster.services.MidonetBackend.{ActiveKey, AliveKey}
...
class SchedulerTest extends FeatureSpec with BeforeAndAfter with Matchers
    ...
    feature("scheduler handles service container group updates") {
        scenario("Host becomes alive after scheduling") {
            ...
            obs.getOnNextEvents.get(0)
                .asInstanceOf[Created].hostId shouldBe host.getId.asJava
        ...
        scenario("A host is removed after a mapping is done (reschedule)") {
            ...
            Then("A new container to host allocation is emitted")
            obs.getOnNextEvents should have size 1
            aliveHosts.contains(obs.getOnNextEvents.get(0)
                                    .asInstanceOf[Created].hostId) shouldBe true
            val first_host = obs.getOnNextEvents.get(0)
                .asInstanceOf[Created].hostId

            And("The host becomes unavailable")
            backdoor.removeValueAs(first_host.toString, classOf[Host],
                                   first_host, AliveKey, "alive").await()
            Then("The container is rescheduled to another host")
            obs.getOnNextEvents should have size 2
            val second_host = obs.getOnNextEvents.get(1)
                .asInstanceOf[Created].hostId
    ...
    feature("The port group policy chooses hosts belonging to a port group") {
        scenario("Several nodes and a container using the port group scheduler") {
            ...
            backdoor.addValueAs(host2.getId.asJava.toString, classOf[Host],
                                host2.getId, AliveKey, "alive").await()
            backdoor.addValueAs(host1.getId.asJava.toString, classOf[Port],
                                port.getId, ActiveKey, "active").await()
        ...
        scenario("Adding a port to a port group and failing one of them (different hosts)") {
            ...
            backdoor.addValueAs(host2.getId.asJava.toString, classOf[Host],
                                host2.getId, AliveKey, "alive").await()
            backdoor.addValueAs(host1.getId.asJava.toString, classOf[Port],
                                port1.getId, ActiveKey, "active").await()
            ...
            val portgroup2 = createPortGroup(portgroup.getId,
                                             portIds = Seq(port1.getId,
                                                           port2.getId))
            store.multi(Seq(CreateOp(port2),
                            UpdateOp(portgroup2)))
            backdoor.addValueAs(host2.getId.asJava.toString, classOf[Port],
                                port2.getId, ActiveKey,
                                "active").await()
            ...
            backdoor.removeValueAs(host1.getId.asJava.toString, classOf[Port],
                                   port1.getId, ActiveKey,
                                   "active").await()

            Then("A new mapping is emitted pointing to the second port")
            obs.getOnNextEvents should have size 2
            obs.getOnNextEvents.get(1)
                .asInstanceOf[Created].hostId shouldBe host2.getId.asJava
        ...
        scenario("Changing the port group associated to a container group") {
            ...
            backdoor.addValueAs(host1.getId.asJava.toString, classOf[Port],
                                port1.getId, ActiveKey, "active").await()
            backdoor.addValueAs(host2.getId.asJava.toString, classOf[Port],
                                port2.getId, ActiveKey, "active").await()
            ...
            val group2 = createServiceContainerGroup(group.getId,
                                                     portGroupId = Some(portgroup2.getId))
            val container2 = createServiceContainer(container.getId,
                                                    groupId = Some(group2.getId))

...
class HealthMonitorV2IT extends C3POMinionTestBase with LoadBalancerManager {
    ...
    private def verifyHealthMonitor(hm: HealthMonitor, id: UUID, delay: Int,
    ...
    "HealthMonitorV2Translator" should "add, update, and delete HM" in {
        ...
        var hmJson = makeHmJson(hmId, delay, timeout, maxRetries, Set(poolId),
                                true)

        insertCreateTask(10, HealthMonitorV2Type, hmJson, hmId)

        storage.exists(classOf[HealthMonitor], hmId).await() shouldBe true
        var hm = storage.get(classOf[HealthMonitor], hmId).await()
        verifyHealthMonitor(hm, hmId, delay, maxRetries, timeout,
                            Set(poolId), true)

        delay = 2
        timeout = 3
        maxRetries = 4
        hmJson = makeHmJson(hmId, delay, maxRetries, timeout,
                            Set(poolId, poolId2), true)

        insertUpdateTask(20, HealthMonitorV2Type, hmJson, hmId)
        hm = storage.get(classOf[HealthMonitor], hmId).await()
        verifyHealthMonitor(hm, hmId, delay, maxRetries, timeout,
                            Set(poolId, poolId2), true)

        hmJson = makeHmJson(hmId, delay, maxRetries, timeout, Set(), true)
        insertUpdateTask(30, HealthMonitorV2Type, hmJson, hmId)
        hm = storage.get(classOf[HealthMonitor], hmId).await()
        verifyHealthMonitor(hm, hmId, delay, maxRetries, timeout, Set(), true)

        insertDeleteTask(40, HealthMonitorV2Type, hmId)

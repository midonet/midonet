...
trait LbaasV2ITCommon { this: C3POMinionTestBase =>
    ...
    protected def healthMonitorV2Json(id: UUID,
                                      delay: Int,
                                      maxRetries: Int,
                                      timeout: Int,
                                      poolIds: Set[UUID],
    ...
    protected def createHealthMonitorV2(taskId: Int, id: UUID = UUID.randomUUID(),
                                        delay: Int = 1, timeout: Int = 1,
                                        maxRetries: Int = 1,
                                        poolIds: Set[UUID] = Set(),
                                        adminStateUp: Boolean = true): UUID = {
        val json = healthMonitorV2Json(id, delay, maxRetries, timeout,
                                       poolIds, adminStateUp)
        insertCreateTask(taskId, HealthMonitorV2Type, json, id)
        id
    }

    protected def updateHealthMonitorV2(taskId: Int, id: UUID, delay: Int = 1,
                                        timeout: Int = 1, maxRetries: Int = 1,
                                        adminStateUp: Boolean = true): Unit = {
        val json = healthMonitorV2Json(id, delay, maxRetries, timeout, Set(),
                                       adminStateUp)

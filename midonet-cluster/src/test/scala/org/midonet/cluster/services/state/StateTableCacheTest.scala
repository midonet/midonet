...
class StateTableCacheTest extends FeatureSpec with Matchers with GivenWhenThen
    ...
    private def tablePath(id: UUID): String = {
        paths.tablePath(classOf[Bridge], id, MidonetBackend.MacTable)
    }
    ...
    : StateTableCache = {
        ...
        new StateTableCache(proxyConfig, paths, zkClient, counter,
    ...
    feature("Cache emits table updates") {
        ...
        scenario("State table with multiple subscribers") {
            Given("A state table cache")
            var closed = false
            val id = UUID.randomUUID()
            val cache = newCache(create = true, id) { closed = true }
    ...
    feature("Cache handles back-pressure") {

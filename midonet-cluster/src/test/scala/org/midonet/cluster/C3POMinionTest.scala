...
class C3POMinionTestBase extends FlatSpec with BeforeAndAfter
    ...
    protected def createComputePort(taskId: Int, networkId: UUID,
                                    portId: UUID = UUID.randomUUID(),
                                    devo: DeviceOwner = DeviceOwner.COMPUTE,
                                    fixedIps: List[IPAlloc] = List[IPAlloc](),
                                    sg: List[UUID] = List[UUID]()): UUID = {
        val json = portJson(portId, networkId, deviceOwner = devo,

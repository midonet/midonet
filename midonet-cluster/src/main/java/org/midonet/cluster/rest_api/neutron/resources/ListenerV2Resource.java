...
public class ListenerV2Resource {

    private final LoadBalancerV2Api api;
    private final URI baseUri;

    @Inject
    public ListenerV2Resource(UriInfo uriInfo, LoadBalancerV2Api api) {
        this.api = api;
    ...
    @GET
    @Path("{id}")
    @Produces(NeutronMediaType.LB_LISTENER_V2_JSON_V1)
    public ListenerV2 get(@PathParam("id") UUID id) {
        ListenerV2 l = api.getListenerV2(id);
        if (l == null) {
    ...
    @GET
    @Produces(NeutronMediaType.LB_LISTENERS_V2_JSON2)
    public List<ListenerV2> list() {
        return api.getListenersV2();
    }

    @POST
    @Consumes(NeutronMediaType.LB_LISTENER_V2_JSON_V1)
    @Produces(NeutronMediaType.LB_LISTENER_V2_JSON_V1)
    public Response create(ListenerV2 l) {
    ...
    @Consumes(NeutronMediaType.LB_LISTENER_V2_JSON_V1)
    @Produces(NeutronMediaType.LB_LISTENER_V2_JSON_V1)
    public Response update(@PathParam("id") UUID id, ListenerV2 l) {

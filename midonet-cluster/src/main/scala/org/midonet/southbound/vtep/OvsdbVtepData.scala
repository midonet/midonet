...
class OvsdbVtepData(val client: OvsdbClient, val dbSchema: DatabaseSchema,
    ...
    private class MacUpdater(`type`: String,
        ...
        override def onCompleted(): Unit = {
            log.debug("Completed {} MAC location updates", `type`)
            unsubscribe()
        }
        override def onError(err: Throwable): Unit = {
            log.warn("Error on {} MAC location updates", `type`, err)
            unsubscribe()
        }
        override def onNext(ml: MacLocation): Unit = {
            log.debug("Received {} MAC location: {}", `type`, ml)
            if (ml != null) {
                applyMac(ml)
            }
        }
        ...
        private def applyMac(ml: MacLocation): Unit = {
            logicalSwitch(ml.logicalSwitchName) map {
                case None =>
                    log.warn("Unknown logical switch for {} MAC location: {}",
        ...
        /** Returns the OVSDB operations to add a unicast MAC entry. If a
          ...
          * referencing the new locator where both operations are executed in
          ...
          * deleted because there is no MAC entry reference to it. */
        ...
        : Seq[Table.OvsdbOperation] = {
            ...
            ops = ops ++: ucastTable.getAll
                ...
                .map(e => ucastTable.table.delete(UcastMac(ls.uuid, ml.mac, e.ipAddr, null)))
                .toSeq

            // Insert the new MAC entry.
            ops = ops :+ ucastTable.table.insert(
                UcastMac(ls.uuid, ml.mac, ml.ipAddr, locatorId), null)

            ops
        }

        /** Returns the OVSDB operations to add a multicast MAC entry. If a
          * locator and locator set already exists for the tunnel IP address,
        ...
        : Seq[Table.OvsdbOperation] = {
            ...
            ops = ops ++: mcastTable.getAll
                .filter(e => e.ls == ls.uuid && e.macAddr == ml.mac)
                .map(e => mcastTable.table.delete(McastMac(ls.uuid, ml.mac, e.ipAddr, null)))
                .toSeq

            // Insert the new MAC entry.
            ops = ops :+ mcastTable.table.insert(
                McastMac(ls.uuid, ml.mac, ml.ipAddr, locatorSetId), null)

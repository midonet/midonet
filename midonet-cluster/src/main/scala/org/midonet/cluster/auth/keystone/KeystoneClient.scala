...
object KeystoneClient {
    ...
    private[keystone] def parseTimestamp(string: String): Long = {
        val tryZonedTime =
            Try(ZonedDateTime.parse(string, DateTimeFormatter.ISO_DATE_TIME)
                             .toInstant.toEpochMilli)
        val tryLocalTime =
            Try(LocalDateTime.parse(string, DateTimeFormatter.ISO_DATE_TIME)
...
class KeystoneClient(config: KeystoneConfig) {
    ...
    /**
      ...
      * password credentials. The method returns a [[AuthResponse]] instance
    ...
    : AuthResponse = withVersion {
        case 2 =>
            ...
            val access = post(auth, classOf[v2.KeystoneAccess], None, "tokens")()
        ...
        case 3 =>
            val auth = v3.KeystoneAuth(v3.Auth(
                ...
                v3.Scope(v3.Project(name = tenantName,
                                    domain = v3.Domain(id = config.domainName)))))
            handleAuthResponse(post(auth, classOf[ClientResponse], None, "auth",
                                    "tokens")())
    }

    /**
      * Authenticates against Keystone for the given tenant using the specified
      * token. The method returns a [[AuthResponse]] instance containing an
    ...
    def authenticate(tenantName: String, token: String): AuthResponse = withVersion {
        case 2 =>
            val access = post(v2.KeystoneAuth(v2.TokenAuth(v2.Token(id = token),
                                                           tenantName)),
                              classOf[v2.KeystoneAccess], None, "tokens")()
            AuthResponse(tokenId = access.access.token.id, r2 = access.access)
        case 3 =>
    ...
    def listUsers(token: String): v2.KeystoneUsers = {
        withVersion {
            case 2 =>
                get(classOf[v2.KeystoneUsers], Some(token), None, "users")()
        }
    }
    ...
    def getUserByName(name: String, token: String): v2.KeystoneUser = {
        withVersion {
            case 2 =>
                get(classOf[v2.KeystoneUser], Some(token), None, "users")(("name", name))
        }
    }
    ...
    def getUserById(id: String, token: String): v2.KeystoneUser = {
        withVersion {
            case 2 =>
                get(classOf[v2.KeystoneUser], Some(token), None, "users", id)()
        }
    }
    ...
    : v2.KeystoneTenants = {
        withVersion {
            case 2 =>
                ...
                get(classOf[v2.KeystoneTenants], Some(token), None, "tenants")(
                    params: _*)
        }
    }
    ...
    : v2.KeystoneTenant = {
        withVersion {
            case 2 =>
                get(classOf[v2.KeystoneTenant], Some(token), None, "tenants")(
                    ("name", name))
        }
    }
    ...
    : v2.KeystoneTenant = {
        withVersion {
            case 2 =>
                get(classOf[v2.KeystoneTenant], Some(token), None, "tenants", id)()
        }
    }
    ...
    : v2.KeystoneRoles = withVersion {
        case 2 =>
            ...
            if (marker.isDefined) params = params :+ ("marker", marker.get)
            if (limit.isDefined) params = params :+ ("limit", limit.get.toString)
            get(classOf[v2.KeystoneRoles], Some(token), None, "tenants", tenantId,
    ...
    private def withAdminToken[R](f: (String) => R): R = {
        ...
        if ((currentToken eq null) ||
            ...
            currentToken = AdminToken(auth.tokenId,
                                      parseTimestamp(auth.token.issuedAt),
                                      System.currentTimeMillis(),
                                      parseExpiresAt(auth.token.expiresAt))

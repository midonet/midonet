...
class RouterInterfaceTranslator(sequenceDispenser: SequenceDispenser,
    ...
    override protected def translateCreate(tx: Transaction,
        ...
        val addresses = buildAddresses(tx, builder,
    ...
    override protected def translateUpdate(tx: Transaction,
        ...
        val allSubnets = nPort.getFixedIpsList.asScala
        val oldMNSubnets = oldRouterPort.getPortSubnetList.asScala
        ...
        val oldSubnetIds = for (sn <- allSubnets if oldMNSubnets.exists { oldsn =>
            oldsn.getAddress == sn.getIpAddress.getAddress
        } ) yield sn.getSubnetId

        val subnetsToRemove = ri.getSubnetIdsList.asScala.toSet
        val keepSubnets = oldSubnetIds.toSet -- subnetsToRemove

        buildAddresses(tx, builder, keepSubnets.toSeq, nPort, isUplink)
        val hasIPv6 =
            tx.getAll(classOf[NeutronSubnet], keepSubnets.toSeq)
                .map(_.getCidr).exists( x => x.getVersion == IPVersion.V6)
        ...
        val addressesToRemove = buildAddresses(tx, Port.newBuilder,
                                               subnetsToRemove.toSeq,
                                               nPort, isUplink)

        addressesToRemove foreach { addr =>
            val nextHopRouteId = RouteManager.
                routerInterfaceRouteId(routerPortId, addr.address)
            val index = builder.getRouteIdsList.indexOf(nextHopRouteId)
            require(index >= 0)
            builder.removeRouteIds(index)

            val localRouteId = RouteManager.
                localRouteId(routerPortId, addr.address)
            val indexLocal = builder.getRouteIdsList.indexOf(localRouteId)
            require(indexLocal >= 0)
            builder.removeRouteIds(indexLocal)
        }
    ...
    : Unit = {
        if (!isUplink) {
            val skipNat4Rule = Rule.newBuilder()
                ...
                .build()
            // FIXME: that would fails if two ipv6 subnets present
    ...
    private def buildAddresses(tx: Transaction, builder: Port.Builder,
                               subnetIds: Seq[Commons.UUID], nPort: NeutronPort,
                               isUplink: Boolean): List[PortAddress] = {
    ...
    private def rollbackRouterSubnetPort4(tx: Transaction,
                                          ...
                                          routerPortId: UUID,
                                          isUplink: Boolean,
                                          addressToRemove: PortAddress)
    : Unit = {
        if (!isUplink) {
            if (addressToRemove.isGateway) {

...
import org.midonet.cluster.data.storage.StateTablePaths
...
class StateTableCache(val config: StateProxyConfig,
    ...
    private val cache = new TableEntries
    ...
    private val path = paths.tablePath(objectClass, objectId, name, args: _*)
    ...
    private def processEntriesUnsafe(entries: util.List[String],
        ...
        // Mark old entries as stale, to remove them if they are not
        // updated.
        var cacheIterator = cache.values().iterator()
        while (cacheIterator.hasNext) {
            cacheIterator.next().stale = true
        }
        ...
        while (entryIterator.hasNext) {
            ...
            if (newEntry != null) {
                ...
                if ((oldEntry eq null) ||
                    oldEntry.version == PersistentVersion) {
                    cache.put(newEntry.key, newEntry)
                    diffAddCache.add(newEntry)
                } else if (newEntry.version == PersistentVersion) {
    ...
    private def requestRefreshUnsafe(subscription: Subscription,
        ...
        val batchCount =
            if (cache.isEmpty) 1 else (cache.size() - 1) / notifyBatchSize + 1

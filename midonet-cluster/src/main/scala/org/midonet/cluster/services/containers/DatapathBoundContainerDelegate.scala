...
object DatapathBoundContainerDelegate {
    final val MaxStorageAttempts = 10
}

class DatapathBoundContainerDelegate (backend: MidonetBackend)
    ...
    override def onUp(container: ServiceContainer, status: ContainerStatus): Unit = {
        log debug s"$name Container up at host ${status.getHostId.asJava} namespace" +
                  s"namespace ${status.getNamespaceName} interface " +
                  s"${status.getInterfaceName}"
    }
    ...
    override def onUnscheduled(container: ServiceContainer, hostId: UUID): Unit = {
        ...
        log info s"$name Container $containerId unscheduled from host " +
                 s"$hostId: unbinding port"
        tryTx { tx =>

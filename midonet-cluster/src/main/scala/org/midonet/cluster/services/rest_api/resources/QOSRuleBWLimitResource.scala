...
class QOSRuleBWLimitResource @Inject()(resContext: ResourceContext)
    ...
    protected override def deleteFilter(ruleId: String,
        ...
        val rule = tx.get(classOf[QOSRuleBWLimit], ruleId)
        val policy = tx.get(classOf[QOSPolicy], rule.policyId)
        if (policy.bandwidthLimitRuleIds.remove(rule.id)) {
            tx.update(policy)
        } else {
            throw new NotFoundHttpException("Rule not found on policy")
        }
...
class QOSPolicyRuleBWLimitResource @Inject()(policyId: UUID,
    ...
    : Unit = {
        ...
        policy.bandwidthLimitRuleIds.add(rule.id)
        tx.update(policy)
    }
}

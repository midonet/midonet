...
import org.midonet.packets.{SbeEncoder, FlowStateStorePackets, Ethernet}
...
object FlowStateMessageHandler {
    ...
    /**
      ...
      *
      * WARNING: This object assumes that the cassandraSession field is
      * initialized before calling a thread calls its first get method on the
    ...
    protected val storageProvider: ThreadLocal[FlowStateStorageWriter] =
        new ThreadLocal[FlowStateStorageWriter] {
            override def initialValue(): FlowStateStorageWriter = {
                ...
                FlowStateStorage(cassandraSession)
            }
        }
}

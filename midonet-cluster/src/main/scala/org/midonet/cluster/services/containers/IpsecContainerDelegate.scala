...
import org.midonet.cluster.data.storage.{NotFoundException, Storage, Transaction}
...
class IpsecContainerDelegate @Inject()(store: Storage)
    ...
    override def onScheduled(container: ServiceContainer, hostId: UUID): Unit = {
        ...
        tryTx { tx =>
            tx update tx.get(classOf[Port], portId).toBuilder
    ...
    private def tryTx(f: (Transaction) => Unit): Unit = {
        var attempt = 1
        while (true) {
            try {
                ...
                case e: ConcurrentModificationException
                    ...
                    log warn s"Write $attempt of $storageAttempts failed due " +

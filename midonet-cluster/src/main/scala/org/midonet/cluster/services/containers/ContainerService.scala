...
class ContainerService @Inject()(nodeContext: Context,
    ...
    private val delegates = new mutable.HashMap[String, ContainerDelegate].empty
                ...
                override def onNext(event: ContainerEvent): Unit = {
                    ...
                    event match {
                        case Allocation(container, hostId) =>
                            getDelegate(container.getServiceType)
    ...
    @inline
    private def getDelegate(st: String): ContainerDelegate = {
        delegates getOrElseUpdate (st, delegateProvider.getInstance(st))
    }
}

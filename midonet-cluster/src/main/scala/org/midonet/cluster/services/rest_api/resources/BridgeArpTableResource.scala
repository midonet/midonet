...
class BridgeArpTableResource(bridgeId: UUID, resourceContext: ResourceContext)
    ...
    : util.List[Ip4MacPair] = {
        val entries = tryLegacyRead {
            Await.result(arpTable.remoteSnapshot, readTimeout)
        }
    ...
    : Response = {
        ...
        tryLegacyWrite {
            Await.result(arpTable.addPersistent(address, mac), readTimeout)
    ...
    override def delete(@PathParam("ip_mac") ipMac: String): Response = {
        ...
        tryLegacyWrite {
            Await.result(arpTable.removePersistent(address, mac), readTimeout)

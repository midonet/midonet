...
abstract class CollectionTracker[M <: ObjectTracker[ME], ME >: Null]
    ...
    /** Indicates whether the port group state has received the data for the
      * port group and all ports that are members of this group.
      */
    ...
    /** Watches the specifies set of members. Current members that are not part
      * of set are removed from the membership list, and their trackers are
      * completed. New members are added to the membership list, and for each a
      * new tracker is created and merged into the output observable.
      */

...
class LoadBalancerV2PoolTranslator
    ...
    override protected def translateCreate(
        ...
        if (nPool.hasSessionPersistence)
            bldr.setSessionPersistence(getSessionPersistence(nPool))
    ...
    override protected def translateUpdate(
        ...
        if (newSessionPersistence != mPool.getSessionPersistence) {
            if (newSessionPersistence == null) {
                bldr.clearSessionPersistence()
            } else {
                bldr.setSessionPersistence(newSessionPersistence)
            }
        }

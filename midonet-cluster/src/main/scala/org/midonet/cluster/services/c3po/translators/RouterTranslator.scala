...
class RouterTranslator(sequenceDispenser: SequenceDispenser,
    ...
    private def createSnatRules(tx: Transaction, nRouter: NeutronRouter,
        ...
        val skipSnatOutGwPortRule = Rule.newBuilder
            .setId(skipSnatOutGwPortRuleId(router.getId))
            .setType(Rule.Type.LITERAL_RULE)
            .setAction(Action.RETURN)
            .setCondition(anyFragCondition.addOutPortIds(portId))
            .build()
        val skipSnatInGwPortRule = Rule.newBuilder
            .setId(skipSnatInGwPortRuleId(router.getId))
            .setType(Rule.Type.LITERAL_RULE)
            .setAction(Action.ACCEPT)
        ...
        tx.create(skipSnatOutGwPortRule)
        tx.create(skipSnatInGwPortRule)
        tx.update(appendRule(
            prependRules(skipSnatChain, skipSnatOutGwPortRule.getId),
            skipSnatInGwPortRule.getId))
    ...
    private def deleteSnatRules(tx: Transaction, routerId: UUID): Unit = {
        ...
        if (tx.exists(classOf[Rule], outSnatRuleId(routerId))) {
            ...
            tx.delete(classOf[Rule], dstRewrittenSnatRuleId(routerId),
                      ignoresNeo = true)
            tx.delete(classOf[Rule], skipSnatOutGwPortRuleId(routerId),
                      ignoresNeo = true)
            tx.delete(classOf[Rule], skipSnatInGwPortRuleId(routerId),
                      ignoresNeo = true)
        }
    }
...
object RouterTranslator {
    ...
    def skipSnatOutGwPortRuleId(routerId: UUID): UUID =
        routerId.xorWith(0x6f90386f458e12ffL, 0x6ec14164bde7cee6L)
    def skipSnatInGwPortRuleId(routerId: UUID): UUID =

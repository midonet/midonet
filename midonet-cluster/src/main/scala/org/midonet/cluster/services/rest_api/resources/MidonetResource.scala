...
object MidonetResource {
    ...
    protected[resources] def tryWrite[R](lockFactory: ZookeeperLockFactory,
                                         f: => Response)(implicit log: Logger)
    : Response = {
        ...
        try {
            ...
            case e: ObjectExistsException =>
                log.warn(e.getMessage)
                return buildErrorResponse(Status.CONFLICT, e.getMessage)
            /* In case the object was modified elsewhere without resorting
               to a lock. */
            case e: ConcurrentModificationException =>
                log.warn("Write to Zoom failed due to a concurrent " +
                          "modification.", e)
                e.printStackTrace
...
abstract class MidonetResource[T >: Null <: UriResource]
    ...
    : Response = {
        ...
        tryWrite(resContext.lockFactory, {

...
class NeutronResource @Inject() (uriInfo: UriInfo,
    ...
    @GET
    @Produces(Array(MidonetMediaTypes.NEUTRON_JSON_V4)) def getV4: Neutron = {
        val neutron = getV3
        val baseUri: UriBuilder = UriBuilder.fromUri(uriInfo.getBaseUri)
        val vpnUri: UriBuilder = baseUri.path("vpnservices")

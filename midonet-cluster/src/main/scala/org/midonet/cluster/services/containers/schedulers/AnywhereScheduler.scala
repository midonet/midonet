...
class AnywhereScheduler @Inject() (store: Storage, stateStore: StateStorage)
    ...
    def doAllocationPolicyFilter(scg: ServiceContainerGroup): Boolean = {
        Option(scg.getHostGroupId).isEmpty && Option(scg.getPortGroupId).isEmpty
    }
    ...
    protected def candidateHosts(scg: ServiceContainerGroup): List[UUID] = {
        store.getAll(classOf[Host])
             .await()
             .map(h => StateHost(stateStore.keyObservable(h.getId.toString, classOf[HostState], h.getId, MidonetBackend.AliveKey), h))

...
class BgpSpeakerTranslator(protected val storage: ReadOnlyStorage,
    ...
    def addNetworks(routerId: UUID): OperationList = {
        val ops = new OperationListBuffer
        ...
        val subIds = nPorts.filter(_.getDeviceOwner == NeutronPort.DeviceOwner.ROUTER_INTERFACE)
                           .filter(_.getDeviceId == UUIDUtil.fromProto(routerId).toString)
        ...
        for (sub <- subs.toList) yield
            Create(makeBgpNetwork(routerId, sub.getCidr, sub.getNetworkId))
    }

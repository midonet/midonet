...
/**
  ...
  */
class PortGroupHostSelector(store: Storage, stateStore: StateStorage) extends HostSelector {
    ...
    private def onPortGroupUpdate(pg: PortGroup): Unit = {
        ...
        for (pid <- pg.getPortIdsList) {
            if (!pgToMapperMap.contains(pid)) {
                ...
                portState.observable.map(makeFunc1 {
                    case (port, false) =>
                        // port is inactive, remove host from knowHostIds
    ...
    private class PortState(val portId: UUID) {
        ...
        private val filteredPortSubject = PublishSubject.create[Port]
        store.observable(classOf[Port], portId)
                .filter(makeFunc1(hostChanged))
                .takeUntil(mark)
                .subscribe(filteredPortSubject)

        private val portStateObservable = stateStore
                .keyObservable(filteredPortSubject.getValue.getHostId.toString,
                               classOf[Port],
                               portId,
                               MidonetBackend.ActiveKey)
                .map[Boolean](makeFunc1(v => { v.nonEmpty }))
                .takeUntil(mark)
        ...
        /**
          ...
          */
        val observable = Observable.zip[Port, Boolean, (Port, Boolean)](
            filteredPortSubject,
            portStateObservable,
            makeFunc2 { case pair => pair }
        )

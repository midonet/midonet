...
object Recycler {
    ...
    class RecyclingContext(val start: Long)
        extends RecyclingPromise[RecyclingContext] {
        ...
        override def fail(e: Throwable): Unit = {
            if (e.isInstanceOf[RecyclingException]) super.fail(e)
            else super.fail(RecyclingException(this, e))
        }
    }
...
class Recycler @Inject()(context: ClusterNode.Context, backend: MidonetBackend,
    ...
    // A callback that verifies that the current NSDB is recyclable by checking
    // that the last time the root ZOOM znode was last modified at a moment in
    // the past before the current time minus the current recycling interval.
    // If the NSDB is recyclable the callback will write to the root znode to
    ...
    // A callback that executes when the recycler collected the current hosts.
    // The hosts set is used to determine the obsolete namespaces, that should
    // be deleted.
    ...
    // A callback that executes when the recycler collected the current state
    // namespaces.
    ...
    // A callback that executes when the recycler verifies whether a namespace
    // can be deleted, by checking whether the creation timestamp of the
    // namespace NSDB znode is previous to the moment that the recycling has
    // started.
    ...
    // A callback that executes when an orphan namespace has been deleted.
    ...
    // A callback that executes when the recycler has collected all topology
    // objects from the NSDB.
    ...
    // A callback that executes when the recycler has collected all state znodes
    // corresponding to topology objects from the NSDB.
    ...
    // A callback that executes when the recycler has verified that the state
    // znode corresponding to an orphan topology object can be deleted, by
    // checking whether the creation timestamp of the namespace NSDB znode is
    // previous to the moment that the recycling has started.
    ...
    // A callback that executes when the recycler has deleted the state znode
    // corresponding to an orphan object.
    ...
    /**
      * Validates that the current backend store is [[ZookeeperObjectMapper]]
    ...
    /**
      * Begins an asynchronous recycling task.
      */
    private def recycle(): Unit = {
        ...
        val context = new RecyclingContext(clock.time)
        curator.getData
    ...
    /**
      * Deletes the orphan namespaces from NSDB according to the list of
      * collected hosts and namespaces.
      */
    ...
    /**
      * Recycles the state NSDB znodes corresponding to deleted topology
      * objects. The method collects all objects in the topology tree, and
      * then the state znodes corresponding to those objects for the namespaces
      * of all current hosts. Upon collecting this information, the method
      * deletes all orphan state znodes that do not have a corresponding
      * topology object.
      */

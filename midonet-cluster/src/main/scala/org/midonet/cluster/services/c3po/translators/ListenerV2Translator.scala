...
class ListenerV2Translator
    ...
    : Unit = {
        ...
        val vipPeerPortId = lbRouter.getPortIdsList.asScala.find { id =>
            tx.exists(classOf[NeutronPort], routerInterfacePortPeerId(id))
        }.getOrElse {
        ...
        bldr.setAddress(tx.get(classOf[Port], vipPeerPortId).getPortAddress)

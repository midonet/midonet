...
class ListenerV2Translator
    ...
    : Unit = {
        ...
        val lbRouterPeers = lbRouter.getPortIdsList.asScala map routerInterfacePortPeerId
        val vipPort = lbRouterPeers.collectFirst {
            case peerId if tx.exists(classOf[NeutronPort], peerId) =>
                tx.get(classOf[NeutronPort], peerId)
        }.getOrElse {
            throw new IllegalStateException(
                "LB router has no router interface port which maps " +
                "to the VIP port")
        }

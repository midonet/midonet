    ...
    private def onServiceContainerGroupUpdate(scg: ServiceContainerGroup): Unit = {
        ...
        if (selector eq null) {
            ...
            selector.discardedHosts
                    .map[UUID](makeFunc1(h =>
                            hostStateWatcher.unsubscribe(
                                h.toString, h, MidonetBackend.AliveKey)))
                    .map[List[ContainerHostMapping]](makeFunc1(h =>
                            hostContainerMappings.get(h)
                                .filter(chm => chm.group.getId == scg.getId)))
                    .map[Unit](makeFunc1(chml => applyMappings(reevaluateMappings(chml))))

            // Initial election of hosts
            selector.candidateHosts
                    .map[Set[UUID]](s => s.map(h =>
                        hostStateWatcher.subscribe(h.toString, h, MidonetBackend.AliveKey)))
                    .onSuccess {
                        case hostSet  =>

...
/**
  ...
  *              scheduled.
  */
case class CreateContainer(container: UUID,
...
/**
 * The type of notifications emitted by the scheduler.
*/
trait Event
case class Created(container: ServiceContainer, group: ServiceContainerGroup,
                   hostId: UUID) extends Event
case class Up(container: ServiceContainer, group: ServiceContainerGroup,
              hostId: UUID) extends Event
case class Down(container: ServiceContainer, group: ServiceContainerGroup,
                hostId: UUID, status: ContainerStatus)
case class Deleted(container: ServiceContainer, group: ServiceContainerGroup,
                   hostId: UUID)
...
class Scheduler @Inject()(store: Storage,
    ...
    private val hostToContainers = mutable.HashMap.empty[UUID, mutable.Set[CreateContainer]]

    private def currentMappings: Set[CreateContainer] = {
        hostToContainers.values.flatten.toSet
    }
    ...
    private val containerLocationSubject =
        ReplaySubject.create[CreateContainer]
    ...
    /**
      ...
      * [[CreateContainer]] events through the observable returned.
      *
      * @return [[rx.Observable]][[CreateContainer]] Observable where
      *         the scheduler emits the mappings of containers to host.
      */
    def startScheduling(): Observable[CreateContainer] = {
    ...
    private def createMappings(group: ServiceContainerGroup, containers: Set[UUID], candidates: Set[UUID]): Unit = {
        ...
        if (aliveCandidates.isEmpty) {
            ...
            val mappings = for (container <- pendingContainers) yield {
                new CreateContainer(container, group, candidateIt.next())
            }
            ...
            for (mapping <- mappings) {
                ...
                val currentMappings = hostToContainers
                    .getOrElseUpdate(mapping.hostId,
                                     mutable.HashSet.empty[CreateContainer])
    ...
    /**
      ...
      *
      * @param chms [[mutable.Set]] The [[CreateContainer]] set of the
      *             containers that should be reescheduled
      * @return [[util.List]][[CreateContainer]]
      */
    private def reevaluateMappings(chms: mutable.Set[CreateContainer]): Unit = {

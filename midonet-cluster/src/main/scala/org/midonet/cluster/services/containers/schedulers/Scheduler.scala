...
case class ContainerHostMapping(container: UUID,
    ...
    override val toString =
        MoreObjects.toStringHelper(this).omitNullValues()
                ...
                .add("groupId", group.getId.asJava)
                .add("hostId", hostId)
                .toString
}
...
class Scheduler @Inject()(store: Storage,
    ...
    private val log = Logger(
        LoggerFactory.getLogger("org.midonet.cluster.containers.container-scheduler"))

    private val hostToContainers =
        new util.HashMap[UUID, mutable.Set[ContainerHostMapping]]()

    private val currentMappings = mutable.Set.empty[UUID]
    ...
    private val containerLocationSubject =
        ReplaySubject.create[ContainerHostMapping]
    ...
    /**
      ...
      */
    def startScheduling(): Observable[ContainerHostMapping] = {
    ...
    private def getHostSelector(scg: ServiceContainerGroup): HostSelector = {
        ...
        val portGroupId = if (scg.hasPortGroupId) scg.getPortGroupId.asJava else null
        val hostGroupId = if (scg.hasPortGroupId) scg.getPortGroupId.asJava else null
        ...
        (portGroupId, hostGroupId) match {
            ...
            case (uuid: UUID, null) => // Port group selector
                selector = new PortGroupHostSelector(store, stateStore, executor)
            case (null, uuid: UUID) => // Host group selector
                selector = new HostGroupHostSelector(store, stateStore, executor)
    ...
    private def onServiceContainerGroupUpdate(scg: ServiceContainerGroup): Unit = {
        log.debug(s"Updated container group ${makeReadable(scg)}")
        val selector = getHostSelector(scg)
        if (selector ne null) {
            scgToSelector.put(fromProto(scg.getId), selector)
            // Subscribing to host no longer candidates
            selector.discardedHosts
                .observeOn(scheduler)
                .doOnNext(makeAction1(h => deleteHostStateSubscription(h, selector)))
                .map[Unit](
                    makeFunc1(h => {
                        val hostMappings = hostToContainers.get(h)
                        val oldMappings = hostMappings
                            .filter(chm => chm.group.getId == scg.getId)
                        // Remove old mappings from internal state
                        for (oldMapping <- oldMappings) {
                            hostMappings -= oldMapping
                            currentMappings -= oldMapping.container
                        }
            ...
            selector.candidateHosts
                    .onSuccess {
                        case hostSet =>
                            onReceiveCandidateHosts(hostSet, selector, scg)
                    }
    ...
    private def createMappings(group: ServiceContainerGroup, containers: Set[UUID], candidates: Set[UUID]): Unit = {
        ...
        // Skip containers that already have a mapping
        val pendingContainers = containers.diff(currentMappings)
        ...
        if (aliveCandidates.isEmpty) {
            ...
            val mappings = for (container <- pendingContainers) yield {
                new ContainerHostMapping(container, group, candidateIt.next())
            }
            ...
            for (mapping <- mappings) {
                ...
                hostToContainers
                    .putIfAbsent(mapping.hostId,
                                 mutable.HashSet.empty[ContainerHostMapping])
                hostToContainers.get(mapping.hostId) += mapping
                currentMappings += mapping.container
    ...
    /**
      ...
      *
      * @param chms [[mutable.Set]] The [[ContainerHostMapping]] set of the
      *            containers that should be reescheduled
      * @return [[util.List]][[ContainerHostMapping]]
      */
    private def reevaluateMappings(chms: mutable.Set[ContainerHostMapping]): Unit = {
    ...
    private def onHostStateChange(hostId: UUID, isAlive: Boolean): Unit = {
        ...
        // containers without mappings because their eligible
        // be down
        ...
        hostToContainers.get(hostId) match {
            ...
            case mappings => {
                ...
                for (mapping <- mappings)
                    currentMappings -= mapping.container
                val removed = hostToContainers.remove(hostId)
                reevaluateMappings(removed)
            }
        }

...
class FloatingIpTranslator(stateTableStorage: StateTableStorage)
    ...
    private def addNatRules(tx: Transaction, fip: FloatingIp,
        ...
        val skipSnatOutRule = Rule.newBuilder
            .setId(fipSkipSnatOutRuleId(fip.getId))
            .setType(Rule.Type.LITERAL_RULE)
            .setAction(Rule.Action.RETURN)
            .setFipPortId(fip.getPortId)
            .setCondition(anyFragCondition.addOutPortIds(rtrPortId))
            .build()
        val skipSnatInRule = Rule.newBuilder
            .setId(fipSkipSnatInRuleId(fip.getId))
            .setType(Rule.Type.LITERAL_RULE)
            .setAction(Rule.Action.ACCEPT)
            .setFipPortId(fip.getPortId)
        ...
        tx.create(dnatRule)
        tx.create(skipSnatOutRule)
        tx.create(skipSnatInRule)
        ...
        tx.update(appendRule(
            prependRules(skipSnatChain, skipSnatOutRule.getId),
            skipSnatInRule.getId))
        if (isGwPort) {
    ...
    private def removeNatRules(tx: Transaction, fip: FloatingIp): Unit = {
        ...
        tx.delete(classOf[Rule], fipReverseDnatRuleId(fip.getId),
                  ignoresNeo = true)
        tx.delete(classOf[Rule], fipSkipSnatOutRuleId(fip.getId),
                  ignoresNeo = true)
        tx.delete(classOf[Rule], fipSkipSnatInRuleId(fip.getId),
                  ignoresNeo = true)
    }

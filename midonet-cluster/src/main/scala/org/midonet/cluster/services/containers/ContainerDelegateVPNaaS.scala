...
import org.midonet.cluster.models.Topology.{Port, Host, ServiceContainer, ServiceContainerGroup}
import org.midonet.cluster.util.logging.ProtoTextPrettifier._
import org.midonet.cluster.util.UUIDUtil._

class ContainerDelegateVPNaaS @Inject()(store: Storage) extends ContainerDelegate {
    ...
    override def onCreate(container: ServiceContainer,
        ...
        val newport = port.toBuilder()
            ...
            .setInterfaceName(s"vpn-${port.getId.toString.substring(0,8)}_dp")
            .build()
        ...
        try {
            ...
            case e: ObjectExistsException =>
                log.info(s"Port ${makeReadable(port.getId)} already exists.")
        }
    }
    ...
    override def onDelete(container: ServiceContainer,
        ...
        val newport = port.toBuilder()
            .setId(container.getPortId)
            // how to remove fields with the builder?
            .build()

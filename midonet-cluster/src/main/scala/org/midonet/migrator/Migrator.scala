...
object Migrator extends App {
    ...
    private def migrateBridges(): Unit = {
        for (b <- legacyImporter.listBridges) {
            ...
            val resp = resources.bridges.create(b, createReplMapNodes = false)
            handleResponse(resp)
    ...
    /** Migrates VTEPs. Prerequisite
      * s: Networks, Tunnel Zones. */
    private def migrateVteps(): Unit = {
        for (v <- legacyImporter.listVteps) {
            log.info("Migrating " + v)
            val resp = resources.vteps.create(v, APPLICATION_VTEP_JSON)
            handleResponse(resp)
            ...
            for (b <- legacyImporter.listVtepBindings(ipAddr)) {
                b.vtepId = v.id
                log.info("Migrating " + b)
                val resp = bindings.create(b, APPLICATION_VTEP_BINDING_JSON)
                handleResponse(resp)

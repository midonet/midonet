...
//trait RtnetlinkConnectionProvider {
     ...
     * @param groups group ID of the Netlink channel
     * @return an instance of the class derives RtnetlinkConnection
     */
    def apply(addr: Netlink.Address, sendPool: BufferPool, groups: Int): T =
        try {
            ...
            constructorMirror(channel, sendPool, groups).asInstanceOf[T]
        } catch {
            case ex: Exception =>

...
object NetlinkRequestReply {
    ...
    private sealed class NetlinkContext {
        ...
        private val sequenceAddress = unsafe.objectFieldOffset(
                        classOf[NetlinkContext].getDeclaredField("sequence"))

        /**
         * Called by the thread handling replies. Protect this context
...
class NetlinkRequestReply(reader: NetlinkReader,
    ...
    private val requests = new Array[NetlinkContext](capacity)

    {
        var i = 0
        while (i < capacity) {
            requests(i) = new NetlinkContext
            requests(i).clear()
            i += 1
        }
    }
    ...
    private def getObserver(seq: Int, ctx: NetlinkContext,
    ...
    private def acquireContext(expiration: Long,
                               observer: Observer[ByteBuffer]): NetlinkContext = {
        var i = 0
        do {

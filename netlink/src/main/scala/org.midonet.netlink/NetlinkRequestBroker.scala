 ...
 */
final class NetlinkRequestBroker(writer: NetlinkWriter,
...

    {
        var i = 0
        while (i < capacity) {
            val start = i * maxRequestSize
            buffer.position(start)
            buffer.limit(start + maxRequestSize)
    ...
    def publishRequest(seq: Int, observer: Observer[ByteBuffer]): Unit = {
        ...
        sequences(pos) = seq
        unsafe.putOrderedInt(this, sequenceAddress, seq)
    }
    ...
    def writePublishedRequests(): Int = {
        val end = unsafe.getIntVolatile(this, sequenceAddress) + 1
    ...
    private def clear(pos: Int): Unit = {
        ...
        buffers(pos).clear()
        val bufferAddress = BASE + (pos * SCALE)
        unsafe.putOrderedInt(sequences, bufferAddress, FREE)
    }
}

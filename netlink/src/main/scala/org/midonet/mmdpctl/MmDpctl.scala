...
object FlowsCtl extends Subcommand("flows") with DpCommand {
    ...
    /* TODO(adrian): Support IPv6 fields
     * current code generates IPv4 FlowMask/FlowMatch keys when L3-L4
     * parameters are passed. This will cause the flow to be rejected when IPv6
     * ether-type is used, thus the conflict.
    */
    conflicts(etherType, List(srcIp, dstIp, tpSrc, tpDst, proto, arp, arpReply))

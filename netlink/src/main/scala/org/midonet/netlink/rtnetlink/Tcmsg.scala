...
/*
 * These consant values were taken from the code of the iproute2 package,
...
object Tcmsg {
    ...
    def ticksPerBurst(burst: Int, rate: Double): Double = {
...
class TcPolice(rate: Int, burst: Int, mtu: Int, tickInUsec: Double)
    ...
    def serializeInto(buf: ByteBuffer): Int = {
        ...
        val pburst = ticksPerBurst(burst, prate)
        val start = buf.position()
        buf.putInt(0) // index
        buf.putInt(Tcmsg.TC_POLICE_SHOT) // action
        buf.putInt(0) // limit
        buf.putInt((pburst*tickInUsec).toInt)
        buf.putInt(mtu)
...
class TcRtab(mtu: Int, rate: Int, tickInUsec: Double) extends NetlinkSerializable {
    ...
    def serializeInto(buf: ByteBuffer): Int = {
        ...
        val bytes = kiloBitsToBytes(rate)
        val cellLog = mtuToCellLog(mtu)

        for (i <- 1 to 256) {
            val sz = i << cellLog
            val time = TIME_UNITS_PER_SEC * (sz.toDouble / bytes.toDouble)

...
class SixwindWriter {
    ...
    private val cLib: Option[Mido6Wind] = {
        try {
            val options = collection.immutable.HashMap(
                Library.OPTION_FUNCTION_MAPPER -> functionMapper
            ).asInstanceOf[java.util.HashMap[_,_]]
    ...
    /**
      ...
      * of bytes written. The buffer must contains a correctly formatted
      * Netlink message
      *
    ...
    def write(src: ByteBuffer): Int = {
        cLib match {
            case Some(instance) => {
                val result = instance.processFlow(src, src.limit())
                if(result != 0) {

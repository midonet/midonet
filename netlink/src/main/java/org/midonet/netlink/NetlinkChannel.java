...
public class NetlinkChannel extends UnixChannel<Netlink.Address> {
    ...
    protected void initSocket() {
        ...
        if (sockoptret != 0) {
            ...
            sobuf.clear();
            log.debug("Successfully set netlink channel RCVBUFF size to {} (got {})",
                      RCVBUF_SIZE, sobuf.getInt());
        }
        ...
        if (sockoptret != 0) {
            ...
            sobuf.clear();
            log.debug("SETSOCKOPT success: NETLINK_BROADCAST_ERROR (got {})",
                      sobuf.get());
        }
        ...
        if (sockoptret != 0) {
            ...
            sobuf.clear();
            log.debug("SETSOCKOPT success: NETLINK_NO_ENOBUFS (got {})",
                      sobuf.getInt());
        }
    }

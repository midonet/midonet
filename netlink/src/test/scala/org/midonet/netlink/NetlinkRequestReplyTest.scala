...
class NetlinkRequestReplyTest extends FeatureSpec
    ...
    feature ("Can make requests to a NetlinkRequestReply") {
        scenario ("A sequence number is added to a netlink request") {
            ...
            requestReply.request(requestBuf, null) should be (512)
        ...
        scenario ("Errors are communicated through the Observer") {
            ...
            requestReply.request(requestBuf, obs) should be (0)
        ...
        scenario ("Number of in-flight requests is bounded") {
            ...
            requestReply.request(requestBuf, null) should be (0)
        }
    }
    ...
    feature ("Can get replies from a NetlinkRequestReply") {
        scenario ("An ACK calls into onComplete") {
            ...
            requestReply.request(requestBuf, obs)
        ...
        scenario ("Can have multiple answers per buffer") {
            ...
            val size = NetlinkMessage.GENL_HEADER_SIZE + 16
            NetlinkMessage.writeHeader(replyBuf, size, 0, 0, seq1, 0, 0, 0)
            replyBuf.position(size)
            NetlinkMessage.writeHeader(replyBuf, size, 0, 0, seq2, 0, 0, 0)
            replyBuf.position(0)
        ...
        scenario ("An upcall packet is passed into a catch-all Observer") {
            ...
            val size = NetlinkMessage.GENL_HEADER_SIZE + 16
            NetlinkMessage.writeHeader(replyBuf, size, 0, 0, 0, 0, 0, 0)
            replyBuf.limit(size)

            requestReply.processReply(obs)

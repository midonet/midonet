...
import org.midonet.api.host.GreTunnelZone;
...
// I need one per type.
public class GreTunnelZoneNameValidator implements
                   ConstraintValidator<IsUniqueGreTunnelZoneName, GreTunnelZone> {

    private final DataClient dataClient;

    @Inject
    public GreTunnelZoneNameValidator(DataClient dataClient) {
        this.dataClient = dataClient;
    }

    @Override
    public void initialize(IsUniqueGreTunnelZoneName constraintAnnotation) {
    }

    @Override
    public boolean isValid(GreTunnelZone zone,
        ...
        try {
            for (TunnelZone tz : dataClient.tunnelZonesGetAll()) {
                if (tz.getName().equalsIgnoreCase(zone.getName()) &&
                    tz.getType().equals(TunnelZone.Type.Gre)) {
                    return false;
                }
            }

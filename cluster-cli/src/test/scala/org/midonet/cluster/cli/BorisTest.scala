...
import org.midonet.cluster.cli.Boris._
...
import org.midonet.cluster.models.Topology.{Port, Network, Rule, Vtep}
...
class BoCLI extends FlatSpec {

    Boris.main(Array())
    Boris.curator.blockUntilConnected()
    Boris.backend.store.isBuilt
...
class BorisPersistenceTest extends FlatSpec
    ...
    "Boris" should "GET existing entities" in {
        val r = boris.execute("Network get id ${fromProto(n1.getId)}").get

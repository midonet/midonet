...
object StorageCommand {
    ...
    private val IPAddrClass = classOf[IPAddr]
    private val IPSubnetClass = classOf[IPSubnet[_]]
...
abstract class StorageCommand[T >: Null <: ZoomObject with Obj, U <: MessageOrBuilder]
    ...
    override def run: Run = super.run orElse {
        ...
        case args if 2 == args.length && args(0).toLowerCase == "delete" =>
            delete(args(1))
        case args if 2 == args.length && args(0).toLowerCase == "state" =>
            state(args(1))
    }
    ...
    protected def newObject[V <: ZoomObject with Obj]
                           (clazz: Class[V], args: Array[String]): V = {
        val obj = clazz.newInstance()
    ...
    private def valueOf(name: String, field: Field, value: String): Any = {
        field.getType match {
            ...
            case IPAddrClass => IPAddr.fromString(value)
            case IPSubnetClass => IPSubnet.fromString(value)
            ...
            case enumClass: Class[_] if enumClass.isEnum =>
                enumClass.getFields.find(field => {
                    field.isEnumConstant && field.getName == value.toUpperCase
                }) match {
                    case Some(enumField) => enumField.get(null)
                    case None =>
                        throw new Exception(s"Unknown enumeration value " +
                                            s"$value for field $name")
                }
            case _ =>
    ...
    protected def state(id: String): CommandResult = {
        try {
            val owners = storageState(UUID.fromString(id)).toList
            println(s"Owners for: $id (${owners.size} owners)")
            for (index <- 0 until owners.size) {
                println(s"\t$index : ${owners(index)}")
            }
            CommandSuccess
        } catch {
            case NonFatal(e) => CommandFailed(e)
    ...
    protected def storageCreate(obj: T): Unit = {
        cli.storage.create(toProto(obj))
    }
    ...
    @throws[ReferenceConflictException]
    @throws[ConvertException]
    protected def storageCreate[V >: Null <: ZoomObject, W <: MessageOrBuilder]
                               (obj: V, pClass: Class[W]): Unit = {
        cli.storage.create(ZoomConvert.toProto(obj, pClass))
    }

    @throws[NotFoundException]
    @throws[ReferenceConflictException]
    @throws[ConvertException]
    protected def storageUpdate(obj: T): Unit = {
    ...
    @throws[ReferenceConflictException]
    @throws[ConvertException]
    protected def storageUpdate[V >: Null <: ZoomObject, W <: MessageOrBuilder]
                               (obj: V, pClass: Class[W]): Unit = {
        cli.storage.update(ZoomConvert.toProto(obj, pClass))
    }
    ...
    @throws[NotFoundException]
    @throws[ConvertException]
    protected def storageGet[V >: Null <: ZoomObject, W <: MessageOrBuilder]
                            (id: UUID, jClass: Class[V], pClass: Class[W]): V = {
        val proto = Await.result(cli.storage.get(pClass, id), timeout)
            .asInstanceOf[W]
        ZoomConvert.fromProto(proto, jClass)
    }
    ...
    @throws[NotFoundException]
    protected def storageState(id: UUID): Set[String] = {
        Await.result(cli.storage.getOwners(u.runtimeClass, id), timeout)
    }

    @throws[NotFoundException]
    @throws[OwnershipConflictException]
    protected def storageStateUpdate(id: UUID, owner: String) = {
        cli.storage.updateOwner(u.runtimeClass, id, owner,
                                throwIfExists = false)
    }

    @throws[NotFoundException]
    @throws[OwnershipConflictException]
    protected def storageStateDelete(id: UUID, owner: String) = {
        cli.storage.deleteOwner(u.runtimeClass, id, owner)
    }

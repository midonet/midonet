...
class HostCommand(cli: ClusterCli)
    ...
    : CommandResult = {
        try {
            ...
            host.portBindings = host.portBindings + (portId -> iface)
            storageUpdate(host)
            println(s"Add binding completed: ${asString(host)}")
            CommandSuccess
        } catch {
    ...
    private def deleteBinding(hostId: UUID, portId: UUID): CommandResult = {
        try {
            val host = storageGet(hostId)
            host.portBindings = host.portBindings - portId
            storageUpdate(host)
            println(s"Delete binding completed: ${asString(host)}")
            CommandSuccess
        } catch {
            case e: Throwable => CommandFailed(e)
        }
    ...
    private def addTunnelZone(hostId: UUID, tzoneId: UUID): CommandResult = {
        try {
            val host = storageGet(hostId)
            host.tunnelZoneIds = host.tunnelZoneIds + tzoneId
            storageUpdate(host)
            println(s"Add tunnel zone completed: ${asString(host)}")
            CommandSuccess
        } catch {
            case e: Throwable => CommandFailed(e)
        }
    ...
    private def deleteTunnelZone(hostId: UUID, tzoneId: UUID): CommandResult = {
        try {
            ...
            host.tunnelZoneIds = host.tunnelZoneIds - tzoneId
            storageUpdate(host)
            println(s"Delete tunnel zone completed: ${asString(host)}")
            CommandSuccess
        } catch {
            case e: Throwable => CommandFailed(e)
        }

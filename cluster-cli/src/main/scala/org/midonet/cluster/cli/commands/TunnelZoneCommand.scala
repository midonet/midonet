...
class TunnelZoneCommand(cli: ClusterCli)
    ...
    : CommandResult = {
        try {
            ...
            tunnelZone.hosts = tunnelZone.hosts + (hostId -> ipAddr)
            storageUpdate(tunnelZone)
            println(s"Add host completed: ${asString(tunnelZone)}")
            CommandSuccess
        } catch {
    ...
    private def deleteHost(tunnelZoneId: UUID, hostId: UUID): CommandResult = {
        try {
            val tunnelZone = storageGet(tunnelZoneId)
            tunnelZone.hosts = tunnelZone.hosts - hostId
            storageUpdate(tunnelZone)
            println(s"Delete host completed: ${asString(tunnelZone)}")
            CommandSuccess
        } catch {
            case e: Throwable => CommandFailed(e)
        }

        ...
        override def parse(token: String): Parser = {
            ...
            materialize(currField, token) match {
                ...
                case v =>
                    ...
                    // list modification
                    builders.top.setField(field.pop(), v)
            }
            this
        }
    ...
    def execute(s: String): Try[Result] = {
        ...
        Try ( parsed match {
            ...
            case p: ContentParser if p.verb == Verb.SUB =>
                ...
                val b = entity.toBuilder
                entity.getAllFields.filter { fv =>
                    fv._1.isRepeated && b.hasField(fv._1)
                } foreach { fv =>
                    val remove = entity.getField(fv._1).asInstanceOf[JList]
                    val list = b.getField(fv._1).asInstanceOf[JList]
                    b.clearField(fv._1)
                    list.foreach { v =>
                        if (!remove.contains(v)) {
                            b.addRepeatedField(fv._1, v)
                        }
                    }
                }

...
- The Neutron importer stops API writes Neutron (this simply freezes the
  state).
- The Neutron importer truncates the recent changes table and add a
  FLUSH command.  This tells the cluster node to point writes at a new
  root directory in ZK.  It does not imply that the old MidoNet backend
  is deleted, threfore agents (and other clients) are still able to
  operate using the current version of the state.

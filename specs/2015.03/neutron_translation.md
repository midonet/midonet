...
Create a pool in MidoNet with the same pool ID.  Update the MidoNet health
monitor with the pool if health monitor association specified..
...
The pool task object does not include router ID, so make sure it keeps this ID
when updating the Neutron pool data.

If the health monitor and pool association was created:

 * Update the MidoNet health monitor with the new pool association
 * Create a health monitor pool association configuration to notify the MidoNet
   health monitor process of the change

If the health monitor and pool association was removed:

 * Update the MidoNet health monitor by removing the pool association
 * Remove the health monitor pool association configuration to notify the
   MidoNet health monitor process of the change
 ...
 * Update the health monitor pool association configuration to notify the
   MidoNet health monitor process of the change

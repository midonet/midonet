...
If the 'type' field is anything other than 'vtep_router', throw an unsupported
operation exception.

Create a gateway device data in MidoNet.
...
Store the remote mac entry.

# DELETE

Fetch the remote mac entry.

Iterate over the VTEP router ports, and for each port that has its VNI set to
the 'segmentation_id' of the remote mac entry, go through its peer table and
delete the paths containing the 'mac_address' and 'vtep_address' pair of the
remote mac entry.
 ...
 * Create a MidoNet network port on the network matching 'network_id' provided
   in the top level 'l2_gateway' object.  Generate the port ID based on
   'network_id' so it will be easy to retrieve it later.
 * Fetch the gateway device object using the 'device_id' value of the device
   object.
 * Fetch the VTEP router using the 'resource_id' field of the gateway device
   object.
 * Create a port on the router, also derived from the 'network_id', set the VNI
   to 'segmentation_id' set in 'l2_gateway_connection' object.
...
remote mac entries associated with the port.

...
class BlockingSpscRwdRingBufferTest extends FeatureSpec
    ...
    feature("Blocking reads") {
        scenario("poll blocks on empty buffer") {
            ...
            a [TimeoutException] shouldBe thrownBy(Await.ready(done, 500.milliseconds))
            ring.complete()
        }
        ...
        scenario("reader awakes on offer") {
            val done = Future {ring.awaitPoll()}
            ring.offer("1")
            Await.result(done, 2.seconds).get.item shouldBe "1"
            ring.complete()
        }
        ...
        scenario("reader awakes on complete") {
            val done = Future {ring.awaitPoll()}
            ring.complete()
            Await.result(done, 2.seconds).isEmpty shouldBe true
            ring.complete()
        }
        ...
        scenario("reader gets data from before complete") {
            ring.add("1")
            ring.complete()
            Await.result(Future {ring.awaitPoll()}, 2.seconds).get.item shouldBe "1"
            Await.result(Future {ring.awaitPoll()}, 2.seconds).isEmpty shouldBe true
            ring.complete()
        }

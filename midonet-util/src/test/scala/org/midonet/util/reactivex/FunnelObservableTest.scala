...
import org.midonet.util.reactivex.observers.{IncrementalSubscriber, AwaitableObserver}
...
class FunnelObservableTest extends FeatureSpec with Matchers {
    ...
    feature("Test single subscriber") {
        ...
        scenario("Subscriber receives OnCompleted from empty sources") {
            ...
            val obs = new AwaitableObserver[String]()

            val sub = funnel.subscribe(obs)

            obs.notifications should contain only OnCompleted()
        ...
        scenario("Subscriber receives OnError from error sources") {
            ...
            obs.notifications should contain only OnError(err)
            sub.isUnsubscribed should be (true)
        }
        ...
        scenario("Subscriber receives updates from constant sources") {
            ...
            obs.notifications should contain inOrderOnly (
                OnNext("1-1"), OnNext("1-2"), OnNext("2-1"), OnNext("2-2"),
                OnCompleted())
            sub.isUnsubscribed should be (true)
        }
        ...
        scenario("Updates received after source completion") {
            ...
            source2.onNext("2-2")
            obs.notifications should contain inOrderOnly (
                OnNext("1-1"), OnNext("2-1"), OnNext("2-2"))
            sub.isUnsubscribed should be (false)
        }
        ...
        scenario("Updates not received after source error") {
            ...
            source2.onNext("2-2")
            obs.notifications should contain inOrderOnly (
                OnNext("1-1"), OnNext("2-1"), OnError(err))
            sub.isUnsubscribed should be (true)
        }
        ...
        scenario("Updates not received after unsubscribe") {
            ...
            source1.onNext("1-2")
            sources.onNext(source2)
            obs.notifications should contain only OnNext("1-1")
            sub.isUnsubscribed should be (true)
        }
        ...
        scenario("Subsequent duplicate sources are repeated") {
            ...
            obs.notifications should contain theSameElementsAs Vector(
                OnNext("1"), OnNext("2"), OnNext("1"), OnNext("2"),
                OnCompleted())
            sub.isUnsubscribed should be (true)
        }
    }
    ...
    feature("Test back-pressure") {
        ...
        scenario("Values cached for single incremental subscriber") {
            ...
            val sub = new IncrementalSubscriber[String]()

            funnel.subscribe(sub)

            sources.onNext(source)
            sub.notifications should contain only OnNext("1")
            sub.consumeOne()
            sub.notifications should contain inOrderOnly (
                OnNext("1"), OnNext("2"))
            sub.consumeOne()
            sub.notifications should contain inOrderOnly (
    ...
    feature("Test subscriber isolation") {
        ...
        scenario("Funnel subscription provides contract protection") {
            ...
            val goodObs = new AwaitableObserver[String]()
            val badObs = new AwaitableObserver[String]() {
            ...
            goodObs.notifications should contain inOrderOnly(
                OnNext("1"), OnCompleted())
            badObs.notifications should contain only OnError(err)
        }
    }

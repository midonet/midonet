...
class FunnelObservableTest extends FeatureSpec with Matchers {
    ...
    feature("Test source removal") {
        scenario("Test removal of existing source") {
            val source1 = PublishSubject.create[String]()

            val funnel = FunnelObservable.create(source1)

            val obs1 = new AwaitableObserver[String]()
            val sub1 = obs1.asSubscriber

            funnel.subscribe(sub1)

            source1.onNext("1:0")
            obs1.notifications should contain only OnNext("1:0")

            funnel.remove(source1)
        ...
        scenario("Test removal of dynamic source") {
            val source1 = PublishSubject.create[String]()

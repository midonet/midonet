...
import org.midonet.util.logging.DailyRollingStrategy.msPerDay
...
class RollingOutputStreamTest extends FeatureSpec
    ...
    feature("RollingOutputStream") {
        ...
        scenario("Performs scheduled rollover") {
            ...
            // Trigger rollover
            advanceToNextMidnight(os.clock)
    ...
    private def advanceToNextMidnight(clock: UnixClock): Unit = {
        val mockClock = clock.asInstanceOf[MockUnixClock]
        mockClock.time = (((mockClock.time + 1) / msPerDay) + 1) * msPerDay
    }

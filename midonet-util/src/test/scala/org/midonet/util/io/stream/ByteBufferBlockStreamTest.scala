...
class ByteBufferBlockStreamTest extends FeatureSpec
    ...
    private def getStreams(blockBuilder: BlockHeaderBuilder[TimedBlockHeader]): (
        ByteBufferBlockWriter[TimedBlockHeader] with TimedBlockInvalidator,
        ByteBufferBlockReader[TimedBlockHeader],
        RingBuffer[ByteBuffer]) = {
        val testBlockSize = blockSize + blockBuilder.headerSize

        val buffers = new RingBuffer[ByteBuffer](
            poolSize, null)

        val blockFactory = new HeapBlockFactory[TimedBlockHeader](testBlockSize,
                                                                  blockBuilder)
        val writer =
            new ByteBufferBlockWriter(blockBuilder, buffers, blockFactory.allocate)
                with TimedBlockInvalidator {
    ...
    feature("ByteBufferExpirationStream handles block expiration") {
        ...
        scenario("Expiring a single block on a multiblock stream.") {
            ...
            currentTime += (timeout toMillis) + 1

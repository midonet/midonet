...
public class SocketTest {
    ...
    public void testSockAddrWrite() {
        ...
        if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
            Assert.assertEquals(unsafe.getByte(ptr), 0x34);
            Assert.assertEquals(unsafe.getByte(ptr + 1), 0x12);
        } else {
            Assert.assertEquals(unsafe.getByte(ptr), 0x12);
            Assert.assertEquals(unsafe.getByte(ptr + 1), 0x34);
        }
        for (int i = 0; i < 14; i++) {
    ...
    public void testSockAddrRead() {
        ...
        if (ByteOrder.nativeOrder() == ByteOrder.LITTLE_ENDIAN) {
            Assert.assertEquals(sockAddr.saFamily, 0x3412);
        } else {

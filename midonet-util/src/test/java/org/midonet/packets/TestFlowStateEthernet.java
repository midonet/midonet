...
public class TestFlowStateEthernet {
    ...
    private static byte[] emptyPacket = new byte[] {
            ...
            (byte) 0x01, (byte) 0x11, (byte) 0x00, (byte) 0x00, // (byte) 0x44, (byte) 0x2e,
    ...
    public static class TestFlowStateEthernetValidPacket {
        ...
        public void TestDeserialize() throws Exception {
            ...
            FlowStateEthernet packet = new FlowStateEthernet();
            packet.deserialize(buff);

            // Compare UDP and its payload
            Assert.assertEquals(this.expected.getPayload().getPayload(),
                    packet.getPayload().getPayload());
            // Compare IP and its payload
            Assert.assertEquals(this.expected.getPayload(), packet.getPayload());
            // Compare Ethenet frame and its payload
            Assert.assertEquals(this.expected, packet);
        }

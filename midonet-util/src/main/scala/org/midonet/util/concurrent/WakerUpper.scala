...
object WakerUpper {
    ...
    private val waiters = new Array[WaitContext](128)
    private val numWaiters = new AtomicInteger()
    ...
    private def register(ctx: WaitContext): Unit = {
        ...
        waiters(threadId.get) = ctx
        if (numWaiters.getAndIncrement == 0) {
            LockSupport.unpark(wakerThread)
        }
    }

    private def deregister(ctx: WaitContext): Unit = {
        ctx.thread = null
        waiters(threadId.get) = null // Cleanup in case of a spurious wake up
        numWaiters.decrementAndGet()
    }

    private def wakerUpperLoop(): Unit = {
        while (true) {
            while (numWaiters.get() == 0) {

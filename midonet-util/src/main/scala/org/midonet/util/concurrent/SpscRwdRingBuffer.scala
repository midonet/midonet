 ...
 * NOTE: it assumes a single producer (meaning that there will only be a
 * single 'offer' at a time, though they can be called from different
 * threads); it has also a single 'consumer' (meaning that there will only
 * be a single 'consume' operation - peek/poll/rewind/curSeqno - at a time,
 * though they can be called from different threads.
 *
...
class SpscRwdRingBuffer[T: ClassTag](minCapacity: Int) {
    ...
    private val capacity: Int = Util.findNextPositivePowerOfTwo(minCapacity)

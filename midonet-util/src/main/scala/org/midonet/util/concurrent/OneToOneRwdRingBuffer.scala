 ...
 */
class OneToOneRwdRingBuffer[T: ClassTag](minCapacity: Int) {
    import OneToOneRwdRingBuffer._
    ...
    def offer(item: T): Boolean = {
        ...
        if ((wrPos - rd.get()) >= capacity)
            ...
            // not using lazySet in case offers come from different threads,
            // one at a time
            wr.set(wrPos + 1)
            true
        }
    ...
    def poll(): Option[SequencedItem] = {
        ...
        if (wr.get() == rdPos)
            ...
            // we can use lazySet here because there is a single consumer thread
            rd.lazySet(rdPos + 1)
            Some(item)
        }
    }
    ...
    def size: Int = {
        ...
        System.out.println("SIZE IS " + (wrPos - rdPos))
        (wrPos - rdPos).toInt
    }

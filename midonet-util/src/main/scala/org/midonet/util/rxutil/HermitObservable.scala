...
package org.midonet.util.rxutil
...
object HermitObservable {
    ...
    class OnSubscribeHermit[T](o: Observable[T]) extends OnSubscribe[T] {
        ...
        class HermitException
            extends RuntimeException("Observable already had a subscription")
        ...
        override def call(subscriber: Subscriber[_ >: T]): Unit = {
            ...
            val onUnsub = new OperatorDoOnUnsubscribe[T](new Action0 {
                    override def call(): Unit = currentSubscription = None
                })
            val s: Observable[T] = o.lift(onUnsub)
            currentSubscription = Some(s.subscribe(subscriber))
        }
    }
}

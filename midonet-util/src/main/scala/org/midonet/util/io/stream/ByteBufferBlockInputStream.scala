...
class ByteBufferBlockInputStream[H <: BlockHeader]
    ...
    var blockIter = buffers.iterator
    ...
    /**
      ...
      * @return The length read (1) or -1 if there were no bytes to read.
      */
    ...
    @inline private def eof = current == null ||
                              (current.remaining() == 0 && !hasNextBlock)

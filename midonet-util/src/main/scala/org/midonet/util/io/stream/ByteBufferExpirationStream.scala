...
trait ExpiringBlockStream
    ...
    private def isBlockStale(blockHeader: ExpirationBlockHeader, tick: Long): Boolean = {
        val bool = tick - blockHeader.lastEntryTime > expirationTime
        val diff = tick - blockHeader.lastEntryTime
        log.debug(s"$bool $tick ${blockHeader.lastEntryTime} $diff ${expirationTime}")
        bool
    }

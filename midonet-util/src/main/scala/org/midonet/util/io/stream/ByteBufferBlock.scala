...
/**
  ...
  * block streams.
  */
trait ByteBufferBlock[H <: BlockHeader] {

    val log = Logger(
        LoggerFactory.getLogger("org.midonet.util.io.stream.bytebuffer-blockstream"))

    /** Ring buffer where blocks are stored */
    val buffers: RingBuffer[ByteBuffer]

    /** Generic block builder for CRUD ops on the header */
    val blockBuilder: BlockHeaderBuilder[H]

    /** Current block where chunks are written to or read from */
    def current: Option[ByteBuffer] = buffers.head

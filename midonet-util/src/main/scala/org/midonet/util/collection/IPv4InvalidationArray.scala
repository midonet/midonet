...
class IPv4InvalidationArray {
    ...
    trait PrefixInvalidator {
        // invalidates a prefix. Returns true if this node became empty after this operation
        def deletePrefix(key: Int, prefixLen: Int, deletions: ArrayList[Int]): Boolean
    }
    ...
    abstract class NonTerminal[ChildNode <: PrefixInvalidator] extends Node[ChildNode] with PrefixInvalidator {
        protected def release(child: ChildNode) {}

        override def deletePrefix(key: Int, prefixLen: Int, deletions: ArrayList[Int]): Boolean = {
            ...
            while (i <= last) {
                if (array(i) != EMPTY) {
                    if (array(i).deletePrefix(key, prefixLen, deletions)) {
                        release(array(i))
                        array(i) = EMPTY
                        size -= 1
                    }
    ...
    final class B1Node extends NonTerminal[B2Node] with PrefixInvalidator {
        ...
        override def release(node: B2Node): Unit = b2Pool.offer(node)
    }
    final class B2Node extends NonTerminal[Leaf] with PrefixInvalidator {
        ...
        override def release(leaf: Leaf): Unit = leafPool.offer(leaf)
    }
    ...
    def isEmpty: Boolean = {
        ...
        var i = 0
        while (i < 256) {
            if ((root.array(i) ne null) && root.array(i).nonEmpty)
                return false
            i += 1
        }
        true

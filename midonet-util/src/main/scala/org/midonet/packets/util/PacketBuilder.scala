...
object PacketBuilder {
    ...
    case class Ip6Pair(src: IPv6Addr = "", dst: IPv6Addr = "") {
        def src(addr: IPv6Addr): Ip6Pair = copy(src = addr)
        def src(addr: String): Ip6Pair =  src(IPv6Addr(addr))

        def dst(addr: IPv6Addr): Ip6Pair = copy(dst = addr)
        def dst(addr: String): Ip6Pair =  dst(IPv6Addr(addr))
        def -->(addr: String): Ip6Pair =  dst(IPv6Addr(addr))
        def -->(addr: IPv6Addr): Ip6Pair =  dst(addr)
    }

    implicit def stringToIp4Pair(src: String): Ip4Pair = Ip4Pair(src)
    implicit def stringToIp6Pair(src: String): Ip6Pair = Ip6Pair(IPv6Addr(src))
    implicit def intToIp4Pair(src: Int): Ip4Pair = Ip4Pair(src)
    implicit def ip4ToIp4Pair(src: IPv4Addr): Ip4Pair = Ip4Pair(src)
    implicit def ip6ToIp6Pair(src: IPv6Addr): Ip6Pair = Ip6Pair(src)
...
case class IPv6Builder(packet: IPv6 = new IPv6()) extends PacketBuilder[IPv6] {
    ...
    def addr(pair: Ip6Pair): IPv6Builder = { src(pair.src) ; dst(pair.dst) }
